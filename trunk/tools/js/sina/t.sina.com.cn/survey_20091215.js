...
function aesurvey(sid, sh, ratio, wd, hg) {
	var pn_refer = document.location.toString().toLowerCase().replace(
			'http://', '');
	if (pn_refer.lastIndexOf('/') == pn_refer.length - 1)
		pn_refer = pn_refer.substr(0, pn_refer.length - 1);
	pn_refer = escape(pn_refer);
	var url = 'http://survey.client.sina.com.cn/survey.php?sid=' + sid
			+ '&pn_refer=' + pn_refer;
	tag = "aesurvey" + sid;
	...
	if (pp > ratio && ratio > 0)
		return;
	window.open(url, '', 'menubar=no,status=no,toolbar=no,resizable=yes,width='
					+ wd + ',height=' + hg + ',scrollbars=no');
}
...
function popupSinaMonitorPage() {
	...
	if ((survey_ids.length > 0) && (!purl.match(r1))) {
		...
		var ws = aesurvey(survey_ids[rid], 0, 100, survey_window_high,
				survey_window_width);
...
function putSinaMonitorBeanInfo(survey_id, ws) {
	...
	putInfo = "Survey||" + globalId + "||" + uidStr + "||" + survey_id + "||"
			+ ws + "||";
	_S_p2Bcn(putInfo, sina_user_monitor_beanurl);
}

function init_SinaMonitorJS(firstSampleRate, secondSampleRate, ckName,
		ckExpires, openCkName) {
