...
/*    Generated:  March 5th 2009 10:39:00 PM PST                               */
/*      Machine:  10.16.140.104                                                */
/*       Source:  Local Cache                                                  */
/*     Location:  rsrc:7svn81ne:zh_CN:/html/js/w69vwt22ras88wkw.pkg.js:141     */
/*       Locale:  zh_CN                                                        */
/*         Path:  js/w69vwt22ras88wkw.pkg.js                                   */
...
var possible=['sidebar_ads','fadbar','home_sponsor','home_sponsor_nile','highlights_ad'];var locations=[];for(var i=0;i<possible.length;i++){if(ge(possible[i])){locations.push(possible[i]);}}
if(locations.length==0){return;}
var data={'page':page,'tab':tab,'locations':locations};var _onResponse=function(response){window.ads_refreshing=false;var divs=response.getPayload();for(var id in divs){if(ge(id)&&divs[id].length>0){DOM.replace($(id),HTML(divs[id]));}else if(ge(id)){$(id).style['display']='none';}}
if(onResponse){onResponse(response);}}
var _onError=function(response){window.ads_refreshing=false;if(onError){onError(response);}}
new AsyncRequest().setURI('/ajax/location_refresh.php').setData(data).setHandler(_onResponse).setErrorHandler(_onError).send();window.ads_refreshing=true;}
function social_ads_pagecache_log_imps(){var ads=DOM.scry(document,'div.pagecache_social_ad');for(var i=0;i<ads.length;i++){var data={'meta':DOMStorage.getData(ads[i],'link_data')};new AsyncRequest().setURI('/ajax/social_ad_impression.php').setData(data).send();}}
...
this._hardpoint=n;}
return this._hardpoint;},initialResourcesReady:function(){return this._initialResourcesReady;},loadInitialResources:function(resources){this._initialResourcesReady=false;this.loadResources(resources,bind(this,function(){this._initialResourcesReady=true;window.onResourceReady&&window.onResourceReady();}));},loadContentFromScript:function(src,div_id,callback){var div=document.getElementById(div_id);if(!div){window.Util&&Util.error('No div has id "'+div_id+'"');return;}
var tmp=document.write,timeout=0,fired=false;document.write=function(m){div.innerHTML+=m;if(!timeout){timeout=1;setTimeout(done,0);}};var done=function(){if(!fired){document.write=tmp;if(callback){callback();}}
fired=true;};var script=loadExternalJavascript(src,done);},_initialResourcesReady:true,_requested:{},_permanent:{},_loaded:{},_pending:[],_componentMap:{},_cssLinkMap:{},_cssLinks:[],_hardpoint:null,_CSS_POLL_EXPIRATION:5000,_CSS_POLL_INTERVAL:20,_UNUSED_CSS_URL:'javascript:void(0)'};
...
copy_properties(Arbiter,{SUBSCRIBE_NEW:'new',SUBSCRIBE_ALL:'all',BEHAVIOR_EVENT:'event',BEHAVIOR_PERSISTENT:'persistent',BEHAVIOR_STATE:'state',ALL:'all',LIVEMESSAGE:'livemessage',BOOTLOAD:'bootload',PAGE_TRANSITION:'pagetransitions/transition',CONTEXT_CHANGE:'ui/context-change',PROFILE_PUBLISHER:'profile/publisher',subscribe:function(types,callback,subscription_policy){var a=Arbiter._getInstance(this);types=types||Arbiter.ALL;a._listeners.push({callback:callback,types:types});subscription_policy=subscription_policy||Arbiter.SUBSCRIBE_ALL;if(subscription_policy==Arbiter.SUBSCRIBE_ALL){if(!(types instanceof Array)){types=[types];}
...
var ua={ie:function(){return ua._populate()||this._ie;},firefox:function(){return ua._populate()||this._firefox;},opera:function(){return ua._populate()||this._opera;},safari:function(){return ua._populate()||this._safari;},safariPreWebkit:function(){return ua._populate()||this._safari<500;},windows:function(){return ua._populate()||this._windows;},osx:function(){return ua._populate()||this._osx;},_populated:false,_populate:function(){if(ua._populated){return;}
ua._populated=true;var agent=/(?:MSIE.(\d+\.\d+))|(?:(?:Firefox|GranParadiso|Iceweasel).(\d+\.\d+))|(?:Opera.(\d+\.\d+))|(?:AppleWebKit.(\d+(?:\.\d+)?))/.exec(navigator.userAgent);var os=/(Mac OS X)|(Windows;)/.exec(navigator.userAgent);if(agent){ua._ie=agent[1]?parseFloat(agent[1]):NaN;if(ua._ie>=8&&!window.HTMLCollection){ua._ie=7;}
...
var aug=no_augment?identity:DOM._getNodeAugment();return aug(element);},setID:function(element,id){element.id=id;},prependContent:function(parent,content){if(!is_node(parent)){throw new Error('DOM.prependContent: reference element is not a node');}
var append_fn=function(fragment){if(parent.firstChild){parent.insertBefore(fragment,parent.firstChild);}else{parent.appendChild(fragment);}};return DOM._addContent(content,append_fn,parent);},insertAfter:function(reference_element,content){if(!is_node(reference_element)||!reference_element.parentNode){throw new Error('DOM.insertAfter: reference element is not a node');}
var append_fn=function(fragment){if(reference_element.nextSibling){reference_element.parentNode.insertBefore(fragment,reference_element.nextSibling);}else{reference_element.parentNode.appendChild(fragment);}};return DOM._addContent(content,append_fn,reference_element.parentNode);},insertBefore:function(content,reference_element){if(!is_node(reference_element)||!reference_element.parentNode){throw new Error('DOM.insertBefore: reference element is not a node or'+'does not have a parent.');}
var append_fn=function(fragment){reference_element.parentNode.insertBefore(fragment,reference_element);};return DOM._addContent(content,append_fn,reference_element.parentNode);},setContent:function(el,content){if(!is_node(el)){throw new Error('DOM.setContent: reference element is not a node');}
DOM.removeAllChildren(el);return DOM.appendContent(el,content,el);},appendContent:function(reference_element,content){if(!is_node(reference_element)){throw new Error('DOM.appendContent: reference element is not a node');}
var append_fn=function(fragment){reference_element.appendChild(fragment);};return DOM._addContent(content,append_fn,reference_element);},replace:function(reference_element,content){if(!is_node(reference_element)||!reference_element.parentNode){throw new Error('DOM.replace: reference element must be a node with a'+' parent');}
var append_fn=function(fragment){reference_element.parentNode.replaceChild(fragment,reference_element);};DOM._addContent(content,append_fn,reference_element.parentNode);},_gettingCaretPosition:false,getCaretPosition:function(element){element=$(element);if(!is_node(element,['input','textarea'])){return{start:undefined,end:undefined};}
...
element.parentNode.removeChild(element);}},removeAllChildren:function(element){element=$(element,true);while(element.firstChild){element.removeChild(element.firstChild);}},contains:function(outer_obj,inner_obj){outer_obj=ge(outer_obj,true);inner_obj=ge(inner_obj,true);if(!outer_obj||!inner_obj){return false;}else if(outer_obj===inner_obj){return true;}else if(is_node(outer_obj,'#text')){return false;}else if(is_node(inner_obj,'#text')){return DOM.contains(outer_obj,inner_obj.parentNode);}else if(outer_obj.contains){return outer_obj.contains(inner_obj);}else if(outer_obj.compareDocumentPosition){return!!(outer_obj.compareDocumentPosition(inner_obj)&16);}else{return false;}},getRootElement:function(){var elem=null;if(window.Quickling&&Quickling.isActive()){elem=ge('content');}
return elem||DOMScroll.getScrollRoot();},NODE_TYPES:{ELEMENT:1,ATTRIBUTE:2,TEXT:3,CDATA_SECTION:4,ENTITY_REFERENCE:5,ENTITY:6,PROCESSING_INSTRUCTION:7,COMMENT:8,DOCUMENT:9,DOCUMENT_TYPE:10,DOCUMENT_FRAGMENT:11,NOTATION_NODE:12},_addContent:function(content,append_fn,target_element){var item,elements=[],callbacks=[];var fragment=document.createDocumentFragment();if(!(content instanceof Array)){content=[content];}
for(var ii=0;ii<content.length;ii++){item=content[ii];if(item instanceof HTML){callbacks.push(item.getAction());var new_nodes=item.getNodes();for(var jj=0;jj<new_nodes.length;jj++){var cloned_item=((ua.ie()||ua.safari())&&item.hasOptionElements())?new_nodes[jj]:new_nodes[jj].cloneNode(true);elements.push(cloned_item);fragment.appendChild(cloned_item);}}else if(is_scalar(item)){var text=document.createTextNode(item);elements.push(text);fragment.appendChild(text);}else if(is_node(item)){elements.push(item);fragment.appendChild(item);}else if(item instanceof Array){Util.error('DOM._addContent: Nested arrays not supported');}else if(item!==null){Util.error('DOM._addContent: No way to set content %q.',item);}}
...
function _check_for_unsaved_forms(link){for(var form_id in _unsaved_forms_to_check_for){var form=ge(form_id);if(form&&form.original_state&&!are_equal(form.original_state,serialize_form(form))){var href=link.href;var submit=_find_first_submit_button(form);var buttons=[];if(submit){buttons.push({name:'save',label:_tx("\u4fdd\u5b58"),handler:function(){submit.click();}});}
buttons.push({name:'dont_save',label:_tx("\u4e0d\u8981\u4fdd\u5b58"),className:'inputaux',handler:function(){window.location.href=href;}});buttons.push(Dialog.CANCEL);new Dialog().setTitle(_tx("\u6ca1\u6709\u4fdd\u5b58\u7684\u66f4\u6539")).setBody(_tx("\u60a8\u7684\u90e8\u5206\u4fee\u6539\u5c1a\u672a\u4fdd\u5b58\u3002\u60a8\u60f3\u73b0\u5728\u4fdd\u5b58\u4fee\u6539\u5417\uff1f")).setButtons(buttons).setModal().show();return false;}}}
...
return result;},implodeQuery:function(obj,name){name=name||'';var r=[];if(obj===null){r.push(URI.encodeComponent(name));}else if(obj instanceof Array){for(var ii=0;ii<obj.length;++ii){try{if(obj[ii]!==undefined){r.push(URI.implodeQuery(obj[ii],name?(name+'['+ii+']'):ii));}}catch(ignored){}}}else if(typeof(obj)=='object'){if(is_node(obj)){r.push('{node}');}else{for(var k in obj){try{if(obj[k]!==undefined){r.push(URI.implodeQuery(obj[k],name?(name+'['+k+']'):k));}}catch(ignored){}}}}else{r.push(URI.encodeComponent(name)+'='+URI.encodeComponent(obj));}
...
var parts=this.domain.split('.');var index=parts.indexOf('facebook');return parts.slice(index).join('.');},getTld:function(){if(!this.domain){return'';}
var parts=this.domain.split('.');return parts[parts.length-1];},getUnqualifiedURI:function(){return new URI(this).setProtocol(null).setDomain(null).setPort(null);},getQualifiedURI:function(){var uri=new URI(this);if(!uri.getDomain()){var current=URI();uri.setProtocol(current.getProtocol()).setDomain(current.getDomain()).setPort(current.getPort());}
...
var dst=uri.getDomain().split('.');var src=this.getDomain().split('.');var dst_tld=dst.pop();if(URI.isValidFacebookTld(dst_tld)&&dst_tld==src.pop()){if(dst.pop()=='facebook'&&src.pop()=='facebook'){this.setDomain(uri.getDomain());return true;}}
...
function AsyncRequest(uri){var dispatchResponse=bind(this,function(asyncResponse){try{this.clearStatusIndicator();if(this.initialHandler(asyncResponse)!==false){if(this.handler){try{this.handler(asyncResponse);}catch(exception){this.finallyHandler(asyncResponse);throw exception;}}
...
if(this.handler||this.errorHandler||this.transportErrorHandler){if(typeof(interp.transportError)!='undefined'){var r=new AsyncResponse();var errorDescription=Util.isDevelopmentEnvironment()?interp.transportError:_tx("\u67d0\u4e9b\u4e1c\u897f\u51fa\u4e86\u95ee\u9898\u3002\u6211\u4eec\u6b63\u5728\u7aed\u5c3d\u6240\u80fd\u5730\u4fee\u590d\u5b83\u3002\u7a0d\u540e\u60a8\u53ef\u4ee5\u91cd\u8bd5\u3002");copy_properties(r,{error:1000,errorSummary:_tx("\u5662\uff01"),errorDescription:errorDescription});if(this.transportErrorHandler){dispatchErrorResponse(r,true);}else{Util.error('Something bad happened; provide a transport error handler for '+'complete details.');}
return;}
...
invokeErrorHandler(1001);}}};copy_properties(this,{onstatechange:onStateChange,invokeResponseHandler:invokeResponseHandler,interpretResponse:interpretResponse,handleResponse:handleResponse,transport:null,method:'POST',uri:'',initialHandler:bagofholding,handler:null,errorHandler:null,transportErrorHandler:null,finallyHandler:bagofholding,statusElement:null,data:{},context:{},readOnly:false,writeRequiredParams:['post_form_id'],remainingRetries:0,option:{asynchronous:true,suppressErrorHandlerWarning:false,suppressEvaluation:false,suppressErrorAlerts:false,retries:1,jsonp:false,useIframeTransport:false},_captchaHtml:'',_confirmationMessage:'',_confirmationTitle:'',_confirmationButton:'',_requiresConfirmation:false});if(typeof ErrorDialog!="undefined"){this.errorHandler=ErrorDialog.showAsyncError;this.transportErrorHandler=ErrorDialog.showAsyncError;}
...
return this.option[opt];},send:function(isRetry){isRetry=isRetry||false;if(this.requiresConfirmation()){this._displayConfirmation();return true;}
...
Arbiter.inform('AsyncRequest.writeOperation',this);}
if(!is_empty(this.context)){copy_properties(this.data,this.context);this.data['ajax_log']=1;}
var impid=env_get('nectar_last_impression_id');if(impid){this.data['nectar_impid']=impid;}
...
+HTML(message)
+'</div>';var title=this._confirmationTitle||_tx("\u5fc5\u987b\u786e\u8ba4\u8981\u6c42");var button=this._confirmationButton||Dialog.CONFIRM;Dialog.createConfirmationDialog(html,title,button,this.finallyHandler.bind(this)).setHandler(this._displayConfirmationHandler.bind(this)).show();},_displayConfirmationHandler:function(){this.data.confirmed=1;this._requiresConfirmation=false;this.send();}});function AsyncResponse(payload){copy_properties(this,{error:0,errorSummary:null,errorDescription:null,onload:null,payload:payload||null});return this;}
...
function intl_set_cookie_locale(locale,uri){setCookie('locale',locale,7*24*3600000);if(URI.getRequestURI().getTld()!='com'){transferCookieToComTld('locale',function(success){if(success){clearCookie('locale');}
...
generic_dialog.prototype.show_loading=function(loading_html){if(!loading_html){loading_html=_tx("\u52a0\u8f7d\u4e2d\u2026\u2026");}
...
generic_dialog.prototype.show_ajax_dialog=function(src,post_vars,use_async_response){post_vars=post_vars||false;var load=_tx("\u52a0\u8f7d\u4e2d\u2026\u2026");return this.show_ajax_dialog_custom_loader(load,src,post_vars,use_async_response);}
...
generic_dialog.prototype.show_message=function(title,content,button){if(button==null){button=_tx("\u786e\u5b9a");}
...
buttons+='<input class="inputsubmit" type="button" value="'+button1+'" id="dialog_button1" />';if(button2){var button2_class='inputsubmit';if(button2==_tx("\u53d6\u6d88")){button2_class+=' inputaux';}
buttons+='<input class="'+button2_class+'" type="button" value="'+button2+'" id="dialog_button2" />';}
if(button3){var button3_class='inputsubmit';if(button3==_tx("\u53d6\u6d88")){button3_class+=' inputaux';}
...
pop_dialog.extend('generic_dialog');pop_dialog.prototype.do_expand_animation=false;pop_dialog.prototype.kill_expand_animation=true;pop_dialog.prototype.show_ajax_dialog=function(src,post_vars,title,use_async_response){post_vars=post_vars||false;if(this.do_expand_animation&&!this.kill_expand_animation){var load=null;this.show_loading_title(title);}else{var load=_tx("\u52a0\u8f7d\u4e2d\u2026\u2026");}
...
pop_dialog.prototype.get_popup_dialog=function(){return DOM.find(this.popup,"#pop_dialog_table");}
...
dropmenu.prototype._documentHideHandler=function(){if(!this.canHide){this.canHide=true;}else{this._hide();}}
...
flyout_menu.prototype._buildDynamicMenu=function(){this.menu=document.createElement('div');CSS.addClass(this.menu,(this.className||'')+' flyout_menu elem_hidden');if(this.alignment==dropmenu.ALIGN_RIGHT){CSS.addClass(this.menu,'flyout_menu_left');}
...
return false;},filterEventTargets:function(event,type){var target=$E(event).getTarget();if(is_node(target,['input','select','textarea','object','embed'])){if(target.type!='checkbox'&&target.type!='radio'){return false;}}
...
copy_properties(Dialog,{OK:{name:'ok',label:_tx("\u786e\u5b9a")},CANCEL:{name:'cancel',label:_tx("\u53d6\u6d88"),className:'inputaux'},CLOSE:{name:'close',label:_tx("\u5173\u95ed")},SAVE:{name:'save',label:_tx("\u4fdd\u5b58")},SUBMIT:{name:'submit',label:_tx("\u63d0\u4ea4")},CONFIRM:{name:'confirm',label:_tx("\u786e\u8ba4")},DELETE:{name:'delete',label:_tx("\u5220\u9664")}});copy_properties(Dialog,{OK_AND_CANCEL:[Dialog.OK,Dialog.CANCEL],_STANDARD_BUTTONS:[Dialog.OK,Dialog.CANCEL,Dialog.CLOSE,Dialog.SAVE,Dialog.SUBMIT],newButton:function(name,label,className,handler){var button={name:name,label:label}
...
return this;},setClassName:Dialog._basicMutator('_class_name'),setCloseHandler:function(close_handler){this._close_handler=call_or_eval.bind(null,this,close_handler);return this;},setFooter:Dialog._basicMutator('_footer'),setAutoFocus:Dialog._basicMutator('_auto_focus'),onloadRegister:function(handler){this._onload_handlers.push(handler.bind(this));return this;},setAsync:function(async_request){var handler=function(response){if(this._async_request!=async_request){return;}
...
this._onload_handlers=[];}else{var title=this._title||_tx("\u52a0\u8f7d\u4e2d\u2026\u2026");this._pd.show_loading_title(title);}
...
Draggable.prototype._ongrab=function(vector){this.ongrab();if(!this.oldPosition){this.oldPosition=this.dom.style.position;}
...
function SortableGroup(){this.namespace='sortable'+(++SortableGroup.instanceCount);this.draggables={};this.droppables={};this.sortables={};this.linkedGroups=[];this.linkedGroups.onlinkjump=bagofholding;this.rootNode=null;this.boundingBox=null;}
...
if(before){this.rootNode.insertBefore(this.placeholder,droppable);}else{this.rootNode.insertBefore(this.placeholder,droppable.nextSibling);}
this.rootNode.insertBefore(draggable,this.placeholder);if(jumped){this.linkedGroups.onlinkjump.call(this,draggableKey);}}
...
SortableGroup.prototype.removeSortable=function(key){this.draggables[key].destroy();this.droppables[key].destroy();delete this.draggables[key];delete this.droppables[key];delete this.sortables[key];}
...
if (window.Bootloader) { Bootloader.done(["js\/w69vwt22ras88wkw.pkg.js"]); }