...
.colorPickerPosition{position:absolute;height:0;margin:-31px 0 0 -222px}');
	...
	getPosition : function() {
		...
		if (headWrap) {
			...
			position[0] = UI.getY(talkBox) - 3;
			position[1] = UI.getX(talkBox) - 1;			
		}
		return position;
	},
...
#Copyright .wrapper,#Copyright .wrapper a{color:<%=color_2%>}\
	...
	showDiy : function(index){ //打开某个页卡
		var Self = this,num;
		if(!Self.dialog._cont){
			Self.show();
		}
		var link = $$(Self.dialog._cont, '.DtabTitle li'),
			len = link.length;
		if(!link.length) {
			setTimeout(function(){
				Self.showDiy(index);
		...
		if(UI.isNumber(index)){
			if(index < 0) num = 0;
			if(index >= len) num = len - 1;
			num = index;
		}else{
			num = len - 1;
		}
		var linked = $$(link[num],'a')[0];
		if(linked){
			Self.load(UI.A(linked, 'href'));
		}else{
			return;
		}
	}
