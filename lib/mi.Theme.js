...
MI.Theme = {
	...
	build : function() {
		...
		.D2 .DTitle,.D2 .ico_skin,.DtabTitle,.ico_lock,.D2 .funBox,.themebg_sel button,.themebg .del,.bgPosition,.bgPosition a.select,.D2 .color,.palettes .mask,.themeDiy .pages a{background:url(http://mat1.gtimg.com/www/mb/images/skinBg1c.png) no-repeat}\
		...
		.themebg_sel .inputTxt{width:180px;margin-right:5px;border:1px solid #C5C5C5;position:relative;z-index:1}\
		.themebg_sel button{width:60px;height:21px;border:0;background-position:0 -145px}\
		.selfilemask{position:absolute;right:18px;width:60px;*width:258px;overflow:hidden;*zoom:1;*margin-top:2px;filter:alpha(opacity:0);opacity:0}\
		.selfilemask input{float:right;height:21px;cursor:pointer}\
		...
		.themebg .nobg{height:106px;color:#999;line-height:106px;text-align:center;background-color:#EFEFEF}\
		.themebg .loading{padding-left:16px;background-position:20% 50%}\
		.themebg .imgbg img{width:106px;height:106px}\
		...
		.Tmain{float:left;width:77px;height:110px}\
		...
		.ThotList .SA{left:0;top:-5px;*top:-6px;_top:-5px}.ThotList .SA *{width:18px;height:5px}.ThotList .SA span{left:0;top:1px}\
		.ThotList{margin-top:3px;height:12px}\
		.palettelist{float:right;width:108px;overflow:hidden}\
		...
		.D2 .themeDiy .pages{width:108px;height:16px;overflow:hidden;padding:0;border:none}\
		.themeDiy .pages *{float:left;margin:0}\
		.themeDiy .pages a{width:12px;height:16px;background-position:-94px -124px}\
		.themeDiy .pages .btn_prev_disabled{background-position:-78px -124px;cursor:default}\
		.themeDiy .pages .btn_next{background-position:-126px -124px}\
		.themeDiy .pages .btn_next_disabled{background-position:-110px -124px;cursor:default}\
		.themeDiy .pageInfo{width:84px;text-align:center}\
		...
		.themeHis .Tmain{width:9px;height:16px;background:#fff}\
		.themeHis .Tside{width:7px;height:16px}\
		...
		Self.dialog = new MI.Dialog({
			tmpl : '<div class="D2">'
					+ (UI.B.ie6
							...
							: '')
					+ '<div class="bg"></div><div class="CR"><div class="DWrap"><div class="DTitle"></div><a title="关闭" class="DClose close" href="#">关闭</a><div class="DLoad"></div><div class="DCont"></div></div></div>'
		});
	...
	load : function(url) {
		...
		Self.xhr = UI.get(url + (url.hasString('?') ? '' : '?') + '&r='
					...
					// data = '<div class="DtabTitle"><ul><li
					// class="select">精选主题</li><li><a
					// href="/setting_theme_frame.php?t=1">世界杯</a></li></ul><span
					// class="right"><a href="#">成为新星主播，获取纪念版皮肤</a></span></div>
					// <div> <ul class="DtempList clear"> <li class="hover"><img
					// src="http://mat1.gtimg.com/www/mb/images/thumb.jpg"><p
					// class="ico_lock"></p><p class="tempName">模板名字</p><div
					// class="mask"></div></li> <li class="locked"><img
					// src="http://mat1.gtimg.com/www/mb/images/thumb.jpg"><p
					// class="ico_lock"></p><p class="tempName">模板名字</p><div
					// class="mask"></div></li> <li class="select"><img
					// src="http://mat1.gtimg.com/www/mb/images/thumb.jpg"><p
					// class="ico_lock"></p><p class="tempName">模板名字</p><div
					// class="mask"></div></li> <li><img
					// src="http://mat1.gtimg.com/www/mb/images/thumb.jpg"><p
					// class="ico_lock"></p><p class="tempName">模板名字</p><div
					// class="mask"></div></li> <li><img
					// src="http://mat1.gtimg.com/www/mb/images/thumb.jpg"><p
					...
					// src="http://mat1.gtimg.com/www/mb/images/thumb.jpg"><p
					// class="ico_lock"></p><p class="tempName">模板名字</p><div
					// class="mask"></div></li> </ul> <div class="pages"><a
					// href="#" class="disabled">上一页</a><a
					// href="/setting_theme_frame.php?t=1&p=2">下一页</a></div>
					// </div> <div class="funBox"><input type="button" value=""
					// class="btn_save"><input type="button" value=""
					// onclick="MI.Theme.hide()"
					// class="btn_cancel"></div></div>';
					if (data != '') {
						if (!Self.loadError) {
	...
	loadError : 0,
	save : function() {
		var Self = this, cont = Self.dialog._cont, selected = Self.themeId;
		if (selected) {
			if (Self.xhr) {
				Self.xhr.abort();
			}
			Self.xhr = UI.ajax({
						...
						success : function(data) {
							...
							if (data.result == 0) {
								/*
								 * MI.confirm({ type : 'success', title :
								 * '皮肤保存成功', content : '发送一条广播，让大家来看看你换的新皮肤吧！',
								 * confirm : function(){
	...
	talk : function() {
		...
		Self.talkBox._txt.value = '我换了新皮肤“' + Self.themeName + '”，快来我的主页看看吧！'
	...
	diy : function() { // 自定义皮肤
		...
		Self._diyImageUpload = $$(Self._diyBody, '.themebg_sel input')[0];
		...
		UI.each(Self._diyColor, function(o, i) {
					...
					o.onclick = function(e) {
						var T = this, E = UI.E(e);
						UI.removeClass($$(Self._body, '.colorBox .select')[0],
								'select');
		...
		UI.each(Self._diyColors, function(o) {
			o.onclick = function() {
				UI.removeClass($$(Self._diyBody, '.palettelist .select')[0],
						'select');
				UI.addClass(this, 'select');
		...
		UI.each($$(Self._diyImagePosition, 'a'), function(o) {
					o.onclick = function() {
		...
		UI.each(Self._diyImageRepeat, function(o) {
					o.onclick = function() {
						Self.diyPreview();
					}
				});
		Self._diyImageDel.onclick = function() {
	...
	diyCss : '.TpreviewBox{background-color:<%=color_0%>}\
		...
		.side{border-left:1px solid <%=color_5%>;background-color:<%=color_1%>;border-radius:0 5px 5px 0;-moz-border-radius:0 5px 5px 0;-webkit-border-radius:0 5px 5px 0}\
		...
		.SM li.current{width:200px;height:32px;border:1px solid <%=color_5%>;border-right:0;background:#fff!important}\
		.SM li a:hover{text-decoration:underline}.SM li.current a:hover{text-decoration:none}\
		...
		.topicIntro{margin-top:3px}\
		.HT .side .SA em,.topicIntro .SA em{color:<%=color_5%>}\
		.topicIntro .SA span{color:<%=color_4%>}\
		.mobileBox a,.vDateBox a{color:#2B4A78}\
		#Copyright .wrapper{color:<%=color_2%>}\
	',
	diyPreview : function() {
		var Self = this, data = {}, position = ['right', 'center', 'left'];
		UI.each(Self._diyColor, function(o, i) {
					data['color_' + i] = UI.A(o, 'rel');
				});
		data.image = UI.A(Self._diyImage, 'src');
		data.imagePosition = position[UI.A(
				$$(Self._diyImagePosition, '.select')[0], 'rel')];
		data.imageRepeat = Self._diyImageRepeat[0].checked
				? 'repeat'
				: 'no-repeat';
		data.imageAttachment = Self._diyImageRepeat[1].checked ? 'fixed' : '';
