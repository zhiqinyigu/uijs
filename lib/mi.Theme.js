...
.colorPickerPosition{position:absolute;height:0;margin:-31px 0 0 -222px}');
	...
	diy : function() { // 自定义皮肤
		...
		Self._diyImageBtn = $$(Self._diyBody, '.themebgWrap form input')[0];
		...
		UI.each(Self._diyColors, function(o) {
			o.onclick = function() {
				...
				Self.diyPreview();
				UI.removeClass($$(Self._diyBody, '.themeHisWrap .select')[0],
						'select');
			}
		});
		...
		UI.each($$(Self._diyBody, '.themeHisWrap .themeHis'), function(o) {
					o.onclick = function() {
						// pic=""
		...
		Self._diyImageForm.onsubmit = function() {
	...
	diyCss : '.TpreviewBox{background-color:<%=color_0%>}\
		...
		#logo a{background-image:url(http://mat1.gtimg.com/www/mb/images/theme/b3_100719.png);_background:none;_filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(sizingMethod=crop,src="http://mat1.gtimg.com/www/mb/images/theme/b3_100719.png")}\
		...
		.FTL li em,.hotTopicList li em{color:#5b5b5b}\
		#mainWrapper{background:<%=color_1%>;}\
		.side{border:none;}\
		.main{background:#FFF;margin:0;border-radius:5px 0 0 5px;-moz-border-radius:5px 0 0 5px;-webkit-border-radius:5px 0 0 5px;border-right:1px solid <%=color_5%>;}\
		...
		.SM li.current{width:200px;height:32px;border-color:<%=color_5%>;background:#fff!important}\
	...
	diyPreview : function(save) {
		 ...
		 */
		var Self = this, data = {}, curTheme = $('curTheme'), position = ['',
				'left', 'center', 'right'];
		if (curTheme) {
			UI.remove(curTheme);
		}
