			...
			success : function(data){
				...
				if (data.result == 0){
					var tag = UI.html('<li><p class="tagFun"><a href="#" class="tag">' + txt + '</a><a href="#" class="delTag" rel="' + data.info.id + '">×</a></p><span class="tag">' + MI.string.cut(txt,12) + '</span></li>')[0],
						_tip = MI.Tag._tip,
						user = data.info.user,
						_user = UI.html('<li><div class="pic"><a href="/' + user.name + '"><img title="' + user.nick + '(@' + user.name + ')" src="' + (user.pic ? user.pic + '/50' : 'http://mat1.gtimg.com/www/mb/images/head_50.jpg') + '" /></a></div><div class="userInfo"><a href="/' + user.name + '" title="' + user.nick + '(@' + user.name + ')">' + user.nick + '</a><br />和你拥有相同标签:<br /><a href="#"><span class="tag">' + txt + '</span>(' + data.info.num + ')</a></div></li>')[0];
					UI.C(tag,'opacity',0);
					UI.append(tag,MI.Tag._body);
					UI.remove(_tip);
					UI.animate(tag,'opacity',1,function(){
						UI.C(tag,'filter','');
						$$(_tip,'.tagTipsNum')[0].innerHTML = data.info.num;
						UI.C(_tip,'opacity',0);
						UI.append(_tip,tag);
					...
					UI.prepend(_user,MI.Tag._users);
