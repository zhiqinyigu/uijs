...
MI.TagWall.Tag = function (el) {
	...
	var Self = this;
	Self._body = el;
	Self.tagName = $$(Self._body, '.tagName')[0];
	Self.tagNum = $$(Self._body, '.tagInfo b')[0];
	...
	paste : function () {
		...
		MI.ajax({
			...
			success : function (result) {
				...
				result = MI.json(result);
				if (!result.result){
					MI.talk(_('添加标签成功！<br />#征用标签#，发条广播分享给大家吧！'), _('我刚刚征用了@'+result.info.user.name+'的标签#'+txt+'#，觉得好玩你也贴上吧！http://t.qq.com/setting/tag'), 40);
				}else if (result.result == -107){
					MI.alert('你最多可以拥有10个标签<br/><a href="http://t.qq.com/setting/tag">去个人标签设置页</a>');
				}
			},
...
MI.FunnyTagBoard.SpecialTip.prototype = {
	...
	_update : function (tip) {
		...
		for (tagId in tip.tags){
			tag = UI.html('<span class="line"><a onclick="MI.Bos(\'btnTagCardGoSearch\');" href="http://t.qq.com/search/tag.php?k=' + encodeURIComponent(tip.tags[tagId]) + '&amp;pos=804">' + tip.tags[tagId] + '</a></span>')[0];
			if (tip.tagsNum - 1 === i++)
