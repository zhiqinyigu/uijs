...
MI.Tag = {
	...
	sayHiTxt : function(obj){
		...
		return account + '我刚通过标签' + tag + '收听了你，原来我们是#同道中人#！';
	},
	sayHiSet : function(obj){
		...
		MI.dialog.show({
			title : '<h1 class="DmainTit">招呼设置</h1>',
			html : '<div class="tagAutoSet" id="tagAutoSet">\
					<div class="cNote">找到同道中人只是第一步，互动交流才是同道之乐！</div>\
					<div><textarea class="inputArea" readonly>' + txt + '</textarea></div>\
					<div class="setSel">下次自动发送：<label><input type="radio" name="tagAutoSet" class="check1" checked="checked" onclick="MI.Bos(\'btnTagFollowReplyYes\')">是</label><label><input type="radio" name="tagAutoSet" class="check1" onclick="MI.Bos(\'btnTagFollowReplyNo\')">否</label></div>\
					<div class="btnBox"><button class="btn2">确定</button><button class="btn2" onclick="MI.dialog.hide()">取消</button></div>\
				</div>'
		});
		var wrap = $('tagAutoSet'),
			input = $$(wrap,'input'),
									...
									MI.confirm({
										...
										confirm : function(){
											if(talkBox) {
												...
												talkBox._txt.value = '对苍白的生活我不接受；对无力的驾驶我不妥协，我存在于后驱时代。我是后驱控，这是我的生活态度！一起来打“后驱控”标签，召唤同道中人现身！http://t.qq.com/setting_tag.php?bmw';
												talkBox.countTxt();
												talkBox.send();
											}
										}
