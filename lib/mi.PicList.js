...
MI.PicList.prototype._addArrowCls = function () {
  ...
  el.innerHTML = '<a class="clk-arrl" href="javascript:void(0)" title="上一张(用键盘←键也可翻页)"></a><a class="clk-arrr" href="javascript:void(0)" title="下一张(用键盘→键也可翻页)"></a>';
...
MI.PicList.prototype.rePosDlg = function (wrap) {
  var toppx, tips, arrl, arrr, wrap, hint, cls,
  ...
  if (UI.B.ie6 || UI.B.ipad) { //ipad do NOT support fixed position
    ...
    settop(arrl, toppx + winxy[1] / 2 - 47);
    settop(arrr, toppx + winxy[1] / 2 - 47);
    settop(wrap, toppx);
    settop(cls, toppx);
    settop(hint, toppx);
  } else {
    settop(arrl, winxy[1] / 2 - 47);
    settop(arrr, winxy[1] / 2 - 47);
    settop(wrap, 2);
    wrap.style.position = 'fixed';
  }
  wrap.style.left = (winxy[0] - MI.PicList.MODAL_WIDTH) / 2 + 'px';
  arrl.style.left = (winxy[0] - MI.PicList.MODAL_WIDTH) / 2 - 77 + 'px';
  arrr.style.left = (winxy[0] + MI.PicList.MODAL_WIDTH) / 2 + 10 + 'px';
  hint.style.left = (winxy[0] + MI.PicList.MODAL_WIDTH) / 2 + 'px';
  ...
  if (justChangeContent) {
    ...
    dlg.show({
      ...
      start : function () {
        ...
        tit.innerHTML = '<a target="_blank" class="imgModOrig" href="javascript:void(0)">查看原图</a>';
      },
      end : function () {
