 ...
 * @config {integer} fitWidth width of the picture inlist, default 120
 * @config {integer} fitHeight height of the picture inlist, default 120
 * @config {boolean} pptMode switch for open ppt mode view, default true
 * @config {string} moreBtn id of the more button for loading, default 'moreList'
 * @config {boolean} showHot show hot number or not, default true
 *            @example
...
MI.PicList = function (id, config) { //Pic List
  ...
  this.config.hasTools = config && typeof config.hasTools !== 'undefined' ?
    config.hasTools : true;

  if (this.config.doMore) {
    moreBtn = UI.G(this.config.moreBtn);
    if (moreBtn){
      this._more = UI.GC(moreBtn, 'a')[0];
      if (this._more) {
...
MI.PicList.genRelayList = function (id) {
  ...
  that[relayList] = UI.html('<div class="imgModWhite"><div class="relayList"><div style="height:5px;line-height:5px;font-size:0;"></div><div class="loading">' + _('正在加载') + '...</div><div class="cont"></div></div><div class="moreImgs"></div></div>')[0];

  that[relayListLoad] = UI.GC(that[relayList], '.loading')[0];
  that[relayListCont] = UI.GC(that[relayList], '.cont')[0];
  that[relayListPic] = UI.GC(that[relayList], '.moreImgs')[0];
...
MI.PicList.PAGE_WIDTH = MI.PicList.getDocumentXY()[0];
...
MI.PicList.prototype._preprocCfg = function (cfg) {
  ...
  UI.each('offsetX offsetY direction arrPos headHeight searchPage fitWidth fitHeight'.split(' '),
    function (o) {
      tmp = cfg[o];
  ...
  obj.owner = cfg.owner || '';
  obj.moreBtn = cfg.moreBtn || 'moreList';
  obj.isGuest = cfg.isGuest === true ? true : false;
  obj.pptMode = cfg.pptMode === false ? false : true;
  obj.showHot = cfg.showHot === false ? false : true;
...
MI.PicList.prototype._addEvents = function () {
  ...
  this.events = {
    mo : MI.PicList.bind(this._hlMouseOver, this),
    mout : MI.PicList.bind(this._hlMouseOut, this),
    click : MI.PicList.bind(this._hlClick, this),
    kd : MI.PicList.bind(this._hlKeyDown, this),
    sr : MI.PicList.bind(this._hlScrResize, this)
  };

  UI.EA(wholeList, 'mouseover', this.events.mo);
  UI.EA(wholeList, 'mouseout', this.events.mout);
  UI.EA(bd, 'click', this.events.click);
  UI.EA(document, 'keydown', this.events.kd);
  UI.EA(window, 'resize', this.events.sr);
  if (UI.B.ie6) {
    UI.EA(window, 'scroll', this.events.sr);
  }

  if (MI.Crs.onLoadResize) {
    UI.each(UI.GC(wholeList, '.clk-1 img'), function (o) {
      UI.EA(o, 'load', MI.Crs.onLoadResize);
      MI.Crs.onLoadResize(o);
    });
  }
...
MI.PicList.prototype._addModalEvents = function () {
  ...
  this.events.modalMout = MI.PicList.bind(this._hlMdMout, this);
  UI.EA(ct, 'mouseover', this.events.mo);
  UI.EA(ct, 'mouseout', this.events.modalMout);
};
...
MI.PicList.prototype._showTip = function (id) {
  ...
  mainCtn = UI.G('mainWrapper');
  if (mainCtn) {
    mainWrap = UI.GC(mainCtn, '.main')[0];
    ...
    //college is 9999
    mainCtn.style.zIndex = MI.user.college ? 10000 : 3;
  }
  ...
  UI.each(this._container.firstChild.getElementsByTagName('img'),
    function (o) {
      if (o.className === 'srs') {
        ...
        var title = that.config.pptMode ? (that.config.hasTools ? '进入幻灯模式查看' :
          '点击进入微群') : '点击查看详情';
        o.title = _(title);
        o.style.width = '16px';
...
MI.PicList.prototype._show = function (isLoading, item, picSize) {
  var x, y, h, w, listImg, imgY, scrollY, winY, picDiv, tipImg,
    ...
    outOfViewPortY, maxh, headWrap, heightOfContainer, shownImgHeight;
  ...
  shownImgHeight = Math.min(listImg.offsetHeight, listImg.parentNode.parentNode.offsetHeight);

  if (this.config.arrPos === 0) {
    icoOffset = shownImgHeight / 2;
  } else {
    icoOffset = Math.min(shownImgHeight, this.config.arrPos);
  }
...
//y = imgY - h - headH + y - Math.min(y, 50);
  ...
  arrowX = (MI.PicList.PAGE_WIDTH < winX ? MI.PicList.PAGE_WIDTH - winX : 0) / 2 + 12;
  if (x < arrowX) {
//do not point out of the window
    arrowX = (w + 12) / 2 - arrowX + x;
    x = (MI.PicList.PAGE_WIDTH < winX ? MI.PicList.PAGE_WIDTH - winX : 0) / 2 + 12;
  } else {
    arrowX = (w + 12) / 2;
  }
...
MI.PicList.prototype.addMore = function (json) {
  ...
  if (data && data.result === 0) {
    ...
    data.info.fav = that.last && that.last.fav ? 1 : 0; //Fav List Tmpl
    data.info.iconPic = that.iconPic;

    data.info.talk = that.removeRepeat(data.info);

    data.info.showHot = that.config.showHot;

    cache = document.createDocumentFragment();
    data.info.fitWidth = this.config.fitWidth;
    data.info.fitHeight = this.config.fitHeight;
    ...
    if (data.info.talk.length) {
      ...
      MI.PV(MI.api.type === 5 ? 'mb/qzone/more' : 'more');
      ...
      success : function (data) {
        ...
        if (data.result === 0) {
          if (type === 1) {
            ...
            if (detail.btnClass == "uptop") {         //置顶成功后发布talk
                that._hide();
                var content = _('我刚置顶了一条微博，大家来看看吧~') + "http://t.qq.com/" + MI.user.account + '?preview';
...
MI.PicList.prototype._addArrowCls = function () {
  ...
  this._elArrLeft = arrl = el.firstChild;
  this._elArrRight = arrr = arrl.nextSibling;
...
MI.PicList.prototype.rePosDlg = function (wrap) {
  ...
  msgWrapH = parseInt(tips.firstChild.offsetHeight, 10);
  arrPadding = (winxy[1] - MI.PicList.ARROW_SIZE) / 2;

  arrl.style.paddingTop = arrr.style.paddingTop = arrPadding + 'px';

  arrPadding = msgWrapH - MI.PicList.ARROW_SIZE - arrPadding;
  arrPadding = arrPadding >= 0 ? arrPadding : 0;
  ...
  */
  ...
  arrr.style.left = (winxy[0] + MI.PicList.MODAL_WIDTH - 16) / 2 - 109 +
    'px';
  ...
  */
  ...
  this._elClose.style.left = (winxy[0] + MI.PicList.MODAL_WIDTH - 18) /
    2 + 'px';
  ...
  //tips.scrollTop = 0;
  //
  tips.style.height = (UI.windowHeight() - 15) + 'px';

  if (MI.PicList.IE_VER === 6) {
    document.body.style.overflow = 'hidden';
    tips.style.position = 'absolute';
    tips.style.width = UI.windowWidth() + 'px';
    //tips.style.top = UI.scrollY() + 'px';
    tips.style.top = 0;
    tips.style.left = (MI.PicList.MODAL_WIDTH - winxy[0]) / 2 + 'px';
  }
...
MI.PicList.prototype._showTalkBox = function (isRelay, talkToAccount) {
  ...
  if (!theSpan.appended) {
    relay = UI.html(MI.tmpl.reply.replace('talkWrap', 'zfWrap').
      ...
      '<div class="left" style="display:none;width:135px;"><label for="replayListCheckbox"><input id="replayListCheckbox" type="checkbox"' +
...
MI.PicList.prototype._genRelayCombo = function (id, container) {
  var relayList,
    ...
    pageLink = function (data) {
      ...
      if (data.pic) {
          relayListPic.innerHTML = data.pic;

          if (UI.B.ie6) {
              var overPic = UI.GC(relayListPic, '.imgBox')[0];
              overPic.onmouseover = function () {
                  UI.each(UI.GC(overPic, 'a'), function (o) {
                      o.style.filter = 'none';
                  });
              };
              overPic.onmouseout = function () {
                  UI.each(UI.GC(overPic, 'a'), function (o) {
      ...
      UI.show(relayListPic);

      var cateTabs = UI.GC(relayListCont, '.tabStyle1');
      if (cateTabs && cateTabs[0]) {
        cateTabs[0].style.display = 'none';
      }
...
//                 }
      ...
      try {
        ...
        if (relayNum) {
          ...
          if (relayNumValue > parseInt(relayListNumValue.innerHTML, 10)){
  ...
  if (!relayList) { //设置转播列表DOM
    ...
    relayListPic = MI.PicList['_relayListPic' + relayListId];
  }
  ...
  if (MI.hostType === 3) {
    sourceTweetId = sourceTweetId.match(/#id=(\d+)/)[1];
  } else {
...
MI.PicList.prototype.showModalDlg = function (id, justChangeContent) {
  ...
  if (!this._modalDlg) {
    ...
    this._modalDlg._wrap.style.padding = '0';
    UI.GC(this._modalDlg._body, '.DWrap')[0].style.padding = '0';
  }
  //this._showModalHint();
  ...
  if (justChangeContent) {
    ...
    dlg.show({
      ...
      start : function () {
        ...
        that._showArrow();
        UI.GC(dlg._cont, '.clk-arrr')[0].focus();
        ...
        */
        ...
        if (UI.B.firefox) {
          document.documentElement.scrollTop = that.oldScrollTop;
        }
  ...
  UI.each(dlg._cont.firstChild.getElementsByTagName('img'),
    function (o) {
      ...
      if (o.className === 'srs') {
        ...
        o.src = 'http://mat1.gtimg.com/www/mb/images/loading-black2.gif';
...
MI.PicList.prototype._adjImgCtnHeight = function (isRotate) {
  ...
  if (UI.B.ie || !isRotate) {
    ...
    img = UI.GC(msgDiv, '.picBox canvas')[0];
  }

  boxHeight = parseInt(UI.C(img, 'marginTop'), 10);
  boxHeight = UI.height(msgDiv) - 2 * (isNaN(boxHeight) ? 0 : boxHeight);
  if (boxHeight > 450) {
...
MI.PicList.prototype._showEmpty = function () {
  ...
  UI.hide(this.config.moreBtn);
};

MI.PicList.prototype._getFunbox = (function () {
  var t1, t2, t3, t4, t_reply, t_cmt_a, t_cmt_b, isReadMythat, urlsuffix='';

  if (MI.guest && MI.guest.user === MI.user.account) {
    isReadMythat = true;
  }
  if(MI.Group && MI.Group.qid) {
  	urlsuffix = '/1';
  }
  ...
  t3 = '</p><p><a href="#" class="clk-fav">' + _('收藏') +
    ...
    _('分享') + '</a></p></div><p><a href="';

  t4 = '" class="clk-detail" target="' + (MI.hostType === 3 ? '_self' : '_blank') + '">' + _('详情') +
  ...
  return function (tid) {
    var t_cmt = t_cmt_a + MI.getInternalUrl(1, tid, urlsuffix) + t_cmt_b;
    return UI.html(MI.user.fun.btnStyle === 1 ?
      t1 + t_reply + t2 + t_cmt + t3 + MI.getInternalUrl(1, tid, urlsuffix) + t4:
      t1 + t_cmt + t2 + t_reply + t3 + MI.getInternalUrl(1, tid, urlsuffix) + t4)[0];
  };
}());
...
MI.PicList.prototype._prepareTip = function () {
  var ctn;
  if (!this._container) {
    this._container = UI.html('<div class="imgModTips cardT" style="display:none;z-index:1000;"><div></div><div class="imgExpd clk-expd">' +
    //_('展开') +
...
MI.PicList.prototype.reset = function () {
  this._lists.innerHTML = '';
...
MI.PicList.prototype.destroy = function () {
  var evs = this.events;

  UI.ER(this._lists, 'mouseover', evs.mo);
  UI.ER(this._lists, 'mouseout', evs.mout);
  UI.ER(document.body, 'click', evs.click);
  UI.ER(document, 'keydown', evs.kd);
  UI.ER(window, 'resize', evs.sr);
  if (UI.B.ie6) {
    UI.ER(window, 'scroll', evs.sr);
  }

  if (evs.am) {
    UI.ER(window, 'scroll', evs.am);
  }

  if (evs.modalMout) {
    UI.ER(this._modalDlg._cont, 'mouseover', this.events.mo);
    UI.ER(this._modalDlg._cont, 'mouseout', this.events.modalMout);
  }

  this.events = null;
  this._lists.innerHTML = '';
  this._lists = null;
  UI.remove(this._container);

  this._modalDlg = null;
  this._container = null;
  this._arrow = null;
  this._more = null;
  this._elArrLeft = null;
  this._elArrRight = null;
  this._elClose = null;
};
...
MI.app({
  Base : function () {
    ...
    MI.PicList.prototype.tmpl = '<% var o,i,n,dt,imgo,nick,hotstyle,zfcnt,urlsuffix;\
    for (i=0,n=talk.length;i<n;i++){\
    o=talk[i];nick=o.bkname||o.nick;urlsuffix=o.qinfo&&o.qinfo.wqid?"/1":"";\
    imgo= o.image && o.image.length ? o : (o.source && o.source.image && o.source.image.length ? o.source : 0);\
    if (o && imgo) {\
      if (imgo.count >= 200 && showHot) {\
  ...
  <div class="imgView" id="<%=o.id%>" rel="<%=o.timestamp%>"<%=hotstyle%>>\
    ...
    <a href="<%=MI.getInternalUrl(1, o.id, urlsuffix)%>" target="<%=MI.hostType==3?"_self":"_blank"%>" class="clk-1"><img src="<%=imgo.image[0]%>/160" alt="" onclick="" resizeto="fitfill,<%=fitWidth%>x<%=fitHeight%>;max,999x999"' + (MI.Crs.onLoadResize ? ' onload="MI.Crs.onLoadResize(this)"' : '') +' /></a>\
    <div class="imgModMore" style="display:none;">\
      <div class="imgView"><a href="<%=MI.getInternalUrl(1, imgo.id, urlsuffix)%>" target="<%=MI.hostType==3?"_self":"_blank"%>" class="clk-2"><img alt="" class="srs" srs="<%=imgo.image[0]%>/460" width="100%"></a></div>\
      <div class="msgWrap clear"><div class="userPic"><a title="" href="<%=MI.getInternalUrl(0, o.name)%>" rel="<%=nick%>(@<%=o.name%>)"><img src="<%=o.pic%>" card="1" ctype="1" title=""></a></div>\
        <div class="msgBox"><div rel="<%=o.name%>" class="userName"><strong><a title="<%=nick%>(@<%=o.name%>)" href="<%=MI.getInternalUrl(0, o.name)%>" card="1" ctype="2" rel="<%=nick%>(@<%=o.name%>"><%=nick%></a><%=o.icon%>' + MIIcon('o.flag')+':</strong></div><div class="msgCnt"><%=o.content%></div><% if(o.type===2) {%>\
    <div class="replyBox" style="clear:both"><div class="msgBox" rel="<%=o.source.name%>"><div class="msgCnt"><strong><em>'+_('[原文]')+'</em>\n<a rel="<%=o.source.name%>" ctype="2" card="1" href="<%=MI.getInternalUrl(0, o.source.name)%>" title="<%=o.source.bkname||o.source.nick%>(@<%=o.source.name%>)"><%=o.source.bkname||o.source.nick%></a><%=o.source.icon%>'+MIIcon('o.source.flag')+':</strong><%=o.source.content%></div></div></div><% } %>\
        <div class="pubInfo"><span class="left"><a from="3" href="<%=MI.getInternalUrl(1, o.id, urlsuffix)%>" target="<%=MI.hostType==3?"_self":"_blank"%>" class="time" rel="<%=o.timestamp%>" title="<%=dt%>"><%=o.time%></a><% if (imgo.count>0) { %><a class="zfNum clk-rc" target="<%=MI.hostType==3?"_self":"_blank"%>" href="<%=MI.getInternalUrl(1, imgo.id, urlsuffix)%>">'+_('查看全部转播和评论')+'(<b class="relayNum"><%=imgo.count%></b>)</a><% } %> </span></div></div>\
      </div>\
    </div>\
    </div>\
    <% }} %>';
