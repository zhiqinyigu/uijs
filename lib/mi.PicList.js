...
MI.PicList.prototype.removeRepeat = function (info) {
  ...
  for(i = 0; i < o.length; i++){
    ...
    */
    ...
    if (!imgurl) {
      imgurl = item.source.image.length > 0 ? getImageHash(item.source.image[0]) : false;
    }
...
MI.PicList.prototype.rePosDlg = function (wrap) {
  ...
  */
  ...
  arrl.style.left = (winxy[0] - MI.PicList.MODAL_WIDTH) / 2 + 'px';
  arrr.style.left = (winxy[0] + MI.PicList.MODAL_WIDTH) / 2 - 109 + 'px';
  ...
  */
  ...
  this._elClose.style.left = (winxy[0] + MI.PicList.MODAL_WIDTH) / 2 +
    'px';
...
MI.PicList.prototype.showModalDlg = function (id, justChangeContent) {
  ...
  mediaWrap.innerHTML = '<div class="picBox big" onmouseover="this.firstChild.style.visibility=\'visible\'" onmouseout="this.firstChild.style.visibility=\'\'"><div class="tools"><a href="javascript:void(0)" class="btnBack clk-rotateimg"><em></em>向左转</a><span>|</span><a href="javascript:void(0)" class="btnPrev clk-rotateimgcke"><em></em>向右转</a><a class="btnOriginal" href="javascript:void(0)" target="_blank">查看原图</a><span class="mask"></span></div>' + imgA.innerHTML + '</div>';
  msgCnts = UI.GC(tmpDiv, '.msgCnt');
  UI.after(mediaWrap, msgCnts[1] || msgCnts[0]);
  UI.remove(imgA);

  if (justChangeContent) {
    dlg._cont.firstChild.innerHTML = tmpDiv.innerHTML;
    this._showArrow();
  } else {
    dlg.show({
      html : '<div class="imgModTips LC" style="border:0;overflow:hidden;"></div>',
