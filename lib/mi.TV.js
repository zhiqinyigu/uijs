...
MI.TV = {
	...
	voteUpdate : function(){ //更新数据
		...
		UI.get(Self.voteXhr,'ids=' + Self.voteIds + '&r=' + UI.random(),function(data){
			...
			if (data.result == 0 && Self.voteStop < 1){
				...
				if (!Self._voteList){
					...
					for (var i in data.info){
						...
						html.push('<td><div class="rateWrap c' + index + '"><div class="rateBox" style="height:' + data.info[i].count + '%"><div class="tface"><div class="rateName">' + data.info[i].count + '%</div></div><div class="mface"></div><div class="bface"></div></div></div><div class="userPic"><img src="' + data.info[i].url + '/100" />' + data.info[i].nick + '<br><span>' + index + '</span></div></td>');
					}
					html.push('</tr>');
					...
					for (var i in data.info){
						...
						Self.voteDelay[index] = UI.animate(Self._voteListBox[index],'height',data.info[i].count * Self.voteBoxScale);
