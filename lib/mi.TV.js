/**
 * MI TV Author : xhlv@tencent.com Datetime : 2010-11-26 Last Eidt:
 */
$ = UI.G;
$$ = UI.GC;
MI = {
		...
		UI.ajax({
			...
			success : function(data) {
				...
				if (data.result == 0 && Self.voteStop < 1) {
					var html = [], index = 0, total = 0, hasZero = 0, num = 0, isDemo;
					if (Self.voteType == 1) { // 百分比类型
						...
						if (Self.voteUpdateTimes > 0 && Self.voteDemoTimes > 0) {
							// 伪造票数据
							for (var i in data.info) {
								if (Math.random() < Self.voteDemoRandom) {
									if (Self.voteNum[i]) {
										Self.voteNum[i] += Self.votePerHeight;
									} else {
										Self.voteNum[i] = Self.votePerHeight;
									}
								}
					...
					if (!Self._voteList) {
						...
						for (var i in data.info) {
							...
							Self.voteDelay[index] = UI.animate(
									Self._voteListBox[index], 'height',
									(Self.voteType == 1
											? data.info[i].count
													* Self.voteBoxScale
											: data.info[i].count));
