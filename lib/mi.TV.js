...
MI.TV = {
	...
	tmpl : {
		list : '<%for(var i=0,n=talk.length;i<n;i++){MI.TV.talks[talk[i].id]=talk[i];%><li id="<%=talk[i].id%>">\
					...
					<div class="msgBox">\
						<%for(var j=0;j<talk[i].image.length;j++){%>\
							<p class="pic"><span><img src="<%=talk[i].image[j]%>/160" onload="if(this.width>152)this.width=152;if(this.height>152)this.height=152"></span></p>\
						<%}%>\
		...
		talk : '<div class="<%=(image.length ? "cntImg" : "cntTxt")%>">\
					...
					<div class="msgBox">\
						<%if(image.length){%><p class="pic" onclick="MI.TV.showPic(\'<%=image[0]%>\')"><span><img src="<%=image[0]%>/460" onload="if(this.width>350)this.width=350;if(this.height>530)this.height=530" /></span><b class="copySign">腾讯微博</b></p><%}%>\
