...
UI.DatePicker = {
	...
	show : function(e,o){ //Parameter:target,lang,format,self,disabledWeekDays,disabledWeeks,disabledDays,showWeek,selectWeek,call
		...
		this.startDay = o.startDay ? (UI.parseDate(o.startDay,this.format).getTime() || 0) : 0;
		this.endDay = o.endDay ? (UI.parseDate(o.endDay,this.format).getTime() || Infinity) : Infinity;

		//Mark Days
		this.markDays = {};
		if (o.markDays) {
			for (var i in o.markDays) {
				this.markDays[UI.parseDate(i,this.format).getTime() || Infinity] = o.markDays[i];
			}
		}
			...
			if (this.format.hasString('d')) {
				for (var col = 1;col < 8;col++) {
					var disabled = false,
						...
						markTitle = '',
						markClassName = '';
					if (this.markDays[displayDateTime]) {
						markClassName = ' ' + this.markDays[displayDateTime].className || '';
						markTitle = this.markDays[displayDateTime].title || '';
					}
					...
					if (disabled || this.disabledWeekDays[col - 1] || this.disabledDays[display_date - 1] || this.disabledWeeks[row] || (col == 1 ? this.disabledWeekDays[7] : false) || displayDateTime < this.startDay || displayDateTime > this.endDay) {
						html.push('<td title="' + markTitle + '" class="' + dateClass + markClassName + ' disabled" rel="' + display_year + ',' + display_month + ',' + display_date + '"><span>' + display_date + '</span></td>');
						...
						html.push('<td title="' + markTitle + '" class="' + dateClass + markClassName + ((col == weekEndA || col == weekEndB) ? ' week_end' : '') + '" onmouseover="UI.addClass(this,\'hover\');" onmouseout="UI.removeClass(this,\'hover\');" onclick="' + O + '.daySelected(this.getAttribute(\'rel\'));" rel="' + selected_year + ',' + selected_month + ',' + selected_date + ',' + wNumTrue + '"><span>' + display_date + '</span></td>');
					}
					display_date++;
