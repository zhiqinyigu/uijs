...
MI.Face = {
	css : '.setFaceSuc{width:390px;padding:5px 0;background:#fff}\
		...
		.cusFaceList{width:456px;margin:15px 0 20px 9px}\
		...
		.cusHisface .talkWrap{padding:9px 10px 0;border-top:1px solid #E5E5E5}',
...
//			}
	...
	view : function(el) { // 查看客人页大头像
		...
		if (!Self.talkBox) {
			...
			Self.talkBox.sourcePic = Self.curPic + '/0';
			$$(Self.talkBox._body, '.replyTitle')[0].innerHTML = _('说说看你对这个头像的看法吧：');
			Self.talkBox.successStart = function() {
				MI.tip(_('发表成功'));
			}
			...
			var html = [], empty, list = $$(Self.talkBox._body,
					'.cusFaceList ul')[0];
			if (Self.data && Self.data.length > 1 && Self.isPriv) {
		...
		MI.dialog.show({
					...
					start : function() {
						setTimeout(function() {
									 ...
									 * Self.viewUser + '?face';
									 */
									Self.talkBox._txt.value = _('#品头论像#一起来围观@{0} 的头像~  http://t.qq.com/{1}?face',Self.viewUser,Self.viewUser);
									//MI.focus(Self.talkBox._txt);
									Self.talkBox.countTxt();
								}, 0);
					},
