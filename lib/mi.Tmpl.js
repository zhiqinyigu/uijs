...
MI.tmpl.listAll = '<%var shield=shield;D=MI.TalkList.date,API=MI.api.type?"?apiType="+MI.api.type:"";for(var i=0,n=talk.length;i<n;i++){\
		var sid=talk[i].id,numAll,numRelay,numComt,numRelay2;numRelay=talk[i].counts[0]||0;numComt=talk[i].counts[1]||0;numRelay2=talk[i].counts[2]||0;numComt2=talk[i].counts[3]||0;\
		...
		numAll=numRelay+numComt;%><li id="<%=talk[i].id%>"<%if(fav){%> fav="<%=talk[i].favortimestamp%>"<%}%> <%if(shield){%> shield="<%=talk[i].shield%>"<%}%> rel="<%=talk[i].timestamp%>" <%if(talk[i].qinfo){%>wqid="<%=talk[i].qinfo.wqid%>"<%}%>>\
			...
			<div class="msgBox"><%\
				...
				<%}%>\
				<div class="pubInfo"><span class="left"><%if(talk[i].type==8){\
						%><em class="ico_sfeed"></em> <em class="time"><%=talk[i].time%></em><%\
					}else if(MI.user.fun.btnStyle==6){\
					 ...
					 <em class="zfNum">' + _('全部转播和评论') + '(<b class="relayNum"><%=numAll%></b>)</em><%}}%><%if(!MI.user.fun.newRelay2 && talk[i].type == 2 && numRelay2){%>\
					...
					<a class="view" href="http://t.qq.com/p/r/<%=talk[i].id%>">' + _('查看对话') + '</a><%}%><%if(MI.user.fun.fastReport == 1 && (talk[i].type == 2 || talk[i].type == 7) && talk[i].name != MI.user.account && '+V1+'.name == MI.user.account){%> <a'+V5+' class="alarm">' + _('删除') + '</a><%}%></span><div class="funBox">\
						<%if(MI.user.account && talk[i].type != 8){\
							if(talk[i].type==7 && MI.user.fun.comtReply){\
								%><a href="/p/t/<%=sid%>" class="comt">' + _('回复') + '</a><%\
							}else{\
								if(MI.user.fun.btnStyle!=6){%><a'+V5+' class="relay" num="<%=(MI.user.fun.newRelay2 && numRelay2 ? numRelay2 : numRelay)%>">' + _('转播') + '<%=(MI.user.fun.newRelay2 && numRelay2 ? "(" + numRelay2 + ")" : "")%></a><span>|</span><%}\
								if(MI.user.fun.btnStyle==5||MI.user.fun.btnStyle==6){\
									...
									%><a href="/p/t/<%=sid%>" class="comt" num="<%=(MI.user.fun.newRelay2 && numComt2 ? numComt2 : numComt)%>">' + _('评论') + '<%=(MI.user.fun.newRelay2 && numComt2 ? "(" + numComt2 + ")" : "")%></a><%\
								}\
							}\
							%><span>|</span><%\
							if(MI.user.fun.btnStyle!=6){%><div class="mFun"><a href="#">' + _('更多') + '<em class="btn_ldrop"></em></a><div class="mFunDrop"><b></b><b class="mask"></b>\
								<%if((MI.user.fun.btnStyle==1 || (MI.user.fun.btnReply && talk[i].type==4)) && !(talk[i].type==7 && MI.user.fun.comtReply)){\
									%><p><a href="/p/t/<%=sid%>" class="comt" num="<%=numComt%>">' + _('评论') + '<%=(MI.user.fun.newRelay2 && numComt2 ? "(" + numComt2 + ")" : "")%></a></p><%\
				...
				<%if(0 && talk[i].type==7 && MI.user.fun.comtReply){%>\
					<div class="inputTxt comtOrg">' + _('回复') + '@<%=talk[i].name%>:</div>\
				<%}%>\
				<%var at=talk[i].at, atNum=talk[i].atNum;if(talk[i].atAll && talk[i].atAll > 1){%>\
					<div class="tipsBox1 at_detail">\
						<a href="#" rel="0" <%=(atNum[0]?"num=" + atNum[0] + "":"")%>>' + _('全部提到我的') + '<em <%=(at[0]?"":"class=checked")%>><%=(at[0] ? "(+" + at[0] + ")" : atNum[0] ? "(" + atNum[0] + ")" : "" )%></em></a> |\
						<a href="#" rel="1">' + _('收听的人') + '<em><%=(at[1] ? "(+" + at[1] + ")" : "")%></em></a> |\
						<a href="#" rel="2">' + _('认证用户') + '<em><%=(at[2] ? "(+" + at[2] + ")" : "")%></em></a>\
					</div>\
				<%}%>\
...
MI.tmpl.face = '<div class="D3 faceWrap" style="display:none">\
	<div class="tabStyle3"><ul>\
	...
	<div class="faceBox">\
		<div class="faceCell">\
			...
			<div class="pFace">\
				<div><a href="#" title="' + _('万圣节') + '" class="f161"></a><a href="#" title="' + _('红包') + '" class="f151"></a><a href="#" title="' + _('喜糖') + '" class="f150"></a><a href="#" title="' + _('百合花') + '" class="f138"></a><a href="#" title="' + _('黄丝带') + '" class="f137"></a><a href="#" title="' + _('祈福') + '" class="f136"></a><a href="#" title="' + _('黑丝带') + '" class="f135"></a><a href="#" title="' + _('月饼') + '" class="f152"></a><a href="#" title="' + _('酒') + '" class="f153"></a><a href="#" title="' + _('蛋黄月饼') + '" class="f156"></a><a href="#" title="' + _('玉兔') + '" class="f155"></a><a href="#" title="' + _('灯笼') + '" class="f157"></a><a href="#" title="' + _('红旗') + '" class="f158"></a><a href="#" title="' + _('日历') + '" class="f159"></a><a href="#" title="' + _('七休哥') + '" class="f160"></a></div>\
			</div>\
		</div>\
	</div>\
