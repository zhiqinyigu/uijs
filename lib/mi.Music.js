...
MI.Music = {
	...
	build : function(talk){
		...
		Self.newMusicBox = MI.user.fun.newMusicBox;
		Self.talkBox = talk;
		...
		//Self._music = talk;
		//Self._musicAnchor = talk.$('.newMusic');
		//Add QQ Music
		//build music dom
		...
		var html1 = '<div>' + (Self.newMusicBox ? '\
			<iframe class="musicCont" height="40" ' + autoHeight + ' frameborder="0" scrolling="no" src="http://music.qq.com/musicbox/app/mblog_share_song.html" src="about:blank"></iframe>' : '') + '\
			<div class="m_searchBox clear" style="display:' + (Self.newMusicBox ? 'none' : '') + '"><input type="text" class="inputTxt musicSearchBoxInput" value="" autocomplete="off" /><button type="button" class="btn2">' + _('搜索') + '</button></div>\
		</div>';

		var html2 = '<div>' + (Self.newMusicBox ? '\
				<iframe class="musicCont" height="40" ' + autoHeight + ' frameborder="0" scrolling="no" src="http://music.qq.com/musicbox/app/mblog_share_list.html" src="about:blank"></iframe>' : '') + '\
			</div>';		
		var upM = talk.upMusic;

		UI.prepend(UI.html(html1)[0],upM._div[0]);
		UI.prepend(UI.html(html2)[0],upM._div[1]);

		//Dom
		talk._musicSearchBox = $$(upM._div[0],'.m_searchBox')[0];
		talk._musicSearchKey = $$(upM._div[0],'.musicSearchBoxInput')[0];
		talk._musicSearchBtn = $$(upM._div[0],'.btn2')[0];
		...
		//Event
		UI.EA(talk._musicSearchKey,'keydown',function(e){
			var E = UI.E(e);
			if(E.key == 13){
				UI.E(e).stop();
				talk._musicSearchBtn.click();
				talk._musicSearchKey.blur();
				Self.stop('',true);
			}
		});
		UI.EA(talk._musicSearchKey,'focus',function(e){
			var E = UI.E(e);
			Self.switchSearchKey();
		});
		UI.EA(talk._musicSearchKey,'blur',function(e){
			var E = UI.E(e);
			Self.switchSearchKey();
		});
		talk._musicSearchBtn.onclick = function(){
			Self.searchMusic(talk._musicSearchKey.value, 0);
			if (talk._rSearch && talk._rSearchPage && talk._rSearchPage.target){
				UI.remove(talk._rSearchPage.target);
				talk._rSearchPage = null;
			}
			Self.stop('',true);
			return false;
		}
		...
		MI.app({
			Base : function(){
				talk.musicBox = new MI.SmartBox(talk._musicSearchKey,'','music',{left:0,top:2,width:212});
				talk.musicBox.url = 'http://soso.music.qq.com/fcgi-bin/cgiSearchKeyWordMusicBox';
				talk.musicBox.param = 'utf8=1&w=%key%';
			}
		});
		//Self.show();
		talk._rSearch = $$(upM._div[0],'.rSearch')[0];
		Self.builded = true;
		Self.talkBoxList.push(talk);
	},
	...
	bindSwitchTabs : function(){
		var Self = this,tabs = Self.talkBox.upMusic._tab;
	...
	bindSwitchTabsClick : function(o,num){
		var Self = this;
		if(!o)	return;
		UI.A(Self.talkBox.upMusic._box,'tabs',num);
		if(Self.lastTabs != o){
			Self.lastTabs = o;
	...
	switchTabs : function(idx){
		var Self = this;
		if(idx == 2 || MI.user.fun.newMusicBox) return;  //录音
		if(!Self.talkBox.hasFav && idx==1) Self.favMusic();  //切换到收藏时再请求
	},
	tmpl : {//
		listAll : '\
				<%if(retcode == 0){%>\
	...
	favMusicCallBack : function(data){
		...
		if(data){
			...
			//if (!Self._rFav){
				// 准备收藏结果层
				Self.talkBox._rFav = Self.talkBox.upMusic._div[1];
			//} 
			// 还没有分页条
			if (!Self.talkBox._rFavPage){
				// 准备分页条
				pages = UI.html('<div class="pages"></div>')[0];
				// 将分页条添加到结果层
				UI.prepend(pages, Self.talkBox._rFav);
				// 构造分布栏
				Self.talkBox._rFavPage = new MI.Page({
					target : pages,
					call : function (i) {
						// 显示第i页的内容
						Self.favPage(i);
			...
			// 显示第一页 无论返回结果是否正确都要显示 这样可以显示错误信息  //SongNum==0的时候，Self.talkBox._rFavPage的call提前return了
			Self.talkBox._rFavPage.show(1, Math.ceil(data.SongNum / Self.PageNum));  
			// 如果返回值不正确&列表中存在少于一页的歌曲 则删除可能存在的分页条
			if ( !!data.code || data.SongNum < Self.PageNum) { 
				if (Self.talkBox._rFavPage && Self.talkBox._rFavPage.target){
					UI.remove(Self.talkBox._rFavPage.target);
					Self.favPage(1);
					Self.talkBox._rFavPage = null;
				}
			}
			noerr = true;
		}else{}
	...
	favPage : function(page){
		...
		UI.each(favResult, function(o){
			if (UI.isElement(o)){
				UI.append(o, frag);
			}
		});
		var m_total = $$(Self.talkBox._rFav, '.m_total')[0];
		var m_list = $$(Self.talkBox._rFav, '.m_list')[0];
		var m_noFav = $$(Self.talkBox._rFav, '.m_noFav')[0];
		if (m_total){
			UI.remove(m_total);
		}
		if (m_list){
		...
		UI.prepend(frag, Self.talkBox._rFav);
		var oSelect = $$(Self.talkBox._rFav,'select')[0];
		oSelect.onchange = function(){
			Self.favMusic(this.value);
		}
		Self.bindSongClickEvent(Self.talkBox._rFav, 1);
	},
	searchMusic : function(key,p,n){
		...
		*/
		...
		Self.talkBox.musicBox && Self.talkBox.musicBox.hide();
	...
	searchCallBack : function(data){
		...
		try{
			if(data){
				...
				// 搜索结果层不存在
				if (!Self.talkBox._rSearch){
					Self.talkBox._rSearch = Self.talkBox.upMusic._div[0];
				}
				if (!Self.talkBox._rSearchPage){
					// 准备分页条
					pages = UI.html('<div class="pages"></div>')[0];
					// 将分页条添加到结果层
					UI.after(pages, Self.talkBox._musicSearchBox);
					// 构造分布栏
					Self.talkBox._rSearchPage = new MI.Page({
						target : pages,
						num : 3,
						max : 40,
						call : function (i) {
					...
					Self.talkBox._rSearchPage.show(0, Math.ceil(data.totalnum / Self.PageNum), false);
				} else {
				...
				// 如果返回值不正确&列表中存在少于一页的歌曲 则删除可能存在的分页条
				if ( !!data.retcode || (data.totalnum < Self.PageNum) || (data.list.length < Self.PageNum)) {
					if (Self.talkBox._rSearchPage && Self.talkBox._rSearchPage.target){
						UI.remove(Self.talkBox._rSearchPage.target);
						Self.talkBox._rSearchPage = null;
					}
				}
				noerr = true;
			}else{}
	...
	searchPage : function (data) {
		...
		UI.each(searchResult, function(o){
			if (UI.isElement(o)){
				UI.append(o, frag);
			}
		});
		var pages = $$(Self.talkBox._rSearch,'.pages')[0];
		var m_total = $$(Self.talkBox._rSearch, '.m_total')[0];
		var m_list = $$(Self.talkBox._rSearch, '.m_list')[0];
		var m_noResult = $$(Self.talkBox._rSearch, '.m_noResult')[0];
		if (m_total){
			UI.remove(m_total);
		}
		if (m_list){
		...
		UI.before(frag,pages);
		
		Self.bindSongClickEvent(Self.talkBox._rSearch, 1);
	//	if (Self.lastTabs.index != 0)
	//		UI.hide(Self.talkBox._rSearch);
	//	else{
	//		UI.show(Self.talkBox._rSearch);
	//	}
	},
	...
	addPreview : function(o,fav,type){
		//type: 1-录音 2-歌曲 3-歌单
		//music{song_id,songurl,singer_name,song_name}
		var Self = this;
		...
		UI.append(Self.talkBox._musicPreview,Self.talkBox._sendStatus);
		...
		//如果有录音按钮，同时置灰录音按钮
		if(Self.talkBox._recordAnchor) {
			UI.addClass(Self.talkBox._recordAnchor,'disabled');
		}
		...
		del.onclick = function(e){
			...
			(UI.children(Self.talkBox._sendStatus,'sendThumbs').length < 2) && (!Self.talkBox._picPreview || (Self.talkBox._picPreview.style.display == 'none')) && UI.removeClass(Self.talkBox._sendCnt,'hasMedia');
			MI.Bos('btnMusicPreviewDel');
			return false;
		}
		Self.talkBox.upMusic.hide();
		...
		MI.ajax({
			...
			success : function(data){
				...
				Self.talkBox.guideTextRemove();
				Self.talkBox.music.shorturl = shorturl;
				Self.talkBox.txtMusic = topic + ' #'
								+ UI.trim(m[3].replace(/\&quot;/gi, '"'))
								+ (m[2] ? '-' : '')
								+ UI.trim(m[2].replace(/\&quot;/gi, '"')) + '#'
								+ (type != 3 ? url : '') + ' ';
				var value = Self.talkBox._txt.value;
				if(!value || (value && !value.hasString(Self.talkBox.txtMusic))){
	...
	smartBoxCallback : function(data){
		...
		var Self = this,musicBox;
		try{
			musicBox = Self.talkBox.musicBox;
			if(data.head.error == 0 && data.item){
				if(data.item.length > 0){			
					...
					if (UI.C(Self.talkBox.upMusic._box, 'display') !== 'none'){
						musicBox.show();
					}
		...
		}catch(e){musicBox && musicBox.hide();}
	},
	...
	clear : function(){
		var Self = this,
			tabs = Self.talkBox.upMusic._tab;
		Self.stop();
		Self.talkBox._musicSearchKey.value = '';
		Self.switchSearchKey();
		Self.bindSwitchTabsClick(tabs[0],0);
//		UI.each($$(Self.talkBox.upMusic._box,'.m_sResult'),function(o){
//			UI.remove(o);
//		});
	},
	show : function(){
		//this.talkBox.upMusic.show();
		try{
	...
	hide : function(){
		UI.hide(this.talkBox.upMusic._box);
		this.stop();
	}
}
...
MI.TalkList.musicPlay = function(music){
	if (MI.user.fun.newMusic){
		...
		//添加收起
		if (!UI.A(mBoxNew,'tools')){
			var tools = UI.html('<div class="vTools bor_bg clear"><a class="vClose" href="#"><em></em>收起</a></div>')[0];
			tools.onclick = function(){
				MI.TalkList.musicStop();
				return false;
			}
			UI.before(tools,mBoxNew);
			UI.A(mBoxNew,'tools',1);
		}
		UI.addClass(MI.TalkList.lastMusic,'large');
		
		var play = function(){
			if (isMlist){ //播放歌单
				window.MUSIC.ICMusic.getMusicListFlash(music.mlistUin,music.mlistUrl,mBoxNew);
			}
			else { //播放歌曲
				/*
					music={
						...
						songName: "寂寞是你给的苦"
						songtype: "1"
						songurl: "http://stream4.qqmusic.qq.com/12815591.wma"
					}
					qqMusic={
						name:"歌曲名",
						singer:"歌手名",
				...
				*/
				...
				if (!music.songID || !music.songurl.hasString('qqmusic.qq.com')){ //非QQ音乐库音乐
					if (music.songurl.hasString('wbyp.store.qq.com')){
						qqMusic.pic = 'http://mat1.gtimg.com/www/mb/images/music_record_none.jpg';
					}
				}
				window.MUSIC.ICMusic.getMusicFlash(qqMusic,mBoxNew);
			}
		}
		if (mBoxNew){
			if (mBoxNew.style.display == 'none'){ //播放器已经加载过
				UI.show(mBoxNew);
			}
			else {
				if (window.MUSIC && MUSIC.BQplay){
					play();
				}
				else {
...
MI.TalkList.musicStop = function(a){
	if (window.MUSIC && window.MUSIC.ICMusic && window.MUSIC.ICMusic.hideFlash){ //关闭Flash版音乐播放
		MUSIC.ICMusic.hideFlash();
		if (MI.TalkList.lastMusic){
			UI.removeClass(MI.TalkList.lastMusic,'large');
		}
		return;
	}
	var musicBoxCur = MI.TalkList.lastMusic,
...
MI.TalkList.musicClose = function(a){
	...
	if (MI.user.fun.newMusic){
		var mThumbsBox = $$(musicBox,'.albumInfo .mThumbsBox')[0],
			mBoxNew = $$(musicBox,'.mBoxNew')[0];
		if (mThumbsBox){ //歌单
			UI.show($$(musicBox,'.albumPic')[0]);
			UI.show($$(musicBox,'.albumInfo')[0]);
		}
		else { //歌曲
			UI.show($$(musicBox,'.mThumbsBox')[0]);
		}
		//UI.hide(mBoxNew);
	}
	...
	MI.TalkList.musicMarqueeStop();
	try{
		var mThumbsBox = $$(musicBox,'.mThumbsBox')[0],
		mBox = $$(musicBox,'.mBox')[0],
...
//IE will auto start after setProcess
      ...
      listenall: function() {
        ...
        UI.each($$('#talkList .songInfo'), function(e) {
          ...
          */
          arr.push([songid, UI.A(e,'songtype'), UI.A(e,'songurl'), UI.A(e,'song'),
            '', UI.A(e,'singer'), '', '', '', '']);
        });
        ...
        ownUin = UI.cookie('o_cookie') || UI.cookie('luin') || UI.cookie('uin');
        ownUin = ownUin.replace(/^o0*/, '');
        this.addSoSoPlaylist(arr, {
          u : ownUin
        });
      },
      nextbunch: function() {
...
//add _delayNextPlayed property to avoid it
    ...
    playNext : function () {
      var alllis = this.els.noDupLists,
        next,
        current,
        oldCurrent;
    ...
    _updateAlbumPic : function (id) {
      ...
      if (aid) {
        ...
        UI.getScript('http://portalcgi.music.qq.com/fcgi-bin/music_mini_portal/cgi_query_download.fcg?id=' + id, function() {}, 'gb2312');
      }
    },
...
//prevent auto jump to next with _progressSetted
    ...
    genRecommandWall: function(usersArray) {
      ...
      UI.each($$(listenerList, 'li img'), function (elem) {
        var url = elem.src;
        if (url && url.substr(url.length - 3) === '/50') {
...
//happens if user's list is empty
	  ...
	  els.noDupLists = [];
	  var tempMap = {};
      UI.each(els.lists, function(o, i) {
	    var songid = UI.A(UI.GC(o, '.songInfo')[0], 'songid');
		if (songid && !tempMap[songid])
		{
		  tempMap[songid] = 1;
		  els.noDupLists[els.noDupLists.length] = o;
		}
	  });
...
*/
    ...
    _hlScrollResize : function () {
      var listPos, scrollY, ctn, newNavHeight,
        els = _musicObj.els;
      ...
      newNavHeight = UI.GC(document.body, '.headWrap').length ? 35 : 0;

      if (_ieVer !== 6) {
        if (listPos - scrollY - newNavHeight < 117) {//117, the height of the player box
          if (!_musicObj._boxFixed) {
            ctn.style.position = 'fixed';
            //ctn.style.top = '5px';
            ctn.style.top = -5 + newNavHeight + 'px';
...
// " + newNavHeight + " +
    ...
    addSoSoPlaylist : function (aList, oPostData) {
      var url = "http://qzone-music.qq.com/fcg-bin/fcg_soso_addsong.fcg";

      var data = {
        formsender : 1,
        uin : oPostData.u,
        songlist : this.getPostData(aList)
      };

      _musicObj.sender.enable();
      var fs = new MUSIC.FormSender(url, "post", data, "GB2312");

      fs.onSuccess = function(o) {
        _musicObj.sender.disable();
        if (!o || typeof(o) != "object") {
        } else {
          switch (parseInt(o.code, 10)) {
          case 0 :
            oPostData.k = o.key;
            break;
          default :
        ...
        _musicObj.openPlayerWindow(o.key, oPostData.u);
      };

      fs.onError = function () {
        _musicObj.sender.disable();
        _musicObj.openPlayerWindow('', oPostData.u);
      };

      if(!_musicObj.isAddingSosoPlaylist) {
        _musicObj.isAddingSosoPlaylist = true;
        this.openEmptyWindow();
        fs.send();
      }
    },

    openPlayerWindow : function (k, uin) {
      var html =
        ...
        '<input name="songList" type="hidden" value="" />' +
        '<input name="u" type="hidden" value="' + uin + '" />' +
        '<input name="k" type="hidden" value="' + k + '" />' +
        '<input name="userName" type="hidden" value="' + MI.user.account + '" />' +
        '<input name="playXsosoFlag" type="hidden" value="0" />' +
      ...
      UI.remove(newdiv);
      _musicObj.isAddingSosoPlaylist = false;
    },

    openEmptyWindow : function (sWindowName) {
    ...
    sender : {
      enable : function () {
         ...
         * @param {Object} [data] hashTable形式的对象
         * @param {String} [charset="gb2312"] 于后台数据交互的字符串
         * @constructor
         *
        ...
        MUSIC.FormSender = function(actionURL, method, data, charset) {
          ...
          this.proxyURL = 'http://imgcache.qq.com/music/qzone/fp_' +
            ((typeof charset === 'string' && charset.match(/utf-?8/i)) ?
              'utf8.html' : 'gbk.html');
          /*
          this.proxyURL = 'http://t.qq.com/fs_' +
            ((typeof charset === 'string' && charset.match(/utf-?8/i)) ?
              'utf8.html' : 'gbk.html');
            */
        ...
        MUSIC.FormSender.prototype.send = function() {
          ...
          function clear(o) {
            ...
            if (UI.B.safari || UI.B.opera) {
              setTimeout(function () {
                UI.remove($('_fp_frm_' + o.name));
              }, 50);
            } else {
              UI.remove($("_fp_frm_" + o.name));
            }
          ...
          if (this._sender === null || this._sender === void(0)) {
            ...
            sender.callback = function(o) {
              clearInterval(interval);
              clear(that);
              that.onSuccess(o);
            };

            sender.errorCallback = function(o) {
              clearInterval(interval);
              clear(that);
              that.onError(o);
            };
            ...
            if (typeof sender.onreadystatechange != 'undefined') {
              sender.onreadystatechange = function() {
                if (that._sender.readyState == 'complete' && that._sender.submited) {
                  clear(that);
                  that.onError(MUSIC.FormSender._errCodeMap[999]);
                }
              };
            } else {
              interval = setInterval(function() {
                try {
                  var _t = that._sender.contentWindow.location.href;
                  if (_t.indexOf(that.uri) === 0) {
                    clear(that);
                    that.onError(MUSIC.FormSender._errCodeMap[999]);
                    clearInterval(interval);
                  }
        ...
        /**
         * MUSIC.FormSender对象自毁方法，用法ins=ins.destroy();
         *
        ...
        MUSIC.dom.createNamedElement = function (type, name, doc) {
          ...
          try{element=_doc.createElement('<'+type+' name="'+name+'">');}catch(ign){}
  ...
  //volume
    ...
    init : function (id) {
      ...
      if (this.els) {
        this.selectedLi = this.els.noDupLists[0];
...
*/
        ...
        if (_ieVer === 6) {
          this._headHeight = UI.height(UI.G('headWrap') ||
            UI.GC(document.body, '.headWrap')[0]);
        }
      ...
      MI.app({
        Base : function () {
          var list = $$('.listenerList')[0];
          if (list) {
            UI.each($$(list, 'li img'), function (elem) {
              var url = elem.src;
              if (url && url.substr(url.length - 3) === '/50') {
                UI.EA(elem, 'mouseover', MI.Card.show);
              }
