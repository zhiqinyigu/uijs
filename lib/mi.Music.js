...
MI.Music = {
	...
	tmpl : {//
				...
				<%if(retcode == 0){%>\
					<%if(list.length > 0){%>\
						<div class="m_total clear">\
							<div class="left">' + _('搜索结果(<%=totalnum%>首)') + '</div>\
		...
		listAll2 : '<div class="m_sResult rFav">\
				<%if(code == 0){%>\
					<div class="m_total clear">\
						<div class="left"<%for(var i=0,n=DirList.length;i<n;i++){if(SongDirID == DirList[i].DirID){%> title="<%=DirList[i].DirName%>"><%=MI.string.cut(DirList[i].DirName,32)%><%;break;}}%>(' + _('首','<%=SongList.length%>') + ')</div>\
						<div class="right">\
							<%if(DirList.length > 0){%>\
							<select>\
