...
MI.tmpl.videoObject = '<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="460" height="372" ><param name="wmode" value="transparent" /><param name="movie" value="$VideoPlayUrl" /><param name="allowScriptAccess" value="never" /><param name="allownetworking" value="internal" /><param name="allowFullScreen" value="true" /><param name="flashvars" value="playMovie=true&amp;isAutoPlay=true&amp;auto=1"><param name="swfversion" value="50536.0.0.0" /><param name="expressinstall" value="Scripts/expressInstall.swf" /></object>';
MI.tmpl.videoEmbed = '<embed src="$VideoPlayUrl" quality="high" pluginspage="http://www.adobe.com/shockwave/download/download.cgi?P1_Prod_Version=ShockwaveFlash" type="application/x-shockwave-flash" width="460" height="372" allowScriptAccess="never" allowFullScreen="true" allownetworking="internal" flashvars="playMovie=true&amp;isAutoPlay=true&amp;auto=1" wmode="transparent"></embed>';
...
MI.Music = {
	...
	build : function(o){
		...
		//Add QQ Music
		//build music dom
		var html = UI.html('<div class="mloadWrap" style="display:none"><div class="musicTab"><ul><li class="select"><b>' + _('搜索音乐') + '</b></li><li><a href="#">' + _('我收藏的音乐') + '</a></li></ul><a href="#" class="close" title="' + _('关闭') + '">' + _('关闭') + '</a></div><div class="m_searchBox clear"><input type="text" class="inputTxt" id="musicSearchBox" value="" autocomplete="off" /><button type="button" class="btn2">' + _('搜索') + '</button></div></div>'),frag = document.createDocumentFragment();
	...
	tmpl : {//
		listAll : '<div class="m_sResult rSearch">\
				<%if(retcode == 0){%>\
					...
					<%}else{%>\
						<div class="m_noResult"><p>' + _('对不起，未找到"{0}"的相关歌曲。','<%=_Key%>') + '</p><p>' + _('请检查关键词是否正确，或更换其他关键字搜索。') + '</p></div>\
					<%}%>\
				<%}else{%>\
	...
	addPreview : function(o,fav){
		//music{song_id,songurl,singer_name,song_name}
		var Self = this;
		var prview = '<span class="preview"><span class="link"><a class="fileName" href="#"><span class="m_name">%song_name%</span><b class="m_Intro">%song_fullname%<span> - %singer_name%</span></b></a></span><a class="del" href="#" title="' + _('删除') + '">[' + _('删除') + ']</a></span>';
...
MI.TalkList.music = function(a,b){ //Music Event
	...
	//Paly All
	MI.TalkList.musicPlayAll.check(musicBox);
...
MI.TalkList.musicPlayAll = { // 自动播放下一首
	on : 1, // 是否可用
	cur : 0, // 当前序号
	list : [], // 播放列表
	build : function(){
		var Self = this;
		Self._body = UI.html('<button>自动播放所有歌曲</button>')[0];
	},
	play : function(){ // 播放下一首
		if (MI.TalkList.musicPlayAll.on) {
			if (Self.list[Self.cur + 1]) {
				MI.TalkList.music(Self.list[Self.cur + 1]);
			}
		}
	},
	check : function(music){
		var Self = this;
		if (Self.on) {
			if (Self.build) {
				Self.build();
...
MI.TalkList.videoOpen = function(vCon,cType,vUrl,vType){
	...
	if((UI.B.ipad && vType == 'flash') || MI.noFlash()){
