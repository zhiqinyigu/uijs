...
//add _delayNextPlayed property to avoid it
    ...
    playByLi : function (li) {
      var infoEl = UI.GC(li, '.songInfo')[0],
        sharerName = UI.GC(li, '.userPic a')[0].title;
    ...
    _startMarqee : function (isResume) {
      var outerCtn = this.els.songInfo,
      ...
      if (isResume) {
        ...
        UI.append(span.cloneNode(true), innerCtn);
        outerCtn.style.width = Math.min(spanwidth + 50, 330) + 'px';
        innerCtn.style.marginLeft = current + 'px';
        //innerCtn.style.width = Math.min(spanwidth + 50, 330) + 'px';
      }
    ...
    _stopMarqee : function (isPause) {
      var innerCtn;

      clearInterval(this._idMarq);
      if (!isPause) {
        innerCtn = this.els.songInfo.firstChild;
