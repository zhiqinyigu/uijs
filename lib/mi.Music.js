...
MI.Music = {
	...
	build : function(o){
		...
		o._musicClose = $$(o._musicBox,'.close')[0];
...
//IE will auto start after setProcess
    ...
    volume : {
      ...
      set : function (percent) {
        ...
        _musicObj.els.volBar.style.width = Math.round(percent * 62) + 'px';
...
//11 : 重新连接
    ...
    playNext : function () {
      ...
      setTimeout(function () {
        _musicObj.playByLi(alllis[next]);
      }, 200);
    },
    playPrev : function () {
      var alllis = this.els.lists,
    ...
    playByLi : function (li) {
      ...
      this.play(UI.A(infoEl, 'song'), UI.A(infoEl, 'songurl'),
        ...
        UI.GC(infoEl, 'a')[0].href);

      this.activeLi = li;
      this._updateLi();
    },

    play : function (name, url, id, singer, playerUrl) {
    ...
    _play : function (name, url, id, singer, playerUrl) {
      /*
        name : 歌曲名
        url : 歌曲url
        qusid : 进行url转换？
    ...
    _updateLi : function () {
      ...
      if (this.activeLi) {
        ...
        if (this.state === this.mapState.S_PLAYING) {
          ...
          UI.addClass(aEl, 'ico_mpl');
          UI.removeClass(aEl, 'ico_mst');
        }
      }
    },
    ...
    _formatTime : function (allSeconds) {
      var minutes, seconds;
    ...
    _updProgress : function () {
      ...
      if (this.state === map.S_PLAYING) {
        ...
        if (progress[1] !== 0) {
          played = progress[0];
          this.els.allTime.innerHTML = this._formatTime(progress[1]);
          this.els.playedTime.innerHTML = this._formatTime(played);
        }
    ...
    init : function (id) {
      ...
      infoEl = UI.GC(this.selectedLi, '.songInfo')[0];

      this.id = UI.A(infoEl, 'songid');
