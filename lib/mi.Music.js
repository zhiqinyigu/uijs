					...
					<div class="m_list">\
						...
						<%for(var i=(curnum*curpage),n=(curnum*curpage+MI.Music.PageNum) > SongList.length ? SongList.length : (curnum*curpage+MI.Music.PageNum);i<n;i++){%>\
							<%var musicid = SongList[i].id,\
								...
								str = MI.Music.cut(MI.Music.getHTML(songname ),MI.Music.getHTML(singername));%>\
							<%if(songurl){%><li music="<%=musicid%>,<%=songurl%>,<%=singername%>,<%=songname%>"><div><a href="#" title="点击添加-<%=songname%> - <%=singername%>"><%=str[0]%><span>- <%=str[1]%></span></a></div><a href="#" class="ico btn_play" id="play_<%=musicid%>" title="试听音乐"></a></li><%}%>\
						<%}%>\
						</ul>\
					</div>\
					<%}else{%>\
	...
	addPreview : function(o,fav){
		...
		UI.ajax({
			...
			success : function(data){
				data = MI.json(data);
				Self.talkBox.music.shorturl = data.url;
