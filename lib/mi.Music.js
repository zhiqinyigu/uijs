...
MI.Music = {
	...
	build : function(o){
		...
		var html1 = '<div>' + (Self.newMusicBox ? '\
			...
			<div class="m_searchBox clear" style="display:' + (Self.newMusicBox ? 'none' : '') + '"><input type="text" class="inputTxt" id="musicSearchBox" value="" autocomplete="off" /><button type="button" class="btn2">' + _('搜索') + '</button></div>\
		</div>';
		...
		//Dom
		o._musicSearchBox = $$(upM._div[0],'.m_searchBox')[0];
		o._musicSearchKey = $('musicSearchBox');
		o._musicSearchBtn = $$(upM._div[0],'.btn2')[0];
	...
	switchTabs : function(idx){
		...
		if(!Self.hasFav && idx==1) Self.favMusic();  //切换到收藏时再请求
	},
	tmpl : {//
		listAll : '\
				<%if(retcode == 0){%>\
	...
	favMusicCallBack : function(data){
		...
		if(data){
			...
			//if (!Self._rFav){
				// 准备收藏结果层
				Self._rFav = MI.talkBox.upMusic._div[1];
			//} 
			// 还没有分页条
			if (!Self._rFavPage){
				// 准备分页条
			...
			// 显示第一页 无论返回结果是否正确都要显示 这样可以显示错误信息  //SongNum==0的时候，Self._rFavPage的call提前return了
	...
	searchCallBack : function(data){
		...
		try{
			if(data){
				...
				// 搜索结果层不存在
				if (!Self._rSearch){
					Self._rSearch = Self.talkBox.upMusic._div[0];
				}
	...
	searchPage : function (data) {
		...
		var pages = $$(Self._rSearch,'.pages')[0];
			...
			if (isMlist){ //播放歌单
				...
				*/
				if (!music.songID || !music.songurl.hasString('qqmusic.qq.com')){ //非QQ音乐库音乐
					window.MUSIC.ICMusic.getMusicFlash({
						name : music.songName || '',
						singer : music.singerName || '',
						url : music.songurl,
						pic : music.songurl.hasString('wbyp.store.qq.com') ? 'http://mat1.gtimg.com/www/mb/images/music_record_none.jpg' : ''
					},mBoxNew);
				}
				else {
					window.MUSIC.ICMusic.getMusicFlash(music.songID,mBoxNew);
				}
			}
		}
		if (mBoxNew){
