...
MI.TalkList.prototype.creatBox = function(obj){//obj 微群新增参数，传递当前的li对象。默认展开的talklist无需创建talkbox，取现有元素即可
	...
	var relayListHtml = Self.talkBoxTmpl.replace('talkWrap',Self.type == 2 ? 'zfWrap comtWrap' : 'zfWrap').replace('<div class="left"></div>','<div class="left" style="display:;"><label style="display:none;"><input id="replayListCheckbox" type="checkbox"' + (isRelay ? ' checked' : '') + ' class="check1">' + _('同时转播给你的听众') + '</label> <label style="display:"><input type="checkbox" ' + (MI.user.fun.syncQzone ? ' checked' : '') + ' class="replayQzoneCheckbox check1">' +  _('同步到空间') + '</label></div>').replace('talkSuc','');
	if(!Self.insTalkbox){
