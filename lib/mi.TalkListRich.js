			...
			if (MI.user.isLab){
				var comtBox = Self.comtBox,
					...
					pageLink = function(data){
						...
						//Update Number
						var num = $$(comtList,'.num')[0],numValue,numDom;
						if (num){
									...
									try{
										...
										if (_replyCite) {
											_replyCite.onclick = function(){
												var account = $$(this.parentNode,'strong a')[0];
												if (account){
...
MI.TalkList.prototype.reply = function(id,type,number){ //type : 0(reply) 1(relay) 2(comt)
	...
	if (!Self.talkBox.display) {
		...
		$$(Self._talk,'.replyTitle')[0].innerHTML = curTitle;
...
MI.tmpl.urlPreview = '<div class="urlPreview"><div class="SA"><em>◆</em><span>◆</span></div><a class="close" href="javascript:void(0)"></a><div class="urlIntro"><a class="ulink" target="_blank" href="#"></a></div><div class="urlCnt"></div></div>';

MI.TalkList.prototype.urlPreview = (function(){
	var _body,_title,_cnt,_ifr,_close,MIN_H = 400,MIN_W = 360,MAX_W = 550,navH = 0,delay,lastSrc,cache = {};
	...
	function init(){
		...
		_arrow = $$(_body,'.SA')[0],
		navH = $$(document.body,'.tSearchNew')[0] ? UI.height($('header')) : 0 ;   //是否是新导航
...
/*
		   UI.addClass(_cnt,'loading');
		   _cnt.innerHTML = _('加载中...');
		   UI.removeClass(_cnt,'loading');
*/		
		lastSrc = src;
		_title.href = src;
		_cnt.innerHTML = info.html || _('很抱歉，没有预览');
		_title.innerHTML = info.title || _('无标题');
	}
	...
	function setPos(pos){
		var w,h,
			...
			winW = UI.windowWidth() - ((UI.B.ie && !UI.B.ie9) ? 0 : 20) - margin; //20为垂直滚动条宽度
		...
		_body.style.cssText = 'left:' + x + 'px;top:' + y + 'px;width:' + w + 'px;height:' + h + 'px;';
	...
	function bindEvent(){
		...
		//兼容取回的html中脚本报错
		window.imgLoad = window.imgLoad || function(){};
	...
	function showPreview(el){
		...
		if(cache[url]) {
			...
			MI.ajax({
				type : 'get',
				url : MI.url.urlPreview || '/aysn/urlPreview.php',
				data : {url : url},
				success : function(data) {
					data = MI.json(data);
					cache[url] = data;
	...
	function show(el,url){
		var P = UI.parents(el,'msgBox')[0].parentNode,
...
/*
MI.tmpl.download = '<div class="comTips1"><div class="downloadWrap"><div class="downloadBox"><p>' + _('请选择下载方式:') + '</p><a href="#">' + _('直接下载') + '</a><a href="#">' + _('QQ旋风') + '</a></div></div></div>';

MI.TalkList.prototype.download = (function(){
	UI.css('.comTips1 .downloadBox {padding:20px 30px;text-align:center;} .comTips1 .downloadBox p {padding:10px;}.comTips1 .downloadBox a {margin:0 10px;}');

	var body,wrap,dl,delayHide,pathCache = {};

	function init(){
		body = UI.html(MI.tmpl.download)[0];
		wrap = $$(body,'.downloadWrap')[0];
		dlBox = $$(body,'.downloadBox')[0];
		dlBtn = UI.GT(dlBox,'a');
		UI.append(body,document.body);
		bindEvent();
	}
	...
	function setPos(el){
		var x = UI.getX(el) - 10,y = UI.getY(el); 
		body.style.cssText = 'left:' + x + 'px;top:' + y + 'px';
	}

    //base64编码，for qqdownload
    base64encode = function (input) {  
        var output = "";  
        var chr1, chr2, chr3, enc1, enc2, enc3, enc4;  
        var i = 0; 
		var _keyStr = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/="; 
        input = UI.utfEncode(input);  
	...
	function setDownLoadUrl (file){
		if(pathCache[file.path]) {
			UI.A(dlBtn[0],'href',pathCache[file.path].link);
			UI.A(dlBtn[1],'href',pathCache[file.path].qqlink);
			return false;
		}
		UI.addClass(wrap,'loading');
		UI.C(dlBox,'visibility','hidden');
		MI.ajax({
			url : 'http://t.qq.com/air/applyDownload.php',   //asyn?
			type : 'post',
			data : {
				fileKey : file.path,
			...
			success : function(data){
				UI.removeClass(wrap,'loading');
				UI.C(dlBox,'visibility','visible');
				  ...
				  if(data.result == 0 && data.info){
					  if(data.info.keys){
						  ...
						  var qqlink = 'qqdl://' + base64encode(link); //qq旋风
						  UI.cookie('FTN5K',keys.attach.cookies);
						  UI.A(dlBtn[0],'href',link);
						  UI.A(dlBtn[1],'href',qqlink);
						  pathCache[file.path] = {link:link,qqlink:qqlink}; //缓存
					  }
				  }else{
					 MI.alert(_('获取文件信息失败')); 
				  }
	...
	function bindEvent(){
		dlBtn[0].onclick = dlBtn[1].onclick = hide;
