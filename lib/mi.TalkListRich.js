...
MI.TalkList.prototype.creatBox = function(){
	...
	relayListBox = UI.html(Self.talkBoxTmpl.replace('talkWrap',Self.type == 2 ? 'zfWrap comtWrap' : 'zfWrap').replace('<div class="left"></div>','<div class="left" style="display:none"><label for="replayListCheckbox"><input id="replayListCheckbox" type="checkbox"' + (isRelay ? ' checked' : '') + ' class="check1">' + _('同时转播给你的听众') + '</label></div>').replace('talkSuc',''))[0];
	box.push(new MI.TalkBox(relayListBox));

	UI.each(['replyBox','relayBox','comtBox','relayListBox'],function(o,i){
	...
	Self.relayListBox.guideTextKeep = 0;
	...
	Self.relayBox._relayCheck = $$(Self.relayBox._body,'.check1')[0] || UI.DC('b');
	...
	//转播列表
	Self.relayBox._relayList = UI.html('<div class="relayList" style="display:none"></div>')[0];
	UI.append(Self.relayBox._relayList,Self.relayBox._body);
	
	//评论列表
	Self.comtBox._relayList = UI.html('<div class="relayList" style="display:none"></div>')[0];
	UI.append(Self.comtBox._relayList,Self.comtBox._body);

	Self.comtBox._numSon = $$(Self.comtBox._body,'.number')[0];
	Self.comtBox._relayCheck = $$(Self.comtBox._body,'.check1')[0] || UI.DC('b');
	Self.comtBox.addListCheck = 0;
	Self.comtBox._relayCheck.onclick = function(){
		var checked = this.checked;
		Self.comtBox.type = checked ? 1 : 5;
		Self.comtBox.talkId = parseInt(Self.cur);
...
MI.TalkList.prototype.report = function(id,el){
	...
	if (el.innerHTML != _('举报')){ // 去掉fun.fastReport限制，强制有删除功能
		...
		jubao_msg(parseInt(id),name);
	}
}
MI.TalkList.prototype.reportQun = function(id, qid, zone){
	zone=zone?zone:1;
	var name = UI.A($$($(id),'.userName')[0] || $$($(id),'.msgCnt strong a')[0],'rel');
	jubao_qun_msg(parseInt(id),qid,zone);
}
...
MI.TalkList.prototype._addEvent = function(el){
	...
	Self.praiseCache = {}; //缓存用户“赞”过的转播和评论
	...
	if (comt) { // 评论
		...
		comt.onclick = function(){
			if(Self.type == 2 && !Self.comtBox){ //如果是群内讨论页面的评论
				Self._comt = UI.html(Self.talkBoxTmpl.replace('<div class="left"></div>','<div class="left"><label for="replayListCheckbox" style="display:"><input id="replayListCheckbox" type="checkbox" class="check1" style="display:none;">'  + '</label></div>'))[0];
			}
			Self.creatBox();
			if(Self.isQun == 1){
				...
				var comtBox = Self.comtBox,
					...
					pageLink = function(data){
						...
						if (num){
							...
							if (numValue){
								numDom = $$(Self.comtBox._body,'.number a')[0];
								if (numDom){
									numDom.innerHTML = numDom.innerHTML.replace(/\d+/g,numValue);
									UI.A(comt,'num',numValue);
								}
								if (MI.user.fun.newRelay2 && replyBox){
									comt.innerHTML = comt.innerHTML.replace(/\d+/g,numValue);
								}
							}
						}
								...
								UI.each(li,function(o){
									...
									if(_praiseCite) {
										_praiseCite.onclick = function(){
											...
											comtBox.talkId = parseInt(o.id);

											//Auto Select CheckBox Of Relay
											comtBox._relayCheck.checked = true;
											comtBox._relayCheck.onclick();
											comtBox.topic = _('赞') + _('/强 ');
											comtBox.focus();
											comtBox.countTxt();
											comtBox.send();
	...
	if (relay) { // 转播
		relay.onclick = function(){
			...
			// 转播列表
			if (Self.comtBoxList && MI.user.fun.newRelay2){
				var comtBox = Self.relayBox,
					comtList = comtBox._relayList,
					Y,
					scrollTop,
					pageLinkClick = function(){
					...
					pageLink = function(data){
						...
						if (num){
							numValue = num.innerHTML;
							if (numValue){
								numDom = $$(Self.relayBox._body,'.number a')[0];
								if (numDom){
									numDom.innerHTML = numDom.innerHTML.replace(/\d+/g,numValue);
									UI.A(relay,'num',numValue);
								}
								if (MI.user.fun.newRelay2 && replyBox){
									relay.innerHTML = relay.innerHTML.replace(/\d+/g,numValue);
								}
							}
						}
				...
				if (replyBox){
					Self.xhr.comt = MI.ajax({
						url : (MI.user.fun.newRelay2 ? MI.url.relayListSecond : MI.url.relayList),
						type : 'get',
						data : messageUrl,
						success : function(data){
							Time_1 = + new Date() - Time;
	...
	if (viewRelay && !viewRelay.innerHTML.hasString(_('对话'))) { // 查看转播
		var relayList,
			...
			pageLink = function(data){
				...
				UI.remove(Self.relayListBox._body);
							...
							try{
								...
								if (_relayCite) {
									_relayCite.onclick = function(){
										...
										UI.removeClass(relayBox._txt,'cNote');
										Self.getRelayOld(o,'.content',relayBox);
										relayBox.txtTipSend = _('转播中');
										relayBox._btn.value = _('转播');
										relayBox.talkId = parseInt(o.id);
			...
			createRelayBox = function(){
				...
				if (!this.appended) {
					Self.creatBox(); //创建转播框
					relayBox = Self.relayListBox;
					relay = Self.relayListBox._body;
				...
				if (MI.user.fun.relayListBoxCondensed){
					UI.addClass(Self.relayListBox._body,'condensed');
					Self.relayListBox.guideTextAdd(isRelay ? _('点击转播') : _('点击评论'));
				}
				else {
					setTimeout(function(){
						relayBox.focus();
				...
				relayBox.txtTipSend = isRelay ? _('转播中') : _('评论中');
				relayBox._btn.value = isRelay ? _('转播') : _('评论');
				relayBox.addList = isRelay ? Self.relayBox.addList : 0;
				//relayBox.talkId = UI.A(viewRelay,'rel');
				relayBox.talkId = elId;
				relayBox.type = isRelay ? 1 : 4;
				relayBox.apiType = Self.apiType;
...
MI.TalkList.prototype.reply = function(id,type,number){ //type : 0(reply) 1(relay) 2(comt)
	...
	if (!Self.talkBox.display) {
		...
		/*Self.talkBox.action = type ? '转播:&nbsp;' : '对<strong class="userTo"><a href="' + url + '" title="' + name + '(@' + account + ')">' + name + '</a>' + (vip ? '<a href="/certification" target="_blank" class="vip" title="腾讯认证"></a>': '') + (expo ? '<a href="/certification" target="_blank" class="ico_expo" title="上海世博"></a>': '') + '说:</strong>';
		if (type && !replyBox) {
		...
		if (type != 0) {
			...
			if (number > 0 && !(MI.user.fun.newRelay2 && relay)){// && !MI.user.isLab （实验室二传手不显示数字）
	...
	var boxs = [Self.replyBox,Self.relayBox,Self.comtBox];
	...
	if (Self.replyBox.display || Self.relayBox.display || Self.comtBox.display) {
...
MI.TalkList.prototype.urlPreview = (function(){
	var _body,_title,_cnt,_ifr,_close,MIN_H = 400,MIN_W = 360,MAX_W = 550,navH = 0,delay,delayShow,_testDiv,lastSrc,cache = {};

	function init(){
		_body = UI.html(MI.tmpl.urlPreview)[0];
		_title = $$(_body,'.ulink')[0];
		_cnt = $$(_body,'.urlCnt')[0];
		_close = $$(_body,'.close')[0];
		_arrow = $$(_body,'.SA')[0];
		_testDiv = UI.DC('div');  //验证返回html
		navH = $$(document.body,'.tSearchNew')[0] ? UI.height($('header') || $('headWrap')) : 0 ;   //是否是新导航
	...
	function setLocation(src){
		...
		if (info.site){
			if (info.site.title){
				result += '<h1>' + info.site.title + '</h1><span class="safeBox" style="float:right;margin-top:-35px;"><em class="ico safe_' + info.safe + '"></em></span>';
			}
			result += '<p style="margin:0 0 20px;">';
			if (info.site["abstract"]){
				result += info.site["abstract"] + '<br>';
			}
			if (info.site.url){
				result += '<a href="' + MI.string.html(decodeURIComponent(info.site.url)) + '" target="_blank">' + decodeURIComponent(info.site.url) + '</a>';
			}
			result += '</p><div style="border-top:1px solid #CCC;height:0;overflow:hidden;margin:0 0 20px;"></div>';
		}
	...
	function setPos(pos){
		var w,h,
			...
			y = pos[1],
			margin = 5,	 //距离浏览器边距
			winH = UI.windowHeight() - 2*margin - navH,  //40为新导航条高度
		...
		if(winW - pos[0] > MAX_W) {
			w = MAX_W;
		} else if (winW - pos[0] < MIN_W) {
			w = MIN_W;
			x = winW - w;
		} else {
			w = winW - pos[0];
		}
		y = UI.scrollY() + margin + navH;
		
		if(UI.B.ie6) {
			_body.style.cssText = 'left:' + ( x - 5 )+ 'px;top:' + y + 'px;width:' + w + 'px;height:' + h + 'px;';
		} else {
			_body.style.cssText =  'left:' + ( x - 5 )+ 'px;top:' + (navH + margin) + 'px;width:' + w + 'px;height:' + h + 'px;position:fixed'
		}
	...
	function bindEvent(){
		...
		if(UI.B.ie6) {
			UI.EA(window,'scroll',function(){
				_body.style.top = UI.scrollY() + margin + navH + 'px';
			});	
		};
...
MI.tmpl.download = '<div class="bigFileInfo dlLink"><b class="trib"><i class="tris"></i></b><span>' + _('下载方式：') + '</span><a href="#">' + _('直接下载') + '</a> <a href="#">' + _('QQ旋风') + '</a><div class="loading" style="display:none;position:absolute;left:5%;top:5%;width:90%;height:90%;background-color:#fff;"></div></div>';


MI.TalkList.prototype.download = (function(){
	var body,dlBtn = [],delayHide,pathCache = {};
	...
	function setDownLoadUrl (file){
		if(pathCache[file.path]) {
			UI.A(dlBtn[0],'href',pathCache[file.path]);
			UI.A(dlBtn[1],'qHref',pathCache[file.path]);
			return false;
		}
		UI.show(loading);
		MI.ajax({
				...
				UI.hide(loading);
				  ...
				  if(data.result == 0 && data.info){
					  if(data.info.keys){
						  ...
						  var link = 'http://'+keys.attach.dns+'/ftn_handler/'+keys.strKey+'/'+file.name;
						  UI.cookie('FTN5K',keys.attach.cookies);
						  UI.A(dlBtn[0],'href',link);
						  UI.A(dlBtn[1],'qHref',link);
						  pathCache[file.path] = link; //缓存
					  }
				  } else if (file.qid && (data.result == -8)) {
					  MI.confirm({
						  type : 'error',
						  title : _('权限不足'),
						  content : _('你需要加入该群才能下载'),
						  confirmTxt : _('申请加入'),
						  confirm : function(){
	...
	function show(el){
		...
		var file = {
			path : P.id,
			qid : UI.A(P,'qid') || 0,
			name : UI.A(P,'rel') || fileName.innerHTML,
			ftype : UI.A(P,'ftype') || '1'
		}
		setPos(fileName);
		UI.show(body);
		setDownLoadUrl(file);
		MI.Bos('btnShowDownload');
	}
...
//文件分享
MI.TalkList.fileEvent = function(fileBox) {
	var Self = this;
	for(var i = 0,num = fileBox.length; i< num; i++){
		...
		if(download) {
			download.onclick = function(){
				MI.TalkList.prototype.download.show(this);
				return false;
			}
		}
	}
