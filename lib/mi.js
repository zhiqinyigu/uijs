...
MI.Dialog.prototype = {
	...
	showLoad : function(){
		this._load.style.cssText = 'width:' + (UI.width(this._cont) - 2) + 'px;height:' + UI.height(this._cont) + 'px';
	},
	hideLoad : function(){
		UI.hide(this._load);
	},
...
MI.Load = function(id,url,type){
	var el = $(id),P = el.parentNode,btn = $$(P,'h3')[0].lastChild;
	if (UI.hasClass(btn,'loading')) {
		return;
	}
	else if (el.innerHTML == '') {
		UI.addClass(btn,'loading');
		UI.get(url,'t=' + type + '&r='+MI.random(10000),function(data){
			data = MI.json(data);
			if (data.result == 0){
				el.innerHTML = data.info[type];
				UI.removeClass(P,'fold');
				UI.removeClass(btn,'loading');
				if (MI.talkList) {
					MI.talkList.bottom()
				}
			}
		...
		UI.toggleClass(P,'fold');
	}
	if (MI.talkList) {
		MI.talkList.bottom()
	}
	btn.blur();
	MI.Bos('btn' + (UI.hasClass(P,'fold') ? 'Un' : '') + 'Fold' + type);
}
MI.PV = function(name){
