...
MI.TalkList.prototype = {
	...
	favor : function(id, type){
		id = String(id);
		var Self = this,talk = $(id);
		if (!talk.sending) {
			UI.get(Self.favorUrl,{id:id,op:type,r:MI.random()},function(data){
				talk.sending = 0;
				data = MI.json(data);
				if (data.result == 0) {
				    if (type == 2)
	...
	addEvent : function(el){
		...
		if (fav) {
			fav.onclick = function(){
				MI.talkList.favor(el.id, 1);
			}
		}
		if (unfav) {
			unfav.onclick = function(){
				MI.talkList.favor(el.id, 2);
			}
		}
		if (del) {
...
MI.ValidateNew = function(o){ //Validate
	/*
		account : {
			rule : function(str){
				return 'error';
