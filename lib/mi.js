...
/*
	...
	other:
		...
		btnUnFoldstar 展开推荐收听
		btnFoldstar 收起推荐收听
		btnFollowstar 收听推荐收听
		btnViewstar 查看推荐收听
		btnUnFoldrecommend 展开我可能认识的人
		btnFoldrecommend 收起我可能认识的人
		btnRefreshrecommend 刷新我可能认识的人
		btnMorerecommend 更多我可能认识的人
		btnFollowrecommend 收听我可能认识的人
		btnViewrecommend 查看我可能认识的人
...
MI = {
	...
	tmpl : {
		listAll : '<%for(var i=0,n=talk.length;i<n;i++){%><li id="<%=talk[i].id%>"<%if(fav){%> fav="<%=talk[i].favortimestamp%>"<%}%> rel="<%=talk[i].timestamp%>"><%if(!guest){%><%if('+V1+' && talk[i].type==4){%><div class="userPic"><a href="/<%=talk[i].name%>"'+V7+' class="masPic"><img src="<%=talk[i].pic%>"'+V6+'/></a><a href="/<%='+V1+'.name%>" title="<%='+V1+'.nick.toTitle()%>(@<%='+V1+'.name%>)" class="cusPic"><img src="<%='+V1+'.pic%>"'+V6+'/></a></div><%}else{%><div class="userPic"><a href="/<%=talk[i].name%>"'+V7+'><img src="<%=talk[i].pic%>"'+V6+'/></a></div><%}%><%}%><div class="msgBox"><div class="userName" rel="<%=talk[i].name%>"><strong><a href="/<%=talk[i].name%>"'+V7+'><%=talk[i].nick%></a>'+MI_icon('talk[i].flag')+'<%=talk[i].icon%><%if(!'+V1+'){%>:<%}%></strong><%if('+V1+' && talk[i].type==2){%>转播:&nbsp;<%}else if('+V1+' && talk[i].type==4){%>对<strong class="userTo"><a href="/<%='+V1+'.name%>" title="<%='+V1+'.nick.toTitle()%>(@<%='+V1+'.name%>)"><%='+V1+'.nick%></a>'+MI_icon(V1+'.flag')+'说:<%}%></strong></div><div class="msgCnt"><%=talk[i].content||"&nbsp;"%></div><%var Imgage=talk[i].image && talk[i].image.length,Videos=talk[i].videos && talk[i].videos.length;if(Imgage || Videos){%><div class="mediaWrap"><%if(Imgage){%><%for(var j=0;j<talk[i].image.length;j++){%><div class="picBox"><a href="<%=talk[i].image[j]%>/460" class="pic"><img class="crs" crs="<%if(iconPic){%>http://mat1.gtimg.com/www/mb/images/vT.png<%}else{%><%=talk[i].image[j]%>/160<%}%>"></a></div><%}%><%}%><%if(talk[i].videos && talk[i].videos.length){%><div class="videoBox" realurl="<%=talk[i].videos[0].realUrl%>" playurl="<%=talk[i].videos[0].playerUrl%>" minipicurl="<%=talk[i].videos[0].miniPicUrl%>" shorturl="<%=talk[i].videos[0].shortUrl%>" reltitle="<%=talk[i].videos[0].title%>"><div class="vTools"><a href="#" class="vClose"><em></em>收起</a><a href="<%=encodeURI(talk[i].videos[0].realUrl)%>" class="vUrl" target="_blank"><em></em><%if(talk[i].videos[0].title){%><%=MI.string.cut(talk[i].videos[0].title,40)%><%}%></a></div><div class="vWrap" thumbs="<%if(iconPic){%>0<%}else{%>1<%}%>"><a href="#" class="vThumbsBox"><span class="vThumbs"><span class="mask"><em></em></span><img class="crs" crs="<%=talk[i].videos[0].miniPicUrl%>" /></span><span class="vSimple"><img class="crs" crs="http://mat1.gtimg.com/www/mb/images/vSimple.png" /></span></a></div></div><%}%></div><%}%><%if('+V1+' && talk[i].type==2){%><div class="replyBox"><div class="msgBox"><div class="msgCnt"><strong><a href="/<%='+V1+'.name%>" title="<%='+V1+'.nick.toTitle()%>(@<%='+V1+'.name%>)"><%='+V1+'.nick%></a>'+MI_icon(V1+'.flag')+'<%='+V1+'.icon%>:</strong><%='+V1+'.content%></div><%var Imgage='+V1+'.image && '+V1+'.image.length,Videos='+V1+'.videos && '+V1+'.videos.length;if(Imgage || Videos){%><div class="mediaWrap"><%if(Imgage){%><%for(var j=0;j<'+V1+'.image.length;j++){%><div class="picBox"><a href="<%='+V1+'.image[j]%>/460" class="pic"><img class="crs" crs="<%if(iconPic){%>http://mat1.gtimg.com/www/mb/images/vT.png<%}else{%><%='+V1+'.image[j]%>/160<%}%>"></a></div><%}%><%}%><%if(Videos){%><div class="videoBox" realurl="<%='+V1+'.videos[0].realUrl%>" playurl="<%='+V1+'.videos[0].playerUrl%>" minipicurl="<%='+V1+'.videos[0].miniPicUrl%>" shorturl="<%='+V1+'.videos[0].shortUrl%>" reltitle="<%='+V1+'.videos[0].title%>"><div class="vTools"><a href="#" class="vClose"><em></em>收起</a><a href="<%=encodeURI('+V1+'.videos[0].realUrl)%>" class="vUrl" target="_blank"><em></em><%if('+V1+'.videos[0].title){%><%=MI.string.cut('+V1+'.videos[0].title,40)%><%}%></a></div><div class="vWrap" thumbs="<%if(iconPic){%>0<%}else{%>1<%}%>"><a href="#" class="vThumbsBox" ><span class="vThumbs"><span class="mask"><em></em></span><img class="crs" crs="<%='+V1+'.videos[0].miniPicUrl%>" /></span><span class="vSimple"><img class="crs" crs="http://mat1.gtimg.com/www/mb/images/vSimple.png" /></span></a></div></div><%}%></div><%}%><div class="pubInfo"><span class="left"><a class="time" rel="<%='+V1+'.timestamp%>" href="/p/t/<%='+V1+'.id%>" target="_blank"><%='+V1+'.time%></a> <%='+V1+'.from%></span><%if(MI.user.account&&UI.isNumber('+V1+'.comt)){%><div class="funBox"><a href="/p/c/<%='+V1+'.id%>" class="comt">评论</a><a href="/p/c/<%='+V1+'.id%>" target="_blank">(<em><%='+V1+'.comt%></em>条)</a></div><%}%></div></div></div><%}%><div class="pubInfo"><span class="left"><a class="time" href="/p/t/<%=talk[i].id%>" target="_blank"><%=talk[i].time%></a> <%=talk[i].from%><%if('+V1+' && '+V1+'.content && '+V1+'.content.length && talk[i].type==4){%> <a class="view" href="/p/r/<%=talk[i].id%>">查看对话</a><%}%><%if(MI.user.fun.report && talk[i].name != MI.user.account){%> <a'+V5+' class="alarm">举报</a><%}%></span><div class="funBox"><%if(MI.user.account){%><a'+V5+' class="fav<%if(fav){%> light<%}%>" title="<%if(fav){%>取消<%}%>收藏"></a><span>|</span><%if(UI.isNumber(talk[i].comt)){%><a href="#" class="comt">评论</a><a href="/p/c/<%=talk[i].id%>" target="_blank">(<em><%=talk[i].comt%></em>条)</a><span>|</span><%}%><%if(MI.user){if((!guest || guest == MI.user.account) && MI.user.account==talk[i].name){%><a'+V5+' class="delBtn">删除</a><span>|</span><a'+V5+' class="relay">转播</a><%}else{%><a'+V5+' class="reply">对话</a><span>|</span><a'+V5+' class="relay">转播</a><%}}}%><%if(talk[i].count){if(MI.user.account){%><span>|</span><a href="/p/z/<%=talk[i].id%>" class="zfNum" target="_blank">　(<b class="relayNum"><%=talk[i].count%></b>)</a><%}else{%> <em class="zfNum">　(<b class="relayNum"><%=talk[i].count%></b>)</em><%}}else if('+V1+' && '+V1+'.count){if(MI.user.account){%><span>|</span><a href="/p/z/<%='+V1+'.id%>" class="zfNum" target="_blank">　(<b class="relayNum"><%='+V1+'.count%></b>)</a><%}else{%><em class="zfNum">　(<b class="relayNum"><%='+V1+'.count%></b>)</em><%}}%></div></div><%if('+V1+' && talk[i].type==4){%><div class="talkDetail"><b><%='+V1+'.nick%>:</b> <%='+V1+'.content%></div><%}%></div></li><%}%>',
	...
	openMusic : function(url){
		if(!url)	return;
		window.open(url,'QQMusic','toolbar=no,location=no,menubar=no,resizable=yes,status=yes,scrollbars=no,width=750,height=520,left=100,top=10');
	},
...
MI.TalkList = function(id){ //Talk List
	...
	Self.replyBox.successStart = Self.relayBox.successStart = function(){
	...
	Self.replyBox.success = Self.relayBox.success = function(){
		...
		var time = $$($(Self.cur),'.time'),length = time.length;
		if (length) {
			setTimeout(function(){
				time[length - 1].focus();
			},100);
...
MI.TalkList.prototype = {
	...
	reply : function(id,type){ //type : undefined(reply) 1(relay) 2(comt)
		...
		if (!Self.talkBox.display) {
			...
			var replyTitle = '转播 <b>' + name + '</b>: "' + MI.string.html(content.slice(0,20)),
				relayTitle = '对 <b>' + name + '</b> 说:',
				delayAddReply;
