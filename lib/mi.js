...
MI = {
	...
	tmpl : {
		list : '<li id="<%=info.id%>" rel="<%=info.timestamp%>"><div class="userPic"><a href="/<%=info.account%>" title="(<%=info.nick.toTitle()%>@<%=info.account%>)"<%if(' + V2 + '.type==2){%> class="masPic"<%}%>><img src="<%=info.userPic%>"' + V6 + '/></a><%if(' + V2 + '.type==2){%><%=' + V2 + '.cusPic%><%}%></div><div class="msgBox"><div class="userName"><strong><a href="<%=info.account%>" title="(<%=info.nick.toTitle()%>@<%=info.account%>)"><%=info.nick%></a><%if(info.vip){' + V4 + '}%><%if(!info.action){%>:<%}%></strong><%=info.action%></div><div class="msgCnt"><%=info.content%></div><%if(' + V2 + '.type==1){%><div class="replyBox"><%=' + V2 + '.content%></div><%}%><div class="pubInfo"><span class="left"><span class="time"><%=info.time%></span> 发自<%=info.sorurce%> <%if(' + V2 + '.type==2){%><a class="view zfNum"' + V5 + '>查看对话</a></b><%}%></span><p class="funBox"><a' + V5 + ' class="del delBtn">删除</a></p></div><%if(' + V2 + '.type==2){%><div class="talkDetail"><%=' + V2 + '.nick%>: <%=' + V2 + '.content%></div><%}%></div></li>',
		listAll : '<%for(var i=0,n=talk.length;i<n;i++){%><li id="<%=talk[i].id%>" rel="<%=talk[i].timestamp%>"><%if(!guest){%><%if(' + V1 + ' && talk[i].type==4){%><div class="userPic"><a href="/<%=talk[i].name%>"' + V7 + ' class="masPic"><img src="<%=talk[i].pic%>"' + V6 + '/></a><a href="/<%=' + V1 + '.name%>" title="<%=' + V1 + '.nick.toTitle()%>(@<%=' + V1 + '.name%>)" class="cusPic"><img src="<%=' + V1 + '.pic%>"' + V6 + '/></a></div><%}else{%><div class="userPic"><a href="/<%=talk[i].name%>"' + V7 + '><img src="<%=talk[i].pic%>"' + V6 + '/></a></div><%}%><%}%><div class="msgBox"><div class="userName" rel="<%=talk[i].name%>"><strong><a href="/<%=talk[i].name%>"' + V7 + '><%=talk[i].nick%></a><%if(talk[i].vip){ ' + V4 + ' }%><%if(!' + V1 + '){%>:<%}%></strong><%if(' + V1 + ' && talk[i].type==2){%>转播 <strong><a href="/<%=' + V1 + '.name%>" title="<%=' + V1 + '.nick.toTitle()%>(@<%=' + V1 + '.name%>)"><%=' + V1 + '.nick%></a><%if(' + V1 + '.vip){' + V4 + '}%>:</strong><%}else if(' + V1 + ' && talk[i].type==4){%>回复 <strong><a href="/<%=' + V1 + '.name%>" title="<%=' + V1 + '.nick.toTitle()%>(@<%=' + V1 + '.name%>)"><%=' + V1 + '.nick%></a><%if (' + V1 + '.vip){' + V4 + ' }%>:<%}%></strong></div><div class="msgCnt"><%=talk[i].content%></div><%if(' + V1 + ' && talk[i].type==2){%><div class="replyBox"><div class="userPic"><a href="/<%=' + V1 + '.name%>" title="<%=' + V1 + '.nick.toTitle()%>(@<%=' + V1 + '.name%>)"><img src="<%=' + V1 + '.pic%>"' + V6 + '/></a></div><div class="msgBox"><div class="msgCnt"><strong><a href="/<%=' + V1 + '.name%>" title="<%=' + V1 + '.nick.toTitle()%>(@<%=' + V1 + '.name%>)"><%=' + V1 + '.nick%></a><%if(' + V1 + '.vip){' + V4 + '}%>:</strong><%=' + V1 + '.content%></div><div class="pubInfo"><span class="left"><span class="time"><%=' + V1 + '.time%></span> 发自<%=' + V1 + '.from%> <a class="zfNum" href="/t/<%=talk[i].id%>" title="点击查看转播路径"><strong class="relayNum"><%=' + V1 + '.count%></strong>人转播</a></span></div></div></div><%}%><div class="pubInfo"><span class="left"><span class="time"><%=talk[i].time%></span> 发自<%=talk[i].from%><%if(' + V1 + ' && ' + V1 + '.content && ' + V1 + '.content.length && talk[i].type==4){%> <a class="view zfNum"' + V5 + '>查看对话</a><%}%></span><p class="funBox"><%if(MI.user){if(!guest && MI.user.account==talk[i].name){%><a class="del delBtn"' + V5 + '>删除</a><%}else{%><a class="reply talk"' + V5 + '>回复</a><a class="relay zf"' + V5 + '>转播</a><a' + V5 + ' class="report" title="举报"></a><%}%><%}%></p></div><%if(' + V1 + ' && talk[i].type==4){%><div class="talkDetail"><%=' + V1 + '.nick%>: <%=' + V1 + '.content%></div><%}%></div></li><%}%>',
