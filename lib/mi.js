...
MI = {
	...
	string : {
		...
		account : function(str){
			...
			return account ? account[0].slice(1,-1) : '';
		}
	},
	number : {
				...
				UI.get(reqUrl,urlData,function(data){
					...
					if (data.result == 0) {
						...
						UI.each(data.info,function(o,i){
							...
							if ((o.type == 3 || o.type == 5 || o.type == 6 || o.type == 7 || o.type == 16) && !MI.noNewCount) {
	...
	reply : function(id,type,number){ //type : 0(reply) 1(relay) 2(comt)
		...
		if (!Self.talkBox.display) {
			...
			setTimeout(function(){
				...
				if (addReplyTxt && !Self.replyCont) { //自动添加转播点评
