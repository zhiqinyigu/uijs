		...
		MI.newCountShow = function(data){
			if (data.result == 0) {
				...
				if(navMsgBox) {
					...
					var msgBoxItem = [msgBoxTotal].concat($$(document.body,'.headWrap .topNavMsg li b'));
					var msgBoxNum = [];
					var msgBoxType = { //消息type : 对应的元素在消息盒子下拉菜单中的位置
						1 : 2, //私信
						2 : 1, //新听众
						4 : [3,4,5], //@消息[所有，我收听的，认证用户]
						14 : [3,4,5], //@有理由的消息[所有，我收听的，认证用户]
						13 : 6  //微群
					};
					maxNum = 999;
					...
					//总数
					msgBoxNum[0] = (msgBoxNum[1] || 0) + (msgBoxNum[2] || 0) + (msgBoxNum[3] || 0);
					...
					UI.each(msgBoxNum,function(p,i){
						...
						if(p) {
							...
							//	(i== 4 || i == 5) ? UI.show(item.parentNode) :  //我收听的和认证用户
								(i== 4 || i == 5) ? UI.hide(item.parentNode) :  //临时隐藏我收听的和认证用户
									UI.show(item); 								//其他
						} else {
							(i == 0) ? UI.removeClass(item.parentNode,'newMsg') :
								(i== 4 || i == 5) ? UI.hide(item.parentNode) :
									UI.hide(item);
						}
						item.innerHTML = p;
					});

					msgBoxItem[6] && UI.hide(msgBoxItem[6].parentNode); //临时隐藏微群
		...
		//var url = 'http://www.qq.com/mb/mat1/mb/',
			js = {
				...
				mi : {
					...
					TalkListRich : '110906c',
					TalkListShare : '110901',
					TalkListFav : '110906e',
					Scroll : '101111',
					Capture : '110817',
					Music : '110906e',
 ...
 * @param {Number} Number 表ID （214-主站, 1125-话题和banner实时统计）
 *            @example
 *            MI.Bos('btnSend');
 */
MI.Bos = function(op,value,random,bossId){ //Boss Using : MI.Bos('frontClick');
...
MI.TalkBox = function(id){ //Talk Box
	...
	if (this._body) {
		...
		var addEvent = function(){
			UI.ER(Self._body,'mouseover',addEvent);
		...
		UI.EA(Self._body,'mouseover',addEvent);
...
MI.TalkBox.prototype = {
	...
	countTxt : function(){
		...
		if(MI.Text2Pic && Self.uploadCollection && !UI.hasClass(document.body,'ipad')) { //只在顶部广播对话框出现
			...
			success : function(data){
				...
				*/
				...
				if (AddedTalkList && Self.addList && data.result == 0 && addCheck) {
					...
					// fill-back样式用于标识消息是否为刚刚发布 拥有叫fill-back标识的消息，被删除时会将它的内容回填到发布框中。富媒体内容不会回填。
					UI.addClass(newTalk,'newMsg fill-back'); //Add Delete CSS
			...
			if (Self._music) {
				MI.app({
					Music : function() {
						...
						MI.Music.addPreview(Self._music, '', false);
						MI.Music.hide();
					}
				});
			}
								...
								if (type == 3) {
									...
									setTimeout(function(){
										if (UI.hasClass(talk,'orginCnt')){ //消息底层页，删除后跳到首页
											...
											//TODO::停止音乐播放
											if (MI.user.fun.newMusic && MUSIC && MUSIC.ICMusic){
...
UI.ready(function(){
	...
	if (MI.hostType == 1){
		...
		setTimeout(function(){
			...
			MI.Bos('btnHost_' + MI.host,0.0001);
		},3000);
		/*if (Math.random() < 0.1){
			MI.app({
				NonTx : function(){
