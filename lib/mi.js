...
MI.TalkBox.prototype = {
	...
	countTxt : function(){
		...
		if (Self.autoSave && MI.Storage(draft) != '$No$') {
	...
	if (Self._new) {
		...
		function hideNew(){
			...
			UI.each(Self._news,function(o){
				if (UI.hasClass(o,'newMsg') && UI.getY(o) + UI.height(o) < y) {
	...
	function _show(){
		...
		if (Self.word) { //E.key == 50
			...
			if (!Self.sending && (!users || new Date().getTime() - MI.Storage('time') > 7200000)){ //3600000
				Self.sending = 1;
				UI.get('/asyn/nicktips.php','type=' + (Self.key ? 0 : 2) + '&num=' + (UI.B.ie ? 1000 : 2000),function(data){
				...
				if (!MI[usersStorageKey]) {
					MI[usersStorageKey] = MI.json(unescape(users));
				}
				Self.show(MI[usersStorageKey]);
			}
		}
...
UI.ready(function(){
	setTimeout(function(){
		...
		if (MI.talkBox) {
			var draft = 'draft_' + MI.user.account;
			if (MI.talkBox.autoSave && MI.Storage(draft) != '$No$') {
		...
		if (MI.talkList && MI.talkBox && MI.talkBox.addList) { //需要区分非主人页，那些页面不需要该功能
			...
			if (location.hash == '#M' && listTime != '$No$') {
