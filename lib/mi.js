...
(function(){
var V1='talk[i].source',V2='info.source',V3='<div class="SA"><em>◆</em><span>◆</span></div>',V4='%><a href="/certification" target="_blank" class="vip" title="腾讯认证"></a><%',V5=' href="#"',V6=' onerror="MI.Pic(this,50)"',V7=' title="<%=talk[i].nick.toTitle()%>(@<%=talk[i].name%>)"';
MI = {
	...
	tmpl : {
		list : '<li id="<%=info.id%>" rel="<%=info.timestamp%>"><div class="userPic"><a href="/<%=info.account%>" title="<%=info.nick.toTitle()%>(@<%=info.account%>)"<%if('+V2+'.type==2){%> class="masPic"<%}%>><img src="<%=info.userPic%>"'+V6+'/></a><%if('+V2+'.type==2){%><%='+V2+'.cusPic%><%}%></div><div class="msgBox"><div class="userName"><strong><a href="/<%=info.account%>" title="<%=info.nick.toTitle()%>(@<%=info.account%>)"><%=info.nick%></a><%if(info.vip){'+V4+'}%><%if(!info.action){%>:<%}%></strong><%=info.action%></div><div class="msgCnt"><%if('+V2+'.type){%> <%}%><%=info.content%>&nbsp;</div><%if('+V2+'.type==1){%><div class="replyBox"><%='+V2+'.content%></div><%}%><%if(info.image && info.image.length){%><div><%for(var i=0;i<info.image.length;i++){%><div class="picBox"><a href="<%=info.image[i]%>/460" class="pic"><img src="<%=info.image[i]%>/160"></a></div><%}%></div><%}%><div class="pubInfo"><span class="left"><span class="time"><%=info.time%></span> 发自<%=info.sorurce%> <%if('+V2+'.type==2){%><a class="view zfNum"'+V5+'>查看对话</a></b><%}%><%if(!info.vip){%> <a'+V5+' class="alarm">举报</a><%}%></span><div class="funBox"><%if(MI.user.vip && UI.isNumber(info.comt)){%><a href="#" class="comt">评论(<em><%=info.comt%></em>)</a><span>|</span><%}%><a'+V5+' class="delBtn">删除</a><span>|</span><a'+V5+' class="relay">转播</a><span>|</span><a'+V5+' class="fav" title="收藏"></a></div></div><%if('+V2+'.type==2){%><div class="talkDetail"><b><%='+V2+'.nick%>:</b> <%='+V2+'.content%></div><%}%></div></li>',
		listAll : '<%for(var i=0,n=talk.length;i<n;i++){%><li id="<%=talk[i].id%>" rel="<%=talk[i].timestamp%>"><%if(!guest){%><%if('+V1+' && talk[i].type==4){%><div class="userPic"><a href="/<%=talk[i].name%>"'+V7+' class="masPic"><img src="<%=talk[i].pic%>"'+V6+'/></a><a href="/<%='+V1+'.name%>" title="<%='+V1+'.nick.toTitle()%>(@<%='+V1+'.name%>)" class="cusPic"><img src="<%='+V1+'.pic%>"'+V6+'/></a></div><%}else{%><div class="userPic"><a href="/<%=talk[i].name%>"'+V7+'><img src="<%=talk[i].pic%>"'+V6+'/></a></div><%}%><%}%><div class="msgBox"><div class="userName" rel="<%=talk[i].name%>"><strong><a href="/<%=talk[i].name%>"'+V7+'><%=talk[i].nick%></a><%if(talk[i].vip){ '+V4+' }%><%if(!'+V1+'){%>:<%}%></strong><%if('+V1+' && talk[i].type==2){%>转播&nbsp;:&nbsp;<%}else if('+V1+' && talk[i].type==4){%>对<strong class="userTo"><a href="/<%='+V1+'.name%>" title="<%='+V1+'.nick.toTitle()%>(@<%='+V1+'.name%>)"><%='+V1+'.nick%></a><%if ('+V1+'.vip){'+V4+' }%>说:<%}%></strong></div><div class="msgCnt"><%=talk[i].content||"&nbsp;"%></div><%if(talk[i].image && talk[i].image.length){%><div><%for(var j=0;j<talk[i].image.length;j++){%><div class="picBox"><a href="<%=talk[i].image[j]%>/460" class="pic"><img class="crs" crs="<%=talk[i].image[j]%>/160"></a></div><%}%></div><%}%><%if('+V1+' && talk[i].type==2){%><div class="replyBox"><div class="msgBox"><div class="msgCnt"><strong><a href="/<%='+V1+'.name%>" title="<%='+V1+'.nick.toTitle()%>(@<%='+V1+'.name%>)"><%='+V1+'.nick%></a><%if('+V1+'.vip){'+V4+'}%>:</strong><%='+V1+'.content%></div><%if('+V1+'.image && '+V1+'.image.length){%><br><%for(var j=0;j<'+V1+'.image.length;j++){%><div class="picBox"><a href="<%='+V1+'.image[j]%>/460" class="pic"><img class="crs" crs="<%='+V1+'.image[j]%>/160"></a></div><%}}%><div class="pubInfo"><span class="left"><span class="time" rel="<%='+V1+'.timestamp%>"><%='+V1+'.time%></span> 发自<%='+V1+'.from%> <b class="relayNum"><%='+V1+'.count%></b>人转播</span></div></div></div><%}%><div class="pubInfo"><span class="left"><span class="time"><%=talk[i].time%></span> 发自<%=talk[i].from%><%if(talk[i].count){%> <b class="relayNum"><%=talk[i].count%></b>人转播<%}%><%if('+V1+' && '+V1+'.content && '+V1+'.content.length && talk[i].type==4){%> <a class="view zfNum"'+V5+'>查看对话</a><%}%><%if(!talk[i].vip){%> <a'+V5+' class="alarm">举报</a><%}%></span><div class="funBox"><%if(talk[i].vip && UI.isNumber(talk[i].comt)){%><a href="#" class="comt">评论(<em><%=talk[i].comt%></em>)</a><span>|</span><%}%><%if(MI.user){if((!guest || guest == MI.user.account) && MI.user.account==talk[i].name){%><a'+V5+' class="delBtn">删除</a><span>|</span><a'+V5+' class="relay">转播</a><%}else{%><a'+V5+' class="reply">对话</a><span>|</span><a'+V5+' class="relay">转播</a><%}%><span>|</span><a'+V5+' class="fav<%if(fav){%> light<%}%>" title="<%if(fav){%>取消<%}%>收藏"></a><%}%></div></div><%if('+V1+' && talk[i].type==4){%><div class="talkDetail"><b><%='+V1+'.nick%>:</b> <%='+V1+'.content%></div><%}%></div></li><%}%>',
		...
		card : '<div class="uCard"><div class="tip">这不是你自己吗？</div><div class="userPic"><a></a><p class="btn"><input type="button" class="addAttention" value="" /><a href="#" class="delAttention" title="取消收听">取消</a></p></div><div class="uCardcnt"><p class="userName"></p><p class="nums loading"></p><p class="btn"><a href="#" class="reply">对话</a><a href="#" class="msg">私信</a></p></div></div>'
	},
...
MI.DropTips.prototype = {
	action : ['/search/user.php?pos=1&k=','/k/'],
	index : 0,
	hide : function(){
		...
		if (Self.word) { //E.key == 50
			...
			if (!Self.sending && (!users || new Date().getTime() - MI.Storage('time') > 7200000)){ //3600000
				...
				UI.get('/asyn/nicktips.php','user=' + MI.user.account + '&type=' + (Self.key ? 0 : 2) + '&num=' + (UI.B.ie ? 1000 : 2000),function(data){
					...
					if (Data.result == 0) {
						MI.Storage(usersStorageKey,data);
...
MI.Dialog = function(o){ //Dialog
	...
	Self._body.innerHTML = o.tmpl || '<div class="bg"></div><div class="CR"><table border="0" cellspacing="0" cellpadding="0" class="tbSendMsg"><tr><td class="tl"></td><td class="tm"></td><td class="tr"></td></tr><tr><td class="lm"></td><td><div class="DWrap"><div class="DTitle"></div><a title="关闭" class="DClose close" href="#">关闭</a><div class="DLoad"></div><div class="DCont"></div></div></div></td><td class="rm"></td></tr><tr><td class="bl"></td><td class="bm"></td><td class="br"></td></tr></table></div>';
	Self._bg = Self.$('.bg');
	Self._wrap = Self.$('.CR');
...
MI.DialogTip = function(o){
	var Self = this;
	o = o || {};
	o.tmpl = '<div class="bg"></div><div class="CR"><div class="DTitle"></div><div class="DClose"></div><div class="DLoad"></div><div class="floatTip"><span class="ico_tsW"><span class="ico_ts"></span></span><b class="DCont"></b></div></div></div>';
...
MI.Load = function(id,url,type){
	var el = $(id),P = el.parentNode,btn = $$(P,'h3')[0].lastChild,_fold='收起',_unfold='展开',_loading='loading',_folded='fold';
	...
	btn.blur();
	btn.title = UI.hasClass(P,_folded) ? _unfold : _fold;
	MI.Bos('btn' + (UI.hasClass(P,_folded) ? 'Un' : '') + 'Fold' + type);
}
MI.PV = function(name){
	...
	if (this._body) {
		...
		UI.ready(function(){
			...
			if (Self._pic) {
				...
				*/
				var html = UI.html('<span class="loading" style="display:none">上传中 <a href="#" class="cancel" title="取消">[取消]</a></span><span style="display:none" class="preview"><span class="link"></span><a href="#" class="del" title="删除">[删除]</a></span><iframe id="imageUpload" name="imageUpload" src="about:blank" style="display:none"></iframe>'),
...
MI.TalkBox.prototype = {
	...
	countTxt : function(){
		...
		if (Self.autoSave && length <= this.txtMax && MI.isStorage) {
	...
	save : function(){
		try{
			MI.Storage('draft_' + MI.user.account,this._txt.value.replace(new RegExp(this.txtTopic,'g'),'') || '');
		}catch(e){};
	},
	...
	send : function(){
		...
		},30000);//定的是15s，但为了安全起见，算上网络延迟、请求排队等因素，将时间扩大一倍
		return;
	},
...
MI.TalkList = function(id){ //Talk List
	...
	Self._comt = UI.html(html.replace('inputTxt','inputTxt noAutoCmt'))[0];
	...
	Self.comtBox.txtTipSend = Self.replyBox.txtTipSend = '发送中';
	Self.relayBox.txtTipSend = '转播中';
	Self.comtBox.url = '/comment/publishComment.php';
	Self.comtBox.countUrl = 0;
	Self.comtBox.hideCall = Self.replyBox.hideCall = Self.relayBox.hideCall = function(){
		UI.removeClass($(Self.current),'cur');
	};
	Self.comtBox.success = function(){
		var id = Self.current,li = $(id),num = $$(li,'.comt em')[0];
		MI.countNum(num,1);
		//$$(Self._talk,'.replyTitle')[0].innerHTML = '共' + num.innerHTML + '条评论';
		Self.talkBox._comts.src = '/comment/comment.php?id=' + id + '&r=' + MI.random();
...
MI.TalkList.prototype = {
	...
	comtNum : function(value){ //Check Comment Num After Iframe Loaded
		var id = this.current,li = $(id),num = $$(li,'.comt em')[0];
		num.innerHTML = value;
		$$(this._talk,'.replyTitle')[0].innerHTML = '共' + value + '条评论';
	},
	reply : function(id,type){ //type : undefined(reply) 1(relay) 2(comt)
		...
		if (type == 1) {
			this.talkBox = this.relayBox;
		}
		else if (type == 2) {
			this.talkBox = this.comtBox;
		}
		else {
			this.talkBox = this.replyBox;
		}
		...
		if (!this.talkBox.display) {
			...
			if (type == 1) { //Don't Change For Reply
			...
			this._relayNum = type == 1 ? $$(li,'.relayNum')[0] : null;
			...
			replyTitle += '"<br>再说两句:';
			if (type == 2) {
				replyTitle = '共' + $$(li,'.comt em')[0].innerHTML + '条评论';
				UI.show(this.talkBox._addComt);
				this.talkBox._comts.style.height = 0;
				this.talkBox._comts.src = '/comment/comment.php?id=' + id + '&r=' + MI.random();
				//UI.show(this.talkBox._comts);
			}
			else {
				UI.hide(this.talkBox._addComt);
				UI.hide(this.talkBox._comts);
			}
			$$(this._talk,'.replyTitle')[0].innerHTML = type ? replyTitle : relayTitle;
			account = url.split('/');
			account = account[account.length - 1];
			if (replyBox && name2) { //添加转播点评
				this.talkBox._addReply.innerHTML = '<a href="#" title="你可以通过“引用”来调出前面人的转播理由">［引用<b>' + name2 + '</b>的点评］</a>';
			...
			if (type == 2) {
				...
				this.talkBox.action = type ? '转播&nbsp;:&nbsp;' : '对<strong class="userTo"><a href="' + url + '" title="' + name + '(@' + account + ')">' + name + '</a>' + (vip ? '<a href="/certification" target="_blank" class="vip" title="腾讯认证"></a>': '') + '说:</strong>';
				if (type && !replyBox) {
				...
				/*else if (userPic) {
					var cusPic = $$(userPic,'img')[0],cusPicParent = cusPic.parentNode;
					cusPicParent.title = UI.A(cusPicParent,'rel');
					UI.A(cusPic,'card','');
					UI.A(cusPicParent,'rel','');
				}*/
				this.talkBox.source = {
					type : type ? 1 : 2,
					content : type ? (replyBox ? replyBox.innerHTML : clone.innerHTML) : MI.string.html(contentBase),
					nick : name,
					cusPic : ( userPic && userPic.innerHTML.replace(/cusPic/gi,'hide').replace(/card="1"/g,'').replace(/title=""/g,'').replace(/rel=/g,'title=').replace(/<a/i,'<a class="cusPic"') ) || 0 //Small Pic For Reply
				};
			}

			contentTopic = (contentTmp || content).match(/(#\s*[^#\s]{1}[^#]{0,59}?\s*#)/g); //Auto Add Topic

			this.talkBox._txt.value = contentTopic ? contentTopic.join('') : '';
			...
			setTimeout(function(){
				...
				MI.selectTxt(Self.talkBox._txt,contentTopicLength,contentTopicLength,contentTopicLength);
			},0);
		...
		if (type == 1) { //有待优化...先这样处理
			this.replyBox.hide();
			this.replyBox.display = 0;
			this.comtBox.hide();
			this.comtBox.display = 0;
		}
		else if (type == 2) {
			this.replyBox.hide();
			this.replyBox.display = 0;
		...
		if (this.comtBox.display || this.replyBox.display || this.relayBox.display) {
...
MI.Card.prototype = {
	...
	show : function(el){
		...
		function show(o){
			...
			Self._num.innerHTML = '广播' + num[0] + '条<span>' + (o.vv ? '' : '|</span>听众' + num[1] + '人<span>');//|</span>收听了' + num[2] + '人
			Self._name.innerHTML = '<a href="' + url + '">' + name + '</a>' + (o.vip ? '<a class="vip" target="_blank" title="腾讯认证" href="/certification"></a>' : '') + '<br /><span class="en">@' + account + '</span>';
			if (notSelf) {
...
MI.Reply = function(){ //Reply Dialog
	...
	//setTimeout(function(){
		UI.append(Self._body,document.body);
		Self.talkBox = new MI.TalkBox(Self._body.id);
...
MI.Msg = function(){ //Msg Dialog
	...
	UI.append(Self._body,document.body);
	Self.talkBox = new MI.MsgBox(Self._body.id);
	talkBox = Self.talkBox;
...
UI.ready(function(){
	...
	setTimeout(function(){
		...
		if (MI.talkBox) {
			var draft = 'draft_' + MI.user.account;
			if (MI.talkBox.autoSave && MI.isStorage) {
				/*UI.EA(window,'beforeunload',function(){
					var txt = MI.talkBox._txt.value;
					MI.Storage(draft,txt || '');
				});*/
				MI.talkBox._txt.value = MI.Storage(draft).replace(/^undefined|^Null/,'') || ''; //有隐患，有时间再查一下
				MI.talkBox.countTxt();
			}
		...
		if (MI.talkList) { //区分非主人页，非主人页不需要该功能
			var account = MI.user.account,
				...
				listTime = MI.Storage('listTime');
			window.onbeforeunload = function(){
				var time = new Date().getTime();
				if (!MI.isStorage || !listStorageName) {
					return;
				}
				MI.Storage(scrollTop,UI.scrollY());
				location.hash = 'M';
				...
				if (MI.talkList && MI.ajax && MI.ajax < 10) {
					try{
						MI.Storage(newList,'[' + MI.talkNew.join(',') + ']');
						MI.Storage(moreList,'[' + MI.talkMore.join(',') + ']');

						var listStorage = MI.Storage(listName),
							list = listStorage ? MI.json(listStorage) : [];
						list.push('"' + listStorageName + '"');
						MI.Storage(listName,'[' + list.join(',') + ']');
					}catch(e){
						clear();
					}
				}
				function clear(){ //Clear Storage
					var list = MI.json(MI.Storage(listName));
					if (UI.isArray(list)) {
						UI.each(list,function(o){
							MI.Storage('new_' + o,'');
							MI.Storage('more_' + o,'');
							MI.Storage('scroll_' + o,'');
						});
					}
					MI.Storage(listName,'');
					MI.Storage('listTime',new Date().getTime());
				}
			};
			if (location.hash == '#M' && MI.isStorage) {
				//if (document.referrer == '' || document.referrer == location.href) {
				//	return;
				//}
				var _newList = MI.Storage(newList),
					_new = MI.json(_newList),
					_moreList = MI.Storage(moreList),
				...
				window.scrollTo(0,MI.Storage(scrollTop));
			}
		}
	...
	var crs = setInterval(function(){
		...
		MI.delay.crs = setTimeout(MI.Crs,300);
	};
});
