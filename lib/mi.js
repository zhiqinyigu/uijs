			...
			if (_y && _y < y && _y > yTop) {
				...
				setTimeout(function(){
					...
					if (iconPic) { //For Big Picture
						...
						if (0 && MI.Crs.iconPic.hasString(md5[0])) {
					...
					if (UI.hasClass(o.parentNode,"vThumbs")) { //For Video Thumbs
						...
						if (0 && MI.Crs.iconPic.hasString(src)) {
							UI.hide(vThumbs);
							UI.show(vSimple);
				...
				*/
				...
				if (UI.hasClass(document.body,'ipad')){ //Disable Upload
					Self._pic.onclick = function(){
						alert('对不起，您的浏览器由于安全限制不支持图片上传，建议您使用其他浏览器。');
			...
			replyMsg.onclick = function(){
				if (MI.talkBox){
					if (MI.talkBox.type == 6){
						MI.dialog.show({width:560,title:'<h1 class="mainTit">修改记事本</h1>',html:$('talkBoxMsg')});
