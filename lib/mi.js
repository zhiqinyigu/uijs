...
MI.Dialog = function(o){ //Dialog
	...
	this._body.className = 'dialog';
	this._body.innerHTML = '<div class="bg"></div><div class="commRound"><div class="top"><span class="left"><em></em></span><span class="right"><em></em></span></div><div class="dialogTitle"></div><a href="javascript:void(0)" class="dialogClose" title="关闭">关闭</a><div class="dialogCont"></div><div class="bot"><span class="left"><em></em></span><span class="right"><em></em></span></div></div>';
	this._bg = this.$('.bg');
	this._wrap = this.$('.commRound');
	this._title = this.$('.dialogTitle');
	this._close = this.$('.dialogClose');
	this._cont = this.$('.dialogCont');
...
MI.Dialog.prototype = {
	...
	show : function(o){
		...
		if (!this._append) {
			...
			this._append = true;
			UI.C(this._bg,'marginLeft','-9999px');
		}

		this.close = o.close != undefined ? o.close : 1;
		if (this.close) {
			UI.show(this._close);
		}
		else UI.hide(this._close);

		this._title.innerHTML = o.title || '';
		UI.C(this._title,'height',o.title ? '' : '0');
		if (UI.isString(o.html)) {
			this._cont.innerHTML = o.html;
		}
		else if (UI.isObject(o.html)) {
			for (var i = 0,num = this._cont.childNodes.length;i < num;i++) {
				UI.remove(this._cont.childNodes[0]);
			}
			UI.append(o.html,this._cont);
			UI.show(o.html);
		}
		if (o.width) {
			this.width = o.width;
		}
		UI.C(this._wrap,'width',this.width + 'px');
		UI.C(this._wrap,'opacity',0);
		UI.show(Self._body);
