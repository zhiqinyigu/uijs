...
MI = {
	...
	tmpl : {
		list : '<li id="<%=info.id%>" rel="<%=info.timestamp%>"><div class="userPic"><a href="/<%=info.account%>" title="(<%=info.nick.toTitle()%>@<%=info.account%>)"<%if(' + V2 + '.type==2){%> class="masPic"<%}%>><img src="<%=info.userPic%>"' + V6 + '/></a><%if(' + V2 + '.type==2){%><%=' + V2 + '.cusPic%><%}%></div><div class="msgBox"><div class="userName"><strong><a href="<%=info.account%>" title="(<%=info.nick.toTitle()%>@<%=info.account%>)"><%=info.nick%></a><%if(info.vip){' + V4 + '}%><%if(!info.action){%>:<%}%></strong><%=info.action%></div><div class="msgCnt">&nbsp;<%=info.content%></div><%if(' + V2 + '.type==1){%><div class="replyBox"><%=' + V2 + '.content%></div><%}%><div class="pubInfo"><span class="left"><span class="time"><%=info.time%></span> 发自<%=info.sorurce%> <%if(' + V2 + '.type==2){%><a class="view zfNum"' + V5 + '>查看对话</a></b><%}%></span><p class="funBox"><a' + V5 + ' class="del delBtn">删除</a></p></div><%if(' + V2 + '.type==2){%><div class="talkDetail"><b><%=' + V2 + '.nick%>:</b> <%=' + V2 + '.content%></div><%}%></div></li>',
		listAll : '<%for(var i=0,n=talk.length;i<n;i++){%><li id="<%=talk[i].id%>" rel="<%=talk[i].timestamp%>"><%if(!guest){%><%if(' + V1 + ' && talk[i].type==4){%><div class="userPic"><a href="/<%=talk[i].name%>"' + V7 + ' class="masPic"><img src="<%=talk[i].pic%>"' + V6 + '/></a><a href="/<%=' + V1 + '.name%>" title="<%=' + V1 + '.nick.toTitle()%>(@<%=' + V1 + '.name%>)" class="cusPic"><img src="<%=' + V1 + '.pic%>"' + V6 + '/></a></div><%}else{%><div class="userPic"><a href="/<%=talk[i].name%>"' + V7 + '><img src="<%=talk[i].pic%>"' + V6 + '/></a></div><%}%><%}%><div class="msgBox"><div class="userName" rel="<%=talk[i].name%>"><strong><a href="/<%=talk[i].name%>"' + V7 + '><%=talk[i].nick%></a><%if(talk[i].vip){ ' + V4 + ' }%><%if(!' + V1 + '){%>:<%}%></strong><%if(' + V1 + ' && talk[i].type==2){%>转播&nbsp;:<%}else if(' + V1 + ' && talk[i].type==4){%>回复&nbsp;<strong><a href="/<%=' + V1 + '.name%>" title="<%=' + V1 + '.nick.toTitle()%>(@<%=' + V1 + '.name%>)"><%=' + V1 + '.nick%></a><%if (' + V1 + '.vip){' + V4 + ' }%>:<%}%></strong></div><div class="msgCnt">&nbsp;<%=talk[i].content%></div><%if(' + V1 + ' && talk[i].type==2){%><div class="replyBox"><div class="userPic"><a href="/<%=' + V1 + '.name%>" title="<%=' + V1 + '.nick.toTitle()%>(@<%=' + V1 + '.name%>)"><img src="<%=' + V1 + '.pic%>"' + V6 + '/></a></div><div class="msgBox"><div class="msgCnt"><strong><a href="/<%=' + V1 + '.name%>" title="<%=' + V1 + '.nick.toTitle()%>(@<%=' + V1 + '.name%>)"><%=' + V1 + '.nick%></a><%if(' + V1 + '.vip){' + V4 + '}%>:</strong><%=' + V1 + '.content%></div><div class="pubInfo"><span class="left"><span class="time" rel="<%=' + V1 + '.timestamp%>"><%=' + V1 + '.time%></span> 发自<%=' + V1 + '.from%> <a class="zfNum" href="/t/<%=talk[i].id%>" title="点击查看转播路径"><strong class="relayNum"><%=' + V1 + '.count%></strong>人转播</a></span></div></div></div><%}%><div class="pubInfo"><span class="left"><span class="time"><%=talk[i].time%></span> 发自<%=talk[i].from%><%if(talk[i].count){%>&nbsp;<b class="relayNum"><%=talk[i].count%></b>人转播<%}%><%if(' + V1 + ' && ' + V1 + '.content && ' + V1 + '.content.length && talk[i].type==4){%> <a class="view zfNum"' + V5 + '>查看对话</a><%}%></span><p class="funBox"><%if(MI.user){if(!guest && MI.user.account==talk[i].name){%><a class="del delBtn"' + V5 + '>删除</a><%}else{%><a class="reply talk"' + V5 + '>回复</a><a class="relay zf"' + V5 + '>转播</a><a' + V5 + ' class="report" title="举报"></a><%}%><%}%></p></div><%if(' + V1 + ' && talk[i].type==4){%><div class="talkDetail"><b><%=' + V1 + '.nick%>:</b> <%=' + V1 + '.content%></div><%}%></div></li><%}%>',
		msg : '<li id="<%=info.id%>"><div class="msgBox"><div class="userName"><a href="/<%=info.account%>"><%=info.nick%></a><%if(info.vip){ ' + V4 + '}%></div><div class="msgCnt"><%=info.content%></div><div class="pubTime"><span class="time"><%=info.time%></span></div></div><p class="btnBox"><a class="btn delBtn"' + V5 + '>删除</a></p></li>',
	...
	newCout : function(type){
		...
		//setTimeout(function(){
			UI.get('/asyn/newMsgCount.php','type=' + type + '&r=' + MI.random(10000),function(data){
				data = MI.json(data);
...
MI.TalkList = function(id){ //Talk List
	...
	//Delete Confirm
	Self._confirm = UI.html('<div class="delChose">真的要删除此广播？<br><input value="确定" type="button">&nbsp;&nbsp;<input value="取消" type="button"></div>')[0];
...
MI.TalkList.prototype = {
	...
	reply : function(id,type){
		...
		if (!this.talkBox.display) {
			var li = UI.G(id),cur = this.current,name = UI.GC(li,'.userName strong a')[0].innerHTML,vip = UI.GC(li,'.userName .vip').length,url = UI.GC(li,'.userName a')[0].href,cont = UI.GC(li,'.msgCnt')[0],content = UI.text(cont),contentBase = cont.innerHTML,replyBox = UI.GC(li,'.replyBox')[0],relay = UI.GC(li,'.replyBox .msgCnt')[0],/*relayUser,*/userPic = UI.GC(li,'.userPic')[0];
			if (relay) {
			...
			var replyTitle = '转播 <b>' + name + '</b>:&nbsp;"' + MI.string.escape(content.slice(0,20)),relayTitle = '回复 <b>' + name + '</b>:';
			...
			this.talkBox.action = type ? '转播&nbsp;:' : '回复&nbsp;<strong><a href="' + url + '">' + name + '</a>' + (vip ? '<b class="vip"></b>': '') + ':</strong>';
			if (type && !replyBox) {
	...
	addEvent : function(el){
		var Self = this,time = UI.GC(el,'.time'),view = UI.GC(el,'.view')[0],report = UI.GC(el,'.report')[0],viewContent = UI.GC(el,'.talkDetail')[0],content = UI.GC(el,'.msgCnt')[0],reply = UI.GC(el,'.reply')[0],relay = UI.GC(el,'.relay')[0],del = UI.GC(el,'.delBtn')[0],replyMsg = UI.GC(el,'.replyMsg')[0];
		MI.addHover(el);
		if (reply) {
