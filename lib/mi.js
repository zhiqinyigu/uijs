...
MI.TalkList.prototype = {
	...
	addEvent : function(el){
		...
		if (picBox) {
			var pic = UI.GT(picBox,'img')[0];
			if (!UI.B.ie) {
				var canvas = document.createElement('canvas');
				canvas.id = 'C' + MI.random();
				MI.canvas[canvas.id] = canvas.getContext('2d');
				pic.parentNode.appendChild(canvas);
			...
			UI.GC(picBox,'.btnBack')[0].onclick = UI.GC(picBox,'.btnPrev')[0].onclick = MI.TalkList.picTool;
		}
...
MI.TalkList.picLoad = function(){
	...
	if (canvas) {
		MI.TalkList.picDraw(this,canvas);
	}
}
