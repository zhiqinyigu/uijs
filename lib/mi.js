...
MI = {
	...
	countTxt : function(value,countUrl,zhCount){
		...
		if (length < 500) {
			...
			length = MI.string.length(UI.trim(value).replace(new RegExp(_('#输入话题标题#'),'g'),'')) + urlNum * 20 + urlExceed;
			if(zhCount) length = Math.ceil(length/2);
		}
		return length;
	},
	fC : { //followConfig
				...
				UI.each(data.info,function(o,i){
					...
					if ((o.type == 3 || o.type == 5 || o.type == 6 || o.type == 7 || o.type == 15 || o.type == 16 || o.type == 32) && !MI.noNewCount) {
						if (MI.talkList && MI.talkList._new) {
							...
							if(num==1)
								...
								tips = _('约{0}新广播，点击查看<i class="l">s</i>',tip);
					...
					if (o.type == 4 || o.type == 13 || o.type == 14) {
						...
						if (target && num) {
							...
							UI.A(target,'onlyAll',atNumOnlyAll);
							if (num || msgNum){
								numTmp = num + msgNum;
								_num.innerHTML =  numTmp > maxNum ? maxNum + '+' : numTmp;
...
MI.TalkBox = function(id){ //Talk Box
	...
	if (this._body) {
		...
		Self._more = Self.$('.smoreFun'),
		Self.moreBox = $$(Self._more,'.moreBox')[0],
		Self._moreAnchor = $$(Self._more,'.more')[0],
		Self._moreDropBox = $$(Self._more,'.dropBox')[0],
		Self._sendCnt = Self.$('.sendCnt');
		Self._autoBackspace = Self.$('.autoBackspace');

		Self._sendStatus = Self.$('.sendStatus');
			...
			*/
			...
			if(Self._more){
				if(Self._moreAnchor){
					Self._moreAnchor.onclick = function(){
						Self._moreDropBox && UI.toggleClass(Self.moreBox,'sopen');
						MI.Bos('btnSendListMore');
						return false;
					}
					Self._moreAnchor.onmouseover = function(){
						clearTimeout(Self._moreDropBox.t1);
						clearTimeout(Self._moreDropBox.t2);
						return false;
					}
					Self._moreAnchor.onmouseout = function(){
						Self._moreDropBox.t1 = setTimeout(function(){
							UI.removeClass(Self.moreBox,'sopen');
						},300);
						return false;
					}
				}
				if(Self._moreDropBox){
					Self._moreDropBox.onmouseover = function(){
						clearTimeout(Self._moreDropBox.t1);
						clearTimeout(Self._moreDropBox.t2); 
						return false;
					}
					Self._moreDropBox.onmouseout = function(){
						Self._moreDropBox.t2 = setTimeout(function(){
							UI.removeClass(Self.moreBox,'sopen');
						},300); 
						return false;
					}
			...
			if (Self._topicNewYear){
				Self._topicNewYear.onclick = function(){
					...
					Self.moreBox && UI.removeClass(Self.moreBox,'sopen');
			...
			if (Self._music) {
				...
				Self.musicMoreBtn = {
					body : UI.html('<div class="cutBox" style="display:none;">\
						...
						<p style="display:' + (UI.B.ipad ? 'none':'') + '"><span class="ico_upAudio"></span><a class="txt mUpload" href="#" title="' + _('自己上传音频') +'">' +_('上传') + '</a></p></div>')[0],
			...
			if (Self._pic) {
				...
				*/
				var imageUploadName = 'imageUpload_' + MI.random(),
					html = UI.html('<div class="cutBox"><p style="display:' + (UI.B.ipad ? 'none':'')   +  '"><a href="#" class="multi txt mOut" title="' + _('支持多张图片拼接') + '"><em class="sico ico_pics"></em>' + _('多图') + '</a></p><p style="display:none;"><a href="#" class="txt mOut" title="'+_("支持粘贴QQ截屏等剪贴板内容") +'"><em class="sico ico_cut"></em>'+_("截屏") +'</a></p><p><a href="#" class="txt mOut" title="'+_("粘贴图片链接")+'"><em class="sico ico_pUrl"></em>'+ _("链接") +'</a></p><p><a href="#" class="text2pic txt mOut" title="'+_("文字转图片")+'"><em class="sico ico_wpic"></em>'+_("文图")+'</a></p></div>'),
					...
					copyfrag = document.createDocumentFragment(),
				...
				UI.each(copyPicHTML,function(o){
					UI.append(o,copyfrag);
				});
				UI.append(frag,Self._pic);
				UI.append(copyfrag,Self._pic);
				...
				Self._picMulti = $$(Self._picInsertBox, "p")[0];//图片多图项
				...
				Self._multiPic.onclick = Self._multi.onclick = function(){
					...
					if($$(document.body,'.D')[0]){
						function remove(){
							if(UI.hasClass(Self._picTxt,'disabled')){
								UI.removeClass(Self._picTxt,'disabled');
							}
						}
						UI.EA($$('.D .DClose')[0],'click',remove);
						UI.EA($$('.D .btnSend')[0],'click',remove);
						UI.EA($$('.D .btnCancel')[0],'click',remove);
					}
				...
				var picNav = function(j){
					...
					if (!(UI.B.ipad)&&!(UI.hasClass(document.body,'ipad'))&&!Self.pic && (!Self._picLoading || UI.C(Self._picLoading,'display') == 'none') && MI.TalkBox.captureEnable()){
				...
				if (UI.hasClass(document.body,'ipad')){ //Disable Upload
					...
					Self._pic.onmouseout = function(){
						if(Self._picInsertBox.style.display != 'none'){
			...
			Self._btn.onclick = function(){
				...
				UI.hasClass(Self._sendCnt,'hasMedia') && UI.removeClass(this._sendCnt,'hasMedia');
				switch (Self.type) {
					case null:	
		...
		if (Self.autoHeight) {
			...
			this.delay.height = setTimeout(function(){
				...
				var scrollHeight = Self._txt.scrollHeight,
					...
					height = (scrollHeight > clientHeight ? scrollHeight : clientHeight) - parseInt(UI.C(Self._txt,'paddingTop')) - parseInt(UI.C(Self._txt,'paddingBottom'));
				if (UI.B.ie) {
	...
	addPic : function(o){
		...
		if (o.result == 0) {
			...
			if (turnPicAble) {
				...
				UI.append(Self._uploadPicS,Self._sendStatus);
				UI.addClass(Self._picTxt,'disabled');
				Self._picLink.innerHTML = '<em class="sico ico_pic"></em><a href="' + pic + '/2000" class="fileName" target="_blank" style="position:relative;">' + fileName + '<span class="simg" style="cursor:default;"><img  src="' + pic +  "/160" + '" /><span style="white-space:nowrap;width:100%;padding:5px 0;display:' + (UI.B.ipad ? 'none':'inline-block' ) + '"><span class="useFilter" style="cursor:pointer;display:inline;" href="javascript:void(0)">' + _('魔法滤镜') + '</span></span></span></a>';
		...
		if (!MI.time || newTime - MI.time > 60) { //Don't Update In 60 Seconds
			...
			UI.each(this.time,function(o,i){
				...
				if (minute <= 0) {
					...
					if(minute>1)
						info = minute + _('分钟前<i class="l">s</i>');
					else
						info = minute + _('分钟前');
				}
	...
	addEvent : function(el,type){
		...
		function addEvent(element){
			UI.each($$(element || document.body,'.addAttention'),function(o){
				var del = UI.html(type ? '<div class="foFun disabled" style="display:none">' + _('已收听') + '</div>' : '<a href="#" class="delAttention" style="display:none">' + _('取消<i class="l">收听</i>') + '</a>')[0];
				UI.after(del,o);
