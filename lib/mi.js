...
MI = {
	 ...
	 *            @example
	 *            MI.addQQ('肖敏','283540706','xiaom565');
	 */
	addQQ : function(Uname,id,account){
	//接入文档 http://platform.server.com/af/guide.html
		if (window.id_af_show){
			...
			UI.getScript('http://id.qq.com/af/js/id_af.js',function(){
				id_af_init({"appid": 10025, "lang": 2052, "defaultlogin": true});
				id_af_show(id,1);
				/**
				 * 重载发送添加好友申请的回调函数
				*/
				window.id_af_callback = function(type,obj){
					if(type == 2){ //发送申请成功
						if(obj.flag == 2){  //发送申请成功，等待对方确认
							setTimeout(function(){
								MI.confirm({
									type : 'success',
									title : _('QQ申请已发出'),
									content : _('再发一条私信通知<b>{0}</b>?', Uname),
									confirmTxt : _('确认'),
									confirm : function(){
										MI.message(account,_('hi,我刚申请加你为QQ好友,快去你微博对应的QQ({0})上通过我的申请吧!',id));
								    },
									cancelTxt : _('取消'),
									cancel :function(){
		...
		MI.app({
			Base : function(){
				...
				if(id == 'recommend') {
					...
					MI.Card.build(el,'.knownInfo .BossHref',2);
				}
			}
		});
	}
			...
			if (Self._pic) {
				...
				*/
				var imageUploadName = 'imageUpload_' + MI.random(),
					html = UI.html('<div class="cutBox" style="display:none;"><p  style="display:none;"><span class="ico_cut"></span><a href="#" class="txt mOut" title="'+_("支持粘贴QQ截屏等剪贴板内容") +'">'+_("截屏") +'</a></p><p><span class="ico_pUrl"></span><a href="#" class="txt mOut" title="'+_("粘贴图片链接")+'">'+ _("链接") +'</a></p><p style="display:' + (UI.B.ipad ? 'none':'')   +  '"><span class="ico_pics"></span><a href="#" class="multi txt mOut">' + _('多图') + '</a></p></div><span class="pidLoad" style="display:none">' + _("上传中") + ' <a href="#" class="cancel">[' + _("取消") + ']</a></span><span style="display:none" class="preview"><span class="link"></span><a href="#" class="del">[' + _("删除") + ']</a></span><iframe id="' + imageUploadName + '" name="' + imageUploadName + '" src="about:blank" style="display:none"></iframe>'),
				...
				Self._multi.onmouseover = function(e){
	...
	addPic : function(o){
		...
		if (o.result == 0) {
			...
			if (MI.user.fun.turnPic && o.info.path) {
				...
				Self._picLink.innerHTML = '<a href="#" class="fileName" onclick="return false">' + fileName + '<span class="simg"><img  src="' + pic +  "/160" + '" /><span style="cursor:default;white-space:nowrap;width:100%;padding:5px 0;display:' + (UI.B.ipad ? 'none':'inline-block' ) + '"><span class="useFilter" style="cursor:pointer;display:inline;" href="javascript:void(0)">' + _('使用魔法滤镜') + '</span></span></span></a>';
				...
				var _fileName = $$(Self._picLink,'.fileName')[0];
				UI.EA(_fileName,'mouseover',function(){
					clearTimeout(_fileName.delay);
					UI.addClass(_fileName,'hover');
				});

				UI.EA(_fileName,'mouseout',function(){
					_fileName.delay = setTimeout(function(){
...
MI.PicTurn = {
	...
	set: function(src) {
		this._imgCt.innerHTML = '<img src="' + src + '/160" /><div style="width:100%;display:' + (UI.B.ipad ? 'none':'inline-block') + ';padding:5px 0;white-space:nowrap;"><a href="#" class="useFilter" style="float:none;display:inline;">' + _('使用魔法滤镜') + '</a></div>';
...
MI.TalkList.prototype = {
	...
	getRelayOld : function(relayBox,o,content){
		...
		if(cont){
			...
			relayBox._txt.value = ' || @' + relayCiteAccount +': ';
		}
		try{
			relayBox._txt.focus();
...
MI.bannerTips = function(obj){
	...
	*/
	...
	if (oMsg && talkBox){
		...
		msg = $(idMsg);
		if (msg){
			msg.innerHTML = '<em class="ico_ntips"></em><span class="left">' + oMsg.content + '</span><a href="#" onclick="UI.hide(\'' + idMsg + '\');UI.ajax({url:\'' + MI.url.updateTips + '\',data:{id:\''+oMsg.id+'\',status:0}});MI.Bos(\'btnMsg_' + oMsg.id + '\');return false" class="close" title="' + _('关闭') + '"></a>';
