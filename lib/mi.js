...
MI = {
	...
	tmpl : {
		list : '<li id="<%=info.id%>" rel="<%=info.timestamp%>"><div class="userPic"><a href="/<%=info.account%>" title="(<%=info.nick.toTitle()%>@<%=info.account%>)"<%if(' + V2 + '.type==2){%> class="masPic"<%}%>><img src="<%=info.userPic%>"' + V6 + '/></a><%if(' + V2 + '.type==2){%><%=' + V2 + '.cusPic%><%}%></div><div class="msgBox"><div class="userName"><strong><a href="<%=info.account%>" title="(<%=info.nick.toTitle()%>@<%=info.account%>)"><%=info.nick%></a><%if(info.vip){' + V4 + '}%><%if(!info.action){%>:<%}%></strong><%=info.action%></div><div class="msgCnt"><%if(' + V2 + '.type){%> <%}%><%=info.content%>&nbsp;</div><%if(' + V2 + '.type==1){%><div class="replyBox"><%=' + V2 + '.content%></div><%}%><%if(info.image && info.image.length){%><br><%for(var i=0;i<info.image.length;i++){%><div class="picBox"><a href="<%=info.image[i]%>/460" class="pic"><img src="<%=info.image[i]%>/160"></a></div><%}}%><div class="pubInfo"><span class="left"><span class="time"><%=info.time%></span> 发自<%=info.sorurce%> <%if(' + V2 + '.type==2){%><a class="view zfNum"' + V5 + '>查看对话</a></b><%}%></span><div class="funBox"><div class="moreFun hide"><h4><a' + V5 + ' hideFocus="true">操作</a></h4><a' + V5 + ' class="relay">转播</a><a' + V5 + ' class="delBtn">删除</a></div></div></div><%if(' + V2 + '.type==2){%><div class="talkDetail"><b><%=' + V2 + '.nick%>:</b> <%=' + V2 + '.content%></div><%}%></div></li>',
		listAll : '<%for(var i=0,n=talk.length;i<n;i++){%><li id="<%=talk[i].id%>" rel="<%=talk[i].timestamp%>"><%if(!guest){%><%if(' + V1 + ' && talk[i].type==4){%><div class="userPic"><a href="/<%=talk[i].name%>"' + V7 + ' class="masPic"><img src="<%=talk[i].pic%>"' + V6 + '/></a><a href="/<%=' + V1 + '.name%>" title="<%=' + V1 + '.nick.toTitle()%>(@<%=' + V1 + '.name%>)" class="cusPic"><img src="<%=' + V1 + '.pic%>"' + V6 + '/></a></div><%}else{%><div class="userPic"><a href="/<%=talk[i].name%>"' + V7 + '><img src="<%=talk[i].pic%>"' + V6 + '/></a></div><%}%><%}%><div class="msgBox"><div class="userName" rel="<%=talk[i].name%>"><strong><a href="/<%=talk[i].name%>"' + V7 + '><%=talk[i].nick%></a><%if(talk[i].vip){ ' + V4 + ' }%><%if(!' + V1 + '){%>:<%}%></strong><%if(' + V1 + ' && talk[i].type==2){%>转播&nbsp;:&nbsp;<%}else if(' + V1 + ' && talk[i].type==4){%>回复 <strong class="userTo"><a href="/<%=' + V1 + '.name%>" title="<%=' + V1 + '.nick.toTitle()%>(@<%=' + V1 + '.name%>)"><%=' + V1 + '.nick%></a><%if (' + V1 + '.vip){' + V4 + ' }%>:<%}%></strong></div><div class="msgCnt"><%=talk[i].content%>&nbsp;</div><%if(talk[i].image && talk[i].image.length){%><br><%for(var j=0;j<talk[i].image.length;j++){%><div class="picBox"><a href="<%=talk[i].image[j]%>/460" class="pic"><img src="<%=talk[i].image[j]%>/160"></a></div><%}}%><%if(' + V1 + ' && talk[i].type==2){%><div class="replyBox"><div class="userPic"><a href="/<%=' + V1 + '.name%>" title="<%=' + V1 + '.nick.toTitle()%>(@<%=' + V1 + '.name%>)"><img src="<%=' + V1 + '.pic%>"' + V6 + '/></a></div><div class="msgBox"><div class="msgCnt"><strong><a href="/<%=' + V1 + '.name%>" title="<%=' + V1 + '.nick.toTitle()%>(@<%=' + V1 + '.name%>)"><%=' + V1 + '.nick%></a><%if(' + V1 + '.vip){' + V4 + '}%>:</strong><%=' + V1 + '.content%></div><%if(' + V1 + '.image && ' + V1 + '.image.length){%><br><%for(var j=0;j<' + V1 + '.image.length;j++){%><div class="picBox"><a href="<%=' + V1 + '.image[j]%>/460" class="pic"><img src="<%=' + V1 + '.image[j]%>/160"></a></div><%}}%><div class="pubInfo"><span class="left"><span class="time" rel="<%=' + V1 + '.timestamp%>"><%=' + V1 + '.time%></span> 发自<%=' + V1 + '.from%> <b class="relayNum"><%=' + V1 + '.count%></b>人转播</span></div></div></div><%}%><div class="pubInfo"><span class="left"><span class="time"><%=talk[i].time%></span> 发自<%=talk[i].from%><%if(talk[i].count){%> <b class="relayNum"><%=talk[i].count%></b>人转播<%}%><%if(' + V1 + ' && ' + V1 + '.content && ' + V1 + '.content.length && talk[i].type==4){%> <a class="view zfNum"' + V5 + '>查看对话</a><%}%></span><div class="funBox"><%if(MI.user){if(!guest && MI.user.account==talk[i].name){%><div class="moreFun hide"><h4><a' + V5 + ' hideFocus="true">操作</a></h4><a' + V5 + ' class="relay">转播</a><a' + V5 + ' class="delBtn">删除</a></div><%}else{%><a' + V5 + ' class="reply">回复</a><span>|</span><div class="moreFun hide"><h4><a' + V5 + ' hideFocus="true">操作</a></h4><a' + V5 + ' class="relay">转播</a><a' + V5 + ' class="alarm" title="举报">举报</a></div><%}%><%}%></div></div><%if(' + V1 + ' && talk[i].type==4){%><div class="talkDetail"><b><%=' + V1 + '.nick%>:</b> <%=' + V1 + '.content%></div><%}%></div></li><%}%>',
	...
	cursorX : function(el){
		if (document.selection){
			el.focus();
			var range = document.selection.createRange(),tmp = el.createTextRange(),position;
			tmp.collapse(true);
			tmp.select();
...
MI.TalkBox.prototype = {
	...
	countTxt : function(){
		var Self = this,
			...
			length,
			url = [],
			url2 = value.match(new RegExp('((news|telnet|nttp|file|http|ftp|https)://)?(([-A-Za-z0-9]+(\\.[-A-Za-z0-9]+)*(\\.[-A-Za-z]{2,5}))|([0-9]{1,3}(\\.[0-9]{1,3}){3}))(:[0-9]*)?(/[-A-Za-z0-9_\\$\\.\\+\\!\\*\\(\\),;:@&=\\?/~\\#\\%]*)*','gi')) || [];
		UI.each(url2,function(o){ //Dont's Match Small Url And Big Url
			if (o.length < 257) { //o.length > 20 && 
				url.push(o);
			}
		});
		value = value.replace(url,'');
		Self.length = length = Math.ceil((MI.string.length(UI.trim(value).replace(new RegExp(Self.txtTopic,'g'),'')) + url.length * 20 ) / 2);
	...
	showTip : function(msg,error){
		this._tip.innerHTML = error ? '<em class="error">' + msg + '</em>' : msg;
	},
	...
	addEvent : function(el){
		...
		if (del) {
			del.onclick = function(){
				...
				if (Self.talkBox && Self.talkBox.display && Self.current == li.id) { //Hide TalkBox When Click Delete Button
					UI.remove(Self.talkBox._body);
					UI.removeClass(li,'cur');
					Self.talkBox.display = 0;
				}
				return false;
			}
		...
		if (moreFun) { //More Function
			...
			moreFun.onclick = function(e){
				...
				for (var i in MI.drop) { //Hide Other Drop Menu
					if (i != this.tid) {
						UI.addClass(MI.drop[i],'hide');
						delete MI.drop[i];
					}
				}
				UI.toggleClass(this,'hide');
				if (!UI.hasClass(this,'hide') && !MI.drop[this.tid]) {
...
MI.TalkList.pic = function(){ //Picture Event
	var pic = UI.B.ie ? this : this.previousSibling || this,
		picBig,
...
MI.AutoCmt = function(o){ //Auto Complete
	/*
		URL : /asyn/nicktips.php?type=1&num=100 //0-All, 1-Following, 2-Followed
		{
			"result":0,
			"msg":"成功",
	...
	*/
	...
	//Dom
	Self._shadow = UI.html('<div class="txtShadow"><span></span><b>|</b></div>')[0];
	UI.after(Self._shadow,o.target);
	Self._txt = Self._shadow.firstChild;
	Self._cursor = Self._shadow.lastChild;
	//Self._body = UI.html('<div class="autoCmt" style="display:none"></div>')[0];
	//UI.after(Self._body,o.target.parentNode);
	...
	function show(e){
		...
		var E = UI.E(e),
			text = Self._target.value.slice(0,Self.cursorX);
		Self.word = text.match(/@\w{0,64}$/g);
		if (Self.word) {
			Self.word = Self.word[0];
			var textLength = Self.word.length;
			if (textLength > 1) {
				text = text.slice(0,- textLength + 1);
			}
		}
		Self._txt.innerHTML = text.breakWord(3).replace(/\n/g,'<br>').replace(/ /g,'&nbsp;');
		...
		if (E.key == 50) {
			if (!Self._body.appended) {
				...
				Self._body.onmouseover = function(e){
					var E = UI.E(e);
					if (E.target != this && Self.index != null) {
						UI.removeClass(Self.list[Self.index],'on');
			...
			var users = MI.Storage('users');
			if (!users || new Date().getTime() - MI.Storage('time') > 3600000){ //3600000
				UI.get('/asyn/nicktips.php','type=0',function(data){
					data = '{"result":0,"info":{"t684000003":"T684000003","t684000004":"T684000004","t684000006":"T684000006","t684000008":"T684000008","t684000009":"T684000009"}}';
					var Data = MI.json(data);
					if (Data.result == 0) {
						MI.Storage('users',data); 
...
MI.AutoCmt.prototype = {
	...
	show : function(o){
		if (this.word) {
			...
			for (var i in o.info) {
				...
				m = i.indexOf(word);
				n = o.info[i].indexOf(word);
				if (m == 0 || n == 0) {
					match.push(o.info[i].replace(reg,html) + ' (' + i.replace(reg,html) + ')')
				}
				index++;
			}
			if (match.length) {
				this._body.innerHTML = '<li class="on">' + match.join('</li><li>') + '</li>';
			...
			if (!this.display) {
				this._body.style.cssText = 'top:' + (UI.getY(this._cursor) - this._target.scrollTop + 22) + 'px;left:' + (UI.getX(this._cursor) - 12) + 'px';
			}
			this.display = 1;
		}
		else {
		...
		//this._body.innerHTML = new UI.tmplString(this.tmpl)(o);
	},
	hide : function(){
		if (this.index) {
			UI.hide(this._body);
	...
	tmpl : '<ul><%for(var i in info) {%><li><%=i%> (<%=info[i]%>)</li><%}%></ul>'
}
