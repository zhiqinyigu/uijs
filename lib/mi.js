...
MI = {
		...
		function send(){
			if (!el.sending) {
				var isFollow = -1,
					...
					classNameFollow = 'addAttention',
					classNameUnfollow = 'delAttention',
					classNameEach = 'delEach',
					followedNum = $('followedNum_' + id),
					followNum = $('followNum_' + id),
					isNotButton = !UI.hasClass(el,classNameFollow) && !UI.hasClass(el,classNameUnfollow) && !UI.hasClass(el,classNameEach);
				if (UI.hasClass(el,classNameFollow) || isNotButton ) {
					isFollow = 1;
					url = MI.url.follow;
				}
				else {
					if(UI.hasClass(el,classNameEach)){
						isFollow = 1;
						isEach = 1;
					}
					url = MI.url.unfollow;
				...
				MI.ajax({
					...
					success : function(data){
						...
						if (data.result == 0) {
							...
							if (!isNotButton) {
								if (isFollow == 1) {
									UI.removeClass(el,classNameFollow);
									if(isEach==1)
										UI.addClass(el,classNameEach);
									else
										UI.addClass(el,classNameUnfollow);
								}
								else {
									UI.addClass(el,classNameFollow);
									if(isEach==1)
										UI.removeClass(el,classNameEach);
									else
										UI.removeClass(el,classNameUnfollow);
								}
							}
						...
						if (MI.api.type == 5 && url == MI.url.follow) {
							MI.Bos('btnQzAppFollow');
						}
	 ...
	 * 18 => 评论的数量
	 * 20 => 任务提醒数
	 * 21-25 => 是web专用的计数位，已经占用 4=>21, 9=>22, 10=>23, 14=>25
	 * 27 => 简版微群提醒
	 * 28 => 评论页 VIP评论我
	 * 29 => 评论页 我收听的人
	 * 30 => 特别收听
	...
	newCount : function(type, url, time){
		...
		setTimeout(function(){ //不加延迟，IE中偶尔请求会被阻断
			...
			//使特别收听只拉取一次，减少调用量
			var reqOnce = /,30/g;
			if (!url && type.match(reqOnce)){
				MI.newCount.type = MI.newCount.type.replace(reqOnce,'');
			}
		},10);
		
		var fail = 0;
		function newCout(){
			var urlData = {type:MI.newCount.type,r:MI.random(10000)}; 
		...
		if(!MI.newTopMsgBox) {
			MI.newTopMsgBox = function(data){
				...
				var maxNum = 999,
					...
					{	type : [4],
						name : _('提到我的'),
						href : 'http://t.qq.com/at',
						bos : 'btnNewNavNews'
					},
					{
						type : 13,
						name : _('群内提到我的'),
						href : 'http://qun.t.qq.com/at.php',
						bos : 'btnNewNavAtQun'
					},
					{
						type : 1,
						name : _('听众'),
						href : 'http://t.qq.com/' + MI.user.account + '/follower',
						bos : 'btnNewNavFollower'
					},
					{
						type : 2,
						name : _('私信'),
						href : 'http://t.qq.com/messages/inbox',
					...
					},
					{
						type : 19,
						name : _('通知'),
						href : 'http://t.qq.com/messages/notice',
						bos : 'btnNewNavNoti',
				...
				UI.each(map,function(p,i){
					formatNum(p,i);
					totalNum += p.num || 0;
					listHtml += '<li style="display:' + ((UI.hasKey(p,'flag')) && !p.flag ? 'none' : 'block') + '"><a href="' + p.href + '" onmousedown="MI.Bos(' + "'" + p.bos + "'" + ',UI.text(this).replace(/\\D/g,\'\') || \'0\')">' + p.name + (p.num ? '<b>' + p.formatNum + '</b>' : '') + '</a></li>';
				});
				...
				function formatNum(p,i){
					...
					if(num) {
						...
						} else if(t == 2 || t == 4 || t == 14 || t== 19 || t== 20 || t==21 || t==13) { //私信等
							num = '+' + num;
						};
					}	
					p.formatNum = num || 0;
				...
				UI.GT(navMsgList,'a')[taskIndex].onclick = function(e){
					...
					MI.Bos(map[taskIndex].bos,UI.text(this).replace(/\D/g,'') || '0');
					return false
				};
			}
		};
...
MI.url = (function(){
	...
	return {
		...
		MoodList : apiHost + '/userEmotionStatistics.php', //心情列表
		...
		killnoreply : host + '/asyn/killnoreply.php',//消灭零回复
		userFriends : host + '/asyn/userFriends.php',//获取亲密朋友
...
MI.Bos = function(op, value, random, bossId) {
	var newBoss, // 是否为新上报方式
		newBossData = { // 新表数据
			iFlow : 0,
			iFrom : '', // 操作来源，feedBack调用的时候上报到1191表，用作上报用户列表的个数
			iPubFrom : '', // 操作二级来源，feedBack调用时上报到1191表，用作区分API类型
			sUrl : '', // 页面URL
			iUrlType : '', // 页面类型
			iPos : '', // 操作在页面上的位置
			sText : '', // 操作对象描述
...
MI.Crs = function(reset){
	...
	function show(o){
		...
		setTimeout(function(){
			...
			if (!isDuplicate && preview && MI.Crs.autoExpandPic) { //Auto open big picture
				if (o.parentNode && o.parentNode.onclick) {
					o.parentNode.onclick();
				}
			} else if (preview && MI.user.fun.autoPic) { //Auto Load Big Picture And Samll Picture
			...
			if (UI.hasClass(o.parentNode,"vThumbs")) { //For Video Thumbs
				...
				if (MI.user.fun.mergePic && MI.Crs.iconPic.hasString(src) && !UI.A(o,'show')) {
					...
					var mediaWrap = UI.parents(o,'mediaWrap')[0];
					if (mediaWrap){
						UI.addClass(mediaWrap, 'iconPic');
					}
				}else {
					MI.Crs.iconPic += src;
				}
...
/**
 * @property {boolean} autoExpandPic auto call picture.onlick() as page scrolls
 * @example
 *   MI.Crs.autoExpandPic = true;
 */
MI.Crs.autoExpandPic = false;
...
MI.TalkList.prototype = {
	...
	addMore : function(json, doNotSendPv){
		var Self = this,
			_new = Self._new,
			ajax = UI.isString(json),
			children = UI.children(Self._body),
		...
		*/
		if (data.result == 0) {
			...
			if (data.info.talk.length) {
				...
				if (!doNotSendPv) {//for qzone app, sometimes addMore is called when a list is created
	...
	if (!pic.loaded) {
		...
		if (MI.user.fun.minSizePicCheck) {//根据图的尺寸 决定 背景容器的宽度
			...
			if (UI.hasClass(this.parentNode, 'big')) {
				...
				MI.TalkList.rotateImg(false, pic.nextSibling);
				MI.TalkList.rotateImg(true, pic.nextSibling);
			}
...
MI.TalkList.picLoad = function(){
	...
	if (this.loaded) { //Show Tools After Loading
		...
		UI.addClass(this.parentNode.parentNode, 'big bgr2');
	}
}
	...
	if (UI.B.ie) {
		...
		if (MI.user.fun.minSizePicCheck) {//修复ie下 旋转 参照坐标的问题
			...
			if (pic.r % 2 != 0) {
				...
				pic.parentNode.style.height=newWidth + "px";
			}
			else {
				picBox.style.height = newHeight + "px";
				pic.parentNode.style.height=newHeight + "px";
			}
