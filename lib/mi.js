...
MI.TalkBox.prototype = {
	...
	send : function(){
		...
		//Go To User's Page
		if (!Self.type && content.match(/^@[\w|-]{0,64}$/g)) {
			...
			if (word) {
				for (var i in users) {
					...
					if (i.match(reg) || users[i].match(reg)) {
						...
						match.unshift(users[i].replace(reg,'<b>' + RegExp.$1 + '</b>') + ' (' + i.replace(reg,'<b>' + RegExp.$1 + '</b>') + ')');
						txt.unshift(i);
						matchLength++;
					}
					else if (i.match(regAll) || users[i].match(regAll)) {
...
UI.ready(function(){
	setTimeout(function(){
		...
		if (MI.talkList && MI.user) { //区分非主人页，非主人页不需要该功能
