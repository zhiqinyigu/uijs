...
MI = {
	...
	tmpl : {
		list : '<li id="<%=info.id%>" rel="<%=info.timestamp%>"><div class="userPic"><a href="/<%=info.account%>" title="(<%=info.nick.toTitle()%>@<%=info.account%>)"<%if(' + V2 + '.type==2){%> class="masPic"<%}%>><img src="<%=info.userPic%>"' + V6 + '/></a><%if(' + V2 + '.type==2){%><%=' + V2 + '.cusPic%><%}%></div><div class="msgBox"><div class="userName"><strong><a href="<%=info.account%>" title="(<%=info.nick.toTitle()%>@<%=info.account%>)"><%=info.nick%></a><%if(info.vip){' + V4 + '}%><%if(!info.action){%>:<%}%></strong><%=info.action%></div><div class="msgCnt"><%if(' + V2 + '.type){%> <%}%><%=info.content%>&nbsp;</div><%if(' + V2 + '.type==1){%><div class="replyBox"><%=' + V2 + '.content%></div><%}%><%if(info.image && info.image.length){%><br><%for(var i=0;i<info.image.length;i++){%><div class="picBox"><a href="<%=info.image[i]%>" class="pic"><img src="<%=info.image[i]%>"></a></div><%}}%><div class="pubInfo"><span class="left"><span class="time"><%=info.time%></span> 发自<%=info.sorurce%> <%if(' + V2 + '.type==2){%><a class="view zfNum"' + V5 + '>查看对话</a></b><%}%></span><div class="funBox"><div class="moreFun hide"><h4><a' + V5 + ' hideFocus="true">操作</a></h4><a' + V5 + ' class="relay">转播</a><a' + V5 + ' class="delBtn">删除</a></div></div></div><%if(' + V2 + '.type==2){%><div class="talkDetail"><b><%=' + V2 + '.nick%>:</b> <%=' + V2 + '.content%></div><%}%></div></li>',
		listAll : '<%for(var i=0,n=talk.length;i<n;i++){%><li id="<%=talk[i].id%>" rel="<%=talk[i].timestamp%>"><%if(!guest){%><%if(' + V1 + ' && talk[i].type==4){%><div class="userPic"><a href="/<%=talk[i].name%>"' + V7 + ' class="masPic"><img src="<%=talk[i].pic%>"' + V6 + '/></a><a href="/<%=' + V1 + '.name%>" title="<%=' + V1 + '.nick.toTitle()%>(@<%=' + V1 + '.name%>)" class="cusPic"><img src="<%=' + V1 + '.pic%>"' + V6 + '/></a></div><%}else{%><div class="userPic"><a href="/<%=talk[i].name%>"' + V7 + '><img src="<%=talk[i].pic%>"' + V6 + '/></a></div><%}%><%}%><div class="msgBox"><div class="userName" rel="<%=talk[i].name%>"><strong><a href="/<%=talk[i].name%>"' + V7 + '><%=talk[i].nick%></a><%if(talk[i].vip){ ' + V4 + ' }%><%if(!' + V1 + '){%>:<%}%></strong><%if(' + V1 + ' && talk[i].type==2){%>转播&nbsp;:&nbsp;<%}else if(' + V1 + ' && talk[i].type==4){%>回复 <strong class="userTo"><a href="/<%=' + V1 + '.name%>" title="<%=' + V1 + '.nick.toTitle()%>(@<%=' + V1 + '.name%>)"><%=' + V1 + '.nick%></a><%if (' + V1 + '.vip){' + V4 + ' }%>:<%}%></strong></div><div class="msgCnt"><%=talk[i].content%>&nbsp;</div><%if(talk[i].image && talk[i].image.length){%><br><%for(var j=0;j<talk[i].image.length;j++){%><div class="picBox"><a href="<%=talk[i].image[j]%>" class="pic"><img src="<%=talk[i].image[j]%>"></a></div><%}}%><%if(' + V1 + ' && talk[i].type==2){%><div class="replyBox"><div class="userPic"><a href="/<%=' + V1 + '.name%>" title="<%=' + V1 + '.nick.toTitle()%>(@<%=' + V1 + '.name%>)"><img src="<%=' + V1 + '.pic%>"' + V6 + '/></a></div><div class="msgBox"><div class="msgCnt"><strong><a href="/<%=' + V1 + '.name%>" title="<%=' + V1 + '.nick.toTitle()%>(@<%=' + V1 + '.name%>)"><%=' + V1 + '.nick%></a><%if(' + V1 + '.vip){' + V4 + '}%>:</strong><%=' + V1 + '.content%></div><%if(' + V1 + '.image && ' + V1 + '.image.length){%><br><%for(var j=0;j<' + V1 + '.image.length;j++){%><div class="picBox"><a href="<%=' + V1 + '.image[j]%>" class="pic"><img src="<%=' + V1 + '.image[j]%>"></a></div><%}}%><div class="pubInfo"><span class="left"><span class="time" rel="<%=' + V1 + '.timestamp%>"><%=' + V1 + '.time%></span> 发自<%=' + V1 + '.from%> <a class="zfNum" href="/t/<%=talk[i].id%>" title="点击查看转播路径" target="_blank"><strong class="relayNum"><%=' + V1 + '.count%></strong>人转播</a></span></div></div></div><%}%><div class="pubInfo"><span class="left"><span class="time"><%=talk[i].time%></span> 发自<%=talk[i].from%><%if(talk[i].count){%> <b class="relayNum"><%=talk[i].count%></b>人转播<%}%><%if(' + V1 + ' && ' + V1 + '.content && ' + V1 + '.content.length && talk[i].type==4){%> <a class="view zfNum"' + V5 + '>查看对话</a><%}%></span><div class="funBox"><%if(MI.user){if(!guest && MI.user.account==talk[i].name){%><div class="moreFun hide"><h4><a' + V5 + ' hideFocus="true">操作</a></h4><a' + V5 + ' class="relay">转播</a><a' + V5 + ' class="delBtn">删除</a></div><%}else{%><a' + V5 + ' class="reply">回复</a><span>|</span><div class="moreFun hide"><h4><a' + V5 + ' hideFocus="true">操作</a></h4><a' + V5 + ' class="relay">转播</a><a' + V5 + ' class="alarm" title="举报">举报</a></div><%}%><%}%></div></div><%if(' + V1 + ' && talk[i].type==4){%><div class="talkDetail"><b><%=' + V1 + '.nick%>:</b> <%=' + V1 + '.content%></div><%}%></div></li><%}%>',
		picTool : '<div class="tools"><a href="#" class="btnBack" title="向左转"></a><span>|</span><a href="#" class="btnPrev" title="向右转"></a><a href="$Url" class="btnOriginal" target="_blank">查看原图</a></div>',
		...
		UI.ready(function(){
			...
			if (Self._pic) {
				...
				*/
				...
				Self._iframe = UI.G('imageUpload');
				Self._picIco = UI.GC(Self._pic,'.ico_pic')[0];
				Self._picTxt = UI.GC(Self._pic,'.txt')[0];
				Self._picLoading = UI.GC(Self._pic,'.loading')[0];
				Self._picPreview = UI.GC(Self._pic,'.preview')[0];
				Self._picLink = UI.GC(Self._pic,'.link')[0];
				Self._picDel = UI.GC(Self._pic,'.del')[0];
...
MI.TalkBox.prototype = {
	...
	countTxt : function(){
		var Self = this,
			txt = Self._txt,
			talkTip,
			length = Math.ceil(MI.string.length(UI.trim(txt.value).replace(new RegExp(Self.txtTopic,'g'),'')) / 2);
	...
	addPic : function(o){
		if (o.result == 0) {
			...
			this._picLink.innerHTML = '<a href="#" class="fileName" onclick="return false">' + fileName + '<img src="' + o.info.image + '" /></a>';
			UI.show(this._picPreview);
			if (this._txt.value == '') {
				this._txt.value = '分享图片';
				this.countTxt();
				this._txt.select();
			}
		}
...
MI.TalkList.prototype = {
	...
	addEvent : function(el){
		var Self = this,
			...
			moreFun = UI.GC(el,'.moreFun')[0],
			replyMsg = UI.GC(el,'.replyMsg')[0];
			picBox = UI.GC(el,'.picBox');
		MI.addHover(el);
		if (reply) {
		...
		if (moreFun) { //More Function
			...
			/*moreFun.onmouseout = function(e){
				...
				if (E.target.nodeName == 'FIELDSET') {
					el = this;
					clearTimeout(MI.delay.moreFun);
					MI.delay.moreFun = setTimeout(function(){
						UI.addClass(el,'hide');
					},200);
					E.stop();
				}
		...
		if (picBox) {
			for (var i = 0,num = picBox.length;i < num;i++) {
				var pic = UI.GT(picBox[i],'img')[0],link = pic.parentNode;
				UI.before(UI.html(MI.tmpl.picTool.replace('$Url',link.href))[0],link); //Add Tools
				...
				UI.GC(picBox[i],'.btnBack')[0].onclick = UI.GC(picBox[i],'.btnPrev')[0].onclick = MI.TalkList.picTool;
			}
		}
...
MI.TalkList.pic = function(){ //Picture Event
	...
	if (!pic.loaded) {
		...
		if (UI.B.ie6) {
			_loading.style.cssText = 'width:' + UI.width(pic) + 'px;height:' + UI.height(pic) + 'px;';
		}
		pic.parentNode.appendChild(_loading);
...
MI.TalkList.picDraw = function(pic,canvas){
	...
	switch (pic.r){
		...
		case 1:
			...
			ctx.drawImage(pic,0,-pic.height);
			break;
		case 2:
			UI.A(canvas,'width',pic.width);
			UI.A(canvas,'height',pic.height);
