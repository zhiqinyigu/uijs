...
MI.TalkList.prototype = {
	...
	remove : function(id,type){
		id = String(id);
		var Self = this,talk = $(id),url = Self.removeUrl,data = {id:id,r:MI.random()};
		if (type) {
			url = Self.favorUrl;
			data.op = type;
		}
	...
	*/
	...
	function _show(){
		...
		if (Self.word) { //E.key == 50
			...
			var usersKey = (Self.key ? 'follow_' : 'followed_') + MI.user.account,users = MI.Storage(usersKey);
			if (users == 'NoStorage') {
				return;
			}
			else if (!Self.sending && (!users || new Date().getTime() - MI.Storage('time') > 7200000)){ //3600000
				Self.sending = 1;
				UI.get('/asyn/nicktips.php','type=0&num=' + (UI.B.ie ? 500 : 2000),function(data){
