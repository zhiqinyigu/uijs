...
MI.DropTips = function(id){
	...
	if (this._key) {
		...
		this._key.onfocus = function(){
			UI.hide(UI.prev(this));
			if (UI.trim(Self._key.value)) {
				Self.select(Self.index);
				UI.show(Self._select);
			}
		}
		...
		this._key.onkeyup = function(e){
			var E = UI.E(e),value = UI.trim(Self._key.value);
			if (value) {
				if (value.length > 3) {
				...
				UI.each(Self._value,function(el){
					el.innerHTML = value;
				});
				UI.show(Self._select);
			}
			else UI.hide(Self._select);
		}
...
MI.DropTips.prototype = {
	...
	select : function(index){
		var maxIndex = this.action.length - 1;
		if (index < 0) {
			index = 0;
		}
