...
MI.swf = (function(){
	...
	return {
		Mood : host + 'Mood_110927.swf', //心情
		MoodDemo : host + 'MoodDemo_110805.swf', //心情
		MusicRecord : host + 'MusicRecord_111010c.swf', //录音
...
MI.TalkBox = function(id){ //Talk Box
	...
	if (this._body) {
		...
		Self.guideText = UI.A(Self._txt,'guide');
		if (Self.guideText){
			//Self._guideText = UI.html('<span class="cNote" style="position:absolute;font-size:14px;top:4px;left:5px;">' + Self.guideText + '</span>')[0];
			//UI.before(Self._guideText,Self._txt);
			setTimeout(function(){
				if (Self._txt.value){
					Self.guideText = null;
				}
				else {
					UI.addClass(Self._txt,'cNote');
					Self._txt.value = Self.guideText;
				}
			},200);
		}
...
MI.TalkBox.prototype = {
	...
	guideTextRemove : function(){ //移除发表框中的引导词
		if (this.guideText){
			this._txt.value = '';
			UI.removeClass(this._txt,'cNote');
			this.countTxt();
			this.guideText = null;
		}
	},
	...
	save : function(){
		try{
			var text = this._txt.value.replace(new RegExp(this.txtTopic,'g'),'') || '';
