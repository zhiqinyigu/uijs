...
MI.TalkList.prototype = {
	...
	reply : function(id,type){
		...
		if (!this.talkBox.display) {
			var li = UI.G(id),cur = this.current,name = UI.GC(li,'.userName strong a')[0].innerHTML,vip = UI.GC(li,'.userName .vip').length,url = UI.GC(li,'.userName a')[0].href,cont = UI.GC(li,'.msgCnt')[0],content = contentBase = UI.text(cont),replyBox = UI.GC(li,'.replyBox')[0],relay = UI.GC(li,'.replyBox .msgCnt')[0],/*relayUser,*/userPic = UI.GC(li,'.userPic')[0];
			if (type && relay) {
...
MI.DropTips = function(id){ //DropTips
	var Self = this;
	this._key = UI.G(id);
	if (this._key) {
