...
MI = {
	...
	tmpl : {
		list : '<li id="<%=info.id%>" rel="<%=info.timestamp%>"><div class="userPic"><a href="/<%=info.account%>" title="(<%=info.nick.toTitle()%>@<%=info.account%>)"<%if(' + V2 + '.type==2){%> class="masPic"<%}%>><img src="<%=info.userPic%>"' + V6 + '/></a><%if(' + V2 + '.type==2){%><%=' + V2 + '.cusPic%><%}%></div><div class="msgBox"><div class="userName"><strong><a href="<%=info.account%>" title="(<%=info.nick.toTitle()%>@<%=info.account%>)"><%=info.nick%></a><%if(info.vip){' + V4 + '}%><%if(!info.action){%>:<%}%></strong><%=info.action%></div><div class="msgCnt"><%if(' + V2 + '.type){%> <%}%><%=info.content%>&nbsp;</div><%if(' + V2 + '.type==1){%><div class="replyBox"><%=' + V2 + '.content%></div><%}%><%if(info.image && info.image.length){%><div><%for(var i=0;i<info.image.length;i++){%><div class="picBox"><a href="<%=info.image[i]%>/460" class="pic"><img src="<%=info.image[i]%>/160"></a></div><%}%></div><%}%><div class="pubInfo"><span class="left"><span class="time"><%=info.time%></span> 发自<%=info.sorurce%> <%if(' + V2 + '.type==2){%><a class="view zfNum"' + V5 + '>查看对话</a></b><%}%></span><div class="funBox"><div class="moreFun hide"><h4><a' + V5 + ' hideFocus="true">操作</a></h4><a' + V5 + ' class="relay">转播</a><a' + V5 + ' class="delBtn">删除</a></div></div></div><%if(' + V2 + '.type==2){%><div class="talkDetail"><b><%=' + V2 + '.nick%>:</b> <%=' + V2 + '.content%></div><%}%></div></li>',
		listAll : '<%for(var i=0,n=talk.length;i<n;i++){%><li id="<%=talk[i].id%>" rel="<%=talk[i].timestamp%>"><%if(!guest){%><%if(' + V1 + ' && talk[i].type==4){%><div class="userPic"><a href="/<%=talk[i].name%>"' + V7 + ' class="masPic"><img src="<%=talk[i].pic%>"' + V6 + '/></a><a href="/<%=' + V1 + '.name%>" title="<%=' + V1 + '.nick.toTitle()%>(@<%=' + V1 + '.name%>)" class="cusPic"><img src="<%=' + V1 + '.pic%>"' + V6 + '/></a></div><%}else{%><div class="userPic"><a href="/<%=talk[i].name%>"' + V7 + '><img src="<%=talk[i].pic%>"' + V6 + '/></a></div><%}%><%}%><div class="msgBox"><div class="userName" rel="<%=talk[i].name%>"><strong><a href="/<%=talk[i].name%>"' + V7 + '><%=talk[i].nick%></a><%if(talk[i].vip){ ' + V4 + ' }%><%if(!' + V1 + '){%>:<%}%></strong><%if(' + V1 + ' && talk[i].type==2){%>转播&nbsp;:&nbsp;<%}else if(' + V1 + ' && talk[i].type==4){%>回复 <strong class="userTo"><a href="/<%=' + V1 + '.name%>" title="<%=' + V1 + '.nick.toTitle()%>(@<%=' + V1 + '.name%>)"><%=' + V1 + '.nick%></a><%if (' + V1 + '.vip){' + V4 + ' }%>:<%}%></strong></div><div class="msgCnt"><%=talk[i].content%>&nbsp;</div><%if(talk[i].image && talk[i].image.length){%><div><%for(var j=0;j<talk[i].image.length;j++){%><div class="picBox"><a href="<%=talk[i].image[j]%>/460" class="pic"><img src="<%=talk[i].image[j]%>/160"></a></div><%}%></div><%}%><%if(' + V1 + ' && talk[i].type==2){%><div class="replyBox"><div class="userPic"><a href="/<%=' + V1 + '.name%>" title="<%=' + V1 + '.nick.toTitle()%>(@<%=' + V1 + '.name%>)"><img src="<%=' + V1 + '.pic%>"' + V6 + '/></a></div><div class="msgBox"><div class="msgCnt"><strong><a href="/<%=' + V1 + '.name%>" title="<%=' + V1 + '.nick.toTitle()%>(@<%=' + V1 + '.name%>)"><%=' + V1 + '.nick%></a><%if(' + V1 + '.vip){' + V4 + '}%>:</strong><%=' + V1 + '.content%></div><%if(' + V1 + '.image && ' + V1 + '.image.length){%><br><%for(var j=0;j<' + V1 + '.image.length;j++){%><div class="picBox"><a href="<%=' + V1 + '.image[j]%>/460" class="pic"><img src="<%=' + V1 + '.image[j]%>/160"></a></div><%}}%><div class="pubInfo"><span class="left"><span class="time" rel="<%=' + V1 + '.timestamp%>"><%=' + V1 + '.time%></span> 发自<%=' + V1 + '.from%> <b class="relayNum"><%=' + V1 + '.count%></b>人转播</span></div></div></div><%}%><div class="pubInfo"><span class="left"><span class="time"><%=talk[i].time%></span> 发自<%=talk[i].from%><%if(talk[i].count){%> <b class="relayNum"><%=talk[i].count%></b>人转播<%}%><%if(' + V1 + ' && ' + V1 + '.content && ' + V1 + '.content.length && talk[i].type==4){%> <a class="view zfNum"' + V5 + '>查看对话</a><%}%></span><div class="funBox"><%if(MI.user){if(!guest && MI.user.account==talk[i].name){%><a' + V5 + ' class="relay">转播</a><span>|</span><a' + V5 + ' class="delBtn">删除</a><span class="last">|</span><div class="moreFun hide"><h4><a' + V5 + ' hideFocus="true">操作</a></h4><a' + V5 + ' class="alarm" title="举报">举报</a></div><%}else{%><a' + V5 + ' class="relay">转播</a><span>|</span><a' + V5 + ' class="reply">回复</a><span class="last">|</span><div class="moreFun hide"><h4><a' + V5 + ' hideFocus="true">操作</a></h4><a' + V5 + ' class="alarm" title="举报">举报</a></div><%}%><%}%></div></div><%if(' + V1 + ' && talk[i].type==4){%><div class="talkDetail"><b><%=' + V1 + '.nick%>:</b> <%=' + V1 + '.content%></div><%}%></div></li><%}%>',
	...
	insertTxt : function(el,text,cursorX,del){
		...
		if (document.selection) {
			if (del) {
				MI.selectTxt(el,cursorX - del,cursorX,cursorX);
...
MI.TalkBox.prototype = {
	...
	send : function(){
		...
		if (Self.topic) {
			data.content = Self.topic + data.content;
		}
		UI.ajax({
			url : Self.url,
			data : data,
...
MI.TalkList.pic = function(e){ //Picture Event
	...
	if (!pic.loaded) {
		...
		pic.src = pic.sUrl;
		//pic.src = pic.src == pic.sUrl ? picBigUrl : pic.sUrl;
		/*setTimeout(function(){
			alert(pic.parentNode.innerHTML);
		},300);*/
		//alert(pic.parentNode.innerHTML);
		pic.parentNode.blur();
...
MI.AutoCmt = function(o){ //Auto Complete
	...
	*/
	...
	//Self._shadow.contentEditable = true;
	//Self._shadow.designMode = 'on';
	...
	function _show(){
		...
		Self._txt.innerHTML = text.replace(/\r\n|\n/g,'<br>').replace(/ /g,'&nbsp;');
		...
		if (Self.word) { //E.key == 50
			if (!Self._body.appended) {
				...
				Self._body.onmouseover = function(e){
					var E = UI.E(e);
					if (E.target.nodeName == 'LI' && Self.index != null) {
						UI.removeClass(Self.list[Self.index],'on');
			...
			var users = MI.Storage('users'),account = MI.Storage('account');
			if (users == 'NoStorage') {
				return;
			}
			else if (!Self.sending && (!users || account != MI.user.account || new Date().getTime() - MI.Storage('time') > 3600000)){ //3600000
				Self.sending = 1;
				UI.get('/asyn/nicktips.php','type=0',function(data){
					...
					if (Data.result == 0) {
						...
						MI.Storage('account',MI.user.account);
				...
				if (!MI.users) {
					MI.users = MI.json(unescape(users));
				}
				Self.show(MI.users);
			}
		}
...
MI.AutoCmt.prototype = {
	...
	select : function(){
		if (UI.isNumber(this.index)) {
			...
			MI.insertTxt(this._target,this.list[this.index].txt,this.cursorX,this.word.slice(1).length);
			if (this.call) {
				this.call();
			}
	...
	show : function(o){
		if (this.word) {
			...
			if (!this.display) {
				this._body.style.cssText += ';top:' + (UI.getY(this._cursor) - this._target.scrollTop + 26) + 'px;left:' + (UI.getX(this._cursor) - 12) + 'px';
			}
			this.display = 1;
		}
		else {
...
MI.DialogMsg = function(o){
	o = o || {};
	o.tmpl = '<div class="bg"></div><div style="position:absolute;z-index:9999;top:0;left:25%;padding:25px" class="CR"><table border="0" cellspacing="0" cellpadding="0" class="tbSendMsg"><tr><td class="tl"></td><td class="tm" style="overflow:hidden"></td><td class="tr"></td></tr><tr><td class="lm"></td><td><div class="floatWrap"><div class="close DClose"></div><div class="DTitle" style="padding:0;border:0;"></div><div class="DCont" style="padding:0;border:0;background:none;"></div></div></td><td class="rm"></td></tr><tr><td class="bl"></td><td class="bm"></td><td class="br"></td></tr></table></div></div>';
