...
(function(){
var varSource = 'talk[i].source',varInfoSource = 'info.source',htmlArrow = '<div class="sideArrow"><em>◆</em><span>◆</span></div>',htmlVip = '%><b class="vip"></b><%',htmlHrefVoid = ' href="javascript:void(0)"',htmlImgError = ' onerror="MI.Pic(this, 50)"',htmlUserTitle = ' title="<%=talk[i].nick.toTitle()%>(@<%=talk[i].name%>)"',htmlDelete = htmlHrefVoid + ' onclick="if(confirm(MI.tips.del))MI.talkList.remove(';
MI = {
	string : {
		length : function(str){
	...
	tmpl : {
		list : '<li id="<%=info.id%>"><div class="userPic"><a href="/<%=info.account%>" title="(<%=info.nick.toTitle()%>@<%=info.account%>)"<%if(' + varInfoSource + '.type==2){%> class="masPic"<%}%>><img src="<%=info.userPic%>"' + htmlImgError + '/></a><%if(' + varInfoSource + '.type==2){%><%=' + varInfoSource + '.cusPic%><%}%></div><div class="msgBox"><div class="userName"><a href="<%=info.account%>" title="(<%=info.nick.toTitle()%>@<%=info.account%>)"><strong><%=info.nick%></strong></a><%if(info.vip){ ' + htmlVip + '}%> <%=info.action%></div><div class="msgCnt"><%=info.content%></div><%if(' + varInfoSource + '.type==1){%><div class="replyBox"><%=' + varInfoSource + '.content%></div><%}%><div class="pubInfo"><span class="left"><%=info.time%> 发自<%=info.sorurce%> <%if(' + varInfoSource + '.type==2){%><a class="view zfNum"' + htmlHrefVoid + '>查看对话</a></b><%}%></span><p class="funBox"><a' + htmlDelete + '\'<%=info.id%>\',this)" class="del">删除</a></p></div><%if(' + varInfoSource + '.type==2){%><div class="talkDetail"><%=' + varInfoSource + '.nick%>: <%=' + varInfoSource + '.content%></div><%}%></div></li>',
		listAll : '<%for(var i=0,n=talk.length;i<n;i++){%><li id="<%=talk[i].id%>" rel="<%=talk[i].timestamp%>"><%if(!guest){%><%if(' + varSource + ' && talk[i].type==4){%><div class="userPic"><a href="/<%=talk[i].name%>"' + htmlUserTitle + ' class="masPic"><img src="<%=talk[i].pic%>"' + htmlImgError + '/></a><a href="/<%=' + varSource + '.name%>" title="<%=' + varSource + '.nick.toTitle()%>(@<%=' + varSource + '.name%>)" class="cusPic"><img src="<%=' + varSource + '.pic%>"' + htmlImgError + '/></a></div><%}else{%><div class="userPic"><a href="/<%=talk[i].name%>"' + htmlUserTitle + '><img src="<%=talk[i].pic%>"' + htmlImgError + '/></a></div><%}%><%}%><div class="msgBox"><div class="userName" rel="<%=talk[i].name%>"><a href="/<%=talk[i].name%>"' + htmlUserTitle + '><strong><%=talk[i].nick%></strong></a><%if(talk[i].vip){ ' + htmlVip + ' }%> <%if(' + varSource + ' && talk[i].type==2){%>转播<%}else if(' + varSource + ' && talk[i].type==4){%>回复<a href="/<%=' + varSource + '.name%>" title="<%=' + varSource + '.nick.toTitle()%>(@<%=' + varSource + '.name%>)"><%=' + varSource + '.nick%></a><%if (' + varSource + '.vip){' + htmlVip + ' }%>:<%}%></div><div class="msgCnt"><%=talk[i].content%></div><%if(' + varSource + ' && talk[i].type==2){%><div class="replyBox">' + htmlArrow + '<div class="userPic"><a href="/<%=' + varSource + '.name%>" title="<%=' + varSource + '.nick.toTitle()%>(@<%=' + varSource + '.name%>)"><img src="<%=' + varSource + '.pic%>"' + htmlImgError + '/></a></div><div class="msgBox"><div class="msgCnt"><a href="/<%=' + varSource + '.name%>" title="<%=' + varSource + '.nick.toTitle()%>(@<%=' + varSource + '.name%>)"><strong><%=' + varSource + '.nick%></a><%if(' + varSource + '.vip){' + htmlVip + '}%></strong>: <%=' + varSource + '.content%></div><div class="pubInfo"><span class="left"><%=' + varSource + '.time%> 发自<%=' + varSource + '.from%> <a class="zfNum" href="/t/<%=talk[i].id%>"><strong class="relayNum"><%=' + varSource + '.count%></strong>人转播</a></span></div></div></div><%}%><div class="pubInfo"><span class="left"><%=talk[i].time%> 发自<%=talk[i].from%><%if(' + varSource + ' && ' + varSource + '.content && ' + varSource + '.content.length && talk[i].type==4){%> <a class="view zfNum"' + htmlHrefVoid + '>查看对话</a><%}%></span><p class="funBox"><%if(MI.user){if(!guest && MI.user.account==talk[i].name){%><a class="del"' + htmlDelete + '<%=talk[i].id%>,this)">删除</a><%}else{%><a' + htmlHrefVoid + ' class="report">举报</a><a class="reply talk"' + htmlHrefVoid + '>回复</a><a class="relay zf"' + htmlHrefVoid + '>转播</a><%}%><%}%></p></div><%if(' + varSource + ' && talk[i].type==4){%><div class="talkDetail"><%=' + varSource + '.nick%>: <%=' + varSource + '.content%></div><%}%></div></li><%}%>',
		msg : '<li id="<%=info.id%>"><div class="msgBox"><div class="userName"><a href="/<%=info.account%>"><%=info.nick%></a><%if(info.vip){ ' + htmlVip + '}%></div><div class="msgCnt"><%=info.content%></div><div class="pubTime"><%=info.time%></div></div><p class="btnBox"><a class="btn"' + htmlDelete + '<%=info.id%>,this)">删除</a></p></li>',
		reply : '<div class="talkWrap">' + htmlArrow + '<div class="top"><span class="left replyTitle"></span><a' + htmlHrefVoid + ' class="close" title="关闭">关闭</a></div><div><textarea class="inputTxt"></textarea></div><div class="bot"><input type="button" class="inputBtn sendBtn" value="" /><span class="countTxt"></span></div></div>'
	},
...
MI.TalkList.prototype = {
	...
	reply : function(id,type){
		...
		if (!this.talkBox.display) {
			...
			this.talkBox.source = {
				type : type ? 1 : 2,
				content : type ? (replyBox ? replyBox.innerHTML : htmlArrow + clone.innerHTML) : contentBase,
				nick : name,
...
MI.Dialog = function(o){ //Dialog
	...
	Self._body.innerHTML = '<div class="bg"></div><div class="commRound"><div class="top"><span class="left"><em></em></span><span class="right"><em></em></span></div><div class="dialogTitle"></div><a' + htmlHrefVoid + ' class="dialogClose" title="关闭">关闭</a><div class="dialogCont"></div><div class="bot"><span class="left"><em></em></span><span class="right"><em></em></span></div></div>';
	Self._bg = Self.$('.bg');
