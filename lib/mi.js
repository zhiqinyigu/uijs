...
MI.TalkList = function(id){ //Talk List
	...
	if (Self._new) {
		Self._new.onclick = function(){
			if (!this.sending) {
				UI.get('/asyn/home.php',{time:UI.A(UI.children(Self._body)[0],'rel'),p:2,type:1,r:MI.random()},function(data){
				//UI.get('/asyn/home.php','time=1262764663&id=6563091904923&r=' + MI.random(),function(data){
					data = eval('(' + data + ')');
					data.info.guest = Self.guest;
					if (data.result == 0) {
						var cache = document.createDocumentFragment(),o = UI.html(new UI.tmplString(MI.tmpl.listAll)(data.info)),length = data.info.talk.length;
						if (length) {
		...
		setInterval(function(){
			UI.get('/asyn/newMsgCount.php',{type:3,r:MI.random()},function(data){
				data = eval('(' + data + ')');
				//data = {"result":0,"msg":"\u6210\u529f","info":[{"type":3,"value":5}]};
				if (data.result == 0) {
					if (data.info && data.info[0].value) {
						UI.C(Self._new,'opacity',0);
						Self._new.innerHTML = '<a href="javascript:void(0)">有<strong>' + data.info[0].value + '</strong>条新消息,点击更新</a>';
						UI.show(Self._new);
...
MI.Talk.prototype = {
	reply : function(type){
		...
		var replyTitle = '转播 "' + UI.trim(Self.name) + ': ' + Self.content.slice(0,30),relayTitle = '回复 ' + Self.name + ':';
...
function setHeight(){
	var copyright = UI.G('Copyright'),dom = document.documentElement,className = 'position';
	if (copyright) {
