...
MI = {
	...
	tmpl : {
		...
		msg : '<li id="<%=info.id%>"><div class="msgBox"><div class="userName"><a href="/<%=info.account%>"><%=info.nick%></a><%if(info.vip){ ' + V4 + '}%></div><div class="msgCnt"><%=info.content%></div><div class="pubTime"><span class="time"><%=info.time%></span></div><p class="btnBox"><a class="btn delBtn"' + V5 + '>删除</a></p></div></li>',
...
MI.TalkBox = function(id){ //Talk Box
	...
	if (this._body) {
		...
		UI.ready(function(){
			...
			Self._txt.onbeforeeditfocus = function(e){
				if (!Self.sending) {
					Self.countTxt();
				}
			};
...
MI.TalkList.prototype = {
	...
	reply : function(id,type){
		...
		if (!this.talkBox.display) {
			...
			var replyTitle = '转播 <b>' + name + '</b>:&nbsp;"' + content.slice(0,20),relayTitle = '回复 <b>' + name + '</b>:';//MI.string.escape()
...
MI.Talk.prototype = {
	reply : function(type){
		...
		var replyTitle = '转播 <b>' + UI.trim(Self.name) + '</b>: "' + Self.content.slice(0,25),relayTitle = '回复 <b>' + Self.name + '</b>:';
