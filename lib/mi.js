...
(function(){
var V1 = 'talk[i].source',V2 = 'info.source',V3 = '<div class="sideArrow"><em>◆</em><span>◆</span></div>',V4 = '%><b class="vip"></b><%',V5 = ' href="javascript:void(0)"',V6 = ' onerror="MI.Pic(this, 50)"',V7 = ' title="<%=talk[i].nick.toTitle()%>(@<%=talk[i].name%>)"',V8 = V5 + ' onclick="if(confirm(MI.tips.del))MI.talkList.remove(';
MI = {
	string : {
		length : function(str){
			...
			UI.get(url,'u=' + id + '&r=' + MI.random(),function(data){
				...
				if (data.result == 0) {
					...
					for (var i = 0,num = MI.fC.numFormat.length;i < num;i++) {
						MI.countNum(MI.fC.numFormat[i],isFollow,true);
					}
...
MI.Talk.prototype = {
	reply : function(type){
		...
		UI.GC(Self.talkBox._body,'.replyTitle')[0].innerHTML = type ? '转播 "' + UI.trim(Self.name) + ': ' + Self.content.slice(0,12) + (Self.content.length > 12 ? '...' : '') + '"<br/>再点评两句:' : '回复 ' + Self.name + ':';
...
MI.Dialog = function(o){ //Dialog
	...
	Self._body.innerHTML = '<div class="bg"></div><div class="commRound"><div class="top"><span class="left"><em></em></span><span class="right"><em></em></span></div><div class="dialogTitle"></div><a' + V5 + ' class="dialogClose" title="关闭">关闭</a><div class="dialogCont"></div><div class="bot"><span class="left"><em></em></span><span class="right"><em></em></span></div></div>';
	Self._bg = Self.$('.bg');
