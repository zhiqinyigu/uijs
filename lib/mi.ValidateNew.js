...
MI.ValidateNew = function(o){ //Validate New For Register
	...
	for (var i in Self.data.inputs) {
		...
		if (target) {
			...
			UI.EA(target,'blur',function (e) {
				Self.validate(e);
			});
			UI.EA(target,'keyup',reset);
		}
		else {
			delete Self.data.inputs[i];
			continue;
		}
	}
	...
	var submit = function(){
		...
		function post(){
			...
			Self.xhr = MI.ajax({url:UI.A(Self._body,'action'),data:dataPost,success:function(data){
...
MI.ValidateNew.prototype = {
	...
	validate : function (e) {
		var Self = this, E, i, input, value, valueReplaced, error = 0, isSubmit = 0, url = {};
		if (UI.isString(e)) {
			i = e;
			isSubmit = 1;
		}
		...
		if (isSubmit || (E.type === 'blur' || E.type === 'keyup') && target === E.target) {
		...
		url[i] = value;
		url.r = MI.random();
		if (!isSubmit && !error && input.url && (target.blured && E.type == 'blur' || E.type === 'keyup') && UI.trim(value)) {
			if (input.urlBefore){
				input.urlBefore();
			}
			MI.ajax({
				url : UI.isFunction(input.url) ? input.url() : input.url,
				type : 'get',
				data : url,
	...
	/**
	 * 使指定的input不再被Validator校验
	 * @param {Array} inputs 数组 存放想放暂停校验的input的DOM 
	 */
	disable : function (inputs) {
		var Self = this;
		if (inputs){
			for (var i=0, len=inputs.length; i<len; i++){
				if (Self.data.inputs[inputs[i]]){
					Self.data.inputs[inputs[i]].removed = true;
				}
			}
		}
	},
	enable : function (inputs) {
		...
		if (inputs){
			for (var i=0, len=inputs.length; i<len; i++){
				if (Self.data.inputs[inputs[i]]){
					Self.data.inputs[inputs[i]].removed = false;
				}
			}
		}
	}
