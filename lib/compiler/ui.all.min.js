...
var UI={resize:function(a,b){a=UI.isObject(a)?[a]:UI.GC(a);UI.each(a,function(c){if("TEXTAREA,SELECT,INPUT,BUTTON,IMG".hasString(c.nodeName)){var g="",o="";if(UI.hasClass(c,"tipBox")){UI.removeClass(c,"tipBox");g=" tipBox";o=' title="'+c.title+'"';c.title="";UI.EA(c,"click",function(m){UI.E(m).stop()})}UI.wrap('<span class="resize_box'+g+'"'+o+'><b class="ico"></b><span></span></span>',c)}else{g=UI.html('<b class="ico"></b>')[0];c.appendChild(g)}new UI.Resize(c,b)})},gotop:function(a){this.Gotop.build(a)},
...
31)<<6|c2&63);c+=2}else{c2=a.charCodeAt(c+1);c3=a.charCodeAt(c+2);b+=String.fromCharCode((g&15)<<12|(c2&63)<<6|c3&63);c+=3}}return b}},parseUrl:function(){var a=document.location.href,b={};if(!a.hasString("?"))return b;a=a.split("?")[1].split("&");for(var c=0;c<a.length;c++){var g=a[c].split("=");b[g[0]]=UI.Browser.ie?g[1]:UI.url.decode(g[1])}return b},cookie:function(a,b,c){if(b==undefined){a=a+"=";b=document.cookie.split(";");for(c=0;c<b.length;c++){for(var g=b[c];g.charAt(0)==" ";)g=g.substring(1,
g.length);if(g.indexOf(a)==0)return decodeURIComponent(g.substring(a.length,g.length))}return null}else{g="";if(c){g=new Date;g.setTime(g.getTime()+c*24*60*60*1000);g="; expires="+g.toGMTString()}document.cookie=a+"="+b+g+"; path=/"}},drag:function(a,b,c){var g=document;c=c!=undefined?c:true;UI.EA(a,"mousedown",function(o){b.start&&b.start(o);if(c)if(a.setCapture)a.setCapture();else window.captureEvents&&window.captureEvents(Event.MOUSEMOVE|Event.MOUSEUP);if(b.drag)g.onmousemove=b.drag;g.onmouseup=
function(){if(c)if(a.releaseCapture)a.releaseCapture();else window.captureEvents&&window.captureEvents(Event.MOUSEMOVE|Event.MOUSEUP);b.stop&&b.stop(o);g.onmousemove=null;g.onmouseup=null;b.call&&b.call(o)}})},animate:function(a,b,c,g){var o=setInterval(function(){var m=UI.C(a,b);if(b=="opacity"){m*=100;c*=100}else m=m=="auto"?0:Number(m.slice(0,-2));if(Math.abs(c-m)<3){m=c;clearInterval(o);eval(g)}UI.C(a,b,b!="opacity"?m+(c-m)*0.4+"px":(m+(c-m)*0.4)/100+"")},40);return o},getX:function(a){return a.offsetParent?
a.offsetLeft+UI.getX(a.offsetParent):a.offsetLeft},getY:function(a){return a.offsetParent?a.offsetTop+UI.getY(a.offsetParent):a.offsetTop},within:function(a,b){var c=UI.getX(b)-UI.scrollX(),g=UI.width(b)+c,o=UI.getY(b)-UI.scrollY();b=UI.height(b)+o;var m={};if(a[0]>c&&a[0]<g&&a[1]>o&&a[1]<b){if(a[0]-c<(g-c)/2)m.left=true;if(a[1]-o<(b-o)/2)m.top=true;return m}},frameX:function(a){return a.frameElement?UI.getX(a.frameElement)+UI.frameX(a.parent):0},frameY:function(a){return a.frameElement?UI.getY(a.frameElement)+
UI.frameY(a.parent):0},width:function(a){return parseInt(a.offsetWidth)},height:function(a){return parseInt(a.offsetHeight)},pageWidth:function(){return document.body.scrollWidth||document.documentElement.scrollWidth},pageHeight:function(){return document.body.scrollHeight||document.documentElement.scrollHeight},windowWidth:function(){var a=document.documentElement;return self.innerWidth||a&&a.clientWidth||document.body.clientWidth},windowHeight:function(){var a=document.documentElement;return self.innerHeight||
a&&a.clientHeight||document.body.clientHeight},scrollX:function(a){var b=document.documentElement;if(a){var c=a.parentNode,g=a.scrollLeft||0;if(a==b)g=UI.scrollX();return c?g+UI.scrollX(c):g}return self.pageXOffset||b&&b.scrollLeft||document.body.scrollLeft},scrollY:function(a){var b=document.documentElement;if(a){var c=a.parentNode,g=a.scrollTop||0;if(a==b)g=UI.scrollY();return c?g+UI.scrollY(c):g}return self.pageYOffset||b&&b.scrollTop||document.body.scrollTop},scrollTo:function(a,b,c){if(a==document.documentElement||
a==document.body)return window.scrollTo(b,c)},hide:function(a){if(!a)return false;if(UI.isString(a))a=this.G(a);var b=this.C(a,"display");if(b!="none")a.__curDisplay=b;a.style.display="none"},show:function(a){if(!a)return false;if(UI.isString(a))a=this.G(a);a.style.display=a.__curDisplay||""},toggle:function(a){if(UI.isString(a))a=this.G(a);this.C(a,"display")=="none"?this.show(a):this.hide(a)},hasClass:function(a,b){if(!a.className)return false;return a.className!=a.className.replace(new RegExp("\\b"+
b+"\\b"),"")},addClass:function(a,b){if(a.className)if(this.hasClass(a,b))return false;else a.className+=" "+b;else a.className=b},removeClass:function(a,b){a.className=a.className.replace(new RegExp("\\b"+b+"\\b"),"")},toggleClass:function(a,b){this.hasClass(a,b)?this.removeClass(a,b):this.addClass(a,b)},node:{ELEMENT:1,ATTRIBUTE:2,TEXT:3,CDATA_SECTION:4,ENTITY:6,COMMENT:8,DOCUMENT:9,DOCUMENT_TYPE:10},next:function(a){a=a.nextSibling;if(a==null)return false;return UI.isElement(a)?a:this.next(a)},
...
var c="wrapObject___",g=RegExp.lastMatch;a=a.replace(g,g+'<pre class="'+c+'"></pre>');a=UI.html(a)[0];UI.before(a,b);UI.replace(b,UI.GC(a,"pre."+c)[0])}else{UI.before(a,b);b.appendChild(b)}},html:function(a){var b=UI.DC("div"),c=[];b.innerHTML=a;UI.each(b.childNodes,function(g){c.push(g)});return c},text:function a(b){var c=[];b=b.childNodes;for(var g=0,o=b.length;g<o;g++)c.push(b[g].nodeType!=1?b[g].nodeValue:a(b[g]));return c.join("")},parent:function(a,b){if(UI.isArray(a)){var c=[];UI.each(a,function(g){if(b&&
UI.hasClass(g.parentNode,b)||!b)c.push(g.parentNode)});return c}return a.parentNode},parents:function(a,b){if(b){var c=[];a=UI.parents(a);UI.each(a,function(g){UI.hasClass(g,b)&&c.push(g)});return c}a=a.parentNode;return a.nodeName=="HTML"?[a]:[a].concat(UI.parents(a))},children:function(a,b){var c=[];if(b)b=b.split("|");UI.each(a.childNodes,function(g){var o=false;if(b)for(var m=0,f=b.length;m<f;m++)if(UI.hasClass(g,b[m])){o=true;break}if(UI.isElement(g)&&(!b||o))c.push(g)});return c},A:function(a,
b,c){if(c==undefined)return a.getAttribute(b);else a.setAttribute(b,c)},C:function(a,b,c){if(c==undefined)if(a.currentStyle){if(b=="opacity")return a.style.filter.indexOf("opacity=")>=0?parseFloat(a.style.filter.match(/opacity=([^)]*)/)[1])/100:"1";return a.currentStyle[b]}else{if(window.getComputedStyle){b=b.replace(/([A-Z])/g,"-$1");b=b.toLowerCase();return window.getComputedStyle(a,null).getPropertyValue(b)}}else if(b=="opacity"&&UI.Browser.ie)a.style.filter=(a.filter||"").replace(/alpha\([^)]*\)/,
"")+"alpha(opacity="+c*100+")";else a.style[b]=c},DC:function(a){return document.createElement(a)},E:function(a){if(a&&a.clone)return a;a=window.event||a;return{clone:true,stop:function(){if(a&&a.stopPropagation)a.stopPropagation();else a.cancelBubble=true},prevent:function(){if(a&&a.preventDefault)a.preventDefault();else a.returnValue=false},target:a.target||a.srcElement,x:a.clientX||a.pageX,y:a.clientY||a.pageY,button:a.button,key:a.keyCode,shift:a.shiftKey,alt:a.altKey,ctrl:a.ctrlKey,type:a.type}},
EA:function(a,b,c,g){if(UI.isString(a)){var o=c;c=function(){eval(o)}}if(a.addEventListener){a.addEventListener(b,c,g);return true}else if(a.attachEvent)return a=a.attachEvent("on"+b,c);else return false},ER:function(a,b,c){if(a.removeEventListener){a.removeEventListener(b,c,false);return true}else if(a.detachEvent)return a=a.detachEvent("on"+b,c);else return false},ET:function(a){return a.target||a.srcElement},G:function(a){return document.getElementById(a)},GT:function(a,b){return a.getElementsByTagName(b)},
GC:function(){var a=/(?:[\w\-\\.#]+)+(?:\[\w+?=([\'"])?(?:\\\1|.)+?\1\])?|\*|>/ig,b=/^(?:[\w\-_]+)?\.([\w\-_]+)/,c=/^(?:[\w\-_]+)?#([\w\-_]+)/,g=/^([\w\*\-_]+)/,o=[null,null];function m(h,k){if(!k){k=h;h=document}h=h||document;var n=/^[\w\-_#]+$/.test(k);if(!n&&h.querySelectorAll)return f(h.querySelectorAll(k));if(k.indexOf(",")>-1){k=k.split(/,/g);n=[];for(var e=0,j=k.length;e<j;++e)n=n.concat(m(k[e],h));return p(n)}k=k.match(a);j=k.pop();n=(j.match(c)||o)[1];e=!n&&(j.match(b)||o)[1];j=!n&&(j.match(g)||
o)[1];if(e&&!j&&h.getElementsByClassName)j=f(h.getElementsByClassName(e));else{j=!n&&f(h.getElementsByTagName(j||"*"));if(e)j=i(j,"className",RegExp("(^|\\s)"+e+"(\\s|$)"));if(n)return(h=h.getElementById(n))?[h]:[]}return k[0]&&j[0]?l(k,j):j}function f(h){try{return Array.prototype.slice.call(h)}catch(k){for(var n=[],e=0,j=h.length;e<j;++e)n[e]=h[e];return n}}function l(h,k,n){var e=h.pop();if(e===">")return l(h,k,true);var j=[],q=-1,r=(e.match(c)||o)[1],t=!r&&(e.match(b)||o)[1];e=!r&&(e.match(g)||
o)[1];var w=-1,v,y,x;for(e=e&&e.toLowerCase();v=k[++w];){y=v.parentNode;do{x=(x=(x=!e||e==="*"||e===y.nodeName.toLowerCase())&&(!r||y.id===r))&&(!t||RegExp("(^|\\s)"+t+"(\\s|$)").test(y.className));if(n||x)break}while(y=y.parentNode);if(x)j[++q]=v}return h[0]&&j[0]?l(h,j):j}var p=function(){var h=+new Date,k=function(){var n=1;return function(e){var j=e[h],q=n++;if(!j){e[h]=q;return true}return false}}();return function(n){for(var e=n.length,j=[],q=-1,r=0,t;r<e;++r){t=n[r];if(k(t))j[++q]=t}h+=1;return j}}();
function i(h,k,n){for(var e=-1,j,q=-1,r=[];j=h[++e];)if(n.test(j[k]))r[++q]=j;return r}return m}(),isDate:function(a){if(!a)return a;if(a.getTime&&a.getFullYear&&a.getTimezoneOffset&&a!="NaN"&&a!="Invalid Date")return true},cloneDate:function(a){if(!a)return a;d=new Date;d.setTime(a.getTime());return d},formatDate:function(a,b){var c=b.replace(/\W/g,",").split(","),g=["yyyy","MM","dd","hh","mm","ss","ww"];a={y:a.getFullYear(),M:a.getMonth()+1,d:a.getDate(),h:a.getHours(),m:a.getMinutes(),s:a.getSeconds(),
w:a.getDay()};for(var o=0,m=c.length;o<m;o++)for(var f=c[o],l=0;l<7;l++){var p=g[l].slice(-1);if(f.hasString(p)){if(p=="w"&&a[p]==0)a[p]=7;b=f.hasString(g[l])?b.replace(RegExp(g[l],"g"),this.addZero(a[p])):b.replace(RegExp(g[l].slice(g[l].length/2),"g"),a[p])}}return b},parseDate:function(a,b){b||(b="yyyy-MM-dd");b=b.replace(/\W/g,",").split(",");a=a.replace(/\D/g,",").split(",");var c=2000,g=0,o=1,m=0,f=0,l=0,p=true;UI.each(b,function(i,h){if(a[h]==""||isNaN(a[h]))p=false;if(i.hasString("y"))c=Number(a[h]);
if(i.hasString("M"))g=Number(a[h])-1;if(i.hasString("d"))o=Number(a[h]);if(i.hasString("h"))m=Number(a[h]);if(i.hasString("m"))f=Number(a[h]);if(i.hasString("s"))l=Number(a[h]);if(i.hasString("w"))l=Number(a[h])});if(!p)return false;return new Date(c,g,o,m,f,l)},isArray:function(a){return a!==null&&UI.isObject(a)&&"splice"in a&&"join"in a},isElement:function(a){return a&&a.nodeType==1},isFunction:function(a){return typeof a=="function"},isNumber:function(a){return typeof a=="number"},isObject:function(a){return typeof a==
"object"},isString:function(a){return typeof a=="string"},isUndefined:function(a){return typeof a=="undefined"},addZero:function(a,b){var c=[],g=String(a).length;b||(b=2);if(g<b)for(var o=0;o<b-g;o++)c.push(0);c.push(a);return c.join("")},trim:function(a){return a.replace(/^\s+|\s+$/g,"")},random:function(a,b){if(a==undefined)a=0;if(b==undefined)b=9;return Math.floor(Math.random()*(b-a+1)+a)},has:function(a,b){for(var c=0,g=a.length;c<g;c++)if(a[c]==b)return true;return false},each:function(a,b){if(UI.isUndefined(a[0]))for(var c in a)UI.isFunction(a[c])||
b(c,a[c]);else{c=0;for(var g=a.length;c<g;c++)UI.isFunction(a[c])||b(a[c],c)}},map:function(a,b){if(UI.isString(b))b=eval("(function(a,i) { return "+b+"})");var c=[];UI.each(a,function(g,o){g=b(g,o);if(UI.isArray(g))c=c.concat(g);else c.push(g)});return c},grep:function(a,b){if(UI.isString(b))b=eval("(function(a,i) { return "+b+"})");var c=[];UI.each(a,function(g,o){b(g,o)&&c.push(g)});return c},merge:function(a,b){var c=[];if(b){UI.each(b,function(g){UI.has(a,g)||c.push(g)});return a.concat(c)}else{UI.each(a,
function(g){UI.has(c,g)||c.push(g)});return c}},apart:function(a,b){var c=[];UI.each(a,function(g){UI.has(b,g)||c.push(g)});return c},sort:{number:function(a,b){return a-b},numberDesc:function(a,b){return b-a},string:function(a,b){return a.localeCompare(b)},stringDesc:function(a,b){return b.localeCompare(a)}},ready:function(a){if(UI.ready.done)return a();if(UI.ready.timer)UI.ready.ready.push(a);else{UI.ready.ready=[a];UI.ready.timer=setInterval(UI.isReady,13)}},isReady:function(){if(UI.ready.done)return false;
if(document&&document.getElementsByTagName&&document.getElementById&&document.body){clearInterval(UI.ready.timer);UI.ready.timer=null;for(var a=0;a<UI.ready.ready.length;a++)UI.ready.ready[a]();UI.ready.ready=null;UI.ready.done=true}},Browser:function(){var a={},b=navigator.userAgent;a.ie6=b.hasString("MSIE 6")&&!b.hasString("MSIE 7")&&!b.hasString("MSIE 8");a.ie=b.hasString("MSIE");a.tt=b.hasString("TencentTraveler");a.opera=window.opera||b.hasString("Opera");a.safari=b.hasString("WebKit");a.maxthon=
!!(window.external&&window.external.max_version);return a}()};UI.Flash=function(a,b,c,g){a.innerHTML='<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="'+c+'" height="'+g+'" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,28,0"><param name="allowScriptAccess" value="always" /><param name="movie" value="'+b+'" /><param name="quality" value="high" /><param name="wmode" value="transparent" /><embed src="'+b+'" width="'+c+'" height="'+g+'" quality="high" wmode="transparent" pluginspage="http://www.adobe.com/shockwave/download/download.cgi?P1_Prod_Version=ShockwaveFlash" allowScriptAccess="always" type="application/x-shockwave-flash"/></object>'};
...
if(this.cover)this.cover.style.cssText="";this.display=false}}})();UI.select=function(a){this.Select.build(a)};
