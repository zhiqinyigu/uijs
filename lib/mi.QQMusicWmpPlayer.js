...
function createMediaPlayer()
{
	return insertWMP({
		...
		canScan : 'true',
		canSeek : 'true',
		src : 'http://qzone-music.qq.com/fcg-bin/fcg_set_musiccookie.fcg?fromtag=22&p='+Math.random()
...
function WMPlayer()
{
	...
	this.mKeyCookieName = "";	
	this.mFromTag=22;
	this.oldPlayState = 1; // @add samlu 上一个WMP播放状态
	this.playStateChange = null; // @add samlu WMP播放状态改变时的回调函数
	...
	this.setMusicCookie = function(){
		...
		this.startPlayer();
		var _th=this;
		setTimeout(function(){_th.mSetedCookie = true;},1000);
	};
	this.initialize = function()
	{
		if(!this.checkPlayer())
		{
			if(this.mInstall)
			{
				alert(_("对不起，您现在的媒体播放器版本太低，请升级媒体播放器。"));
	...
	this.runPlayer = function(ul,name,singerName,iSongId,iSongType)
	{
		...
		else
		{
			//alert("runPlayer 2");
			var _th=this;
			window.idRunPlayer = setTimeout(function(){_th.runPlayer(ul,name,singerName,iSongId,iSongType)},500);
		}
	...
	this.setBalance = function()
	{
		...
		return(oplay.settings.balance=='100'?_('右声道'):_('左声道'));
	};
	...
	this.printPlayList = function()
	{
		...
		{
			list = list+_("第[{0}]播放记录:",i)+this.mPlayList.getObject(i-1).mPlayURL+"\n";  
		}	
		return list;
	};	
}
...
function checkPlayer()
{
	try {
		...
		switch (iStatus) {
			...
			case 3 :// 正在播放
				...
				*/
				...
				break;
			case 6 : // 缓冲中
				// @add samlu 与Music中的事件缓冲(buffering:4)标识码不符，进行修正
				iStatus = 4;
				break;
			case 8 : // 播放结束
				// @add samlu 与Music中的播放结束事件(PlayEnd:6)标识码不符，进行修正
				iStatus = 6;
				break;
			case 10 : // 准备就绪
				//BQplay();
		...
		// @add samlu 如果WMP发生播放状态改变 回调处理函数
		if (MediaPlayer.oldPlayState != iStatus){
			if (MediaPlayer.playStateChange){
				MediaPlayer.playStateChange(iStatus);
			}
			// 更新WMP播放状态
			MediaPlayer.oldPlayState = iStatus;
		}
		loopCheckPlayer(); // 重新激活播放状态循环检测
	} catch (e) {
