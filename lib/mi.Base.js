...
//举报
function jubao(urlParam,type){
	var url="http://jubao.qq.com/cn/jubao?";
	//如果是申诉
	if(type && type==2)
		url="http://jubao.qq.com/cn/im_appeal?";
	...
	MI.dialog.show({title:'<h3 class="DmainTit">' + ((type && type==2)? _('腾讯申诉平台') : _('腾讯举报平台')) + '</h3>',width:562,html:wrap});
	MI.dialog.showLoad();
	oIFrame.src = url;
}
...
function jubao_url(id,name,url,type){
	url = encodeURIComponent(url);
	if(type==1)
		jubao('appname=micoblog&subapp=web&jubaotype=url&tw_msg_id='+id+'&tw_name='+name+'&url='+url);
	else if(type==2)
		jubao('appname=appeal&subapp=micoblog_web&jubaotype=url&tw_name='+name+'&langcode=0&encoding=2&url='+url,type);	
}
...
MI.loadPM = function(_afJsOnload)
{
	var _sHost = location.protocol + "//pm.t.qq.com/",
		...
		_sJsUrl = [_sStaticHost, "zh_CN/htmledition/js/", _jsName, "?version=120119"].join(""),
		_sCssUrl = _sStaticHost + "zh_CN/htmledition/style/wbpm.css?v=120119";
	if (MI.user.fun.msgCgiLab){
		_sHost = location.protocol + "//t.qq.com/";
		_sJsUrl = MI.version.Message;
		_sCssUrl = MI.version.message;
	}
		...
		setTimeout(function()
			{
				if (!_bLoaded)
				{
					...
					if (MI.user.fun.msgCgiLab){
						//a.src = _sHost + "inbox/getinvestigate.php?stat=wbpm&type=pmloginfail&subtype=" + qq + ",8" + r;
					}
					else {
...
MI.TalkBox.prototype.addFace = function(){
	...
	MI.app({
		Tmpl : function(){
			if (!Self._faceBox){
				...
				UI.each(Self._faceTab,function(o,i){ //页卡切换
					o.onclick = function(){
						if (Self._faceTabCur != i){
							...
							Self._faceTabCur = i;
							var faceTabSub = $$(Self._faceCont[Self._faceTabCur],'.subTab1 a');
							if (faceTabSub.length){ //二级Tab
								UI.each(faceTabSub,function(tab){
									tab.onclick = function(){
										Self._faceList = $$(Self._faceCont[Self._faceTabCur],'.faceCell');
										UI.each(Self._faceList,function(list){
											UI.hide(list);
										});
										UI.removeClass($$(this.parentNode,'.cur')[0],'cur');
										UI.addClass(this,'cur');
										var page = UI.A(this,'page'), //翻页设置
											start, //开始索引
											pageNum; //页数
										if (page){
											page = page.split('|');
											start = page[0];
											pageNum = page[1];
											Self._faceList = Self._faceList.splice(start - 1,pageNum);
											Self.page.show(1,pageNum);
										}
										return false;
									}
		
								});
								faceTabSub[0].onclick();
							}
							else { //无二级Tab
								Self._faceList = $$(Self._faceCont[Self._faceTabCur],'.faceCell');
					...
					if(!UI.isUndefined(Self.faceHideTabIndex)) { //隐藏页卡
						if(Self.faceHideTabIndex[i]) {
							UI.hide(o);
						}
					}
				...
				UI.each($$(Self._faceBox,'.faceBox .faceCell a'),function(o,i){ //表情事件
					o.onclick = function(){
						...
						*/
						if (type == 'art' && Self.addFaceArt[title]){
							Self.addPic({
								result : 0,
								msg : _('上传成功'),
								info : {
									image : 'http://t0.qpic.cn/mblogpic/' + Self.addFaceArt[title],
									fileName : title + '.jpg'
								}
							});
						}
						...
						MI.Bos('btnFaceInsert' + type,temp);
						Self.hideFace();
						return false;
					}
					o.onmouseover = function(){
						...
						if (fold == 'z'){ //Emoji
							file = 'emoji/' + this.className;
							UI.A(this,'type','z');
							fileType = 'png';
							num = 15;
						}
						if (fold != '/art'){ //过滤掉艺术表情
							Self._facePreview.innerHTML = '<div><p class="faceImg"><img src="http://mat1.gtimg.com/www/mb/images/' + file + '.' + fileType + '" alt=""></p><p class="faceName">' + this.title + '</p></div>';
							Self._facePreview.style.cssText = index % num > 7  ? 'left:12px;' : '';
						}
					}
					/*
					var fold = UI.A(o.parentNode.parentNode,'fold'),
						file;
					if (fold == 'z'){
						file = 'http://mat1.gtimg.com/www/mb/images/emoji/' + o.className + '.png';
						o.style.background = 'url(' + file + ') no-repeat 50% 50%';
					}
					*/
			...
			if (Self._faceBox.style.display == 'none'){
				setTimeout(function(){
					...
					if (Self._faceArrow && UI.text(Self._face)){
						var iconWidth = UI.width(Self._face); //小于16px说明没有文字
						UI.C(Self._faceArrow,'left',(iconWidth > 16 ? (iconWidth + 16) : iconWidth) / 2 + 'px');
					}
					else {
			...
			if(Self.faceSingleTabIndex) UI.fireEvent(Self._faceTab[Self.faceSingleTabIndex],'click'); //触发点击，打开单个tab
		}
	});
}
...
;(function(){ //艺术表情 http://t0.qpic.cn/mblogpic/f260699400457a9f1e3e/2000
	var face = {};
	face[_('年夜饭')] = 'b90fbff36b7e513fdc32';
	face[_('恭喜发财')] = 'd82a45604c6740e23796';
	face[_('年年有余')] = '04968f0a62129fa1242c';
	face[_('花开富贵')] = '01e0936320513214b88e';
	face[_('2012')] = 'f260699400457a9f1e3e';
	face[_('火箭')] = 'f41cb3c61f6550d3956a';
	face[_('船票')] = '7b984c57b94f166dfe54';
	face[_('合家欢')] = 'e30e064caf31e6e3698e';
	face[_('祝福礼花')] = 'e941afa4483a62e6fb76';
	face[_('雪人')] = '29b8d2eaec0e269e36b2';
	face[_('圣诞烟火')] = '1c23d4080e30fbb60bd0';
	face[_('礼物房子')] = '23959546ff28c74584ba';
	face[_('发礼物')] = '9c3532623d4c5ef975b6';
	face[_('圣诞老人')] = '7f22468bed15d5aae5a6';
	face[_('圣诞树')] = 'c48b7e5b478660f27ec0';
	face[_('生日蛋糕')] = 'ed8c7c6c30f5df5e78ce';
	face[_('下雪')] = '869a47be445d7463af96';
	face[_('婚礼')] = 'e4aaf9f23078063e7d8a';
	face[_('我爱你')] = '86788bf3dc7277bc3cac';
	face[_('情书')] = '721d0d88eec90c8c2216';
	face[_('玫瑰花')] = '1fce104dc920d0355362';
	face[_('打电话')] = '8f3d82cafd85a27f8e62';
	face[_('史胖纸')] = 'fb9d4d9b70073f4245e2';
	face[_('怪蜀黍')] = '429704c34f9ebcb27308';
	face[_('鲸鱼女宝宝')] = 'd57486c609f64b92e208';
	face[_('UFO')] = '8cadbe0227828d84d058';
	face[_('缺乏睡眠的可怜人儿')] = '6a4f100d8a7e64e596ce';
	face[_('假期')] = 'd2488b94eab71dfe1cc4';
	face[_('出游')] = '9c5d9c3d8cb38363ba78';
	face[_('小雨')] = 'b56e68af54b4ed73f090';
	face[_('海滩')] = 'c0bd5e2c160c5387329a';
	face[_('下雨')] = 'ebb26b4ac0ad5a927e7c';
	face[_('电话')] = '9ff10874ed02e1fdb1c0';
	face[_('钻戒')] = '81324487027d5e37974e';
	face[_('啤酒')] = '522c8bbe9e521d6d4bd6';
	MI.TalkBox.prototype.addFaceArt = face;
})();
...
MI.TalkList.prototype.urlTips = function(tips){
	if (this.nodeName == 'A') {
		var target = this,
			urlTipsTxt = [_('原链接'),_('此网址可能含有恶意内容或被大量用户举报，不建议打开原链接'),_('腾讯网'),_('原链接'),_('搜搜')],
			href = target.href,
	...
	function show(data){
		if (data.result == 0) {
			...
			if(data.info.safe == 1 || data.info.safe == 2){
				...
				var text2 = '<a href="###" class="jubao">' + ((data.info.safe == 1) ? _('举报链接') : _('申诉链接')) + '</a>';
			...
			MI.tips.position(target);
			//举报弹出举报浮层
			var jubaoDom = $$(tips,'.jubao')[0];
			if(jubaoDom){
				jubaoDom.onclick=function(){
					var msgBox = UI.parents(target,'msgBox')[0],
						liBox,
						name='',
						id='';
					if(msgBox){
						var account = $$(msgBox,'a')[0].href;
						name = MI.string.id(account);
						liBox = UI.parent(msgBox);
					}
					if(liBox){
						if(liBox.nodeName.toLowerCase()=='li')
							id = UI.A(liBox,'id');
						else{//该url是被转播原创里的url，寻找原创的id
							var time = $$(msgBox,'.pubInfo .time')[0];
							if(time) id=MI.string.id(time.href);
						}
					}
					jubao_url(id,name,data.info.url,data.info.safe);
				}
			}
		}
	}
								...
								var validate = new MI.Validate({
									...
									success : function(){
										MI.tip(_('保存成功'),function(){
											document.location.href = document.location.href.replace('pagesetMyhome','');
										});
									}
								});
							}
...
MI.navMenu = function(){ //Nav Menu
	...
	if (!nav){
		(function(){
			...
			UI.each([navSet,navMsg],function(o) {
				if(o) {
					if (UI.B.ipad){
						//o.ontouchstart = show;
						var link = $$(o,'a')[0];
			...
			setTimeout(function(){
				...
				var pathList = {
					...
					news : {
						element : topNav[1],
						pathname :['/p/news','/p/city','/p/top','/college_anas.php','/c/default']
					},
					qun : {
						element : topNav[2],
					...
					people : {
						element : topNav[3],
						pathname :['/people','/potential.php','/rank.php','/lists.php','/list2.php','/find/friends.php','/find/interest.php']
					},
					app : {
						element : topNav[4],
				...
				for(name in pathList){
					for(str in pathList[name]['pathname']){
						if(pathList[name]['pathname'].hasOwnProperty(str) && new RegExp(pathList[name]['pathname'][str]).test(selfPathname)){
			...
			;(function(){
				//应用
				var appBtn = $$(document.body,'.apps a')[0];
				if(appBtn ) {
					var appDropBox = new MI.DropBox({
						target : appBtn,
						dir : 2,
						start : 1,
						className : 'topNavSub',
						url : MI.url.htmlContent,
						data : {type:1},
						add : '<p class="right"><a href="http://app.t.qq.com/" target="_blank">' + _('应用频道') + '<span class="ffsong">&gt;&gt;</span></a></p><p class="left"><a href="http://app.t.qq.com/?g=2" target="_blank">' + _('进入我的应用') + '</a></p>',
						html : null,
						dataFilter : function(data){
							return data.replace(/\{account\}/g,MI.user.account || '');
						},
						domHandler : function(dom) {
							var listItem = UI.GT(dom,'li');
							if(MI.user.fun.app != 1) {
								UI.hide(listItem[1]);
							}; 
							/*
							if(!MI.user.medal || MI.user.medal[1] <= 2) {
								UI.hide(listItem[2]);
							};
							*/
						}
				...
				//微群
				var groupBtn = $$(document.body,'.headWrap .groups a')[0];
				if(groupBtn) {
					var appDropBox = new MI.DropBox({
						htmlStr : '<div style="display:none; width:310px; position: absolute;"><br /><br /><br /><br /></div>',
						target : groupBtn,
						dir : 2,
						start : 1,
						className : 'topNavSub lay_topdrop_qun',
						url : MI.url.topNavGroup,
						html : null,
						defaultHTML : '<div class="lay_topdrop_inner clear"><p class="desc">你还没有加入微群，现在就去<a href="http://qun.t.qq.com/" onmousedown="MI.Bos(\'btnNewMenuGroupIndex0\')">微群广场</a>逛逛吧。</p></div>'
					});
				}
			})();	

			//发表框
			var nav_pub_wrap = $('nav_pub_wrap'),
				nav_pub,
				nav_pub_box,
				nav_pub_box_object,
				nav_pub_tips,
				nav_pub_tips_close = 'option_nav_publish_' + MI.user.account,
				nav_pub_box_create = function(){
					if (!nav_pub_box){
						nav_pub_box = UI.html('<div class="nav_pub_box"><div class="talkBox">\
							<div class="sendCnt">\
								<div class="cntBox">\
									<textarea id="msgTxt" accesskey="u" name="content" autocomplete="off" tabindex="1"></textarea>\
								</div>\
								<div class="insertFun clear">\
									<div class="sendList insertFace"><a class="txt" href="#" title="' + _('表情') + '" tabindex="3"><em class="sico ico_face"></em></a></div>\
									<div class="sendList newTopic"><a href="#" class="creatNew txt" title="' + _('汇聚相同热点的广播') + '"><em class="sico ico_topic"></em>' + _('话题') + '</a></div>\
									<div class="sendList atSome"><a class="txt" href="#" title="' + _('@朋友帐号就可以提到他') + '"><em class="sico ico_at"></em>' + _('朋友') + '</a></div>\
									<div class="sendList newFeel"><a class="txt" sign="" title="' + _('心情') + '" href="#"><em class="sico ico_feel"></em>' + _('心情') + '</a></div>\
								</div>\
							</div>\
							<div class="sendFun">' + MI.html.button(_('广播'),'gb_btn1','sendBtn') + '<span class="countTxt c_tx5">' + _('还能输入{0}字','<em>140</em>') + '</span></div></div></div>')[0];
						UI.append(nav_pub_box,nav_pub_wrap);
						UI.EA(document.body,'click',function(e){
							var target = UI.E(e).target;
							if (!UI.parents(target,'nav_pub_wrap')[0] && !UI.parents(target,'atWrap')[0]){
								UI.removeClass(nav_pub_wrap,'nav_pub_acitve');
							}
						});
						nav_pub_box_object = new MI.TalkBox(nav_pub_box,{
							feelTip : 0,
							data : {},
							successStart : function(){
								if (MI.isS){
									MI.S(nav_pub_tips_close,'1');
								}
								setTimeout(function(){
									UI.removeClass(nav_pub_wrap,'nav_pub_acitve');
								},2000);
								MI.Bos('btnNewMenuPubBoxSuccess');
							}
						});
					}
					setTimeout(function(){
						if (UI.hasClass(nav_pub_wrap,'nav_pub_acitve')){
							if (nav_pub_box_object){
								nav_pub_box_object.focus();
							}
						}
					},0);
				};
			if (nav_pub_wrap && !UI.B.ipad){
				//隐藏提示
				nav_pub_tips = $$(nav_pub_wrap,'.nav_pub_tips')[0];
				if (MI.isS && MI.S(nav_pub_tips_close) == 1){
					UI.hide(nav_pub_tips);
				}
				//点击发表按钮
				nav_pub = $$(nav_pub_wrap,'a')[0];
				nav_pub.onclick = function(e){
					UI.toggleClass(nav_pub_wrap,'nav_pub_acitve');
					nav_pub_box_create();
					UI.E(e).stop();
					MI.Bos('btnNewMenuPubBox');
					return false;
				}
				nav_pub = null;
				if (UI.B.ie6){
	...
	//消息盒子
	setTimeout(function(){
		if (MI.user.account && (!MI.newCountShow || document.location.href.hasString('/k/') || document.location.href.hasString('/search/'))){
			MI.newCount && MI.newCount((MI.user.fun.relayFilter ? '14' : '4') + ',2,1,20,19');
		}
	},0);
...
MI.homeTab = function(){ //主页页卡
	//qq好友下拉菜单
	var qqFriendBtn = $('home_myfriends_top');
	var qqFriendArr = qqFriendBtn ? $$(qqFriendBtn,'.btn_ldrop')[0] : null; //没有箭头，不展示下拉
	if(qqFriendArr) {
		new MI.NewListDrop({
			tab : UI.GT(qqFriendBtn,'a')[0], 	//列表按钮
			className : 'listDrop',			//下拉列表class，用于css
			listData : [{listId:null,listName:MI.currQQType || 'QQ群',href:MI.currQQHref || 'http://t.qq.com/ilist/qqqun'}],  //listName和href从页面里获取
			bos : 'btnQQFriendDropView' 			 //显示列表时上报代码	
		});
	};
...
MI.MayBeKnow= (function(){
	...
	function changeAfter(e){
		...
		if(domlist.length>=6){
			...
			for(var i=3;i<6;i++){
				UI.show(domlist[i]);
				var account = UI.A(domlist[i],'uin')+'.'+UI.A(domlist[i],'reason');
				totalAccount.push(account);
			}
			MI.feedBack({name:'timeline',iPos:11,iBak1:1,sBak2:totalAccount.join(','),id:1191});
		}
		else{ 
			getData();
			return false;
	...
	function buildList(arr) {
		...
		UI.each(arr,function(p,i){
			...
			str += '<li account="' + p.account + '"' + ((i>= SHOW_NUM)? 'style="display:none"' : '')+' uin="' + p.uin +'" reason="' + p.reason_type + '"><div class="userPic">';
	...
	function showNext(P){
		UI.animate(P,'opacity',0,function(){
				...
				for(var i=0; i < userList.length; ++i)
				{
					if(count >= SHOW_NUM)
					{
						var uin = UI.A(userList[count-1],'uin')+'.'+UI.A(userList[count-1],'reason');
						MI.feedBack({name:'timeline',iPos:11,iBak1:1,sBak2:uin,id:1191});
						break;
					}
					UI.show(userList[i]);
					count++;
	...
	function bindEvent(){
		UI.each($$(_list,'.attentBox'),function(o,i){
			...
			var uin = UI.A(UI.parents(o)[1],'uin')+'.'+UI.A(UI.parents(o)[1],'reason');
			_follow.onclick = function(e){
				...
				MI.feedBack({name:'timeline',iPos:11,iBak1:3,sBak2:uin,id:1191});
				//收听以后，消失，补充下一个
			...
			_unFollow.onclick = function(e){
				...
				MI.feedBack({name:'timeline',iPos:11,iBak1:8,sBak2:uin,id:1191});
			}			
		});
		UI.each($$(_list,'.del'),function(o,i){
			o.onclick = function(){
				...
				var uin = UI.A(P,'uin')+'.'+UI.A(P,'reason');
				setTimeout(function(){
					showNext(P);
				},500);
				MI.feedBack({name:'timeline',iPos:11,iBak1:6,sBak2:uin,id:1191});
		...
		UI.each(people,function(o,i){
			...
			var account = UI.A(o,'uin')+'.'+UI.A(o,'reason');
			$$(o,'.userPic a')[0].onclick = function(e){
				MI.feedBack({name:'timeline',iPos:11,iBak1:2,sBak2:account,id:1191});
			};
			$$(o,'.uname')[0].onclick = function(e){
				MI.feedBack({name:'timeline',iPos:11,iBak1:5,sBak2:account,id:1191});
			};
		...
		MI.Card.build(_list,'.userPic img',2);
...
MI.listDrop.prototype = {
	...
	show : function(data){
			...
			if(UI.hasClass(Self._arrow,'btn_drop'))
				UI.addClass(Self._arrow.parentNode,'hover');
			else UI.addClass(Self._arrow,'lopen');
			UI.show(Self._body);
			Self.pos();
			Self.page(0);
	...
	hide : function(){
		...
		Self.delay.hide = setTimeout(function(){
			...
			if(Self._listDropBtn != Self._selectedBtn){ 
				UI.removeClass(Self._listDropBtn,'select');
				if(UI.hasClass(Self._arrow,'btn_drop'))
					UI.removeClass(Self._arrow.parentNode,'hover');
			}
			if (Self.hideCall){
				Self.hideCall();
			}
...
//新的dropList，拉取json格式数据或者静态json数据,目前用于心情的下拉和qq好友的切换
...
MI.NewListDrop = function(args){
	...
	this.listData = args.listData || []; //静态数据，不异步拉取
...
MI.NewListDrop.prototype = {
	...
	load : function(call){
		var Self = this;
		if(Self.listData && Self.listData.length) {
			Self.show();
		}	
		else {
			MI.ajax({
	...
	creatList : function(){
		...
		UI.each(Self.listData,function(o){
			html += (Self.current && (Self.current == o.listId)) ? '<li class="on"><a style="cursor:default">' + o.listName + '</a></li>' :
	...
	setPos : function(){
		...
		Self._body.style.left = UI.getX(Self._tab.parentNode) +  'px';
...
MI.Flash = {
	...
	/**
	 * 获取或插入flash结构
	 * @param {Object} Element 要插入到的父元素节点
	 * @param {String} src flash文件的url
	 * @param {Integer} width flash的宽度
	 * @param {Integer} height flash的高度
	 * @param {String} id flash的Id
	 *
	 * @return {String} flash的html结构
	 *  @example
	 *		MI.Flash.getObject(el,MI.swf.MultiPhoto,460,200,'MultiPhoto');
	 *
	 */	
	getObject : function(o,src,width,height,id) {
		var html = '<object ' + (id ? 'id="' + id + '"' : '') + ' classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="'+width+'" height="'+height+'" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,28,0"><param name="allowScriptAccess" value="always" /><param name="movie" value="'+src+'" /><param name="quality" value="high" /><param name="wmode" value="transparent" /><param value="langVer=' + window.MILang + '" name="FlashVars" /><embed ' + (id ? 'name="' + id + '"' : '') + ' src="'+src+'" width="'+width+'" height="'+height+'" quality="high" wmode="transparent" pluginspage="http://www.adobe.com/shockwave/download/download.cgi?P1_Prod_Version=ShockwaveFlash" allowScriptAccess="always" FlashVars="langVer=' + window.MILang + '" type="application/x-shockwave-flash"/></object>';
		if (o){
			o.innerHTML = html;
		}
		else {
	 ...
	 * 获取当前浏览器下的flash插件版本号
	 *
	 * @return {Integer} flash的版本号，没装flash为0
	 *  @example
	 *		MI.Flash.getVersion()
	 *
	 */	
	...
	/**
	 * 插入无flash或版本低提示
	 * @param {Integer} 需要的最低版本号
	 *
	 * @return {String} 提示html结构
	 *  @example
	 *		if(MI.Flash.getVersion()<10) {
	 *			el.innerHTML = MI.Flash.updataTip(10);
	 *		}
	 *
	 */	
	updateTip : function(version){
		var v = this.getVersion(),html,version=version || '';
		if(UI.B.ipad) return '<div class="flashVersion"><p>你的系统不支持Flash,不能使用此功能</p></div>';
...
MI.SmartBox = function(id,callback,type,style){ //input element,callback,search type (keyword:用户输入的关键字提示,布尔值,可以为空）
    ...
    if (this.tip) {
        ...
        if (!this._style.isnano) {
            if (MI.boss == 13) 
                Self._tip.innerHTML = _('搜我的广播/搜全站');
            else 
                if (MI.guest) 
                    Self._tip.innerHTML = _(MI.string.sprintf('搜%s的广播/搜全站', MI.guest.sex));
        }
    }
		...
		if (E.key == 40 || E.key == 38) {
			...
			if(Self._valTip){
				...
				if(UI.hasClass(Self._list[Self.index],'special') || UI.hasClass(Self._list[Self.index],'speak')) {
					...
					else UI.A(parentNodePos,'value','179');
	...
	this._key.onkeyup = function(e){
		...
		//this.value=='' 修复遨游，TT，火狐等浏览器，中文状态下输入英文后回车，也要弹出smartBox，不return
		if(E.key == 40 || E.key == 38 || (E.key == 13 && this.value=='') || E.key == 27) return;
		Self.getMsg(this.value);
	}
}
MI.SmartBox.prototype = {
	...
	removePara : function(){
		...
		//在需要传不同pos来源参数时，将原先form的pos值去掉
		var posInput = $$(this._key.parentNode,'input');
		if(posInput){
			UI.each(posInput,function(o){
				if (UI.A(o,'name') && UI.A(o,'name')=='pos' && Self._valTip){
						UI.remove(o);				
				}
			});
		}
	...
	buildDom : function(data,value){
		...
		if (data.result == 0) {
			...
			if(Self._valTip){
				...
				//不同li点击搜索时pos参数的添加放在这里统一处理
				UI.EA(Self._select, 'mousedown', function(event) {
					var E = UI.E(event),
						$target = E.target,
						$el;
					Self.removePara();
					var parentNodePos = $$(Self._key.parentNode,'.pos')[0];
					if(!parentNodePos){
						var html = UI.html('<input type="hidden" class="pos" name="pos" value="">')[0];
						UI.after(html,Self._key);
						parentNodePos = $$(Self._key.parentNode,'.pos')[0];
					}
					if(UI.hasClass($target,'special')){
						UI.A(parentNodePos,'value','173');
						UI.after(Self._query,Self._key);
					}
					else if(UI.hasClass($target,'searchUser')){
						UI.A(parentNodePos,'value','176');
					}else if(UI.hasClass($target,'searchQun')){
						UI.A(parentNodePos,'value','178');
						UI.after(Self._queryQun,Self._key);
					}
					else if(UI.hasClass($target,'speak')){
						UI.A(parentNodePos,'value','179');
					}
				});
			}
			//Event
				...
				el.onclick = function(){
					if(UI.hasClass(el,'userGet')){
						...
						if(UI.hasClass(el,'qunGet')){
							...
							if(this.nodeName != 'DT' && Self._valTip){
								Self.removePara();
	...
	submit : function(el){
		...
		if(this._callback){
			...
			if(el && UI.hasClass(el,'searchUser')){
				if(this._form != null) {
					this._form.action="http://t.qq.com/search/user.php";
					if (this._style.isnano){
						this._form.action="http://1.t.qq.com/search/user.php";
					}
				}
			}
...
MI.AutoCmt = function(o){ //Auto Complete
	...
	*/
	...
	Self.moreName = o.moreName || 0; //允许input展示多个用户名
...
MI.AutoCmt.prototype = {
			...
			if (UI.isNumber(Self.index) && Self.list[Self.index]) {
				...
				if (Self.word){					
					if (Self.key) {
						...
						MI.insertTxt(Self._target, txt + ' ', Self.cursorX, Self.word.length);
					}
				}
				else {
				...
				MI.Bos('btnAutoAtAccount' + (Self.model ? Self.model : ''),0,Self.model ? 0.01 : 0);
			}
		}
	...
	getData : function(call){ //获取数据
		...
		if ((!MI[usersStorageKey] && !Self.sending && (!users || (new Date().getTime() - MI.S('time') > 43200000) || !newData(users))) || !MI.S('time')){ //12小时 2小时-7200000
			Self.sending = 1;
			var Time,Time_1,Time_2,Time_3;
			Time = + new Date();
			MI.ajax({
	...
	showAll : function(el){
		var Self = this;
		Self.creatDom();
		if (Self._all.innerHTML == '' || !MI.S('time')){
			Self.word = '';
	...
	show : function(o, py, model){
		...
		Self.listEvent = function(type){ //1-all,2-key
			var isAll = type == 1,
				listName = isAll ? 'allList' : 'keyList',
				listDom = isAll ? '_all' : '_key';
			Self[listName] = $$(Self[listDom],'li');
			UI.each(Self[listName],function(o,i){
				o.onclick = select;
				o.onmouseover = hover;
				o.onmouseout = out;
				o.txt = txt[i];
			...
			Self.index = 0;
			Self.indexMax = Self.list.length - 1;
			UI.addClass(Self.list[0],'on');
			
			if (Self.showCall){ //显示回调
				Self.showCall();
			}
		}
		//if (this.word) {
			var match = [],
				...
				word = model ? UI.trim(Self._input.value.replace(/^@/g,'')) : (Self.key ? Self.word.slice(1) : Self.word),
				...
				txt = [],
				txtObj = {},
				resultObj = {}, //最终匹配的结果，用于listTmpl渲染
				x,
				y,
				users = o.info;
			if (word) {
				for (var i in users) {
					...
					if (users[i]){
						...
						if(i.match(reg) || users[i].match(reg)){
							var matchId = i.replace(reg,reRegtxt),
								matchName = users[i].replace(reg,reRegtxt);
							match.push(matchName + ' (' + matchId + ')' + ((i.substr(0,1)=="*") ? '<em class="ico_qGroup"></em>' : ''));
							if (Self.listTmpl){
								resultObj[i] = [users[i],matchId,matchName];
							}
							txt.push(i);
							txtObj[i] = 1;
							matchLength++;
				...
				if (matchLength < Self.max) {
					...
					for (var i in users) {
						...
						//如果搜索word是以*开始的
							var matchId = i,
								matchName = users[i].substring(0, rs[0]) + '<b>' + users[i].substring(rs[0], rs[0]+rs[1]) + '</b>' + users[i].substring(rs[0]+rs[1]);
							match.push(matchName + ' (' + matchId + ')' + ((i.substr(0,1)=="*") ? '<em class="ico_qGroup"></em>' : ''));
							if (Self.listTmpl){
								resultObj[i] = [users[i],matchId,matchName];
							}
				...
				if (matchLength < Self.max) {
					for (var i in users) {
						...
						if (users[i] && ( i.slice(1).match(regAll) || users[i].slice(1).match(regAll) ) && !txtObj[i]) {
							//如果搜索word是以*开始的
							var matchId = i.replace(regAll,reRegtxt),
								matchName = users[i].replace(regAll,reRegtxt);
							match.push(matchName + ' (' + matchId + ')' + ((i.substr(0,1)=="*") ? '<em class="ico_qGroup"></em>' : ''));
							if (Self.listTmpl){
								resultObj[i] = [users[i],matchId,matchName];
							}
					...
					if(regAll1 && regAll1 != regAll){ //简繁体逻辑
						for (var i in users) {
							...
							if (users[i] && ( i.slice(1).match(regAll1) || users[i].slice(1).match(regAll1) ) && !txtObj[i]) {
								//如果搜索word是以*开始的
								var matchId = i.replace(regAll1,reRegtxt),
									matchName = users[i].replace(regAll1,reRegtxt);
								match.push(matchName + ' (' + matchId + ')' + ((i.substr(0,1)=="*") ? '<em class="ico_qGroup"></em>' : ''));
								if (Self.listTmpl){
									resultObj[i] = [users[i],matchId,matchName];
								}
				...
				if (MI.usersArr && MI.usersArr.length) {
					...
					for (var i = 0,num = MI.usersArr.length;i < num;i++) {
						...
						if(userNick) { //判断常用联系人是否在当前联系人中
							...
							matchLength++;
							if (Self.listTmpl){
								resultObj[userName] = [userNick,matchId,matchName];
							}
							txt.push(userName);
							txtObj[userName] = 1;
						}
			...
			if (1 || matchLength || model) {
				if (matchLength > 10) {
					Self._cont.style.cssText = Self.maxCss;
					Self._cont.scrollTop = 0;
				}
				else {
					Self._cont.style.cssText = '';
				}
				...
				var html;
				if (Self.listTmpl){
					html = new UI.tmplString(Self.listTmpl)({data:resultObj});
				}
				else {
					html = '<ul><li><span>' + match.join('</span></li><li><span>') + '</span></li></ul>';
				}
				if (!matchLength){
					html = '';
				}
				if (!word && !Self.allList && model){
					Self._all.innerHTML = html;
				}
				else if (word || !model){
					Self._key.innerHTML = html;
				}
				UI.removeClass(Self.list[Self.index],'on');
				if (!word && model) { //显示所有人
					UI.hide(Self._key);
					UI.show(Self._all);
					if (Self.allList && Self.allList.length > 10) {
						Self._cont.style.cssText = Self.maxCss;
					}
					Self.listEvent(1);
				}
		...
		function hover(){
			...
			UI.addClass(this,'on');
			Self.index = this.index;
			if (Self.mouseHoverCall){
				Self.mouseHoverCall(this);
			}
		}
		function out(){
			if (Self.mouseOutCall){
				Self.mouseOutCall(this);
			}
		}
	},
	hide : function(){
	...
	position : function(){
		...
		if (!this.display) {
			...
			if (this.align == "top") {
				y = UI.getY(this._target) - UI.height(this._body);
			}
...
MI.Dialog = function(o){ //Dialog
	...
	var _titleMousedown = UI.bind(Self._titleMousedown,Self,true);
	UI.EA(Self._title,'mousedown',_titleMousedown);
	UI.EA(Self._title,'touchstart',_titleMousedown);
	_titleMousedown = null;
	
	//UI.EA(document,'keyup',Self.key);
	UI.EA(window,'resize',UI.bind(Self.resizeBg,Self,true));
	//UI.EA(Self._close,'mousedown',UI.bind(Self._closeClick,Self,true));
	if (self != parent){
		try{
			if (parent.document.domain == 'qq.com'){
				Self.iframe = 1;
			}
		}catch(e){}
	}
...
MI.Dialog.prototype = {
	width : 462,
	iframe : 0, //被iframe，且父页面被降到qq.com
	close : 1,
	_titleMousedown : function(e) { //Move
		var wrap = this._wrap;
		var E = UI.E(e);
	...
	show : function(o){
		var Self = this,css;
		if (!Self._close.onclick){
			Self._close.onclick = UI.bind(Self._closeClick,Self,true);
		}
		if (o.html) {
		...
		if (Self.iframe){
			windowHeight = parent.innerHeight || (parent.document.documentElement && parent.document.documentElement.clientHeight) || parent.document.body.clientHeight;
			scrollY = parent.pageYOffset || (parent.document.documentElement && parent.document.documentElement.scrollTop) || parent.document.body.scrollTop;
			do {
				frameY += frameEl && frameEl.offsetTop;
				frameEl = frameEl.offsetParent;
			} while (frameEl);
			scrollY -= frameY;
		}
		
		css = 'width:' + Self.width + 'px;top:' + ( scrollY + windowHeight / 2 - UI.height(Self._wrap) / 2 ) + 'px;left:' + (UI.windowWidth() / 2 - Self.width / 2) + 'px;';
		if (o.left != undefined) {
 ...
 * 旧私信框对象
 * @constructor
 * @param {Object|String} Element或者节点id属性 
 * @param {Integer} 私信来源字段
 *
 *  @example
 *		new MI.MsgBox(el,1716);
 *
 */	
MI.MsgBox = function(id,source){ //MsgBox
	var Self = new MI.TalkBox(id,{noFlash:1});
	Self.txtTipSend = _('发送中');
	Self._msgTo = Self.$('.msgTo');
	Self._num = $$('.msgNum');
	Self.url = MI.url.inboxSend || 'http://t.qq.com/inbox/send.php';
//	Self.type = 3;
	Self.msgTo = {};
	Self.data = source ? {source:source} : {};
	
	// 私信的权限交由后端判断，错误提示在publish的返回码中返回。
	...
	*/	
	...
	Self.successStart = function(data){
		...
		*/	
			MI.tip(_('发送成功'));
			UI.removeClass(Self._body,'pubSuc');
		}
	}
...
MI.Reply = function(boxType){ //Reply Dialog
	...
	Self._body = UI.html(MI.tmpl.reply.replace('<div class="left"></div>','<div class="left" style="display:;"><label style="display:none"><input type="checkbox" ' + (MI.user.fun.syncQzone ? ' checked' : '') + ' class="replayQzoneCheckbox check1">' +  _('同步到空间') + '</label></div>').replace('talkSuc',''))[0];
	UI.hide(Self._body);

	Self.talkBox = new MI.TalkBox(Self._body);
	Self._talkTo = $$(Self._body,'.replyTitle')[0];
	Self._msg = $$(Self._body,'.msg')[0];
	Self.talkBox._syncQzone =$$(Self._body,'.replayQzoneCheckbox')[0];
...
MI.Reply.prototype = {
	...
	show : function(obj){
		...
		if (obj){
			...
			if(obj.syncQzone){
				 UI.show(talkBox._syncQzone.parentNode);
			} else {
				talkBox._syncQzone.checked = 0;
			}
 ...
 *						{
 *	 						tab : _('发广播'),
 *	 						content : MI.tmpl.reply
 *						},
 *						{
 *	 						tab : _('发私信'),
 *							content : MI.tmpl.msgBox
 *						}
 *					]
 *				})
 */
MI.TabDialog = function(opt){
	this.list = opt.list;							//标签和内容的列表，一个数组	
	this.selectClass = opt.selectClass || 'select';	//选中tab的class	
	this.bodyClass = opt.bodyClass || '';			//主结构的class，用于css
	this.openIndex = opt.openIndex || 0;			//首次打开选中的tab
	this.width = opt.width || 0;					//默认宽度
	this.build();
};

MI.TabDialog.prototype = {
	build : function(list){
		var Self = this;
		Self.tab = [];
		Self.div = [];
		Self.explain = [];
		Self._body = Self.getBody(Self.list);
		Self.bindEvent();
		Self.toggleTab(Self.openIndex);
	},
	getBody : function(list){
		if(!list || !list.length) return;
		var Self = this;
		var _body = UI.html('<div class="DshareWrap dialogTalkBox ' + Self.bodyClass + '"><div class="tabStyle1 shareTitle"><ul></ul></div><div class="explain"></div></div>')[0];
		var _tab = UI.GT(_body,'ul')[0];
		var _explain = $$(_body,'.explain')[0];
		UI.each(list,function(o,i){
			var tab = UI.html('<li class="' + ((i == Self.openIndex) ? Self.selectClass : '') + '"><a href="javascript:void(0)">' + o.tab + '</a></li>')[0];
			var explain = UI.html('<div style="padding:10px 0"></div>')[0];
			var content = UI.isElement(o.content) ? o.content : UI.html(o.content)[0];
			Self.tab.push(tab);
			Self.div.push(content);
			Self.explain.push(explain);
			UI.append(tab,_tab);
			UI.append(explain,_explain);
			UI.append(content,_body);
		});
		return _body;
	},
	/**
	 ...
	 * @param {Array} 说明文字数组
	 *            @example
	 *            		var o = new MI.TabDialog({....});
	 *					o.changeExplain(['发广播给xxx','发私信给xxx']);
	 */
	changeExplain : function(explainList){
		var Self = this;
		UI.each(explainList,function(o,i){
			Self.explain[i].innerHTML = o;
		});
	},
	bindEvent  : function(){
		var Self = this;
		UI.each(Self.tab,function(o,i){
			UI.EA(o,'click',function(){
				Self.toggleTab(i);
				if(UI.isFunction(Self.list[i].tabEvent)) Self.list[i].tabEvent(o,i);
			});
		});
	},
	toggleTab : function(curr){
		var Self = this;
		UI.each(Self.div,function(o,i){
			if(i == curr) {
				UI.show(o);
				UI.show(Self.explain[i]);
				UI.addClass(Self.tab[curr],Self.selectClass);
			} else {
				UI.hide(o);
				UI.hide(Self.explain[i]);
				UI.removeClass(Self.tab[i],Self.selectClass);
			};
		});
		Self.lastIndex = curr;	
	},
	...
	/**
	 * 重置浮层中的talkBox对象状态
	 * @param {Object} TalkBox实例
	 * @param {String} 发表框重置后的默认文本
	 *            @example
	 *            		var o = new MI.TabDialog({....});
	 *					var talk = new TalkBox(o._div[0]);
	 *					o.reset(talk,'请输入你要说的话');
	 */
	reset : function(talkBox,txt){ //重置talkBox对象
		MI.app({
			TalkBoxRich : function(){
				talkBox._txt.value = txt || '';
				talkBox.hideMedia();
				talkBox.reset();
				talkBox.delPic();
				talkBox.delMusic();
...
MI.Card.prototype = {
	...
	addEvent : function(){
		var Self = this,
			...
			uneach = 'delEach',
			replyBox,msgBox,reFollowBox,txtMax;
		if (Self.addEvented){
			return;
		}
		...
		Self._hotLine = $$(_body,'.hotLine')[0]; //热线
	...
	//		recomBox = Self.recom.talkBox;
		...
		Self._follow.onclick = function(){
			...
			//if (!this.send) {
				MI.follow(Self.account,T,function(isFollow){
					...
					MI.feedBack({name:'usercard',iPos:52,iBak1:3,sBak1:'',id:1191});
					//this.send = 0;
				});
			//}
			//this.send = 1;
		...
		Self._uneach.onclick = Self._unfollow.onclick = function(){
			...
			//if (!this.send) {
				MI.follow(Self.account,T,function(isFollow){
					...
					MI.feedBack({name:'usercard',iPos:52,iBak1:8,sBak1:'',id:1191});
					//this.send = 0;
				});
			//}
			//this.send = 1;
		...
		Self._recom.onclick = function() {
			var explainList = [_('向大家介绍一下<b>{0}</b>吧:',Self.bkname || Self.name),_('可以单独向某个朋友推荐<b>{0}</b>',Self.bkname || Self.name)];
			var topic = _('#推荐收听# @{0} ',Self.account);
			if(!Self.recom) {
				var botHtml = '<div class="bot clear">';
				var tips = '<div class="recomTip">' + _('提示：TA是什么样的人，TA的微博好玩么，哪些是最值得你推荐的？') + '</div>' + botHtml;
				Self.recom = new MI.TabDialog({
					width : 530,
					list : [
						{ 
							tab : _('公开推荐'),
							content : MI.tmpl.fullMediaTalk.replace(botHtml,tips)
						},
						{
							tab : _('私信推荐'),
							content : MI.tmpl.msgBox.replace(botHtml,tips)
						}		
					]	
				});
			}	
			...
			//公开推荐	
			if(!Self.recomPublishBox) {		
				Self.recomPublishBox = new MI.TalkBox(Self.recom.div[0],{noFlash:1});
				Self.recomPublishBox.data = {commAccount:Self.account};
				Self.recomPublishBox.success = function(){
					MI.ajax({url:MI.url.comm,data:{accounts:Self.account}});
					MI.tip(_('发送成功'));	
				};
			}
			...
			//私信推荐
			if(!Self.recomMsgBox) {			
				Self.recomMsgBox = new MI.MsgBox(Self.recom.div[1]);
			}
			
			Self.recom.reset(Self.recomPublishBox);
			Self.recomPublishBox.topic = topic;
			Self.recomPublishBox.countTxt();
			
			Self.recom.reset(Self.recomMsgBox);
			Self.recomMsgBox.topic = topic;
			Self.recomMsgBox._msgTo.value = '';
			Self.recomMsgBox.countTxt();
			
			Self.recom.changeExplain(explainList);
			Self.recom.show(function(){
				([Self.recomPublishBox,Self.recomMsgBox][Self.recom.lastIndex]).focus();
			});
			MI.Bos('btnCardRecom');
			return false;	
		};
		...
		//热线
		Self._hotLine.onclick = function(){
			var topic = '#'+(Self.topic || '热线平台')+'# ';
			var explainList = [_(topic)+'@' + (Self.bkname || Self.name),_(topic)+'@'+(Self.bkname || Self.name)];
			if(!Self.hotLine) {
				Self.hotLine =  new MI.TabDialog({
					width : 530,
					list : [
						{ 
							tab : _('发广播'),
							content : MI.tmpl.fullMediaTalk
						},
						{
							tab : _('发私信'),
							content : MI.tmpl.msgBox
			...
			//发广播
			if(!Self.hlPublishBox) {		
				Self.hlPublishBox = new MI.TalkBox(Self.hotLine.div[0],{noFlash:1});
				Self.hlPublishBox.data = {};
				Self.hlPublishBox.success = function(){
					MI.tip(_('发送成功'));	
				};
			}
			...
			//发私信
			if(!Self.hlMsgBox) {			
				Self.hlMsgBox = new MI.MsgBox(Self.hotLine.div[1],1712);
			}
			
			Self.hotLine.reset(Self.hlPublishBox);
			Self.hlPublishBox.topic = topic + '@' + Self.account + ' ';
			Self.hlPublishBox.txtMax = 140 - MI.countTxt(Self.hlPublishBox.topic);
			Self.hlPublishBox.countTxt();
			
			Self.hotLine.reset(Self.hlMsgBox);
			Self.hlMsgBox.topic = topic;
			Self.hlMsgBox._msgTo.value = Self.account;
			Self.hlMsgBox.txtMax = 140 - MI.countTxt(Self.hlMsgBox.topic);
			Self.hlMsgBox.countTxt();
			Self.hotLine.changeExplain(explainList);
			
			Self.hotLine.show(function(){
				([Self.hlPublishBox,Self.hlMsgBox][Self.hotLine.lastIndex]).focus();
			});
			MI.Bos('btnCardHotLine');
			return false;
		}
		//更多
	...
	getPos : function(el){
		...
		if(this.cType == '2') {
			...
			var eHeight = UI.height(el); 	//目标元素的高度,链接换行的话，取单行行高17
	...
	getCommenFriend : function(account,type){
		...
		if(MI.Card.commenFriends && MI.Card.commenFriends[account] && MI.Card.commenFriends[account][type]){
			...
			var commenUrl = MI.url.relations + '/' + account + ',' + MI.user.account + '/idol_follow.json';
	...
	showCommenFriend : function(o,type){
		...
		if(o.list && o.list.length){
			...
			UI.each(o.list,function(p){
				if(index<4){
					//var href = 'http://t.qq.com/' + p.name;
					var href = MI.getInternalUrl(0, p.name);
					str += '<a href="' + href + '">'
					str += '<img onerror="MI.Pic(this,30)" title="' + p.nick + '" src="' + (p.avatar ? p.avatar + '/30' : 'http://mat1.gtimg.com/www/mb/images/head_50.jpg') + '" />';
					str += '</a>';
					index ++;
				}
			});
			...
			$$(Self._rf,'.commenNum')[type].innerHTML = type==1 ? _('这些人也收听了{0}:',Self.gender) : _('收听{0}的人还收听:',Self.gender);
	...
	show : function(el){
		...
		if(el.nodeName.toLowerCase() == 'a') {   //talkList中的账户链接
			var P = el;
			var pic = '';
		}
		else {
		...
		var chName = title.match(reg)[0].slice(1,-1), //含中文ID
			...
			url = MI.getInternalUrl(0, account),
			uRela = UI.A(P,'uRela') || '',
			Time,Time_1,Time_2,Time_3;
		...
		function show(o){
			...
			*/
			...
			if(o.hotLine) {
				UI.show(Self._hotLine);	
			} else {
				UI.hide(Self._hotLine);	
			}
	...
	setName : function(o){
		...
		var name = (o.bkname || o.nick);
		if(o.flag.member)
			name = '<span class="c_vip" title="会员参加#给生活找点乐#点亮红名">' + name + '</span>';
		Self._name.innerHTML = '<a href="'+ MI.getInternalUrl(0, Self.account) + '">' + name + '</a>' + icon + '<br /><span class="en">' + (o.bkname ? o.nick : '') + '@' + Self.chName + '</span>';
	},
	updateName : function(){
	},
	num : function(o){
...
MI.Card.build = function(el,className,cType){
	...
	UI.each($$(el,className),function(o){
		var P = (o.nodeName.toLowerCase()=='a') ? o : o.parentNode;
		if (!UI.A(P,'card')) {
			UI.A(P,'card','1');
			UI.A(P,'ctype',cType);
			UI.A(P,'rel',P.title);
			...
			//qzone app私信逻辑，暂时由这里帮私信修改地址
			if (MI.hostType === 3) {
				if (P.href && P.href.match(MI.Card.build.REG_QZAPP_HREF)) {
					P.href = MI.getInternalUrl(0, RegExp.$2);
				}
			}
...
MI.Card.build.REG_QZAPP_HREF = /^(http:\/\/z.t.qq.com)?\/([A-Za-z][A-Za-z0-9_\-]*)$/;
 ...
 *
 */	
MI.DropBox = function(opt){
	this.target = opt.target;  //目标元素
	this.dir = opt.dir;			//目标元素所在工具条方位序列，0:'left',1:'right',2:'top',3:'bottom'
	this.start = opt.start || 1; //是否对齐浮层起始位置，0：对齐尾部，1：对齐头部
	this.add = opt.add; 		//额外的html内容
	this.url = opt.url;  		//异步拉取列表内容
	this.data = opt.data || {};	//异步拉取是的url参数
	this.data._ = + new Date();
	this.html = opt.html; 		//静态html内容
	this.htmlStr = opt.htmlStr || this.htmlStr;	//下拉菜单容器html
	this.className = opt.className; //className  用于css定义样式
	this.dataFilter = opt.dataFilter || function(data) {return data};  //处理异步数据
	this.domHandler = opt.domHandler || function(dom) {}; //处理生成的dom结构
	this.delayTime = 0; //请求延迟时间
	this.defaultHTML = opt.defaultHTML || '';//接口读取失败默认显示内容
	
	this.build();
}

MI.DropBox.prototype = {
	htmlStr : '<div style="display:none;position:absolut;"><div class="itemBox clear"><br /><br /><br /><br /></div><div class="func"><div class="clear"></div></div></div>',
	activeClass : 'active',
	dirList : ['left','right','top','bottom'],
	setPos : function(){
		var Self = this;
		var align = 'left';
		if(!Self.dir) return false;
		UI.C(Self.body,Self.dirList[Self.dir],'100%');
		if(Self.dir < 2) {
			align = [Self.dirList[3],Self.dirList[1]][Self.start];
		} else {
			align  = [Self.dirList[1],Self.dirList[0]][Self.start];
		}
		UI.C(Self.body,align,0);
	},
	build : function(){
		var Self = this;
		Self.body = UI.html(Self.htmlStr)[0];
		Self.listBox = $$(Self.body,'.itemBox')[0] || Self.body;
		Self.funcBox = $$(Self.body,'.func .clear')[0];
		Self.funcBox && (Self.funcBox.innerHTML = Self.add);
		if(Self.className) UI.addClass(Self.body,Self.className);
		UI.append(Self.body,Self.target.parentNode);
		Self.setPos();
		
		//Event
		if (UI.B.ipad){
			Self.target.ontouchstart = function(){
				Self.show();
			};	
			...
			Self.target.onmouseout = Self.body.onmouseout = function(){
				Self.hide();
			}
	...
	show : function(){
		...
		UI.addClass(Self.target,Self.activeClass);
		UI.show(Self.body);
		clearTimeout(Self.delayShow);
		if(!Self.listData) Self.delayShow = setTimeout(function(){ //防止请求错误，反复循环
			Self.refresh(Self.html);
		},Self.delayTime);
	},
	hide : function(){
		var Self = this;
		...
		Self.delayHide = setTimeout(function(){
			UI.hide(Self.body);
			UI.removeClass(Self.target,Self.activeClass);
		},20);
	},
	refresh : function(html){
		var Self = this;
		if(html || Self.listData) {
			Self.listBox.innerHTML = Self.listData = html || Self.listData;
			Self.domHandler(Self.listBox);
		}
		else if(Self.url) {
			UI.addClass(Self.listBox,'loading');
			MI.ajax({
				url : Self.url,
				type : 'get',
				data : Self.data,
				success : function(data){
					UI.removeClass(Self.listBox,'loading');
					data = MI.json(data) 
					if(data.result == 0) {
						if(data.info) {
							var o = Self.dataFilter(data.info);
							Self.refresh(o);
							Self.listData = o;
							Self.delayTime = 0;
						}
					} else {
						data.msg = data.msg ? '<p style="text-align:center;padding:20px 0;">' + data.msg + '</p>' : Self.defaultHTML;
						Self.listBox.innerHTML = data.msg;
						Self.delayTime += 500;
					}
				}
			})
 ...
 *  分类筛选展开按钮      
 */
MI.subTabToggle = function(){
	
		var subTab = $$(document.body,'.homeFilter .subTab1')[0];
		if(!subTab) return;
		
		var	btn = $$(document.body,'.homeFilter .more_link')[0],
			items = $$(subTab,'.homeFilter .subTab1 a'),
		//	currTagPage = UI.A(subTab,'expand'),
			cookieName = btn && UI.A(btn,'expand'),
			defaultH = 53,
			currH =  UI.height(subTab),
			fullH = subTab.scrollHeight; //scrollHeight取值有偏差
			fullH = !UI.B.ie ? fullH - 7 :
					UI.B.ie9 ? fullH -8 : 
					UI.B.ie8 ? fullH - 7 : fullH;
			
		//记录展开高度和标签
		UI.each(items,function(o){
			o.onclick = function(){
				if(cookieName) UI.cookie(cookieName,UI.height(subTab),7,'t.qq.com'); //展开高度
		//		if(currTagPage) UI.cookie(currTagPage,UI.A(this,'qtagid'),7,'t.qq.com'); //当前标签
			}	
		});
		...
		btn.onclick = function(){
			if(fullH > currH + defaultH) {
				currH = currH + defaultH;
			} else if((fullH > currH) && (fullH <= currH + defaultH)) {
				currH = fullH;
				slideUp();
			} else {
				currH = defaultH;
			...
			clearInterval(MI.subTabToggle.animate);
			MI.subTabToggle.animate = UI.animate(subTab,'height',currH,function(){
				if(cookieName) UI.cookie(cookieName,UI.height(subTab),7,'t.qq.com');
			});
			return false;
		}
}
...
MI.Shortcuts = {
	...
	add : function(obj){
		...
		//放到一个数组中批量添加
		if (UI.isArray(obj)){
			UI.each(obj,function(o){
				Self.add(o);
			});
		}
		else {
			//注册事件
			if (!Self.builded[obj.event]){
				Self.build(obj.event);
			}
			
			//添加事件
			if (!Self.key[obj.event]){
				Self.key[obj.event] = [];
			}
			Self.key[obj.event].push(obj);
		}
	},
	/**
	 * 删除快捷键
	 ...
	 *                }
	 *            });
	 *            MI.Shortcuts.del(MI.Shortcuts.talkList);
	 */
	del : function(obj){
		var Self = this;
		...
		if (UI.isArray(obj)){
			...
			//移出事件
			var index = 0;
			for (var i in Self.key[obj.event]){
				if (Self.key[obj.event][i] === obj){
					index = i;
				}
			}
			Self.key[obj.event].splice(index,1);
			
			//注销事件绑定
			if (Self.key[obj.event].length == 0){
				UI.ER(document.documentElement,obj.event,Self.builded[obj.event]);
			}
		}
	},
	/**
	 ...
	 */
	key : {},
	keyDo : function(e,event){
		var Self = this,
			E = UI.E(e);
		UI.each(Self.key[event],function(o){
			var nodeName = E.target.nodeName;
			if (nodeName != 'INPUT' && nodeName != 'TEXTAREA' && E.key == o.key){
				o.call(E);
			}
		});
	},
	...
	build : function(event){
		var Self = this;
		Self.builded[event] = function(e){
			Self.keyDo(e,event);
		};
		UI.EA(document.documentElement,event,Self.builded[event]);
	},
	talkList : [
		//P 切换图标模式
		{
			...
			call : function(){
				...
				if (viewModel) {
					...
					viewModel.onclick && viewModel.onclick();
				}
				else {
					if (MI.talkList) {
						MI.talkList.iconPicToggle();
		...
		*/
		//R 刷新消息
		{
			...
			call : function(){
				if (MI.talkList && MI.talkList._new){
					UI.fireEvent(MI.talkList._new,'click');
				}
			}
		},
		//T 返回顶部
		...
		//B 返回底部
		{
			...
			key : 66,
			call : function(){
				window.scrollTo(0,UI.pageHeight());
			}
		},
		//S 搜索
		{
...
MI.GoTop = {
	...
	build : function(target) {
		...
		if (UI.B.mac){ //修正Mac滚动条宽度
			rightFix += 2;
			if (navigator.userAgent.hasString('Mac OS X 10_7')){
				rightFix += 13;
			}
		}
