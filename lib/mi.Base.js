...
MI.MayBeKnow= (function(){
	...
	function buildList(arr) {
		...
		UI.each(arr,function(p,i){
			...
			str += '<a href="#" class="del" title="不再推荐此人">×</a>';
			str += '</div></li>';
		});
		return str;
	}
	...
	function bindEvent(){
		...
		UI.each($$(_list,'.del'),function(o,i){
			o.onclick = function(){
				var P = UI.parents(o)[2];
				var account = UI.A(P,'account');
				setTimeout(function(){
					showNext(P);
				},500);
				MI.feedBack({"scene":11,"type":6,"account":account});
...
MI.listDrop.build = function(classStr){				//外部调用静态方法
	...
	var btns = $$(classStr),
		dropList = {
			...
			'fav_myfav_top':{
				type:2,
				url:MI.url.favList || 'asyn/favoritemsggroup.php',
				data : {op:4,type:1},
				current:null,
				bos:'btnFavView',
				tab:_('更多')
	...
	setTimeout(function(){
		...
		UI.each(hotTopic,function(o,i){
			o.onmousedown = function(){
				var url = this.getAttribute('href',2).replace(/http:\/\/t.qq.com/g,'');
