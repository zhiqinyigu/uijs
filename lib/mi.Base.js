...
MI.badge = {
	...
	show : function(el){
		var Self = this,data = '',id = UI.A(el,'rel'),user = UI.A(el,'user');
		if (!Self.appended){
			Self._body = UI.html('<div class="badgeTips"><div class="SA"><em>◆</em><span>◆</span></div><div class="badgeCnt"></div></div>')[0];
	...
	position : function(el,data){
		var Self = this;
		el.title = '';
		Self._body.style.cssText = 'top:' + UI.getY(el) + 'px;left:' + UI.getX(el) + 'px;';
