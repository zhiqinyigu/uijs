...
MI.Validate = function(o){ //Validate
	...
	var Self = this,msgFunction,msgText,index = 0,input,target;
	...
	for (var i in Self.data.inputs) {
		...
		if (target) {
			...
			msgText = UI.A(input.message,'rel');
			if (!msgText && msgText != ''){
	...
	UI.EA(Self._body,'submit',function(e){
		var E = UI.E(e),dataPost = {},target,targetName,tmp;
		...
		if (Self.success && Self.isAjax) {
			for (var i in Self.data.inputs) {
				target = $(i);
				targetName = UI.A(target,'name');
				if (target.type == 'radio' || target.type == 'checkbox'){
					tmp = [];
					UI.each($$(Self._body,'input'),function(o){
						if (UI.A(o,'name') == targetName){
							tmp.push(o);
						}
					});
				}
				if (target.type == 'radio') {
					...
					if (tmp.length == 1){
						// 单个checkbox
						if (!$(i).disabled){ //过滤掉禁用的checkbox
							dataPost[i] = Self.data.inputs[i].result ? Self.data.inputs[i].result($(i).value) : $(i).value;
						}
						...
						for (var j = 0,num = tmp.length;j < num;j++) {
							if (tmp[j].checked) {
								dataPost[i].push(tmp[j].value);
							}
						}
					}
				}
				else {
					if (Self.data.inputs[i].result){
						dataPost[i] = Self.data.inputs[i].result($(i).value);
	...
	function validate(e){
		...
		if (isSubmit ? 1 : target.blured) {
			error = input.rule ? input.rule(target.value,isSubmit) : 0;
			if (!error) { //Not Error
