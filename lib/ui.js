...
UI = {
	 ...
	 */
	ajax : function(o) {
		var xmlHttp = o.xhr || UI.xmlHttp(),
			complete,
			timeout;
		o.async = UI.isUndefined(o.async) ? true : o.async
		...
		if (o.type == 'get') {
			xmlHttp.open('GET',o.url + (o.url.hasString('?') ? '&' : '?') + o.data, o.async);
			xmlHttp.send(null);
		}
		else {
	 ...
	 * @method GET only
	 * @param {String} String url地址
	 * @param {String} String 回调函数名
	 * @param {Function} Function 加载完后的回调函数
	 *            @example
	 *            UI.crossAsynJson('/test.php?callback=mbJsonCallback','mbJsonCallback',function(){
	 *            },5000,'utf-8')
	 *                                                
	 *            @callback
	 *            mbJsonCallback({'result':0,info:''})
	 */
	crossAsynJson : function(url,func,call,charset){
		var el = UI.DC('script'),head=UI.GT(document,'head')[0];
		window[func] = function(data){
			window[func] = undefined;
			try{delete window[func];} catch(e){}
			call(data);
			if (head){
				setTimeout(function(){head.removeChild(el);},5);
			}
		}
		if(charset){
	 ...
	 *            })
	 */
	getScript : function(url,call,charset,keep){
		var el = UI.DC('script');
		if (UI.B.ie) {
	 ...
	 *            })
	 */
	getCss : function(url,call,target){
		var link = target ? target : UI.DC('link');
		if (call) {
			link.onload = call;
		}
		if (!target) {
			UI.A(link,'rel','stylesheet');
			UI.A(link,'type','text/css');
			UI.GT(document,'head')[0].appendChild(link);
		}
		UI.A(link,'href',url);
	},
	/**
	 ...
	 */
	cookie : function(n,v,d,domain) { //Cookie
		if (v == undefined) {
	...
	animate : function(o,name,num,call,type,rate) {
		var type = type || 0.4,isScroll = name.hasString('scroll'),isScale = 'height,width,marginLeft,marginTop'.hasString(name),curTmp,delay = setInterval(function(){
			var cur,
				last,
				increase,
				isOpacity = name == 'opacity';
			//Get Style
			if (isScale) { //Kill Webki Bug When Zoom In Page
				cur = o.style[name];
			}
			else {
				cur = isScroll ? o[name] : UI.C(o,name);
			}
			...
			//Set Style
			if (isScale) {
				o.style[name] = last + 'px';
			}
			else if (isScroll) {
				o[name] = parseInt(last);
			}
			else {
				UI.C(o,name,(!isOpacity ? last + 'px' : last/100 + ''));
			}
	 ...
	 */
	getX : function(o) {
		return (o.offsetParent ? o.offsetLeft + UI.getX(o.offsetParent) : o.offsetLeft) + (UI.C(o,'position') == 'fixed' ? UI.scrollX() : 0);
	},
	/**
	 * 获取Dom的页面Y轴坐标值
	 * 
	 ...
	 */
	getY : function(o) {
		return (o.offsetParent ? o.offsetTop + UI.getY(o.offsetParent) : o.offsetTop) + (UI.C(o,'position') == 'fixed' ? UI.scrollY() : 0);
	},
	/**
	 ...
	 * @param {Object} Object style标签对象
	 *            @example
	 *            UI.css('.error{color:red}');
	 */
	css : function(string,style) {
		var text;
		if (!style){
			style = UI.DC('style')
	 ...
	 */
	text : function(el) {
		var str = [],e = el.childNodes,nodeName;
		for (var i = 0,num = e.length;i < num;i++) {
			nodeName = e[i].nodeName.toUpperCase();
			if(nodeName == 'STYLE' || nodeName == 'SCRIPT') continue; //排除css和js子元素
			str.push(e[i].nodeType != 1 ? e[i].nodeValue : UI.text(e[i]));
		}
		return str.join('');
	},
	/**
	 ...
	 * @param {Boolean} clear 是否清除母体对象
	 *            @example
	 *            var e = UI.bind(objA, funB);
	 */
	bind : function(fn, scope, clear) {
		if (clear){
			UI.clear(function(){
				scope = null;
			});
	...
	EA : function (o,n,f,capture) {
		if (o){
			...
			if(o.addEventListener) {
				if (n == 'mousewheel' && UI.B.firefox) {
					//only firefox support DOMMouseScroll
					n = 'DOMMouseScroll';
				}
	 ...
	 */
	G : function(n) {
		return UI.isElement(n) ? n : document.getElementById(n);
	},
	/**
	 * 获取某标签名的Dom
	 * 
	...
	$ : (function(){
		...
		return function(selector,o){
			...
			if (!o.querySelectorAll) {
				...
				var results = [],parts;
				if (selector.hasString(',')) {
					parts = selector.split(',');
					for (var i = 0,num = parts.length;i < num;i++) {
						results.concat( UI.$(parts[i],o) );
					}
					return results;
				}
				else {
					if (selector.hasString(' ')) {
						...
						elems = o.getElementsByTagName(query),
						results = [];
  ...
  /**
   * 通过样式选择器从包含自身的节点向父级查找最邻近的父级元素
   * 
   * @param {Object} Object 查找所依赖的子元素DOM对象
   * @param {String} String 样式选择器
   * @param {Object} Object Dom对象查找范围
   * @return {Object} Object Dom对象
   *            @example
   *            <div id="x"><div id="y"></div></div>
   *
   *            UI.closest(UI.G('y'), 'div'); //[div#x]
   */
  closest : function(el, selector, context) {
    var els;
    contenxt = context || document;
    els = UI.GC(context, selector);
    while(el) {
      if(UI.has(els, el)) {
        return el;
      }
  ...
  /**
   * 检测事件发生的关联元素是否在事件响应元素中，用在mouseover和mouseout事件中
   * @param {Object} Object 事件发生的对象
   * @param {String} String 事件对象
   * @return {Boolean} Boolean 是否包含
   */
  withinElement: function(el, event) {
    event = UI.E(event || UI.getEvent());
	 ...
	 * @return {Boolean} Boolean 是否为时间对象
	 *            @example
	 *            UI.isDate(new Date());
	 */
	isDate : function(o) {
	 ...
	 * 
	 * @param {Object} Object 时间对象
	 * @return {Object} Object 克隆的时间对象
	 *            @example
	 *            var date = new Date();
	 *            var cloneDate = UI.cloneDate(date);
	 */
	 ...
	 *            @example
	 *            var date = new Date();
	 *            var dateString = UI.formatDate(time,'yyyy-MM-dd hh:mm:ss');
	 */
	 ...
	 * 获取时区时间对象
	 * 
	 * @param {String} String 毫秒时间
	 * @param {String} String 时区
	 * @return {Object} Object 时间对象
	 *            @example
	 *            var date = UI.zoneDate('1301639737395',8);
	 *            var date = UI.zoneDate(1301639737395,8);
	 */
	zoneDate : function(time,zone){
		var date = new Date(Number(time)),
			utc = date.getTime() + (date.getTimezoneOffset() * 60000);
		return new Date(utc + (3600000 * zone));
	},
	/**
	 * 判断是否为对象
	 * 
	 ...
	 *            @example
	 *            var data = UI.json2str(data);
	 */
	json2str : function(data){
		var string = [],isArray = UI.isArray(data);
		if (UI.isObject(data)) {
			if (data === null) {
				return 'null';
			}
			...
			for (var i in data) {
				string.push((isArray ? '' : ('"' + i + '":')) + UI.json2str(data[i]));
			}
			string = string.join();
			return isArray ? ('[' + string + ']') : ('{' + string + '}');
		} else if (UI.isNumber(data) || UI.isFunction(data)) {
			return data.toString();
		} else if (UI.isUndefined(data)) {
			return 'undefined';
		}
		return !data ? '""' : ('"' + data + '"');
	},
	...
	each : function(o,f) {
		if (o){
			if(UI.isUndefined(o[0])){
				for (var key in o){
					if(!UI.isFunction(o[key])) f(o[key],key);
				}
			}
			else{
				for(var i = 0,n = o.length;i < n;i++){
	 ...
	 * @return {Array} Array 数组
	 *            @example
	 *            UI.merge([1,2,3],[3,4,5]); //[1,2,3,4,5]
	 */
	merge : function(A,B) {
		var tmp = [];
		if (B) { //Merge A + B
	...
	isReady : function(){
		...
		if (document.readyState == 'complete'){
			...
			if (document.readyState == "interactive") { // 防止Dom加载完了，但是某些Image还在加载，卡住页面了
				UI.onReady();
			}
			else {
	...
	B : (function(){ //Browser
		/*
			...
			Iphone: Mozilla/5.0 (iPhone; U; CPU iPhone OS 3_1_2 like Mac OS X; zh-cn) AppleWebKit/528.18 (KHTML, like Gecko) Version/4.0 Mobile/7D11 Safari/528.16
			Iphone QQ浏览器: MQQBrowser/28 Mozilla/5.0 (iPhone; CPU iPhone OS 5_0_1 like Mac OS X) AppleWebKit/534.46 (KHTML, like Gecko) Mobile/9A405 Safari/7534.48.3
							MQQBrowser/27 Mozilla/5.0 (iPhone; U; CPU iPhone OS 4_1 like Mac OS X; zh-cn) AppleWebKit/532.9 (KHTML, like Gecko) Mobile/8B117
			Mac: Mozilla/5.0 (Macintosh; Intel Mac OS X 10_7_2) AppleWebKit/535.2 (KHTML, like Gecko) Chrome/15.0.874.121 Safari/535.2
			Android: Mozilla/5.0 (Linux; U; Android 2.1-update1; zh-cn; Nexus One Build/ERE36B) AppleWebKit/530.17 (KHTML, like Gecko) Version/4.0 Mobile Safari/530.17
		*/
		var b = {},i = navigator.userAgent;
		b.win = i.hasString('Windows') || i.hasString('Win32');
		b.ie6 = i.hasString('MSIE 6') && !i.hasString('MSIE 7') && !i.hasString('MSIE 8');
		b.ie8 = i.hasString('MSIE 8');
		b.ie9 = i.hasString('MSIE 9');
		b.ie = i.hasString('MSIE');
		b.tt = i.hasString('TencentTraveler');
		b.opera = window['opera'] || i.hasString('Opera');
		b.safari = i.hasString('WebKit');
		b.chrome = i.hasString('Chrome');
		b.mac = i.hasString('Mac');
		b.ipad = i.hasString('iPad');
		b.iphone = i.hasString('iPhone');
		b.os4 = i.hasString('OS 4');
		b.os5 = i.hasString('OS 5');
		b.qq = i.hasString('QQBrowser');
...
UI._clearCache = [];
UI.clear = function(call){
	UI._clearCache.push(call);
}
UI.EA(window,'unload',function(){
	for (var i = 0,num = UI._clearCache.length;i < num;i++){
		UI._clearCache[i]();
	}
});
window.console || (console = { //Set Console
	log : function(msg){
		document.title += '_' + msg;
		//alert(msg);
	},
	dir : function(){
	}
