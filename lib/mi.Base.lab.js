					...
					<a class="view" href="http://t.qq.com/p/r/<%=talk[i].id%>">' + _('查看对话') + '</a><%}%><%if(MI.user.fun.fastReport == 1 && (talk[i].type == 2 || talk[i].type == 7) && talk[i].name != MI.user.account && '+V1+'.name == MI.user.account){%> <a'+V5+' class="alarm">' + _('删除') + '</a><%}%></span><div class="funBox">\
						<%if(MI.user.account){\
							...
							if(MI.user.fun.btnStyle==5||MI.user.fun.btnStyle==6){\
								...
								<div class="shareBtn"><p><a href="#">' + _('邮件分享') + '</a></p><%if(MI.user.fun.btnStyle!=5&&MI.user.fun.btnStyle!=6){\
								%><p><a href="#">' + _('QQ分享') + '</a></p><%}%></div>\
...
MI.pmcompose = function(name, from)
{
	...
	var _sHost = location.protocol + "//pm.t.qq.com/",
		...
		_sJsUrl = [_sHost, "zh_CN/htmledition/js/", _jsName, "?version=110512"].join(""),
		_sCssUrl = _sHost + "zh_CN/htmledition/style/wbpm.css?v=110512",
		_bClose = false;
...
MI.TalkList.prototype.geolocation = (function(){
	...
	return {
	'show' : function(el){			//两个地图加载完后才能创建地图对象
			if(loadedNum < 2) {
				UI.getScript('http://cache.soso.com/30d/js/map20100926/mapsoso20101028.js',function(){
					loadedNum++;
					if(loadedNum==2) show(el);
				},'utf-8',1);
...
MI.TalkList.prototype.shareMail = (function(){
		...
		var html = UI.html(MI.tmpl.shareMail)[0],
			...
			textArea2 = $$(mailCnt,'.inputArea')[1],     	//textarea
		...
		function sendMail(){
			...
			MI.dialog.showLoad();
			slideUpConact();
			var reason = textArea2.value.slice(0,140);
			UI.ajax({
				url:'/mail/mailShare.php',
				data:{mailAddr:UI.GT(sender,'select')[0].value,mlist:contact,subject:title,body:msgId,reason:reason},  //只发20个联系人
				success:function(data){
					MI.dialog.hideLoad();
					data = MI.json(data);
					if(data.result == 0){
...
MI.tmpl.trans = '<div class="comTips1"><div class="transWrap"><div class="transBox"><span class="cNote">' + _('译：') + '</span><span class="transResult"></span></div><div class="transNote"><span class="left">' + _('此翻译由金山词霸提供，仅供参考') + '</span><span class="right"><a class="transMe" href="#">'+ _('我来翻译') + '</a></span></div></div><div class="loading"></div></div>';
...
MI.SmartBox = function(id,callback,type,style){ //input element,callback,search type (keyword:用户输入的关键字提示,布尔值,可以为空）
	...
	this._query = UI.html('<span><input type="hidden" id="label" name="label" value="8" /><input type="hidden" id="poster" name="poster" value="2" /></span>')[0];
		...
		if (E.key == 40 || E.key == 38) {
			...
			if(Self._valTip){
				...
				if(Self.index == 0) {
					...
					this.value =$$(Self._select,'span')[0].innerHTML;
				}else{
					...
					function(data){
						...
						if (data.result == 0) {
							if(data.info && data.info.length){
								...
								Self.indexMax = Self._valTip ? l : l - 1;
								Self.index = -1;
								s.push(Self._valTip? '<dl><dt class="s">搜我的广播：<span index="0" value=\"'+ value +'\">'+value+'</span></dt><dt>搜全站：</dt><dd><ul>' : '<ul>');//s.push('<ul>');
								...
								if(Self._valTip){
									Self._list.unshift($$(Self._select,'dt')[0]);
									UI.EA($$(Self._select,'dt')[0],'click',function(){
										UI.after(Self._query,Self._key);
									});
								}
								//Event
								UI.each(Self._list,function(el,i){
									...
									el.onclick = function(){
										Self._key.value = (this.nodeName == 'DT') ? $$(this,'span')[0].innerHTML : this.innerHTML;
	...
	bulidList : function(data){
		...
		if (data.result == 0) {
			if(data.info){
				...
				s.push('<ul>');//s.push('<ul>');
				UI.each(data.info,function(o,i){
					s.push('<li value=\"'+ o +'\" index="' + (parseInt(i)+1) + '">' + '<span class="mainContent">' + o +'</span>' + '<span style="float : right">' + ((data.stat[Number]==1) ? '<b class="ico_rss">订</b>' : '<b class="ico_his">历史</b>') +'</span>'+'</li>');//等待icon制作成功即可加icon
					Number++;
				});
				s.push('</ul>');
				if (s.length == 2) {
				...
				UI.each(Self._list,function(el,i){
					...
					el.onclick = function(){
						Self._key.value = (this.nodeName == 'DT') ? UI.text($$(this,'span')[0]) : UI.text($$(this,'.mainContent')[0]);
						UI.A(Self._key,'index',UI.A(this,'index'));
						Self.submit();
					}
				});
