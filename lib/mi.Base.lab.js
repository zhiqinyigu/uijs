...
MI.tmpl.listAll = '<%for(var i=0,n=talk.length;i<n;i++){\
		...
		numAll=numRelay+numComt;%><li id="<%=talk[i].id%>"<%if(fav){%> fav="<%=talk[i].favortimestamp%>"<%}%> rel="<%=talk[i].timestamp%>">\
			...
			<%}%>\
			<div class="msgBox">\
				<div class="userName" rel="<%=talk[i].name%>"><strong><a href="/<%=talk[i].name%>"'+V7+'><%=talk[i].bkname||talk[i].nick%></a>'+MIIcon('talk[i].flag')+'<%=talk[i].icon%><%if(!'+V1+' || talk[i].type==5){%>:<%}%></strong><%if('+V1+' && talk[i].type==2){%>' + _('转播') + ':&nbsp;<%}else if('+V1+' && talk[i].type==7){%>' + _('评论') + ':&nbsp;<%}else if('+V1+' && (talk[i].type==4)){%>' + _('对') + '<strong class="userTo"><a href="/<%='+V1+'.name%>" title="<%=('+V1+'.bkname||'+V1+'.nick).toTitle()%>(@<%='+V1+'.chName || '+V1+'.name%>)"><%='+V1+'.bkname||'+V1+'.nick%></a>'+MIIcon(V1+'.flag')+'' + _('说') + ':<%}%></strong>\
				</div>\
				<div class="msgCnt"><%=talk[i].content||"&nbsp;"%></div>\
				<%var Imgage=talk[i].image && talk[i].image.length,Videos=talk[i].videos && talk[i].videos.length,Music=talk[i].music && talk[i].music.length,Vote=talk[i].votes && talk[i].votes.length;if(Imgage || Videos || Music || Vote){%><div class="mediaWrap">\
					<%if(Vote || !(Imgage || Videos || Music)){%>\
						<div class="videoBox voteBox" realurl="<%=talk[i].votes[0].realUrl%>" shorturl="<%=talk[i].votes[0].shortUrl%>" reltitle="<%=talk[i].votes[0].title%>">\
							<div class="vTools"><a href="#" class="vClose"><em></em>' + _('收起') + '</a><a href="<%=encodeURI(talk[i].votes[0].realUrl)%>" class="vUrl" target="_blank"><em></em><%if(talk[i].votes[0].title){%><%=MI.string.cut(talk[i].votes[0].title,40)%><%}%></a></div>\
				...
				<%if('+V1+' && (talk[i].type==2 || talk[i].type==7)){%>\
					<div class="replyBox">\
						<div class="msgBox">\
							<div class="msgCnt"><strong><a href="/<%='+V1+'.name%>" title="<%=('+V1+'.bkname||'+V1+'.nick).toTitle()%>(@<%='+V1+'.chName || '+V1+'.name%>)"><%='+V1+'.bkname||'+V1+'.nick%></a>'+MIIcon(V1+'.flag')+'<%='+V1+'.icon%><%if(!'+V1+'.source){%>:<%}%></strong><%if('+V1+'.source && ('+V1+'.type==4 || '+V1+'.type==5)){%><em class="userTo">' + _('对') + '</em><strong class="userTo"><a href="/<%='+V1+'.source.name%>" title="<%=('+V1+'.source.bkname||'+V1+'.source.nick).toTitle()%>(@<%='+V1+'.source.chName || '+V1+'.source.name%>)"><%='+V1+'.source.bkname||'+V1+'.source.nick%></a>'+MIIcon(V1+'.source.flag')+'' + _('说') + ':<%}%></strong>\<%='+V1+'.content%></div>\
							<%var Imgage='+V1+'.image && '+V1+'.image.length,Videos='+V1+'.videos && '+V1+'.videos.length,Music='+V1+'.music && '+V1+'.music.length,Vote='+V1+'.votes && '+V1+'.votes.length;if(Imgage || Videos || Music || Vote){%><div class="mediaWrap">\
								<%if(Vote || !(Imgage || Videos || Music)){%>\
									<div class="videoBox voteBox" realurl="<%='+V1+'.votes[0].realUrl%>" shorturl="<%='+V1+'.votes[0].shortUrl%>" reltitle="<%='+V1+'.votes[0].title%>">\
										<div class="vTools"><a href="#" class="vClose"><em></em>' + _('收起') + '</a><a href="<%=encodeURI('+V1+'.votes[0].realUrl)%>" class="vUrl" target="_blank"><em></em><%if('+V1+'.votes[0].title){%><%=MI.string.cut('+V1+'.votes[0].title,40)%><%}%></a></div>\
										<div class="vWrap"><a href="#" class="vtThumbs"><img class="crs" crs="http://mat1.gtimg.com/www/mb/images/voteResulte.png" /></a></div>\
									</div>\
								<%}%>\
								<%if(Imgage){%>\
							...
							<div class="pubInfo"><span class="left"><a class="time" rel="<%='+V1+'.timestamp%>" href="/p/t/<%='+V1+'.id%>" target="_blank"><%='+V1+'.time%></a> <%='+V1+'.from%><%if('+V1+'.city){%> <%='+V1+'.city%><%}%><%if(1 && '+V1+' && numAll){if(MI.user.account){%>\
								 <a href="/p/t/<%='+V1+'.id%>" class="zfNum" target="_blank">' + _('查看转播和评论') + '(<b class="relayNum"><%=numAll%></b>)</a>\
							<%}else{%>\
				...
				<%}%>\
				<div class="pubInfo"><span class="left"><a class="time" href="/p/t/<%=talk[i].id%>" target="_blank"><%=talk[i].time%></a> <%=talk[i].from%><%if(talk[i].city){%> <%=talk[i].city%><%}%><%if(1 && !'+V1+' && numAll){if(MI.user.account){%>\
					 <a href="/p/t/<%=talk[i].id%>" class="zfNum" target="_blank">' + _('查看转播和评论') + '(<b class="relayNum"><%=numAll%></b>)</a><%}else{%>\
					 <em class="zfNum">' + _('查看转播和评论') + '(<b class="relayNum"><%=numAll%></b>)</em><%}}%><%if('+V1+' && '+V1+'.content && '+V1+'.content.length && talk[i].type==4){%>\
					<a class="view" href="/p/r/<%=talk[i].id%>">' + _('查看对话') + '</a><%}%><%if(MI.user.fun.fastReport == 1 && (talk[i].type == 2 || talk[i].type == 7) && talk[i].name != MI.user.account && '+V1+'.name == MI.user.account){%> <a'+V5+' class="alarm">' + _('删除') + '</a><%}%></span><div class="funBox">\
						<%if(MI.user.account){%><a'+V5+' class="relay" num="<%=numRelay%>">' + _('转播') + '</a><span>|</span><%\
								if(MI.user.fun.btnStyle){\
									...
									%><a href="/p/t/<%=talk[i].id%>" class="comt" num="<%=numComt%>">' + _('评论') + '</a><%\
								}%><span>|</span><div class="mFun">\
								<a href="#">' + _('更多') + '<em class="btn_ldrop"></em></a><div class="mFunDrop"><b></b><b class="mask"></b>\
									<%if(MI.user.fun.btnStyle){\
										%><p><a href="/p/t/<%=talk[i].id%>" class="comt" num="<%=numComt%>">' + _('评论') + '</a></p><%\
									}else{\
									...
									<div class="shareBtn"><p><a href="#">' + _('分享') + '</a></p></div>\
									<%if(MI.user.fun.report && talk[i].name != MI.user.account){%><p><a'+V5+' class="alarm">' + _('举报') + '</a></p><%}%>\
								</div>\
							</div><%}%>\
						<%if(0){%><%if(talk[i].count){%><%if(MI.user.account){%>\
							<span>|</span><a href="/p/t/<%=talk[i].id%>" class="zfNum zfNumShow" target="_blank">' + _('评论') + '<b class="relayNum">(<%=talk[i].count%>)</b></a>\
						<%}else{%>\
							<em class="zfNum zfNumShow">' + _('评论') + '<b class="relayNum">(<%=talk[i].count%>)</b></em>\
						<%}}else if('+V1+' && '+V1+'.count){if(MI.user.account){%>\
							<span>|</span><a href="/p/t/<%='+V1+'.id%>" class="zfNum zfNumShow" target="_blank">' + _('评论') + '<b class="relayNum">(<%='+V1+'.count%>)</b></a>\
						<%}else{%>\
							<em class="zfNum zfNumShow">' + _('评论') + '<b class="relayNum">(<%='+V1+'.count%>)</b></em>\
						<%}}else{if(MI.user.account){%>\
							<span>|</span><a href="/p/t/<%=talk[i].id%>" class="zfNum" target="_blank">' + _('评论') + '<b class="relayNum"></b></a>\
						<%}else{%>\
							<em class="zfNum">' + _('评论') + '<b class="relayNum"></b></em>\
						<%}}%><%}%><%if(0 && MI.user.account){%><span>|</span><a href="/p/t/<%=talk[i].id%>" class="comt" target="_blank">' + _('评论') + '</a><%}%>\
					</div>\
				</div>\
...
MI.tmpl.msg = '<%for(var i=0,n=talk.length;i<n;i++){%><li id="<%=talk[i].id%>" rel="<%=talk[i].timestamp%>">\
			<div class="msgBox"><div class="userName">' + _('发给') + ' <a href="/<%=talk[i].name%>" title="<%=talk[i].bkname||talk[i].nick%>(@<%=talk[i].chName || talk[i].name%>)"><%=talk[i].bkname||talk[i].nick%></a>'+MIIcon('talk[i].flag')+'</div><div class="msgCnt"><%=talk[i].content%></div><div class="pubTime"><span class="time"><%=talk[i].time%></span> <%=talk[i].from%></div><p class="btnBox"><a href="#" class="replyMsg btn" rel="<%=talk[i].chName || talk[i].name%>">' + _('再写一封') + '</a><a class="btn delBtn" href="#">' + _('删除') + '</a></p></div>\
		</li><%}%>';
...
MI.tmpl.card = '<div class="uCard"><div class="tip">' + _('这是你自己') + '</div><div class="tip"></div><div class="userPic"><a></a><p class="btn"><input type="button" class="addAttention" value="" /><a href="#" class="delAttention" title="' + _('取消收听') + '">' + _('取消') + '</a></p></div><div class="uCardcnt"><p class="userName"></p><p class="nums"></p><p class="btn"><a href="#" class="note">' + _('备注') + '</a><a href="#" class="reply">' + _('对话') + '</a><a href="#" class="msg">' + _('私信') + '</a><a href="#" class="black">' + _('拉黑') + '</a></p><div class="uInfobox"><div class="sepline"></div><p class="uIntro"></p><p class="tagBox"></p><p class="company"></p><p class="school"></p></div><a href="#" class="ico_sFo"></a></div><div class="uloadBox"><em class="loading"></em>资料卡加载中</div><div class="SA"><em>◆</em><span>◆</span><b>◆</b></div></div>';
...
MI.navMenu = function(){ //Nav Menu
	...
	if (topic){
		subTopicNav = UI.html('<div class="subNav" onclick="MI.Bos(\'btnTopMenuTopic\')"><p><a href="/p/topic">' + _('热议') + '</a></p><p class="last"><a href="/p/event">' + _('活动') + '</a></p></div>')[0];
	...
	if (!Self._voteWrap){
		Self._voteWrap = UI.html('<div class="mloadWrap">\
			...
			<div class="m_voteBox clear">\
				...
				<table>\
					<tr class="voteAddTr"><th>' + _('标题：') + '</th><td valign="top"><input type="text" name="sbjtitle" id="sbjtitle" class="inputTxt"></td><td valign="top"><span class="voteAddError"></span></td></tr>\
					<tr class="voteAddTr"><th>' + _('类型：') + '</th><td valign="top"><label><input class="check1" type="radio" name="ftype" id="ftype" value="0" checked>' + _('单选') + '</label> <label><input class="check1" type="radio" name="ftype" value="1">' + _('多选') + '</label></td><td><b class="voteAddError"></b></td></tr>\
					<tr class="voteAddTr"><th>' + _('选项：') + '</th><td valign="top">\
						...
						<a href="#" class="voteAddOption">' + _('增加选项') + '>></a>\
			...
			Validate : function(){
				new MI.Validate({
					...
					success : function(data){
						...
						if (data.result == 0){
							Self._txt.value += _('我发起了一个投票「{0}」，地址：',data.info.title) + data.info.url;
							Self.focus();
...
MI.TalkBox.prototype.addVoteSubmit = function(){ // 新建投票提交
	var Self = this,
		data = {},
		input = $$(Self._voteWrap,'td input')[0],
		_title = input[0],
		_type = input[1],
		_option = $$(Self._voteWrap,'td ul input')[0];
	data.sbjtitle = _title.value;
	data.ftype = _type.checked ? 0 : 1;
	UI.ajax({
		url : '/asyn/vote.php',
		data : data, //opttitle[]=test&sbjtitle=test&ftype=0
		success : function(data){
			data = MI.json(data);
		}
	});
...
MI.TalkList.prototype.shareMail = function(id){ // 分享到邮件
	var title = _('腾讯微博分享');
	var el = $(id);
	var reply = $$(el,'.replyBox .msgCnt')[0];
	var str = [];
	str.push(_('Hi，我在腾讯微博看到了这条广播，想与你分享：'));
	str.push('\n\n\x20\x20\x20“');
	if ($$(el,'.replyBox')[0]){
		str.push(UI.text($$(el,'.replyBox .msgBox .msgCnt')[0]));
	}
	else {
		str.push(UI.text($$(el,'.msgBox .userName')[0]) + UI.text($$(el,'.msgBox .msgCnt')[0]));
	}
	str.push('”');
	if ($$(el,'.mediaWrap .picBox')[0]){
		str.push(_('【图】'));
	}
	str.push('\n\n\x20\x20\x20' + _('微博是最近很受欢迎的一种交流平台，点击链接，来亲身体验一下吧！'));
	str.push('\n\x20\x20\x20http://t.qq.com');
	var body = str.join('');
	UI.ajax({
		url:'/asyn/utf2gbk.php',
		data:{subject:title,body:body},
		success:function(data){
			data = MI.json(data);
			if (data.result == 0){
				window.location = 'mailto:?subject=' + data.info.subject + '&body=' + data.info.body;
			}
		}
	});
	MI.Bos('btnShareMail');
...
MI.TalkList.vote = function(el){ //显示投票
	var P = el.parentNode.parentNode,
		...
		iframe;
	UI.addClass(P,'vPlay');
	iframe = '<div style="height:200px"></div><iframe style="display:none" onload="UI.show(this);var height=(this.contentWindow.document.body.scrollHeight||this.contentWindow.document.documentElement.scrollHeight)+5+\'px\';this.style.height=height;UI.hide(this.previousSibling);" src="http://page.vote.qq.com/?id=931111&result=yes&tpl=mini" frameborder="0" scrolling="no"></iframe>';
	if (box){
...
MI.AutoCmt = function(o){ //Auto Complete
	...
	*/
	...
	Self._body = UI.html('<div class="atWrap" style="display:none">\
		...
		<div class="tips" style="display:none">' + _('@朋友帐号,他就能在[提到我的]页收到') + '</div>\
	</div>')[0];
...
MI.Card.prototype = {
	...
	show : function(el){
		...
		function show(o){
			...
			//o.avatar = 'http://t0.qlogo.cn/mbloghead/3b93625a5b16a7900aca';  //伪代码，要删除
			...
			//o.uIntro = '简介：俄爱生活、喜欢幽默、喜欢和平凡普通…喜欢和平凡普通喜欢和平凡普和平凡普通喜欢和平凡普通通';    //伪代码，要删除
			//o.company = {comName:'绝无仅有无处不在不可思议硕果仅存有限公司',href:'http://www.google.com'};
			//o.school =  {name:'中国北京科技大学海淀分校',href:'http://www.google.com',objectName:'电子信息与自动化专业',objectHref:''};
...
MI.Card.build = function(el,className,cType/*clickHead*/){
	cType = cType || 1;
