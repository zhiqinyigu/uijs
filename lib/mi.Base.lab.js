	...
	MI.app({
		Tmpl : function(){
			...
			if (Self._faceBox.style.display == 'none'){
				setTimeout(function(){
					Self._faceBox.style.cssText = 'top:' + (UI.getY(Self._face) + UI.height(Self._face) + 5) + 'px;left:' + (UI.getX(Self._face) - 6) + 'px';
				},0);
			}
			else {
...
MI.searchKey = function(){ //Search Key
	...
	if (searchKey){
		...
		MI.searchKeyBox = new MI.SmartBox('searchKey',function(){$('searchForm').submit(); $('searchForm').action="http://t.qq.com/search/index.php"},'',{left:(newNav ? -4 : -8),top:UI.B.ie?4:6,width:(newNav ? 172 : 202),type:1,history:1});
...
MI.MayBeKnow= (function(){
	...
	function change(e){
		var arr= [],o = dataCache;
		if(o.list.length != 0) MI.MayBeKnow.returnResult = 1;
...
MI.linkBoss = function(){ //Link Boss
	setTimeout(function(){
		var link = $$('#UIn a,.SM a,.mobileBox a,.proposal a'),hotTopic = $$('#hotTopic li'),topMenu = $$('.topMenu a');
...
MI.GoTop = {
	...
	build : function() {
		...
		if (!MI.newCountShow || document.location.href.hasString('/k/') || document.location.href.hasString('/search/')){
			MI.newCount((MI.user.fun.relayFilter ? '14' : '4') + ',2,1');
		}
