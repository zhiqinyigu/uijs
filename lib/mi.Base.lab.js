...
MI.tmpl.know = '<div class="tjRelaWrap"><div class="title"><div class="left"><h3>' +_('可能认识的人') + '</h3><a class="cSign"></a><a class="change" href="#">[' +_('换一换') +']</a><a href="http://t.qq.com/potential.php" target="_blank">[' +_('全部') + ']</a></div><div class="right"><a href="#" class="btn flod"><em class="arrow1"></em>' +_('收起') + '</a></div></div><ul class="tjRelaList clear"><ul></div>';
...
MI.pmcompose = function(name, from, content, bostype)
{
	...
	var _sStaticHost = location.protocol + "//rescdn.qqmail.com/pmweb/",
		...
		_sJsUrl = [_sStaticHost, "zh_CN/htmledition/js/", _jsName, "?version=110704"].join(""),
		_sCssUrl = _sStaticHost + "zh_CN/htmledition/style/wbpm.css?v=110704",
...
MI.MayBeKnow= (function(){
	var _body,_list,_newSign,_change,_all,_toggle,SHOW_NUM=6,dataCache;
    var followingNum =  $('followingNum') && parseInt($('followingNum').innerHTML);
    
	function getData(){
        if(followingNum>30) return;
		MI.ajax({
	...
	function buildList(arr) {
		...
		UI.each(arr,function(p,i){
			...
			str += '<li account="' + p.account + '"><div class="userPic">';
			str += '<a title="' + title + '" href="' + href + '">';
			...
			str += '<p><a class="uname" title="' + title + '" href="' + href + '">' + (p.bkName || p.nick) + '</a></p>';
			...
			str += '<p><a class="gray" onclick="MI.Bos(\"btnMayBeKnowReason\")" href="http://t.qq.com/potential.php?t=' + p.type + '" target="_blank">' + p.reason + '</a><p>';
			str += '</div></li>'
...
MI.TalkBox.prototype.addFace = function(){
	...
	MI.app({
		Tmpl : function(){
			if (!Self._faceBox){
				Self._faceBox = UI.html(MI.tmpl.face)[0];				UI.C(Self._faceBox,'z-index',9999);
...
//多图上传开始
MI.PhotoMultiUpload = {
	html :'<div class="multiPic" style="padding:5px 0"><p style="padding-bottom:2px;color:#999;letter-spacing:0.5px;">' + _('按住Ctrl 键可一次选中并上传多张图片（最多8张），拖动图片可以调整顺序') + '</p><object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" id="MultiPhoto" width="462" height="215" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab"><param name="movie" value="http://mat1.gtimg.com/www/mb/swf/MultiPhoto_110706.swf" /><param name="wmode" value="transparent" /><param name="quality" value="high" /><param name="allowScriptAccess" value="always" /> <embed src="http://mat1.gtimg.com/www/mb/swf/MultiPhoto_110706.swf" quality="high" width="462" height="215" name="MultiPhoto" align="middle" play="true" loop="false" quality="high" wmode="transparent" allowScriptAccess="always" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer"></embed></object><div class="clear"><p style="text-align:right;padding:10px 5px 5px 5px;" class="btnBox"><button class="btn3 btnSend">' + _('确定') + '</button>&nbsp;<button class="btnCancel btn_cancel">' + _('取消') + '</button></p></div></div>',
	body:null,
	sendBtn:null,
	cancelBtn:null,
	uploadUrl:'',
	build:function(uploadUrl){		//外部调用build方法
		var Self = this;
		...
		Self.cancelBtn.onclick = function(){Self.clear()};  //调用clear清除 不是hide隐藏
		window['_flash_resizeFlashHeight'] = Self.resetSize;  //注册给flash调用，_flash_resetFilterSize名字不能改
	...
	show:function(){
		var Self = this;
		MI.dialog.show({
			title:'<h3 class="DmainTit">' + _('多图上传') + '</h3>',
			html:Self.body,
			width:492
		});
		MI.dialog.showLoad();
	...
	clear:function(){
	  	document.MultiPhoto.multiPic_clearStage();
		MI.dialog.hide();
	},
	hide:function(){
		var Self = this;
	...
	resetSize:function(h){	//调整flash宽高
		if(document.MultiPhoto) document.MultiPhoto.height = h;
		...
		if(data.result ==0) {
			setTimeout(function(){
				MI.tip(_('上传成功'),function(){
					UI.hide(MI.talkBox._multiPic);
					MI.talkBox.addPic({'result':0,'msg':_('上传成功'),info:{'image':data.info.image,'path':data.info.path,'host':data.info.host,'fileName':_('多图') + '.jpg'}});
				});
			},0);		
		} else {
			MI.alert(data.msg);
...
MI.Text2Pic = {
    html :'<div class="Text2Pic" style="padding:5px 0;text-align:center"><textArea class="longText" style="height:300px;width:450px;padding:10px;border:1px solid #ccc;font-size:14px;">' + _('一条微博说不完的话，转成图片试试吧~不要超过2000字哦：）') + '</textArea><div style="height:0;overflow:hidden;position:absolute;z-index:-10;"><object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" id="Text2Pic" width="462" height="10" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab"><param name="movie" value="http://mat1.gtimg.com/www/mb/swf/Text2Pic.swf" /><param name="wmode" value="transparent" /><param name="quality" value="high" /><param name="align" value="middle" /><param name="allowScriptAccess" value="always" /> <embed src="http://mat1.gtimg.com/www/mb/swf/Text2Pic.swf" quality="high" width="462" height="215" name="Text2Pic" align="middle" play="true" loop="false" quality="high" wmode="transparent" allowScriptAccess="always" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer"></embed></object></div><div class="clear"><p style="text-align:right;padding:5px 0;" class="btnBox"><span class="tip" style="position:absolute;z-index:9;display:none;padding:20px 20px 20px 40px;bottom:150px;left:150px;border:5px solid #ccc;font:18px/21px \'MicroSoft YaHei\',\'Simhei\';color:#333;background:#fff url(\'http://mat1.gtimg.com/www/mb/images/loading.gif\') no-repeat 15px 50%;"></span><button class="btn3 btnSend">' + _('确定') + '</button>&nbsp;<button class="btnCancel btn_cancel">' + _('取消') + '</button></p></div></div>',
    body:null,
    txt:null,
    sendBtn:null,
    cancelBtn:null,
    ...
    build:function(url){
        var Self = this;
        if(Self.body) UI.remove(Self.body);
        Self.body = UI.html(Self.html)[0];
        Self.sendBtn = $$(Self.body,'.btnSend')[0];
		Self.cancelBtn = $$(Self.body,'.btnCancel')[0];
        Self.tip = $$(Self.body,'.tip')[0];
        Self.txt = $$(Self.body,'.longText')[0];
    	Self.sendBtn.onclick = function(){Self.upload()};
        ...
        window['_flash_Text2PicLoaded'] = function(){  //flash加载完成时调用
            MI.dialog.hideLoad();
            UI.C(Self.body,'visibility','visible');
            Self.loaded = true;
        }
        
        var style = Self.txt.style.cssText;
        Self.txt.style.cssText = style + 'line-height:20px;font-size:12px;color:#999;';

        Self.txt.onfocus = function(){
            if(this.value == this.defaultValue) {
                this.value = '';
                Self.txt.style.cssText = style;
            }    
        }
        //window['_flash_setText2PicTip'] = function(str) {Self.setTip(str)};
        Self.uploadUrl = url;
        Self.show();
        return false;
    },
    show:function(){
		var Self = this;
		MI.dialog.show({
			title:'<h3 class="DmainTit">' + _('文字转图片') + '</h3>',
			html:Self.body,
			width:462,
            start:function(){
                //Self.txt.focus();
        ...
        UI.C(Self.body,'visibility','hidden');
        MI.dialog.showLoad();
	},
    upload:function(){
        var Self = this;
        ...
        if(len>4000) {
            Self.setTip(_('字数超过<span class="error">{0}</span>字，请删减',Math.ceil((len-4000)/2)),true);
            return;
        }  
        clearTimeout(Self.delay);
        MI.dialog.showLoad();
        ...
        document.Text2Pic.buildImg(Self.uploadUrl,'\n' + txt + '\n\n\n\n','_flash_Text2PicUploadCall');
    ...
    uploadCall:function(data){
		...
		if(data.result ==0) {
            UI.hide(Self.tip);
            Self.setSummary(MI.string.cut(Self.txt.value,100,'...'));
			setTimeout(function(){
				MI.tip(_('上传成功'),function(){
					MI.talkBox.addPic({'result':0,'msg':_('上传成功'),info:{'image':data.info.image,'path':data.info.path,'host':data.info.host,'fileName':_('文图') + '.jpg'}});
				})
			},0);
		} else {
    ...
    setSummary:function(text){
        var Self= MI,_txt = Self.talkBox._txt;
        Self.talkBox.txtSummary = text;
        if(!_txt.value.hasString(text)){
            _txt.value += text;
        }
...
//滤镜开始
MI.photoFilter = {
	html :'<div class="filterPic" style="width:517px;padding:5px 0;text-align:center"><object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" id="FilterPhoto" width="462" height="250" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab"><param name="movie" value="http://mat1.gtimg.com/www/mb/swf/FilterPhoto.swf" /><param name="wmode" value="transparent" /><param name="quality" value="high" /><param name="align" value="middle" /><param name="allowScriptAccess" value="always" /> <embed src="http://mat1.gtimg.com/www/mb/swf/FilterPhoto.swf" quality="high" width="462" height="215" name="FilterPhoto" align="middle" play="true" loop="false" quality="high" wmode="transparent" allowScriptAccess="always" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer"></embed></object><div class="clear"><p style="text-align:right;padding:5px;" class="btnBox"><span class="error" style="padding-right:20px"></span><button class="btn3 btnSend">' + _('确定') + '</button>&nbsp;<button class="btnCancel btn_cancel">' + _('取消') + '</button></p></div></div>',
	body:null,
	sendBtn:null,
	cancelBtn:null,
	sourceUrl:'',
	...
	uploadCall:function(data){
		...
		if(data.result ==0) {
			setTimeout(function(){
				MI.tip(_('上传成功'),function(){
					MI.talkBox.addPic({'result':0,'msg':_('上传成功'),info:{'image':data.info.image,'path':data.info.path,'host':data.info.host,'fileName':_('滤镜') + '.jpg'}});
				})
			},0);
		} else {
			MI.alert(data.msg);
...
MI.SmartBox = function(id,callback,type,style){ //input element,callback,search type (keyword:用户输入的关键字提示,布尔值,可以为空）
	...
	//用户搜索页参数
	this._queryUser = UI.html('<span><input type="hidden" id="pos" name="pos" value="401" /></span>')[0];
	...
	if (this.tip) {
		...
		else if(MI.guest) Self._tip.innerHTML =  _('搜{0}的广播/搜全站：',MI.guest.sex);
	}
	...
	this._key.onkeydown = function(e){
		...
		if (E.key == 40 || E.key == 38) {
			$('searchForm').action="http://t.qq.com/search/index.php"
			if (E.key == 38) {
			...
			if(Self._valTip){
				...
				if(UI.hasClass(Self._list[Self.index],'special')) { //搜我（TA）的广播需要传递不同于搜全站的参数
					...
					this.value = UI.text($$(Self._select,'span')[0]);
					$('searchForm').action="http://t.qq.com/search/user.php";
				}else{
					if(Self._query )UI.remove(Self._query);
					else if(Self._queryUser) UI.remove(Self_queryUser);
					if(UI.hasClass(Self._list[Self.index],'userGet')){
						var uname = UI.A(Self._list[Self.index],'name');
						$('searchForm').action='http://t.qq.com/' + uname;
...
MI.SmartBox.prototype = {
					...
					success : function(data){
						...
						if (data.result == 0) {
							...
							if(data.info.user){
			         		...
			         		//搜用户结束
							if(data.info.msg.length != 0){ //有自动匹配的搜索关键词
								len = data.info.msg.length;
								Self.index = -1;
								//搜全站
								Self.s.push(Self._valTip ?  total + '<dd><ul>' : '<ul>');
								UI.each(data.info.msg,function(o,i){
							...
							else if(data.info.msg.length == 0){ //没有自动匹配的搜索关键词，右侧搜索框也出现“搜全站”和”搜我(他)的广播“
							    if(Self._valTip){
								    if (MI.boss == 13 || MI.boss == 7){ 
								    	Self.indexMax = 2;
										if(MI.guest){
							...
							Self.indexMax = Self._valTip ? len1 + len2 : len-1;
							if (Self.s.length == 2)
								return;
							...
							if(Self._valTip){
								...
								Self._list.splice(len2+1,0,$$(Self._select,'.speak')[0]); 
								if (MI.boss == 13 || MI.boss == 7){  //如果是广播页或者客人页
									Self._list.push($$(Self._select,'.special')[0]);
									UI.EA($$(Self._select,'.special')[0],'mousedown',function(){
										UI.after(Self._query,Self._key);
									});
								}
	...
	submit : function(el){
		$('searchForm').action="http://t.qq.com/search/index.php";
		if(this._callback){
			//搜用户的时候，需要更改action值，在callback里要改回来
			if(el && UI.hasClass(el,'searchUser')){$('searchForm').action="http://t.qq.com/search/user.php"; this._callback();}
			else
				this._callback(); 
		}
...
MI.GoTop = {
	_body : UI.html('<div class="toolbar" style="display:">\
		<div class="tipsBox2">\
			<div class="qTalkBox"><a href="#" class="ico_qTips" style="display:none"></a><a href="#" class="btn_qTalk">' + _('QQ临时对话') + '<b class="mask"></b></a></div>\
			<div class="toolBox" style="display:none">\
				<a href="/at?filter=0" class="ico_at" title="提到我的" onmousedown="MI.Bos(\'btnMsgBoxAt\')"></a><span id="newCout2At" class="bubbleW"><a href="/at?filter=0" onmousedown="MI.Bos(\'btnMsgBoxAtNum\')"></a><span></span></span>\
				<div class="tipsBox1" style="display:none">\
					<p><a href="/at?filter=1" onmousedown="MI.Bos(\'btnMsgBoxAtFollowing\')">我收听的<em rel="1"></em></a></p>\
					<p><a href="/at?filter=2" onmousedown="MI.Bos(\'btnMsgBoxAtVip\')">认证用户<em rel="2"></em></a></p>\
					<p><a href="/at?filter=0" onmousedown="MI.Bos(\'btnMsgBoxAtAll\')">所有提到<em rel="0"></em></a></p>\
				</div>\
			</div>\
			<!--<div class="toolBox" style="display:none"><a href="#" class="ico_wq" title="微群" onmousedown="MI.Bos(\'btnMsgBoxQun\')"></a></div>-->\
			<div class="toolBox" style="display:none"><a href="/messages/inbox" class="ico_letter" title="私信" onmousedown="MI.Bos(\'btnMsgBoxMsg\')"></a><span id="newCout2Msg" class="bubbleW"><a href="/messages/inbox" onmousedown="MI.Bos(\'btnMsgBoxMsgNum\')"></a><span></span></span></div>\
			<div class="toolBox" style="display:none"><a href="/' + MI.user.account + '/follower" class="ico_foer" title="听众" onmousedown="MI.Bos(\'btnMsgBoxFollower\')"></a><span id="newCount2Follower" class="bubbleW"><a href="/' + MI.user.account + '/follower" onmousedown="MI.Bos(\'btnMsgBoxFollowerNum\')"></a><span></span></span></div>\
			<div class="toolBox" style="display:none"><a href="/k/%25E6%2584%258F%25E8%25A7%2581%25E5%258F%258D%25E9%25A6%2588" class="ico_new" title="公告" onmousedown="MI.Bos(\'btnMsgBoxNotice\')"></a></div>\
		</div>\
		<div class="sToolbar">\
			<div class="sToolbarBg"><em></em></div>\
			<div class="mSimpleBox hover" style="display:none"><a href="#" class="ico_mState" style="display:none"></a><button class="btn_mPlay" title="' + _('播放') + '">' + _('播放') + '<b class="mask"></b></button><button class="btn_mPause" title="' + _('暂停') + '" style="display:none">' + _('暂停') + '<b class="mask"></b></button><b class="mbg"></b></div>\
			<div class="goTopBox" style="display:"><a href="#" class="btn_goTop" onclick="window.scrollTo(0,0);MI.Bos(\'btnTop\');this.blur();return false" title="' + _('返回顶部') + '">' + _('返回顶部') + '<b class="mask"></b></a></div>\
			<div class="sToolbarBg"><em></em></div>\
		</div>\
	</div>')[0],
	...
	btnNum : 1,
	rightFix : -40,
	btnClass : ['sToolbar single','sToolbar single','sToolbar double'],
	btnSet : function(){ //外层按钮样式名控制
		var Self = this;
		Self._toolBar.className = Self.btnClass[Self.btnNum - 1];
	},
	chat : 0, //是否有聊天按钮
	ipad : 0, //是否为iPad
	position : function(){
	...
	qqBoxSet : function(scrollY){
		var Self = this,
			bottom = '',
			activeElementName = '',
			rightFix = 0,
			right = ( UI.windowWidth() - UI.width($('mainWrapper')) ) / 2 - 8 + Self.rightFix;
		if (UI.B.ie6){
			rightFix += 8;
		}
		else if (UI.B.ie){
		...
		if (UI.windowHeight() == UI.pageHeight()){ //修正滚动条宽度
			if (!UI.B.ie){
				rightFix += 6;
			}
		}
		if (Self.ipad){
			activeElementName = document.activeElement.nodeName;
			if (activeElementName != 'TEXTAREA' && activeElementName != 'INPUT'){
				rightFix += 7;
				bottom = 'bottom:-' + (scrollY - 1) + 'px;';
			}
			//document.body.scrollLeft = 0;
		}
		Self._qqBox.style.cssText += ';display:block;right:' + (right + rightFix + Self.msgBoxWidth) + 'px;' + bottom;
		if (bottom){
			Self._body.style.cssText += ';display:block;' + bottom;
		}
	},
	build : function() {
		if (!$('mainWrapper')) {
		...
		if (UI.hasClass(document.body,'ipad')) {
			Self.ipad = 1;
			UI.C(document.body,'overflow-x','hidden');
			UI.C(document.documentElement,'overflow-x','hidden');
			return;
		}
		...
		if (_main) {
			Self._at = $$(Self._body,'.ico_at')[0]; //At
			Self._atTips = $$(Self._body,'.tipsBox1')[0]; //At浮层
			Self._mBox = $$(Self._body,'.mSimpleBox')[0]; //音乐
			Self._toolBar = $$(Self._body,'.sToolbar')[0]; //返回顶部和音乐播放
			Self._msgBox = $$(Self._body,'.toolBox'); //消息盒子
			Self._tBox = $$(Self._body,'.goTopBox')[0]; //返回顶部
			Self._qBox = $$(Self._body,'.qTalkBox')[0]; //假的聊天按钮
			UI.append(Self._body,document.body);
			Self._atNum = $('newCout2At'); //At数字
			Self.musicBoxEvent();
			if(state!=null){
				UI.show(Self._mBox);
			}
			...
			if (Self.ipad){
				document.body.ontouchmove = function(){
					if (Self._body.style.display != 'none'){
						UI.hide(Self._body);
						UI.hide(Self._qqBox);
					}
				}
				document.body.ontouchend = position; //不绑定touched，手指左右滑动时不会触发position
			}
			UI.EA(window,'scroll',position);
			UI.EA(window,'resize',function(e){
				if(UI.B.ie6) {				//解决ie6下resize事件反复触发的问题
					var h = UI.windowHeight(),w = UI.windowWidth();
					if(windowHeight == h && windowWidth == w) return;
					windowHeight = h;
					windowWidth = w;
				}
				position(e);
			...
			Self._at.onmouseover = Self._atNum.onmouseover = Self._atTips.onmouseover = function(){
				clearTimeout(delay);
				if (UI.text(Self._atNum) && !UI.A(Self._atNum,'onlyAll')){
					UI.show(Self._atTips);
				}
			}
			Self._at.onmouseout = Self._atNum.onmouseout = Self._atTips.onmouseout = function(){
				delay = setTimeout(function(){
					UI.hide(Self._atTips);
				},200);
			}
		}
		resize = 1;
		...
		function position(e){
			...
			Self.delay.position = setTimeout(set,UI.B.ie6 ? 400 : 10);
			if (MI.card){
				MI.card.hide();
			}
		}
		function set(){
			...
			if (resize) {
				windowHeight = UI.windowHeight()
				left = UI.getX(_main) + UI.width(_main) - leftFix;
				right = UI.windowWidth() - UI.getX(_main) - UI.width(_main) - 30 - 31 + 6 + rightFix;
				Self.resize = 0;
			}
			scrollY = UI.scrollY();
			Self._body.style.cssText = 'right:' + (right + 38 + Self.rightFix) + 'px;';
			Self._qqBox.style.cssText = '';
			if(scrollY < 52 || Self.ipad){
				UI.hide(Self._tBox);
				Self.btnNum = state != null ? 2 : 1;
				if (state == null){
					UI.hide(Self._toolBar);
				}
			}
			else{
				Self.btnNum--;
				...
				UI.show(Self._toolBar);
				Self.btnNum = state != null ? 3 : 2;
			}
			Self.qqBoxSet(scrollY);
			Self.btnSet();
			y = scrollY + windowHeight - pageHeight;
			if (y >= -foot || noScroll) {
				Self._body.style.cssText += ';position:absolute;top:' + (pageHeight - 37 - foot + 1) + 'px';
				Self._qqBox.style.cssText += ';position:absolute;top:' + (pageHeight - foot) + 'px';
			}
		}
	},
	msgBoxWidth : 0,
	showMsgBox : function(){ //显示消息盒子
		if (!(MI.user.fun.goTop && MI.user.account)){
			return;
		}
		var Self = this
			countmsgBoxWidth = !Self.msgBoxWidth;
		UI.each(Self._msgBox,function(o){
			UI.hide(o);
			if (countmsgBoxWidth){
				Self.msgBoxWidth += UI.width(o);
			}
		});
		/*
		if (UI.scrollY() < 453){
