...
MI.tmpl.listAll = '<%for(var i=0,n=talk.length;i<n;i++){\
		...
		numAll=numRelay+numComt;%><li id="<%=talk[i].id%>"<%if(fav){%> fav="<%=talk[i].favortimestamp%>"<%}%> rel="<%=talk[i].timestamp%>">\
			...
			<%}%>\
			<div class="msgBox">\
				<div class="userName" rel="<%=talk[i].name%>"><strong><a href="/<%=talk[i].name%>"'+V7+'><%=talk[i].bkname||talk[i].nick%></a>'+MIIcon('talk[i].flag')+'<%=talk[i].icon%><%if(!'+V1+' || talk[i].type==5){%>:<%}%></strong><%if('+V1+' && talk[i].type==2){%>' + _('转播') + ':&nbsp;<%}else if('+V1+' && talk[i].type==7){%>' + _('点评') + ':&nbsp;<%}else if('+V1+' && (talk[i].type==4)){%>' + _('对') + '<strong class="userTo"><a href="/<%='+V1+'.name%>" title="<%=('+V1+'.bkname||'+V1+'.nick).toTitle()%>(@<%='+V1+'.chName || '+V1+'.name%>)"><%='+V1+'.bkname||'+V1+'.nick%></a>'+MIIcon(V1+'.flag')+'' + _('说') + ':<%}%></strong>\
				</div>\
					...
					<div class="replyBox">\
						<div class="msgBox">\
							...
							<div class="pubInfo"><span class="left"><a class="time" rel="<%='+V1+'.timestamp%>" href="/p/t/<%='+V1+'.id%>" target="_blank"><%='+V1+'.time%></a> <%='+V1+'.from%><%if(1 && '+V1+' && numAll){if(MI.user.account){%>\
								 <a href="/p/t/<%='+V1+'.id%>" class="zfNum" target="_blank">' + _('查看转播和点评') + '(<b class="relayNum"><%=numAll%></b>)</a>\
							<%}else{%>\
								 <em class="zfNum">' + _('查看转播和点评') + '(<b class="relayNum"><%=numAll%></b>)</em>\
							<%}}%></span></div>\
						</div>\
					</div>\
				<%}%>\
				<div class="pubInfo"><span class="left"><a class="time" href="/p/t/<%=talk[i].id%>" target="_blank"><%=talk[i].time%></a> <%=talk[i].from%><%if(1 && !'+V1+' && numAll){if(MI.user.account){%>\
					 <a href="/p/t/<%=talk[i].id%>" class="zfNum" target="_blank">' + _('查看转播和点评') + '(<b class="relayNum"><%=numAll%></b>)</a><%}else{%>\
					 <em class="zfNum">' + _('查看转播和点评') + '(<b class="relayNum"><%=numAll%></b>)</em><%}}%><%if('+V1+' && '+V1+'.content && '+V1+'.content.length && talk[i].type==4){%>\
					<a class="view" href="/p/r/<%=talk[i].id%>">' + _('查看对话') + '</a><%}%><%if(MI.user.fun.fastReport == 1 && (talk[i].type == 2 || talk[i].type == 7) && talk[i].name != MI.user.account && '+V1+'.name == MI.user.account){%> <a'+V5+' class="alarm">' + _('删除') + '</a><%}else if(MI.user.fun.report && talk[i].name != MI.user.account){%> <a'+V5+' class="alarm">' + _('举报') + '</a><%}%></span><div class="funBox">\
						<%if(MI.user.account){%><a'+V5+' class="relay" num="<%=numRelay%>">' + _('转播') + '</a><span>|</span><a href="/p/t/<%=talk[i].id%>" class="comt" num="<%=numComt%>">' + _('点评') + '</a><span>|</span><div class="share">\
								<a href="#">' + _('更多') + '<em class="btn_ldrop"></em></a><div class="shareDrop"><b></b><b class="mask"></b>\
									...
									<div class="shareBtn"><p><a href="#">邮件分享</a></p></div>\
									<p><a href="#" class="alarm">举报</a></p>\
								</div>\
							</div><%}%>\
						<%if(0){%><%if(talk[i].count){%><%if(MI.user.account){%>\
							<span>|</span><a href="/p/t/<%=talk[i].id%>" class="zfNum zfNumShow" target="_blank">' + _('点评') + '<b class="relayNum">(<%=talk[i].count%>)</b></a>\
						<%}else{%>\
							<em class="zfNum zfNumShow">' + _('点评') + '<b class="relayNum">(<%=talk[i].count%>)</b></em>\
						<%}}else if('+V1+' && '+V1+'.count){if(MI.user.account){%>\
							<span>|</span><a href="/p/t/<%='+V1+'.id%>" class="zfNum zfNumShow" target="_blank">' + _('点评') + '<b class="relayNum">(<%='+V1+'.count%>)</b></a>\
						<%}else{%>\
							<em class="zfNum zfNumShow">' + _('点评') + '<b class="relayNum">(<%='+V1+'.count%>)</b></em>\
						<%}}else{if(MI.user.account){%>\
							<span>|</span><a href="/p/t/<%=talk[i].id%>" class="zfNum" target="_blank">' + _('点评') + '<b class="relayNum"></b></a>\
						<%}else{%>\
							<em class="zfNum">' + _('点评') + '<b class="relayNum"></b></em>\
						<%}}%><%}%><%if(0 && MI.user.account){%><span>|</span><a href="/p/t/<%=talk[i].id%>" class="comt" target="_blank">' + _('点评') + '</a><%}%>\
					</div>\
				</div>\
...
MI.TalkList.prototype.shareMail = function(id){ // 分享到邮件
	var title = '腾讯微博分享';
	var reply = $$($(id),'.replyBox .msgCnt')[0];
	var str = [];
	str[0] = '嗨，你好！\n\x20\x20\x20我是';
	str[1] = MI.user.name;
	str[2] = '，最近我在玩腾讯微博，看到这条有意思的广播，想要分享给你，没准儿你也会喜欢呢！常联系，关心常在温馨常在~\n\n\x20\x20\x20';
	str[3] = UI.text($$($(id),'.msgBox .msgCnt')[0]);
	str[4] = '\n\x20\x20\x20http://t.qq.com/p/t/'+id;
	if(!!reply){str[3]= UI.text(reply);}
	var body = str.join('');
	UI.ajax({
...
MI.TalkList.prototype.share = function(btn,id){ // 分享
	var Self = this,
		sameBtn,
		displayClassName = 'shareDis';
	/*if (!Self._share) {
		Self._share = UI.html('<div class="shareDrop"><b></b><b class="mask"></b><p><a href="#">' + _('分享到邮件') + '</a></p><p><a href="#">' + _('分享到QZone') + '</a></p>' + (MI.user.flag.auth ? '<p><a href="#">' + _('复制链接分享') + '</a></p>' : '') + '</div>')[0];
		var a = $$(Self._share,'a'),
			mail = a[0],
			qzone = a[1],
			copy = a[2];
		if (mail){
			mail.onclick = function(){
				Self.shareMail(UI.A(Self._share,'rel'));
				return false;
			}
		}
		if (qzone){
			qzone.onclick = function(){
				Self.shareQzone(UI.A(Self._share,'rel'));
				return false;
			}
		}
		if (copy){
			copy.onclick = function(){
				Self.shareCopy(UI.A(Self._share,'rel'));
				return false;
			}
		}
		UI.EA(document.body,'click',function(){
			UI.removeClass(Self._shareBtn,'shareDis');
		});
	}
	if (Self._shareBtn){
		UI.removeClass(Self._shareBtn,'shareDis');
	...
	Self._shareBtn = btn;
	UI.A(Self._share,'rel',id);
	UI.append(Self._share,btn);
	UI.toggleClass(btn,'shareDis');*/
	if (!Self._shareBtn){
		UI.EA(document.body,'click',function(){
	...
	Self._shareBtn = btn;
	UI.toggleClass(btn,displayClassName);
	var a = $$(btn,'.shareBtn a'),
		mail = a[0],
		qzone = a[1],
		copy = a[2];
	if (mail){
...
MI.Card = function(){ //Info Card    资料卡
	...
	Self._body.onmouseout = function(){
		//鼠标离开时隐藏,右侧资料卡由于small的加载图标位置和最终资料卡的位置不一致，导致加载结束时多触发一次mouseout事件
		//MI.Card.hide();
		Self.hide();   
	}
	Self._body.onmouseover = function(){
...
MI.Card.prototype = {
	...
	getPos : function(el){
		...
		if(this.cType == '2') {
			...
			var eHeight = UI.height(el); 	//目标元素的高度
	...
	show : function(el){
		...
		function show(o){
			...
			//o.uIntro = _('简介：俄爱生活、喜欢幽默、喜欢和平凡普通…喜欢和平凡普通喜欢和平凡普和平凡普通喜欢和平凡普通通');    //伪代码，要删除
			...
			//简介
			if (o.uIntro) {
				if(MI.string.length(o.uIntro) > 72) {
					Self._uIntro.title = o.uIntro;
				    o.uIntro = MI.string.cut(o.uIntro,72,'...');
				}
				else {
					Self._uIntro.removeAttribute('title');
				}
				Self._uIntro.innerHTML = _(o.uIntro);
			...
			if (o.company) {
				...
				Self._company.innerHTML =  _('公司：') + '<a href="' + o.company.href + '">' + comName + '</a>';
			...
			if (o.school) {
				var schoolName = o.school.name || '';
				var schoolHref = o.school.href || '';
				var objectName = o.school.objectName || '';
				...
				if(objectHref) {
					Self._school.innerHTML =  _('学校：') + '<a href="' + schoolHref + '">' + schoolName + '</a> <a href="' + objectHref + '">' + objectName + '</a>';
				}
				else {
					Self._school.innerHTML =  _('学校：') + '<a href="' + schoolHref + '">' + schoolName + '</a> <span>' + objectName + '</span>';
				}
...
MI.Card.type = {
	'default' : 1,
	talkListAvatar : 1,   //talkList 头像
	talkListAccount : 2,  //talkList @账户链接
	star : 3,             //推荐收听
	recommend : 4         //可能感兴趣的人
} 
MI.Card.build = function(el,className,cType/*clickHead*/){
	cType = cType || 'default';
	var cTypeValue = MI.Card.type[cType];	
	UI.each($$(el,className),function(o){
		if (!UI.A(o,'card')) {
			UI.A(o,'card','1');
			UI.A(o,'ctype',cTypeValue);
			var P = cTypeValue === 2 ? o : o.parentNode;
			if(cTypeValue == 3) {
				//推荐收听，暂时这样处理，如果改成现在资料卡显示的话，建议html做调整，和其他资料卡保持一致
				UI.A(P,'rel',UI.next(P).title);
			}
			else {	
				UI.A(P,'rel',P.title);
			}
