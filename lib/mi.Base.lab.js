					...
					<a class="view" href="/p/r/<%=talk[i].id%>">' + _('查看对话') + '</a><%}%><%if(MI.user.fun.fastReport == 1 && (talk[i].type == 2 || talk[i].type == 7) && talk[i].name != MI.user.account && '+V1+'.name == MI.user.account){%> <a'+V5+' class="alarm">' + _('删除') + '</a><%}%></span><div class="funBox">\
						<%if(MI.user.account){%><a'+V5+' class="relay" num="<%=numRelay%>">' + _('转播') + '</a><span>|</span><%\
							if(MI.user.fun.btnStyle){\
								...
								<div class="shareBtn"><p><a href="#">' + _('写邮件') + '</a></p></div>\
...
MI.TalkList.prototype.shareMail = (function(){
		...
		function sendMail(){
			...
			UI.ajax({
				...
				data:{mlist:contact,subject:title,body:msgId},  //只发20个联系人
				success:function(data){
					MI.dialog.hideLoad();
					data = MI.json(data);
					if(data.result == 0){
		...
		return function(id){
			MI.dialog.hide();
			MI.dialog.show({title:'<h1 class="DmainTit">' + _('通过QQMail分享给好友') + '</h1>',html:html,width:444,close:1});
			if(!hasCheckedMail) {
				firstCheck();
			};
			msgId = id;
...
MI.tmpl.shareQQ = '<div class="shareToFriend"><div class="shareMenu"><ul class="shareTab"><li class="select"><a href="#">' + _('QQ好友') + '</a></li><li><a href="#">' + _('QQ群') + '</a></li></ul><div class="shareSearch"><input type="text" class="inputTxt"><input type="button" class="btn_search2"></div></div><div style="height:300px" class="shareFList"></div><div style="display:none;height:300px" class="shareFList"><dl><dt><em class="btn_ldrop"></em>' + _('我的好友') + '</dt></dl><dl><dt><em class="btn_ldrop"></em>' + _('我的好友') + '</dt></dl><dl><dt><em class="btn_ldrop"></em>' + _('我的好友') + '</dt></dl></div><div class="btnBox"><span class="error" style="float:left">' + _('') + '</span><button class="btn1">' + _('分享') + '</button><button class="btn1" onclick="MI.dialog.hide();">' + _('取消') + '</button></div></div>';
...
MI.TalkList.prototype.shareQQ = (function(){
	...
	function pullData(){
		MI.dialog.showLoad();
		UI.get('qqlist.php','account=' + MI.user.account + '&r=' + MI.random(),function(data){
			MI.dialog.hideLoad();
			data = MI.json(data);
			if(data.result == 0) {
	...
	function createList(data){
		...
		UI.each(data.friend,function(o,i){
			...
			UI.each(o.member,function(p){
				f.push('<p class="f' + index + '"><label><input type="checkbox" class="check1" id="' + p.qq +'"><img src="' + p.pic + '" />');
				f.push('<span>' + p.nick + '(' + p.qq + ')</span></label></p>');
				sKey[0]['f' + index] = p.nick + p.qq;
				index++;
			});
			f.push('</dd></dl>');
		});
		...
		UI.each(data.group,function(o,i){
			g.push('<dl ' + (i==0 ? 'class="on"' : '') + '><dt><em class="btn_ldrop"></em>' + o.name + '</dt><dd>');
			UI.each(o.member,function(p){
				g.push('<p class="g' + j + '"><label><input type="checkbox" class="check1" id="' + p.qq +'"><img src="' + p.pic + '" />');
				g.push('<span>' + p.nick + '(' + p.qq + ')</span></label></p>');
				sKey[1]['g' + j] = p.nick + p.qq;
				j++;
			});
			g.push('</dd></dl>');
		});
	...
	function checkNum(){
		...
		setError(o.num > n ? _('一次只能分享给') + n +  _('个') + _(['好友','群'][curr]) : '');
	...
	function sendMsg(){
		...
		UI.each(selectedId,function(o,i){
			if(o.num > [3,2][i])  {
				setError(_('一次只能分享给') + [3,2][i] +  _('个') + _(['好友','群'][i]));
				outnumber = true;
			}
		...
		UI.ajax({
			url:'qqShare.php',
			data:{id:postId,friend:conf[0].join(','),group:conf[1].join(',')},  
			success:function(data){
				MI.dialog.hideLoad();
				data = MI.json(data);
				if(data.result == 0){
	...
	return function(id){
		setError('');
		MI.dialog.hide();
		MI.dialog.show({title:'<h3>' +  _('分享给QQ好友') + '</h3>',html:html,width:444,close:1});
	...
	build : function() {
		...
		function set(){
			...
			if (y >= -foot) {
				...
				Self._qqBox.style.cssText += ';position:absolute;top:' + (pageHeight - 37 - foot) + 'px';
			}
		}
	},
