...
MI.Card = function(){ //Info Card    资料卡
	...
	Self._addQQ.onclick = function(){
		if(UI.hasClass(Self._addQQ,'disabled') != true)
		{
			MI.Bos('btnCardQQ');
			var mine = MI.Uin();
			var text = _('发出申请后，会将你的QQ号告知给<b>{0}</b>',Self.name);
			var tem = _('可能是{0}尚未设置允许互听好友向{0}发起QQ好友申请，发条私信把你的QQ告诉<b>{1}</b>?',Self.gender,Self.name);
			var textMessage = _('hi,我想加你为QQ好友,我的QQ是{0},可以加我吗?你可以在隐私设置里勾上允许相互收听的人加你为QQ好友那项，以后就可以在QQ上直接收到申请啦！设置地址是http://t.qq.com/setting/privacy',mine);
			//B设置为不允许的情况
			if(Self.addQQ == 0){
				MI.confirm({
					type : 'error',
					...
					confirmTxt : _('继续'),
					confirm : function(){
						MI.message(Self.account,textMessage);
					},
					cancelTxt : _('取消'),
					cancel :function(){
			...
			//只有用户设置为允许加为QQ好友时，才发送qq号
				...
				MI.confirm({
		              type : 'error',
		              title : _('确定将{0}加为QQ好友?',Self.gender),
		              content : text,
		              confirmTxt : _('确定'),
		              confirm : function(){ //确认的回调函数
						  MI.dialog.hide();
						  MI.addQQ(Self.name,Self.qq,Self.account);
		              }, //end confirm
		              cancelTxt : _('取消'),
		              cancel : function(){ //取消的回调函数
