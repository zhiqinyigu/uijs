...
MIIcoHtml = ['<a href="/certification" target="_blank" class="vip" title="' + _('腾讯认证') + '"></a>','<a href="http://ent.qq.com/zt2010/star2010/fans.htm" class="ico_star" title="' + _('星光达人') + '" target="_blank"></a>','<a href="http://z.t.qq.com/sz2011/index.htm" target="_blank" class="ico_dyh" title="' + _('大运会') + '"></a>'];
/*MIIcoWC = function(id){
					...
					<a class="view" href="http://t.qq.com/p/r/<%=talk[i].id%>">' + _('查看对话') + '</a><%}%><%if(MI.user.fun.fastReport == 1 && (talk[i].type == 2 || talk[i].type == 7) && talk[i].name != MI.user.account && '+V1+'.name == MI.user.account){%> <a'+V5+' class="alarm">' + _('删除') + '</a><%}%></span><div class="funBox">\
						<%if(MI.user.account){\
							...
							if(MI.user.fun.btnStyle==5||MI.user.fun.btnStyle==6){\
								...
								<div class="shareBtn"><p><a href="#">' + _('邮件分享') + '</a></p><p><a href="#">' + _('QQ分享') + '</a></p></div>\
...
MI.TalkList.prototype.trans = (function(){
	...
	transMe.onclick = function(){
		if(!transBox) transBox = new MI.TransMe();
		...
		success : function(data){
			data = MI.json(data);
			 if(data.result==0){
				...
				tips.innerHTML ='<div>' + _('361° 腾讯微博火炬传递参与：{0}人次',num)+'</div>';
				UI.show(tips);
...
MI.TalkList.prototype.urlTips = function(tips){
	if (this.nodeName == 'A') {
		...
		if (!href.hasString('?')){
			...
			UI.A(target,'href',(UI.B.ie ? ' ' : '') + href + '?type=1&from=19&u=' + MI.user.account + '&s=' + timeFrom + '&f=1&skey='+ UI.cookie("skey"));
		}
