...
MI.tmpl.shareMail = '<div class="shareToMail"><div class="mailCnt" style="visibility:hidden"><dl class="clear"><dt>' + _('发件人：') + '</dt><dd><p class="sender"><select></select></p><p class="cNote">(' + _('不想透露您的QQ号码？') + '<a href="https://mail.qq.com" target="_blank">' + _('注册英文邮箱账户') + '</a>)</p></dd><dt>' + _('收件人：') + '</dt><dd><textarea class="inputArea">' + _('一次最多可分享给20个好友') + '</textarea></dd><dt></dt><dd><div class="comPeople" style="display:none"><a href="#">' + _('常用联系人') + '<em class="btn_ldrop"></em></a><div class="mail_listWrap" style="display:none;"><div class="mail_lHead"><a class="close" title="' + _("关闭") + '"></a><table cellspacing="0" cellpadding="0" border="0" class="tHead"><tbody><tr><td class="t1"><input type="checkbox" class="selectAll"></td><td class="t2">' + _('姓名') + '</td><td class="t3">Email地址</td></tr></tbody></table></div> <div class="mailBox" style="height:0"><ul class="mail_lcont"></ul></div> </div></div></dd></dl><div class="DmailCnt clear"><div class="dt">' + _('正文：') + '</div><div class="msgContent"><div class="shareReason" contenteditable="true">' + _('Hi,分享一条腾讯微博给你：') + '</div><div class="msgSource"></div></div></div>\
...
MI.TalkList.shareCollection = (function(){
	...
	UI.each(tabs,function(o,i){
		o.onclick = function(){
			...
			MI.dialog.hideLoad();	
			MI.TalkList.shareCollection[tabArray[i]](currId);  //调用各自的分享接口
	...
	var shareCountObject = {
		...
		function setReason(id){
			...
			if(wrap && $$(wrap,'.replyBox').length>0) {
				...
				reason.value = txt.replace(nickRe,'|| $1');
			}else {
				reason.value = '';
			}	
			count();
		...
		function send(){
			...
			MI.dialog.showLoad();
			UI.ajax({
					url : '/share/shareQzone.php',   //请求广播正文
...
MI.TalkList.shareCollection.shareMail = (function(){
	...
	//	var html = UI.html(MI.tmpl.shareMail)[0],
		...
		function sendMail(){
			...
			var reasonText = reason.innerHTML;
			slideUpConact();
			MI.dialog.showLoad();
			UI.ajax({
		...
		function count(isBlink){
			var txtMax = 5000, 
				...
				check = true,
				length = Math.ceil(len(UI.text(reason))/2 + sourceNum/2);
		...
		UI.EA(msgCntBox,'keyup',function(){count()});
		UI.EA(msgCntBox,'blur',function(){count()});
...
MI.TalkList.shareCollection.shareQQ = (function(){
	//var html = UI.html(MI.tmpl.shareQQ)[0],
	  var html = MI.TalkList.shareCollection.qqBox,
		...
		sValue = ['',''],	//存储搜索关键词
		sDefaultValue = [_('输入QQ好友昵称'),_('输入QQ群名称')], //初始提示 
		sNode = [],			//存储p
...
MI.TalkList.prototype.trans = (function(){
	...
	function showTransResult(o){
		var str = '';
     	for(dict in o.result) {
			str += '<p><span class="cNote">' + dict + '：</span>' + o.result[dict] + '</p>';
		}
	...
	function trans(){
		...
		UI.ajax({
			...
			success:function(data){
				...
				data = MI.json(data);
				if(data.result == 0){
					showTransResult(data.info);
					resultList[transId] = data.info;
					resetPos();
				}
				else {
					MI.alert(data.msg);
	...
	transMe.onclick = function(){
		...
		transBox.show({cont:_('#我来翻译# '),title:_('我来更准确的翻译:'),talkId:transId,type:1});
		MI.Bos('btnTransByMe');
		return false;
	}
		...
		success : function(data){
			data = MI.json(data);
			 if(data.result==0){
				...
				tips.innerHTML ='<div>' + _('361° 腾讯微博梦想传递参与：{0}人次',num)+'</div>';
				UI.show(tips);
...
MI.SmartBox = function(id,callback,type,style){ //input element,callback,search type (keyword:用户输入的关键字提示,布尔值,可以为空）
	...
	UI.EA(this._key,'focus',function(){
		if (Self.tip) {
			UI.hide(Self._tip);
			if(Self._style.history == 1){   //历史搜索和订阅搜索
				Self.getHistory();  
			}
		}
	...
	this._key.onkeydown = function(e){
		...
		if (E.key == 40 || E.key == 38) {
			...
			if(Self._valTip){
				...
				if(Self.index == Self.indexMax) { //搜我的广播在最后一项
					if(Self._list[Self.index].nodeName == 'DT') //如果是订阅和历史搜索，最后一项也是搜全站
						...
						if (data.result == 0) {
							if(data.info){
								...
								if(data.info.length){ //有自动匹配的搜索关键词
									...
									Self.index = -1;
									s.push(Self._valTip ? '<dl><dt class="s">' + _('搜全站：') + '<span index="0" value=\"'+ value +'\">'+value+'</span></dt><dd><ul>' : '<ul>');
									UI.each(data.info,function(o,i){
										s.push('<li value=\"'+ o +'\" index="' + (parseInt(i)+1) + '">' + o + '</li>');
									});
									if(MI.guest){
										var txt = _('搜{0}的广播: ',MI.guest.sex);
										s.push(Self._valTip
															? ('</ul></dd><dt class="s">' + txt +'<span index=\"' + Self.indexMax + '\" value=\"'
																			+ value
																			+ '\">'
																			+ value
																			+ '</span></dt></dl>')
										...
										s.push(Self._valTip? '</ul></dd><dt class="s">' + _('搜我的广播： ') + '<span index=\"' + Self.indexMax + '\" value=\"'+ value +'\">'+value+'</span></dt></dl>' : '</ul>');//s.push('<ul>');								
									if (s.length == 2) {
										return;
									}
								}
								else{ //没有自动匹配的搜索关键词，也出现搜全站和搜我(他)的广播
									Self.indexMax = 1;
									s.push(Self._valTip ? '<dl><dt class="s">' + _('搜全站：') + '<span index="0" value=\"'+ value +'\">'+value+'</span></dt>' : '<ul>');									
									if(MI.guest){
										var txt = _('搜{0}的广播: ',MI.guest.sex);
										s.push(Self._valTip ? ('<dt class="s">' + txt +'<span index="1" value=\"' + value + '\">' + value + '</span></dt></dl>'): '</ul>');
									}
									else
										s.push(Self._valTip ? '<dt class="s">' + _('搜我的广播： ') + '<span index="1" value=\"'+ value +'\">'+value+'</span></dt></dl>' : '</ul>');//s.push('<ul>');								
								}
								Self._select.innerHTML = s.join("");
								Self.show();
								Self._list = $$(Self._select,'li');
								if(Self._list == null) //如果没有自动匹配的关键词列表
									Self._list = []; //新建一个数组
								if(Self._valTip){
...
MI.Reply = function(boxType){ //Reply Dialog
	...
	talkBox.successStart=function(){
		...
		if(boxType=='recom') {
			UI.ajax({url:'/asyn/comm.php',data:{accounts:talkBox.recomAccount}});
		}
	}
