			...
			<div class="msgBox">\
				...
				<%var Imgage=talk[i].image && talk[i].image.length,Videos=talk[i].videos && talk[i].videos.length,Music=talk[i].music && talk[i].music.length,Vote=talk[i].vote && talk[i].vote.length;if(Imgage || Videos || Music || Vote){%><div class="mediaWrap">\
					<%if(Vote || !(Imgage || Videos || Music)){%>\
						<div class="videoBox voteBox" realurl="http://vote.t.qq.com/vote/vote.php?id=<%=talk[i].vote[0].realUrl%>&u=<%=MI.user.account%>&type=<%=talk[i].vote[0].type%>&result=yes&tpl=mini" shorturl="http://url.cn/<%=talk[i].vote[0].shortUrl%>" realtitle="<%=talk[i].vote[0].title%>">\
							<div class="vTools"><a href="#" class="vClose"><em></em>' + _('收起') + '</a><a href="http://vote.t.qq.com/vote/vote.php?id=<%=encodeURI(talk[i].vote[0].realUrl)%>&u=<%=MI.user.account%>&type=<%=talk[i].vote[0].type%>" class="vUrl" target="_blank"><em></em><%if(talk[i].vote[0].title){%><%=MI.string.cut(talk[i].vote[0].title,40)%><%}%></a></div>\
						...
						<div class="msgBox">\
							...
							<%}else{%>\
								...
								<%var Imgage='+V1+'.image && '+V1+'.image.length,Videos='+V1+'.videos && '+V1+'.videos.length,Music='+V1+'.music && '+V1+'.music.length,Vote='+V1+'.vote && '+V1+'.vote.length;if(Imgage || Videos || Music || Vote){%><div class="mediaWrap">\
									<%if(Vote || !(Imgage || Videos || Music)){%>\
										<div class="videoBox voteBox" realurl="http://vote.t.qq.com/vote/vote.php?id=<%='+V1+'.vote[0].realUrl%>&u=<%=MI.user.account%>&type=<%='+V1+'.vote[0].type%>&result=yes&tpl=mini" shorturl="http://url.cn/<%='+V1+'.vote[0].shortUrl%>" realtitle="<%='+V1+'.vote[0].title%>">\
											<div class="vTools"><a href="#" class="vClose"><em></em>' + _('收起') + '</a><a href="http://vote.t.qq.com/vote/vote.php?id=<%=encodeURI('+V1+'.vote[0].realUrl)%>&u=<%=MI.user.account%>&type=<%='+V1+'.vote[0].type%>" class="vUrl" target="_blank"><em></em><%if('+V1+'.vote[0].title){%><%=MI.string.cut('+V1+'.vote[0].title,40)%><%}%></a></div>\
											<div class="vWrap"><a href="#" class="vtThumbs"><img class="crs" crs="http://mat1.gtimg.com/www/mb/images/voteResulte.png" /></a></div>\
										</div>\
									<%}%>\
									<%if(Imgage){%>\
...
MI.pmcompose = function(name, from)
{
	...
	var _sStatusHost = location.protocol + "//rescdn.qqmail.com/pmweb/",
		_sHost = location.protocol + "//pm.t.qq.com/",
		_jsName = { zh_TW : "wbpm.zh_TW.js" }[window.MILang] || "wbpm.zh_CN.js",
		_sJsUrl = [_sStatusHost, "zh_CN/htmledition/js/", _jsName, "?version=110518"].join(""),
		_sCssUrl = _sStatusHost + "zh_CN/htmledition/style/wbpm.css?v=110518",
		_bClose = false;
			...
			MI.ajax({
				...
				success : function(data){
					...
					if (data.result == 0) {
						...
						listWrap.innerHTML = '<div>' + floatListBtn + '</div><div class="ldList"><a href="#" class="ld_btn pageUp" title="' + _('向上滚动') + '" style="display:none"><em></em></a><div class="ldListBox"><ul>' + data.info + '</ul></div><a href="#" class="ld_btn pageDown" title="' + _('向下滚动') + '" style="display:none"><em></em></a></div><p><a href="http://t.qq.com/lists.php">' + _('集市') + '</a>|' + create + '<a href="http://t.qq.com/list_mine.php" onclick="MI.Bos(\'btnListViewAdmin\')">' + _('管理') + '</a></p>';
						if (keyOnly){
							show();
						}
					}
...
MI.black = function(o){
	var btn,
		...
		type = isBlack ? 'un' : '',
		url = isBlack ? MI.url.block : MI.url.unblock,
		txt = [_('拉黑名单'),_('取消拉黑')];
	...
	*/
	...
	if (oMsg && talkBox){
		var idMsg = 'homeMsgTip',
			...
			followAll,
			user = [];
		msg = UI.html('<div class="newsTips1 h1" id="' + idMsg + '"><em class="ico_ntips"></em><span class="left">' + oMsg.content + '</span><a href="#" onclick="UI.hide(\'' + idMsg + '\');UI.ajax({url:\'' + MI.url.updateTips + '\',data:{id:\''+oMsg.id+'\',status:0}});MI.Bos(\'btnMsg_' + oMsg.id + '\');return false" class="close" title="' + _('关闭') + '"></a></div>')[0];
...
');*/
		var id = 'homeBannerTip',
			link,
			close = 'UI.ajax({url:\'' + MI.url.updateTips + '\',data:{id:\''+oBanner.id+'\',status:0}});',
			bannerHtml = [],
		...
		if (target){
			...
			*/
			...
			function closeTips(el){
				var url = el.href;
				UI.ajax({url:MI.url.updateTips,data:{id:oTips.id,status:0}});
...
//多图上传开始
MI.PhotoMultiUpload = {
	html :'<div class="multiPic" style="padding:5px 0"><object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" id="MultiPhoto" width="462" height="215" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab"><param name="movie" value="http://mat1.gtimg.com/www/mb/swf/MultiPhoto.swf" /><param name="wmode" value="transparent" /><param name="quality" value="high" /><param name="allowScriptAccess" value="always" /> <embed src="http://mat1.gtimg.com/www/mb/swf/MultiPhoto.swf" quality="high" width="462" height="215" name="MultiPhoto" align="middle" play="true" loop="false" quality="high" wmode="transparent" allowScriptAccess="always" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer"></embed></object><div class="clear"><p style="text-align:right;padding:5px;" class="btnBox"><button class="btn3 btnSend">' + _('上传') + '</button>&nbsp;<button class="btnCancel btn_cancel">' + _('取消') + '</button></p></div></div>',
	body:null,
	sendBtn:null,
	cancelBtn:null,
	uploadUrl:'',
	...
	uploadCall:function(data){
		...
		data = MI.json(data);
		if(data.result ==0) {
			setTimeout(function(){MI.tip(_('上传成功！'))},0);
			MI.talkBox.addPic({'result':0,'msg':_('上传成功'),info:{'image':data.info.image,'fileName':_('多图') + '.jpg'}});
		} else {
			MI.alert(data.msg);
		}
	},
...
//滤镜开始
MI.photoFilter = {
	html :'<div class="filterPic" style="padding:5px 0"><object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" id="FilterPhoto" width="462" height="250" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab"><param name="movie" value="http://t.qq.com/FilterPhoto.swf" /><param name="wmode" value="transparent" /><param name="quality" value="high" /><param name="allowScriptAccess" value="always" /> <embed src="http://t.qq.com/FilterPhoto.swf" quality="high" width="462" height="215" name="FilterPhoto" align="middle" play="true" loop="false" quality="high" wmode="transparent" allowScriptAccess="always" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer"></embed></object><div class="clear"><p style="text-align:right;padding:5px;" class="btnBox"><span class="error" style="padding-right:20px"></span><button class="btn3 btnSend">' + _('上传') + '</button>&nbsp;<button class="btnCancel btn_cancel">' + _('取消') + '</button></p></div></div>',
	body:null,
	sendBtn:null,
	cancelBtn:null,
	sourceUrl:'',
	...
	show:function(){
		var Self = this;
		MI.dialog.show({
			title:'<h3 class="DmainTit">' + _('图片滤镜') + '</h3>',
			html:Self.body,
			width:462
		});
		MI.dialog.showLoad();
	...
	uploadCall:function(data){
		...
		if(data.result ==0) {
			setTimeout(function(){MI.tip(_('上传成功！'))},0);
			MI.talkBox.addPic({'result':0,'msg':_('上传成功'),info:{'image':data.info.image,'fileName':_('滤镜') + '.jpg'}});
		} else {
			MI.alert(data.msg);
		}
	},
...
MI.tmpl.shareMail = '<div class="shareToMail"><div class="mailCnt" style="visibility:hidden"><dl class="clear"><dt>' + _('发件人：') + '</dt><dd><p class="sender"><select></select></p><p class="cNote">(' + _('不想透露您的QQ号码？') + '<a href="https://mail.qq.com" target="_blank">' + _('注册英文邮箱账户') + '</a>)</p></dd><dt>' + _('收件人：') + '</dt><dd><textarea class="inputArea friendTextArea">' + _('一次最多可分享给20个好友') + '</textarea></dd><dt></dt><dd><div class="comPeople" style="display:none"><a href="#">' + _('常用联系人') + '<em class="btn_ldrop"></em></a><div class="mail_listWrap" style="display:none;"><div class="mail_lHead"><a class="close" title="' + _("关闭") + '"></a><table cellspacing="0" cellpadding="0" border="0" class="tHead"><tbody><tr><td class="t1"><input type="checkbox" class="selectAll"></td><td class="t2">' + _('姓名') + '</td><td class="t3">Email地址</td></tr></tbody></table></div> <div class="mailBox" style="height:0"><ul class="mail_lcont"></ul></div> </div></div></dd></dl><div class="DmailCnt clear"><div class="dt">' + _('正文：') + '</div><div class="msgContent"><textArea class="shareReason">' + _('Hi,分享一条腾讯微博给你：') + '</textArea><div class="msgSource"></div></div></div>\
...
MI.TalkList.shareCollection = (function(){
	var collectionTab = '<div class="DshareWrap"><div class="tabStyle1 shareTitle"><ul><li class="select"><a href="#">' + _('邮箱分享') + '</a></li><li><a href="#">' + _('QQ分享') + '</a></li><li><a href="#">' + _('空间分享') + '</a></li></ul></div>',	
		tmpHtml = collectionTab + MI.tmpl.shareMail + MI.tmpl.shareQQ + MI.tmpl.shareQzone + '</div>',
		...
		tabArray = ['shareMail','shareQQ','shareQzone'],  //和tab对应
		boxArray = [mailBox,qqBox,qZoneBox],  	//和tab对应
		lastIndex = 0,			//最后一次点击的tab
		currId = '';			//当前id

	function init(){
		MI.TalkList.shareCollection[tabArray[lastIndex]](currId);  //初始化时显示邮件共享
		//init = function(){};
	}

	function show(id){
		currId = id;
		MI.dialog.show({title:'<h1 class="DmainTit"> </h1>',html:body,width:548,start:init});
	...
	var shareCountObject = {
		...
		function setReason(id){
			...
			if(wrap && $$(wrap,'.replyBox').length>0) {
				...
				reason.value = (' || ' + UI.A($$(wrap,'.userName a')[0],'rel')  + ': ' +  txt).replace(nickRe,'|| $1');
			}else {
				reason.value = '';
			}	
			count();
...
MI.TalkList.shareCollection.shareMail = (function(){
	...
	//	var html = UI.html(MI.tmpl.shareMail)[0],
		...
		function autoHeight(){
			...
			/*	
				if (height >= maxHeight) {		//取消了最大高度
					height = maxHeight;
					UI.C(el,'overflowY','auto');
				}
				else {
					UI.C(el,'overflowY','hidden');
				}
			*/
		...
		UI.EA(reason,'keyup',autoHeight);
		UI.EA(reason,'blur',autoHeight);
		UI.EA(reason,'focus',count);
		/*if(UI.B.ie9) {	//ie9下退格键删除字符，不触发propertychange事件
			UI.EA(reason,'keyup',function(e){
				if(UI.E(e).key == 8) autoHeight();
			})
		}*/
		...
		UI.EA(textArea2,'keyup',count); //IE和FF中的执行顺序有差异，比较郁闷...
		UI.EA(textArea2,'keydown',count);
		UI.EA(textArea2,'blur',count);

		function count(){
				MI.TalkBox.prototype.countTxt.call(shareCountObject);
				sendBtn.disabled = sendBtn.disable;
		}
		*/
...
MI.tmpl.trans = '<div class="comTips1"><div class="transWrap"><div class="transBox"></div><div class="transNote"><span class="right"><a class="transMe" href="#">'+ _('我来翻译') + '</a></span></div></div><div class="loading"></div></div>';
	...
	function showTransResult(o){
		var str = '';
     	for(i=0;i<o.length;i++) {
			...
			str += o[i].link ? (_('网友') + '<a target="_blank" href="' + o[i].link + '">@' + o[i].name + '</a>') : o[i].name;
			str += _('提供，仅供参考') + '</span></p>';			
		}
...
MI.TalkList.prototype.urlTips = function(tips){
	if (this.nodeName == 'A') {
		var target = this,
			urlTipsTxt = [_('QQ电脑管家提醒您，原链接有风险'),_('QQ电脑管家提醒您，不建议打开原链接'),_('腾讯网'),_('原链接'),_('搜搜')],
			href = target.href,
					...
					function(data){
						...
						if (data.result == 0) {
							if(data.info){
								...
								if(data.info.length){ //有自动匹配的搜索关键词
								...
								else{ //没有自动匹配的搜索关键词，也出现“搜全站”和”搜我(他)的广播“
									Self.indexMax = 1;
								    if (MI.boss == 13 || MI.boss == 7){ 
										if(MI.guest)
											s.push(Self._valTip ? '<dl>' + localHis : '<ul>');
										else
											s.push(Self._valTip ? '<dl>' + localMy : '<ul>');
										s.push(Self._valTip ? total + '</dl>' : '</ul>');
								    }
								    else{
										s.push(Self._valTip ? '<dl>' + total : '<ul>');
										if(MI.guest)
											s.push(Self._valTip ? localHis + '</dl>' : '</ul>');
										else
											s.push(Self._valTip ? localMy + '</dl>' : '</ul>');
								    }
								}
