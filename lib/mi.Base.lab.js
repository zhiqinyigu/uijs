					...
					<a class="view" href="/p/r/<%=talk[i].id%>">' + _('查看对话') + '</a><%}%><%if(MI.user.fun.fastReport == 1 && (talk[i].type == 2 || talk[i].type == 7) && talk[i].name != MI.user.account && '+V1+'.name == MI.user.account){%> <a'+V5+' class="alarm">' + _('删除') + '</a><%}else if(MI.user.fun.report && talk[i].name != MI.user.account){%> <a'+V5+' class="alarm">' + _('举报') + '</a><%}%></span><div class="funBox">\
						<%if(MI.user.account){%><a'+V5+' class="relay" num="<%=numRelay%>">' + _('转播') + '</a><span>|</span><a href="/p/t/<%=talk[i].id%>" class="comt" num="<%=numComt%>">' + _('点评') + '</a><span>|</span><div class="share">\
								<a href="#">' + _('更多') + '<em class="btn_ldrop"></em></a><div class="shareDrop"><b></b><b class="mask"></b>\
									...
									<div class="shareBtn"><p><a href="#">分享到邮件</a></p><p><a href="#">分享到QZone</a></p><p><a href="#">复制链接分享</a></p></div>\
...
MI.TalkList.prototype.shareMail = function(id){ // 分享到邮件
	...
	str[4] = '\nhttp://t.qq.com/p/t/'+id;
	if(!!reply){str[3]= UI.text(reply);}
	var body = str.join('');
	window.location = 'mailto:?subject='+UI.encode(title)+'&body='+UI.encode(body);
	MI.Bos('btnShareMail');
}
...
MI.AutoCmt = function(o){ //Auto Complete
	...
	*/
	...
	Self._body = UI.html('<div class="atWrap" style="display:none">\
		...
		<div class="tips" style="display:none">@朋友帐号,他就能在[提到我的]页收到</div>\
	</div>')[0];
...
MI.AutoCmt.prototype = {
	...
	getData : function(call){ //获取数据
		...
		if (!MI[usersStorageKey] && !Self.sending && (!users || (new Date().getTime() - MI.S('time') > 7200000) || !newData(users))){ //3600000
			Self.sending = 1;
			var Time,Time_1,Time_2,Time_3;
			Time = + new Date();
	...
	show : function(o, py, model){
		...
		//if (this.word) {
			var match = [],
				...
				word = model ? UI.trim(Self._input.value) : (this.key ? this.word.slice(1) : this.word),
