	...
	*/
	...
	Self.uploadCollection = function(o){
		...
		up.setArrPos = function(btn){
			...
			if(MI.Nano){
				x = up.arrPos || (UI.getX(btn) + UI.width(btn)/2 - UI.getX(up._box.parentNode)-17/2);			
				...
				success : function(data){
					...
					if((data.result == 0) && data.info.s) {
						...
						UI.each(UI.GT(tpObj._relTopicList,'a'),function(a){
							a.onclick = function(){
								var topic = a.innerHTML,added = false;
	...
	if (Self._music) {
		...
		window.getRecDuration = function() { // [总时间,警告开始时间,倒数开始时间]
			var result = [180,120,170,15]; //3分钟, 上传类型：私信语音微博 from=16  主站语音微博from=15
	...
	if (Self._pic) {
		...
		*/
		var imageUploadName = 'imageUpload_' + MI.random(),
			action = MI.Nano ? ("http://t.qq.com" +MI.url.uploadPic):MI.url.uploadPic,
			...
			picHtml = '<div class="picDiv singlePic">\
				...
				<div class="picDiv hideDcnt searchPic" style="height:208px">\
				</div>';
...
MI.SearchPic = {
		...
		var Self = this;
			if(data && data.errCode == 0){
				...
				if (!Self._rSearchPage){ // 分页
					...
					Self._rSearchPage.show(0, Self._pageNow, false);
	...
	searchPage : function (data) {
		...
		if(Self._pageNow==999) 
			$$(Self._rSearch,'.p_sNum b')[0].innerHTML = Self._pageNow * 16;
...
//视频录制上传开始
MI.swf.VideoRecord = 'http://mat1.gtimg.com/www/mb/swf/VideoRecord_111026b.swf';
MI.VideoRecord = {
	html :'<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" id="VideoRecord" width="462" height="335" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab"><param name="movie" value="' + MI.swf.VideoRecord + '" /><param name="wmode" value="transparent" /><param name="quality" value="high" /><param name="allowScriptAccess" value="always"/><param value="langVer=' + window.MILang + '" name="FlashVars" /><embed src="' + MI.swf.VideoRecord + '" quality="high" width="462" height="335" name="VideoRecord" align="middle" play="true" loop="false" quality="high" wmode="transparent" allowScriptAccess="always" FlashVars="langVer=' + window.MILang + '" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer"></embed></object>',
	uploadUrl:'',	
	build:function(target, uploadUrl){
		var Self = this;
		if(MI.Flash.getVersion() < 10.0){
			target.innerHTML = MI.Flash.updataTip(9.0);
			return;
		}
		Self.uploadUrl = uploadUrl || 'http://upload.t.qq.com';
		if(Self.builded){
