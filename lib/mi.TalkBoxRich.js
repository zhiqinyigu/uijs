...
MI.TalkBox.prototype.addEvent = function(){ // 事件绑定
	...
	Self._order = Self.$('.newOrder .txt'); //订单
	...
	*/
	...
	Self.uploadCollection = function(o){
		...
		//箭头定位
		up.setArrPos = function(){
			if(up.showed) return;
			var btnX = UI.getX(up._open);
			var x = UI.getX(up._box);
			up._arrows.style.left = (btnX - x) + UI.width(up._open)/2 + 'px';
		}
	...
	if (Self._order) {
		var orderHtml = '<div class="orderBox"><p>' + _('粘贴订单地址') + '</p><p class="clear"><input type="text" class="inputTxt"><button class="btn1">' + _('确定') + '</button></p><p class="cError" style="display:none"></p></div>';
		...
		Self._order.onclick = function(){
			Self.uploadCollection({
					...
					buildEvent : function(o){
						...
						orderTip = [_('你输入的商品链接已提交，请等待'), _('商品地址不能为空，请在输入框粘贴或输入') ,_('你输入的商品地址不是正确的URL格式，请重新输入')];
						...
						o._submit.onclick = function(){
							...
							if(strRegex.test(value)){
								...
								UI.show(o._tip);
								MI.ajax({
									url : MI.url.uploadUrlPic, //拉取地址需要修改
									type : 'get',
									data : "orderlnk=" + UI.utfEncode(value)+'&r='+MI.random(),
									success : function(data){
										data =  MI.json(data);
										if (data.result == 0){
											var txt = _('#拍拍商品# ')  + data.info.name + ' ' + data.info.sortUrl;
											if(!Self._txt.value.hasString(txt)) Self._txt.value += txt;
											o._tip.innerHTML = orderTip[0];
											o.hide();
										}else{
	...
	if (Self._music) {
		...
		var recordHtml = (MI.user.isLab ? '<div class="flashWrap"><object width="404" height="160" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab" id="musicRecord" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000"><param value="' + MI.swf.MusicRecord + '" name="movie"><param value="high" name="quality"><param value="#FFFFFF" name="bgcolor"><param value="langVer=' + window.MILang + '" name="FlashVars" /><param name="wmode" value="transparent" /><param value="always" name="allowScriptAccess"><param value="all" name="allowNetworking"><embed width="404" height="160" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash" wmode="transparent" allownetworking="all" allowscriptaccess="always" loop="false" play="true" name="musicRecord" FlashVars="langVer=' + window.MILang + '" bgcolor="#FFFFFF" quality="high" src="' + MI.swf.MusicRecord + '"></object></div>' : '');
...
MI.SearchPic = {
	...
	build : function(o){
		...
		o._picSearchBtn.onclick = o._picSearchFrom.onsubmit= function(){
			Self.searchPic(o._picSearchKey.value,0);//Self._resetPage
			if(window.__sosostat && window.__sosostat.pr_get) __sosostat.pr_get(document.location.href, 't.1.1.0' , 0,'wb_bq_search');//上报soso
			return false;
		}
	...
	searchPicCallBack : function(data){
		var Self = this;
			if(data && data.errCode == 0){
				...
				UI.hide(Self._sLoading);
				if (Self._rSearch){
					UI.remove(Self._rSearch);
				}
				if(Self._noResult){
					UI.remove(Self._noResult);
				}
				var noResult = UI.html('<div class="p_noResult"><p>' + _('对不起，未找到')+'<b>"'+Self._Key+'"</b>'+_('的相关表情。') + '</p><p>' + _('请检查关键词是否正确，或更换其他关键字搜索。') + '</p></div>')[0];
	...
	searchPage : function (data) {
		...
		if(p_pages && UI.trim(Self._segword) != UI.trim(Self._Key)){//刷新页码
			Self._rSearchPage.show(0, Math.ceil(data.general.allnum / Self.PageNum), false);
			Self._segword = UI.trim(Self._Key);
		}
		UI.prepend(frag, Self._rSearch);
		Self.m_total = $$(Self._rSearch, '.p_sNum')[0];
		if(Self.m_total){
			Self.cSign = $$(Self.m_total, '.cSign')[0];
			Self.cSign && (Self.cSign.innerHTML = UI.trim(Self._Key));
		}
	...
	bindEvent : function(o){
		...
		p_sList.onclick = function(e){
			...
			if(E.target.nodeName == 'IMG'){
				...
				if(window.__sosostat && window.__sosostat.pr_get) __sosostat.pr_get(document.location.href, 't.2.2.'+ E.target.id, 0, 'wb_bq_search');//上报soso
			}
			E.stop();
			return false;
		}
...
MI.PhotoCameraUpload = {
	html :'<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" id="CameraPhoto" width="462" height="335" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab"><param name="movie" value="' + MI.swf.CameraPhoto + '" /><param name="wmode" value="transparent" /><param name="quality" value="high" /><param name="allowScriptAccess" value="always"/><param value="langVer=' + window.MILang + '" name="FlashVars" /><embed src="' + MI.swf.CameraPhoto + '" quality="high" width="462" height="335" name="CameraPhoto" align="middle" play="true" loop="false" quality="high" wmode="transparent" allowScriptAccess="always" FlashVars="langVer=' + window.MILang + '" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer"></embed></object>',
	sendBtn:null,
	cancelBtn:null,
	uploadUrl:'',
	build:function(target,uploadUrl){		//外部调用build方法
...
MI.Text2Pic = {
	html :'<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" id="Text2Pic" width="462" height="10" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab"><param name="movie" value="' + MI.swf.Text2Pic + '" /><param name="wmode" value="transparent" /><param name="quality" value="high" /><param name="align" value="middle" /><param name="allowScriptAccess" value="always" /><param value="langVer=' + window.MILang + '" name="FlashVars" /> <embed src="' + MI.swf.Text2Pic + '" quality="high" width="462" height="10"  name="Text2Pic" align="middle" play="true" loop="false" quality="high" wmode="transparent" allowScriptAccess="always" FlashVars="langVer=' + window.MILang + '" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer"></embed></object>',
	theme : 0,
...
MI.photoFilter = {
	...
	flashHtml:'<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" id="FilterPhoto" width="462" height="250" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab"><param name="movie" value="' + MI.swf.FilterPhoto + '" /><param name="wmode" value="transparent" /><param name="quality" value="high" /><param name="align" value="middle" /><param name="allowScriptAccess" value="always" /><param value="langVer=' + window.MILang + '" name="FlashVars" /> <embed src="' + MI.swf.FilterPhoto + '" quality="high" width="462" height="215" name="FilterPhoto" align="middle" play="true" loop="false" quality="high" wmode="transparent" allowScriptAccess="always" FlashVars="langVer=' + window.MILang + '" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer"></embed></object>',
	body:null,
	sendBtn:null,
	cancelBtn:null,
	sourceUrl:'',
