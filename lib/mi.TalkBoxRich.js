			...
			*/
			...
			if (MI.code.check(data.result)) { //Verify code
				...
				if (data.info){		
					...
					if (MI.user.fun.gotoNanoUserProfile) {
						Self.goUser('http://1.t.qq.com/' + data.info);
					}
					else {
	...
	*/
	...
	if (Self._music) {
		...
		window.getRecDuration = function() { // [总时间,警告开始时间,倒数开始时间,来源]
			var result = [60, 40, 50, 15]; // 3分钟, 上传类型：微群语音微博 from=16  私信语音微博 from=16  主站语音微博from=15 主站无权限限制语音微博from=18 
...
MI.SendGift = function(opt){
	...
	this.friendTitle = opt.friendTitle;							//亲密朋友wording
...
MI.SendGift.prototype = {
	...
	init : function(){
		...
		Self.domTitle = '<ul class="pr_rep_tit va_tab_re">';
		UI.each(Self.title,function(o,i){
			Self.domTitle += '<li class="vaTab' + (i+1) + '"><a href="#">' + o + '</a></li>';	
		});
		Self.domTitle += '</ul>';	
	/*		
		Self.domTitle = '<ul class="pr_rep_tit va_tab_re"><li class="vaTab1 on"><a href="#@">' + Self.title[0] + '</a></li><li><a href="#@">' + Self.title[1] + '</a></li><li><a href="#@">' + Self.title[2] + '</a></li><li style="' + ((window.MILang != 'en_US' && MI.user.fun.audioRecord) ? '' : 'display:none') + '"><a href="#@">' + Self.title[3] + '</a></li></ul>';
	*/
	
		Self.myFriend = UI.html('<div class="pr_friend_pn"><p class="pr_friend_hd">' + Self.friendTitle + '</p><div class="pr_friend_bd"><div  class="loading">' + _('数据加载中') +'</div></div></div>')[0];	
...
*/
		UI.css('.anony_pn th {display:none;}.anony_pn .sendList{display:none !important;}.anony_pn .txtWrap {display:none;}');

		Self.div[1] = UI.html('<div class="art_pn sendCnt anony_pn" style="display:none;"><p class="anonyFont">' + _('爱TA在心口难开？树洞帮你来告白！') + '</p><p class="anonyInfo">（' + _('树洞会把你的告白广播出去，勇敢@她/他，没人知道是你发的哦~详见') + '<a target="_blank" href="http://t.qq.com/k/%E6%A0%91%E6%B4%9E%E5%91%8A%E7%99%BD">#' + _('树洞告白') + '#</a>）</p>' + MI.tmpl.msgBox.replace('uploadPic','') + '</div>')[0];
		UI.after(Self.div[1],Self.div[0]);
		UI.after(Self.myFriend,Self.div[1]);	
		
		
		//私信
		Self.msgBox = new MI.MsgBox(Self.div[1]);
		Self.msgBox.noRelTopic = 1;
		Self.msgBox.topic = Self.topic[1];
		Self.msgBox._msgTo.value = 'weiboshudong';
		Self.msgBox._btn.value = _('扔进树洞');
		Self.msgBox.txtMax = 140 - MI.countTxt(Self.msgBox.topic);
		Self.msgBox.success = function(data){
			Self.publishSuccess(data,1);
		};
	...
	build : function(){
		...
		//TalkBox
		Self.talk = Self.bindTalkEvent($$(Self.body,'.chrisFrm')[0],0,1);
	...
	selectHandler : function(){
		...
		Self.select.pullData(function(data){
			...
			UI.each(data,function(o,i){
				if(index < 13) {
					...
					Self.myFriendArray[o.name] = index;
				}
				index++;	
			});
		...
		UI.EA(Self.myFriend,'click',function(e){
			...
			if(Self.select.selected.num > Self.select.max) {
				Self.select.show();	
			} else {
				Self.select.hide();
			}
		...
		//根据选择输入框的值，改变亲密朋友的选中状态
		Self.select.refreshDataEvent = function(data){
			if(!Self.myFriend) return;
			UI.each(Self.myFriendArray,function(i,name){
			//	if(name in data) {
				if(UI.hasKey(data,name)) {
			...
			UI.each(data,function(o,i) {
				var name = o.name;
			//	if(!(name in Self.myFriendArray)) {
				if(!UI.hasKey(Self.myFriendArray,name)) {
					Self.changeTalkValue(name,1);	
				}
			});
		}
	...
	changeSelectData : function(talk){
		...
		UI.each(Self.select.selected.data,function(o){
			var id = o.name;
			if(!UI.has(atArr,'@' + id)) {
		...
		UI.each(Self.select.unSelected.data,function(o){
			var id = o.name;
			if(UI.has(atArr,'@' + id)){
	...
	bindTalkEvent : function(el,index,noFlash){
		...
		talk.success = function(data){
			MI.tip(_('发送成功'));
			Self.publishSuccess(data,index);
		};
		//覆盖send方法
	...
	changeTab : function(index){
		...
		UI.each(Self.tab,function(o,i){
			if(index == i) {
				UI.show(Self.div[i]);
				o.className = o.className.replace(/on$/,'') + 'on';
				//UI.addClass(o,'on');
			} else {
				UI.hide(Self.div[i]);
				//UI.removeClass(o,'on');
			...
			if(index == 1) { //私信拜年不显示亲密朋友
				UI.hide(Self.myFriend);
				Self.msgBox.countTxt();
			} else {
	...
	*/		
	...
	//发送成功
	publishSuccess : function(data,index){
		var Self = this;
		Self.publishBos && MI.Bos(Self.publishBos,index,(data.info && data.info.talk && data.info.talk[0] && data.info.talk[0].id));
	},
	//切换礼物
	changeGift : function(i){
		...
		var defaultValue = '';
		var value = Self.sendWord.value;
		UI.each(Self.giftConfig,function(o,index){
			if(value.hasString(o.word)){
				defaultValue = o.word;
			}
		});
		if(defaultValue) {
			Self.sendWord.value = value.replace(defaultValue,Self.giftConfig[i].word);
		} else if(!UI.trim(value)) {
			Self.sendWord.value = Self.giftConfig[i].word;
		}	
	...
	//	Self.talkArray[3].countTxt();
		...
		//私信拜年
	/*	if(Self.div[1]) { //每次打开都执行
			MI.pmembedcompose({
				type : "compose",
				to : 'weiboshudong',
				from : null,
				dom : $$(Self.div[1],'.msgSpring')[0],
				content : Self.wordingList[1] || '',
				'onload' : function(){
					MI.dialog.hideLoad();
					var sendBtn = $$(Self.div[1],'.btn_send')[0];
					sendBtn.innerHTML = _('扔进树洞');
				},
				onsendcomplete : function(data){
					alert(data);
					MI.tip(_('发送成功'));
				}
			});	
...
/**
 * 打开现有送礼浮层
 * @param {Integer} eventId (eventId : 1-圣诞，2-元旦，3-情人节，4-说爱)
...
MI.sendGiftShow = function(eventId,tabIndex){
	//if(eventId == 4) {
		MI.springFestivalGift =  MI.springFestivalGift || new MI.SendGift({
			...
			btnValue : _('祝福'),
			title : [_('爱的礼物'),_('密语告白')],
			bodyClass : 'valentineWrap',
			placeHolder : _('输入好友姓名，最多可添加5个好友'),
			friendTitle : _('情人节，不要忘记给好友也送上祝福哦'),
		//	recordTabName : _('语音拜年'),
			publishBos : 'btnBlessPublished',
			wordingList : ['',''],
			giftConfig : [
					{
						name : _('爱心'),
						pic : 'http://mat1.gtimg.com/www/mb/images/theme/chrisTip/xinxiangyin.png',
						word : _('我数遍漫天繁星，最明亮的是你的眼，我看遍世间芳华，最灿烂的是你的脸，我踏遍山野清涧，最甘醇的是你的心，我寻遍所有的宝藏，最珍贵的，是我们的爱。')
					},
					{
						name : _('玫瑰'),
						pic : 'http://mat1.gtimg.com/www/mb/images/theme/chrisTip/meigui.png',
						word : _('我一直在寻找着，生命中那朵玫瑰，终于让我遇见你，值得用一生追随。')
					},
					{
						name : _('巧克力'),
						pic : 'http://mat1.gtimg.com/www/mb/images/theme/chrisTip/qiaokeli.png',
						word : _('你是我的巧克力情人，我是你最甜蜜的缘分，只要轻轻咬下一口，便能体会那种丝般的柔情。')
					},	
					{
						name : _('钻戒'),
						pic : 'http://mat1.gtimg.com/www/mb/images/theme/chrisTip/zhuanshi.png',
						word : _('无名指上的钻戒像阳光，陪我渡过风和霜，对爱情我很平凡，只想做你的另一半。')
					},
					{
						name : _('汽车'),
						pic : 'http://mat1.gtimg.com/www/mb/images/theme/chrisTip/che.png',
						word : _('如果这是末日来临前的最后一个情人节，趁我还年轻，可以毫无保留的为爱付出；趁我还无知，可以完完全全的奉献自己；趁我还天真，可以相信爱你是一辈子的事；请你爱我的青春，爱我的痴狂，爱我的执着，完成我“不程式的爱情”——Jeep指南者')
					}
				]
		});
