		...
		var html = '<div class="DshareWrap">\
			...
			function toggleWording(){
				...
				if(total>0 && oldMoods.length>0) {
					...
					_lastMCont.innerHTML = '<a href="#" title="' + cnt.text + '">' + cnt.cont + '<span class="time">' + fmtTime(o.timestamp) + '</span></a>'; 
					UI.show(_lastMLabel);
					UI.show(_lastMCont);
			...
			function createList(start,end){
				if(!currData.talk.length) return;
				var str = '<ul>',o,len,time,ft = MI.Mood.ft,url,cnt;
				for(i=start;i<end;i++){
					o = currData.talk[i];
					if(!o) continue;
			...
			function fmtCont(str,len){
				...
				var cnt = tmpTxt.replace(/<(a|em|span)[^<>]+?>|<\/(a|em|span)>/gi,'');
