		...
		var html = '<div class="DshareWrap">\
			...
			function bindEvent(){
				...
				MI.Mood.listDrop = new MI.NewListDrop({
						url : MI.url.moodListSingleData || '/asyn/getList.php',	//拉取列表	
			...
			function getMoodListData(listId,type,listName){
				if(!MI.Mood.listData[listId]) {
					MI.dialog.showLoad();
					MI.ajax({
						url : MI.url.MoodList || '/asyn/userEmotionStatistics.php',   
...
MI.NewListDrop.prototype = {
	...
	setPos : function(){
		...
		Self._body.style.left = UI.getX(Self._tab.parentNode) + 1 + 'px';
