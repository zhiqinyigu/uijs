...
MI.List = {
	...
	build : function(){
		...
		Self._form = UI.html('<form action="' + Self.url.add + '" method="post" id="listForm" onsubmit="return false" style="display:none"><div class="tbAddList"><table border="0" cellspacing="0" cellpadding="0">\
			...
			<tr><th>' + _('浏览权限：') + '</th><td class="viewRight"><label for="priv"><input type="radio" checked name="priv" id="priv" value="0" class="check1"><b></b>' + _('所有人可见') + '</label><label for="privB"><input type="radio" name="priv" id="privB" value="1" class="check1"><b></b>' + _('只有我自己可见') + '</label></td></tr>\
	...
	listFollow : function(id){ //收听成功，加入名单
		...
		UI.get(Self.url.list,'position=follow&account=' + id + '&r=' + MI.random(),function(data){
			...
			if (data.result == 0){
				...
				MI.dialog.show({
					title : '<h1 class="DmainTit">' + _('收听 {0} 成功',data.nick || id) + '</h1>',
					width : 420,
					start : function(){
					...
					closeEnd : function(){		
						...
						if (MI.isS) {
							var closeCnt = parseInt(MI.S('option_iUserCloseCnt_' + MI.user.account));
							if (closeCnt != NaN && closeCnt == 10) {
								MI.S('option_iUserCloseCnt_' + MI.user.account, 11);
								MI.confirm({
									title: _('以后不再提示设置备注和名单分组？'),
									confirm: function(){
										UI.ajax({
											url : '/asyn/others_save.php',
											type : 'post',
											data : {ft : 0}
										});
										MI.dialog.hide();
					...
					html : '<div class="foedSets" id="foedSets"><div class="foset">[<a href="/setting_others.php#co" target="_blank" onclick="MI.Bos(\'btnListFollowSet\')">' + _('收听设置') + '</a>]</div><table border="0" cellspacing="0" cellpadding="0"><tbody>\
						...
						<tr><th></th><td class="funBox"><button class="btn1" onclick="MI.List.listFollowSave(\'' + id + '\')">' + _('完成') + '</button></td></tr>\
					</tbody></table></div>'
