...
MI.List = {
	...
	build : function(){
		...
		Self._form = UI.html('<form action="' + Self.url.add + '" method="post" id="listForm" onsubmit="return false" style="display:none"><div class="tbAddList"><table border="0" cellspacing="0" cellpadding="0">\
			...
			<tr><th>' + _('名单权限：') + '</th><td class="viewRight"><p><label for="priv"><input type="radio" checked name="priv" id="priv" value="0" class="check1"><b></b>' + _('公开<span class="cNote">（所有人可见，可订阅）</span>') + '</label></p><p><label for="privB"><input type="radio" name="priv" id="privB" value="1" class="check1"><b></b>' + _('隐私<span class="cNote">（只有我可见，不通知被收录者）</span>') + '</label></p></td></tr>\
	...
	listJoin : function(id,account,call,name){
		var Self = this;
		MI.ajax({
			url : Self.url.join,
		...
		if (Self.listCache[Self.listAccount] && Self.listCache[Self.listAccount].info[id]) {
			Self.listCache[Self.listAccount].info[id].inlist = true;
		}
		else if(name){
			Self.listCache[Self.listAccount].info[id] = {inlist:true,locked:1,name:name,userNum:"1"};
		}
		MI.Bos('btnListJoin');
	},
	...
	listShow : function(data){
		...
		for (var i in data.info) {
			...
			listName[data.info[i].name]=1;
			index++;
		}
		//加入默认的名单列表
		j=index;
		if(index <= 10){
			UI.each(Self.defaultList,function(o,i){
				//判断defaultList中是否和已有名单重复
				if(!listName[i]){
					listName[i] = 1;
					html.push('<li><label for="' + j + '" title="' + i + '"><input type="checkbox"  name="inputlist" id="' + j + '" class="check1 noid" value="' + j + '">' + MI.string.cut(i,14) + '<em class="ico_slock"></em></label></li>');
					j++;
				}				
			});
			index = j;
		...
		UI.each($$(Self._listCont,'input'),function(o){
			o.onclick = function(){
				...
				if (this.checked) {
					//如果是第一次点击默认名单，首先创建名单id,然后加入名单
					if(UI.hasClass(T,'noid')){
						MI.ajax({
							url : Self.url.add,
							data : {name:P.title,priv:1},
							success : function(data){
								data_create = MI.json(data);
								if (data_create.result == 0) {
									T.value = data_create.createId;
									//T.id = 'list_' + data_create.createId;
									id = T.value;
									UI.removeClass(T,'noid');
									Self.listJoin(id,account,callBack,P.title);
								}
							}
						});
					}
	...
	_memberForm : '<div class="pop_chs_wrap">\
		<h3 class="pop_chs_top">' + _('选择要添加到该名单的用户') + '</h3>\
		<div class="gb_chsuser" id="pop_chs_inner"></div>\
		<div class="pop_cont_bottom">\
			<a class="gb_btn gb_btn2" href="#"><span class="t">完成</span></a>\
			<a class="gb_btn gb_btn6" href="#"><span class="t">取消</span></a>\
		</div>\
	</div>',
	member : function(id,name,descrip,priv,tags){ //批量设置成员
		var Self = this;
		
		MI.dialog.show({
			title : '<h1 class="DmainTit">' + _('名单') + '-' + name + '</h1>',
			html : Self._memberForm,
			width : 500
		});
		
		MI.app({
			ItemSelect : function(){
				new MI.ItemSelectMember({
					target : $('pop_chs_inner')
				});
			}
		});
	},
...
MI.List.member("7769713123937528","b");