...
MI.List = {
	...
	build : function(){
		...
		Self._form = UI.html('<form action="' + Self.url.add + '" method="post" id="listForm" onsubmit="return false" style="display:none"><div class="tbAddList"><table border="0" cellspacing="0" cellpadding="0">\
			<tbody><tr><th>名称：</th><td><input type="text" name="name" id="name" class="inputTxt"><p class="cNote">1-13个中文、字母、数字、下划线或减号</p></td></tr>\
			<tr><th>描述：</th><td><textarea class="inputArea" name="descrip" id="descrip"></textarea><p class="cNote">100字以内</p></td></tr>\
			<tr><th>标签：</th><td class="tb_tag"><input type="text" class="inputTxt" name="listTags" id="listTags"><p class="cNote">每个标签8个字以内，以空格间隔，最多填写10个</p></td></tr>\
					...
					inputs : {
						name : {
							rule : function(str){
								...
								if (str.match(/[^\u4e00-\u9fa5\w-\s]/g)) return '仅支持中文、字母、数字、下划线或减号';
								if (length < 1 || length > 13) return '仅支持1-13个中文、字母、数字、下划线或减号';
							},
							noIco : true
						},
						descrip : {
						...
						listTags : {
							rule : function(str){
								str = UI.trim(str);
								var length = str.length,w=str.split('');
								if (str.match(/[^\u4e00-\u9fa5\w\s]/g)) return '仅支持中文、字母、数字';
							},
							noIco : true
						},
						lid : {
	...
	listFollowCss : 0,
	listFollow : function(id){ //收听成功，加入名单
		var Self = this;
		if (!Self.listFollowCss){
			UI.css('.fotoList{width:310px;padding:10px}.fotoList .foset{width:70px;overflow:hidden;position:relative;top:-34px;float:right}.fotoList label{position:static;padding:0;margin-right:15px;color:#333;white-space:nowrap}.fotoList .btn2{margin:0 5px}.myCreateList{width:310px;padding:5px 0;_margin-right:-15px}.myCreateList label{cursor:default;display:block;float:left;width:140px;overflow:hidden}.fotoList .createList{margin-bottom:5px;padding:6px 15px 8px;border:1px solid #F4E4B4;background:#FFFFE9}.fotoList .createList p{padding-top:5px}.fotoList .btnBox{padding:5px}.fotoList .btnBox .btn2{margin:0 20px 0 120px}');
			Self.listFollowCss = 1;
		}
		UI.get(Self.url.list,'account=' + id + '&r=' + MI.random(),function(data){
			data = MI.json(data);
			if (data.result == 0){
				var html = [],
					index = 0;
				for (var i in data.info){
					html.push('<label for="list_' + i + '" title="' + data.info[i].name + '"><input type="checkbox" id="list_' + i + '" class="check1" value="' + i + '"' + (data.info[i].inlist ? ' checked inlist="1"' : '') + (!data.info[i].inlist && data.info[i].userNum >= Self.listMaxUserNum ? ' disabled title="成员数超过上限"' : '') + '>' + data.info[i].name + (data.info[i].locked ? '<em class="ico_slock"></em>' : '') + '</label>');
					index++;
				}
				MI.dialog.show({
					title : '<h1 class="DmainTit">收听成功，用名单分组</h1>',
					width : 380,
					end : function(){
						MI.Bos('btnListFollowClose');
						UI.remove($('fotoList'));
					},
					html : '<div class="fotoList" id="fotoList"><div class="foset">[<a href="/setting_others.php#co" onclick="MI.Bos(\'btnListFollowSet\')">收听设置</a>]</div><h4>为“' + (data.nick || id) + '”选择名单：</h4>\
	<div class="myCreateList clear">' + html.join('') + '</div>\
	<div class="createlistWrap">\
		' + (index < Self.listMaxNum ? '<a href="#" class="ffsong" onclick="MI.List.listFollowCreate();return false">+创建新名单</a>' : '') + '\
		<div class="createList" style="display:none">\
			<form action="' + Self.url.add + '" method="post" id="listFollowForm" onsubmit="return false">\
				<p><span><input type="text" id="name" name="name" class="inputTxt"></span><button type="submit" class="btn2">创建</button><a href="#" onclick="MI.List.listFollowCreateCancel();return false">取消</a><br' + (UI.B.ie ? ' style="display:none"' : '') + '><span></span></p>\
				<p>浏览权限：<label><input type="radio" class="check1" id="priv" name="priv" value="0" checked>所有人可见</label><label><input type="radio" class="check1" name="priv" value="1">仅自己可见</label><span></span></p>\
			</form>\
		</div>\
	</div>\
	<div class="btnBox"><button class="btn2" onclick="MI.List.listFollowSave(\'' + id + '\')">保存</button><a href="#" onclick="MI.Bos(\'btnListFollowCancel\');MI.dialog.hide();return false">暂不收录</a></div></div>'
				});
