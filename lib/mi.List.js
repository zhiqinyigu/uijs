 ...
 */
MI.List = {
	listMaxNum : 10, //List Max Num
	validate : null, //Validate Object
	xhr : {},
	call : null, //Call Back Function
	addCall : function(){ //Call Back Function
	...
	build : function(){
		var Self = this;
		//Css
		UI.css('.tbAddList{line-height:19px}\
			.tbAddList .inputTxt{ width:170px; border:1px solid #86A1B4;}\
			.tbAddList .inputNum { width:375px;}\
			.tbAddList .inputArea { width:375px; height:75px;border:1px solid #86A1B4;}\
			.tbAddList{ padding:15px 0 10px;}\
	...
	list : function(el,account){
		...
		*/
		...
		if (!Self._list) {
			//Dom
			Self._list = UI.html('<div class="listFun"><ul></ul><div class="newList"><a href="#">新建List</a></div></div>')[0];
			...
			Self._listNew.onclick = function(){
				...
				Self.call = function(data){
					...
					for (var i in Self.listCache) {
						Self.listCache[i].info[data.createId] = {
	...
	listShow : function(data){
		...
		for (var i in data.info) {
			html.push('<li><label for="list_' + i + '"><input type="checkbox"' + (data.info[i].inlist ? ' checked' : '') + ' name="inputlist" id="list_' + i + '" class="check1" value="' + i + '">' + data.info[i].name + '</label></li>');
			index++;
		}
	...
	add : function(){
		...
		MI.dialog.show({
			title : '<h1 class="DmainTit">新建List</h1>',
			html : Self._form,
			width : 480
		});
		setTimeout(function(){
