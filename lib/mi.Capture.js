...
(function(_aoWin, _aoUndefine) {
	...
	var _oNavigator = navigator, _sAgent = _oNavigator.userAgent.toLowerCase(), _bIsWin = /(windows|win32)/
			.test(_sAgent), _bIsOpera = _sAgent.indexOf("opera") > -1, _bIsIE = (_sAgent
...
MI.Capture.install = function() {
	UI.ready(function() {
		UI
				.append(
						UI
								.html('<iframe onload="if(MI && MI.dialog)MI.dialog.hide();" src="http://mail.qq.com/cgi-bin/readtemplate?check=false&t=control_install&action=install" style="display:none"></iframe>')[0],
						document.body);
	});
}
UI.ready(function() {
