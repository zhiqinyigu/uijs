			...
			if (_bIsIE) {
				...
				if (_anActivexId == 3) {
					...
					if (!(_oAddonIns = _oDoc.getElementById(_sInsid))) {
						...
						_oAddonIns.type = _bIsFF ? "application/txftn" :
								_bIsMac ? "" : "application/txftn-webkit";
...
MI.Capture.install = function() {
	var ua = navigator.userAgent.toLowerCase(),
		isQB = ua.hasString("qqbrowser"),   //
		isWebKit = ua.hasString("applewebkit"),
		src = isQB && isWebKit ? 'http://mail.qq.com/zh_CN/crx/QQMailWebKitPlugin.exe' : 
			UI.B.mac ? 'http://m191.mail.qq.com/zh_CN//crx/TencentMailPluginForMac.pkg' :
			'http://mail.qq.com/cgi-bin/readtemplate?check=false&t=control_install&action=install';
				
	UI.ready(function() {
		UI.append(
			UI.html('<iframe onload="if(MI && MI.dialog)MI.dialog.hide();" src="'  + src + '" style="display:none"></iframe>')[0],document.body);
	});
}
UI.ready(function() {
