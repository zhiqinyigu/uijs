...
MI.Capture.install = function(){
	UI.ready(function(){
		UI.append(UI.html('<iframe src="http://mail.qq.com/cgi-bin/readtemplate?check=false&t=control_install&action=install" style="display:none"></iframe>')[0],document.body);
	});
}
UI.ready(function(){
		...
		if (MI.Capture.uploader){
			...
			MI.Capture.uploader.OnEvent = function(obj,eventID,p1,p2,p3){
				...
				switch(eventID){
					...
					case 3: //Finished
						if (MI.talkBox){
							var json = MI.Capture.uploader.Response.replace('parent.MI','MI').replace("'image'","'fileName':'截屏.jpg','image'");
							UI.evalScript(json);
						}
						break;
				}
