...
MI.ItemSelectMember = function(o){
	var Id = '' + MI.random(), //数据存储ID
		_target = UI.html('<textarea style="width:0;height:0;position:absolute;left:-999px;"></textarea>')[0],
		_member = o.target;
	...
	Self._member.innerHTML = '<div class="gb_chsuser_top clear">\
			...
			%><li class="unit<%if(selected && selected[i]){ %> select<% }%>" user="<%=i%>" nick="<%=data[i][0]%>" matchUser="<%=data[i][1]%>" matchNick="<%=data[i][2]%>"><div class="gb_ucard gb_ucard">\
				<div class="gb_ucard">\
					<div class="gb_ucard_l">\
						<span class="avatar"><img src="http://mat1.gtimg.com/www/mb/images/head_50.jpg" user="<%=i%>"></span>\
					</div>\
	...
	//console.log(Self,Self._target,Self._all,Self._key);
	...
	Self.clickCall = function(el){ //点击人
		...
		if (user){
			if (selected){
				...
				if (Self.memberTabSelect){ //已选页卡的情况
					//UI.remove(el);
				}
			}
			else {
		...
		MI.Bos('btnItemSelectMember');
	}
	Self.searchCall = function(){ //搜索
		if (Self.memberTabSelect){ //假如在已选页卡，切换到全部页卡
			UI.addClass(Self._memberTabAll,'cur');
			UI.removeClass(Self._memberTabSelect,'cur');
		}
		Self.memberAvatarsShow($$(UI.trim(Self._input.value) ? Self._key : Self._all,'.avatar img'));
	}
	Self.mouseHoverCall = function(el){ //鼠标滑过
		UI.addClass(el,'hover');
	}
	Self.mouseOutCall = function(el){ //鼠标滑出
		UI.removeClass(el,'hover');
	}
	Self.showCall = function(){ //显示回调
		Self.memberAvatarsGet();
		UI.hide(Self._body);
	}
	
	UI.EA(Self._memberCont,'scroll',function(){
		Self.memberAvatarsShow();
	});
	...
	if (Self._memberTabSelect){
		...
		Self._memberTabSelect.onclick = function(){
			...
			Self.listEvent(2);
			UI.show(Self._key);
			Self.memberAvatarsShow($$(Self._key,'.avatar img'));
...
MI.ItemSelectMember.prototype = {
	...
	memberAvatars : null, //缓存头像
	memberAvatarsGet : function(){ //拉取头像
		var Self = this;
		if (!Self._memberAvatars.length){
			var data = [];
			Self._memberAvatars = $$(Self._all,'.avatar img');
			UI.each(Self._memberAvatars,function(o){
				var user = UI.A(o,'user');
				Self._memberAvatars.push(o);
				data.push(user);
			});
			MI.ajax({
				url : MI.url.getAvatar,
				type : 'post',
				data : {id:data.join(',')},
				success : function(data){
					data = MI.json(data);
					if (data.result == 0){
						Self.memberAvatars = data.info;
						Self.memberAvatarsShow();
					}
				}
			});
		}
		else if (Self.memberAvatars){
			Self.memberAvatarsShow();
		}
	},
	memberAvatarsShowNum : 10, //一轮显示的头像个数
	memberAvatarsShowCur : 0, //一轮显示的头像个数
	memberAvatarsShow : function(avatars){ //显示头像
		var Self = this,
			showNum;
		if (Self.memberAvatars){
			if (avatars){
				Self._memberAvatars = avatars;
				Self._memberCont.scrollTop = 0;
				Self.memberAvatarsShowCur = 0;
			}
			
			showNum = Self.memberAvatarsShowNum + Self.memberAvatarsShowCur;
			for (var i = Self.memberAvatarsShowCur,num = Self._memberAvatars.length;i < num;i++){
				if ((i < showNum) || (UI.getY(Self._memberAvatars[i]) - UI.getY(Self._memberCont) < UI.scrollY(Self._memberCont))){
					show(Self._memberAvatars[i]);
				}
				else {
					break;
				}
		...
		function show(img){
			var user = UI.A(img,'user');
			if (user){
				img.src = Self.memberAvatars['onlyavivi'] + '/50';
				UI.A(img,'user','');
			}
			Self.memberAvatarsShowCur++;
		}
	},
