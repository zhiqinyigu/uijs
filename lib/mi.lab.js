...
MI = {
	...
	confirm : function(obj){ //obj = {type:'success',title:'',content:'',confirmTxt:'下一步',confirm:function(){},cancelTxt:'取消',cancel:function(){}}
		...
		if(obj) {
			...
			str = '<div class="blackTip"><h2><span class="ico_tsW"><span class="ico_t' + obj.type + '"></span></span>' + obj.title + '</h2><p>' + obj.content + '</p><p><input type="button" value="' + obj.confirmTxt + '" id="__confirmBtn__" /><input type="button" value="' + obj.cancelTxt + '" id="__cancelBtn__" /></p></div>'
			MI.dialog.show({html:str});
			if (obj.confirm) {
	...
	app : {
		datePicker : function(call){
			UI.getCss('http://mat1.gtimg.com/www/mb/css/datePicker_100827.css');
			UI.getScript('http://mat1.gtimg.com/www/mb/js/ui.DatePicker_100827.js',call);
		}
	}
}
/*
...
MI.Crs = function(reset){
	...
	UI.each(MI.crs,function(o){
		if (UI.hasClass(o,className) && UI.getY(o.parentNode) < y) {
		...
		if (viewRelay && !viewRelay.innerHTML.hasString('对话')) { //View Relay List
			var relayList = Self._relayList,
				...
				createRelayBox = function(){
					...
					var relay;
					if (!this.appended) {
						relay = UI.html(MI.tmpl.reply.replace('talkWrap','zfWrap').replace('<div class="left"></div>','<div class="left"><label for="replayListCheckbox"><input id="replayListCheckbox" type="checkbox" checked class="check1">同时转播给你的听众</label></div>').replace('talkSuc',''))[0];
						...
						$('replayListCheckbox').onclick = function(){
							relayBox.url = this.checked ? '/publish.php' : '/comment/publish.php';
							relayBox.addList = this.checked ? Self.relayBox.addList : 0;
						}
						this.appended = 1;
					}
					else {
...
MI.TalkList.video = function(a,b){ //Video Event
	...
	var vUrl = unescape(box.getAttribute('playurl'));
	var vMp4Url;
...
MI.TalkList.videoOpen = function(vCon,cType,vUrl,vType){
	...
	if(cType == 1){
		var tTools = $$(box,'.vTools')[0],oTools = UI.html(MI.tmpl.videoTool.replace('$VideoTitle',title).replace('$VideoRealUrl',encodeURI(realurl)))[0],flash = _videoObject.replace(/\$VideoPlayUrl/g,playurl).replace(/\$MiniPic/g,picurl),vClose = $$(oTools,'.vClose')[0];
		if(vThumbsBox){
...
UI.ready(function(){
	...
	if (setTheme) {
		setTheme.onclick = function(){
			...
			if (!UI.A(T,'loaded')) {
				UI.getScript('http://mat1.gtimg.com/www/mb/js/mi.Theme_100906.js',function(){
					MI.Theme.build();
					UI.A(T,'loaded',1)
				});
			}
	...
	setTimeout(function(){
		if (MI.user.college) {
			UI.getScript('http://mat1.gtimg.com/www/mb/js/mi.College_100906.js',function(){
				MI.College.build();
			});
		}
	},100);
