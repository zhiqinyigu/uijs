...
/*
	...
	other:
		...
		btnRemoveKnowUsers 收听可能认识的人
		btnChangeKnowUsers 收听可能认识的人
*/
...
MI = {
	...
	tmpl : {
		...
		code : '<div class="verify"><span class="ico_tsW"><span class="ico_te"></span></span><h3></h3><div class="clear"><div class="left">验证码：</div><div class="verifyInput"><input type="text" class="inputTxt" /><div class="cNote">输入下图中的字符，不区分大小写</div><img width="130" height="53">&nbsp;<a href="#" class="codeChange">看不清，换一张</a></div></div><div class="btnBox"><button class="btn2">确定</button><button class="btn2">取消</button></div></div>'
	},
...
MI.Dialog.prototype = {
	...
	alert : function(str){
		var Self = this,btn;
		Self.show({width:9999});
		Self.show({html:'<div class="popBox"><span class="ico_tsW"><span class="ico_te"></span></span><h3>' + str + '</h3><p><input type="button" value="知道了" onclick="MI.dialog.hide()"></p></div>'});
		Self.show({width:UI.width(Self._cont) + 40});
...
MI.Code.prototype = {
	show : function(o){ // o = {msg:'您的操作出现异常，请输入验证码',code:'http://ptlogin2.qq.com/getimage?aid=13001401&Math.random()',call:function(){}}
		...
		MI.dialog.show({html:this._body,width:o.width || 363});
		setTimeout(function(){
			Self._value.focus();
		},0);
	}
...
MI.TalkList = function(id){ //Talk List
	...
	Self._relayList = UI.html('<div class="relayList"><div class="top"><span class="left">大家转播时的看法</span><a href="#" class="close">关闭</a><a href="#" class="right" target="_blank">查看全部</a></div><div class="loading">正在加载...</div><div class="cont"></div></div>')[0];
...
MI.TalkList.prototype = {
	...
	addEvent : function(el){
		...
		if (viewRelay && viewRelay.innerHTML.hasString('转播')) { //View Relay List
			...
			viewRelay.onclick = function(){
				var href = this.href,
					id = href.match(/[^\/]+$/g)[0],
					list = Self._relayList,
					li = id == el.id ? id : el.id,
					Y,
					windowHeight = UI.windowHeight(),
					scrollTop;
				if (list.display && Self.current == li) {
					$$(list,'.close')[0].onclick();
				}
				else {
					$$(list,'.right')[0].href = href;
					UI.hide(Self._relayListCont);
					UI.show(Self._relayListLoad);
					UI.append(list,$$(el,'.msgBox')[0]);
					UI.get('/message_relay_frame.php','id=' + id + '&r=' + MI.random(),function(data){
						pageLink(data);
					});
