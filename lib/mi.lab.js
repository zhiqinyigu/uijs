...
/*
	...
	start:
		...
		btnRegisterChangeAccount 换一批推荐（注册）
	...
	side:
		...
		btnUnFoldtags 展开我的标签
		btnFoldtags 收起我的标签
		btnUnFoldmylist 展开我的名单
		btnFoldmylist 收起我的名单
	...
	talk:
		btnTalkLogin 马上登录（底层页）
		btnTalkRegist 申请开通（底层页）
		btnTalkRelay 转播（底层页）
		btnTalkComt 点评（底层页）
		btnTalkReply 对话（底层页）
		btnTalkFav 收藏（底层页）
		btnTalkRelayHot 转播热门广播（底层页）
		btnTalkComtHot 点评热门广播（底层页）
		btnTalkReplyHot 对话热门广播（底层页）
		btnTalkFavHot 收藏热门广播（底层页）
		btnTalkHotTopic 热门话题（底层页）
	...
	music:
		...
		btnMusicPreviewDel 音乐-删除预览音乐
		btnMiniPlayerPlay 音乐-mini播放器点击播放
		btnMiniPlayermPause 音乐-mini播放器点击暂停
	...
	tips:
		...
		btnTipsIMGroupClose 邀请群成员-关闭
		btnTipsIMGroupCheck 邀请群成员-复选框
		btnTipsIMGroupConfirm 邀请群成员-确定
		btnTips_ Tips点击
...
MI = {
	...
	appLoad : function(obj){
		...
		for (var i in obj || MI._app) {
			if (!MI[i]) {
				if (!MI._appLoading[i]){
					UI.getScript(MI.version[i],(function(i){
						return function(){
							MI.appLoadDo(i);
						}
					})(i));
