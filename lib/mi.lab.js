...
MI.Tips.txt = ['这条消息来自手机微博。<br>想要成为微博手机达人？点<a href="/client.php?t=mobile" target="_blank" class="ulink">这里</a>'];
MI.Tips.url = {};
	...
	urlTips : function(tips){
		...
		function show(data){
			if (data.result == 0) {
				...
				tips.innerHTML = '<em class="ico safe_' + data.info.safe + '"></em><div class="safeType">' + urlTipsTxt[data.info.safe - 1] + '：' + url + '</div></div>';
				UI.show(tips);
		...
		if (viewRelay && viewRelay.innerHTML.hasString('转播')) { //View Relay List
			var relayList = Self._relayList,
				...
				pageLink = function(data){
					...
					if (relayNum) {
						$$(relayList,'.relayNum')[0].innerHTML = $$(relayListNum,'.relayNum')[0].innerHTML = relayNum.innerHTML;
					}
			...
			viewRelay.onclick = function(e){
				...
				if (relayList.display && Self.current == li) {
					...
					$$(relayList,'.relayNum')[0].innerHTML = $$(this,'.relayNum')[0].innerHTML;
