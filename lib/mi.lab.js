/*
	...
	LocalStorage:
		...
		tips_[type]_[account] 提示/广告：1显示、-1隐藏 （type = relayList,college,QQx360）
*/
/*
	MI.Bos('');
			1 /list/xxxx
		...
		numAll=numRelay+numComt;%><li id="<%=talk[i].id%>"<%if(fav){%> fav="<%=talk[i].favortimestamp%>"<%}%> rel="<%=talk[i].timestamp%>">\
			...
			<div class="msgBox">\
					...
					<div class="replyBox">\
						<div class="msgBox">\
							...
							<div class="pubInfo"><span class="left"><a class="time" rel="<%='+V1+'.timestamp%>" href="/p/t/<%='+V1+'.id%>" target="_blank"><%='+V1+'.time%></a> <%='+V1+'.from%><%if(1 && '+V1+' && numAll){if(MI.user.account){%>\
								 <a href="/p/z/<%='+V1+'.id%>" class="zfNum" target="_blank">查看转播/点评(<b class="relayNum"><%=numAll%></b>)</a>\
							<%}else{%>\
				...
				<div class="pubInfo"><span class="left"><a class="time" href="/p/t/<%=talk[i].id%>" target="_blank"><%=talk[i].time%></a> <%=talk[i].from%><%if(1 && !'+V1+' && numAll){if(MI.user.account){%>\
					 <a href="/p/z/<%=talk[i].id%>" class="zfNum" target="_blank">查看转播/点评(<b class="relayNum"><%=numAll%></b>)</a><%}else{%>\
					 <em class="zfNum">查看转播/点评(<b class="relayNum"><%=numAll%></b>)</em><%}}%><%if('+V1+' && '+V1+'.content && '+V1+'.content.length && talk[i].type==4){%>\
					<a class="view" href="/p/r/<%=talk[i].id%>">查看对话</a><%}%><%if(MI.user.fun.report && talk[i].name != MI.user.account){%> <a'+V5+' class="alarm">举报</a><%}%></span><div class="funBox"><%if(MI.user.account){%><a'+V5+' class="fav<%if(fav){%> light<%}%>" title="<%if(fav){%>取消<%}%>收藏"></a><span>|</span><%if(MI.user){if((!guest || guest == MI.user.account) && MI.user.account==talk[i].name){%><a'+V5+' class="delBtn">删除</a><span>|</span><a href="/p/z/<%=talk[i].id%>" class="comt" num="<%=numComt%>">点评</a><span>|</span><a'+V5+' class="relay" num="<%=numRelay%>">转播</a><%}else{%><a'+V5+' class="reply">对话</a><span>|</span><a href="/p/z/<%=talk[i].id%>" class="comt" num="<%=numComt%>">点评</a><span>|</span><a'+V5+' class="relay" num="<%=numRelay%>">转播</a><%}}}%><%if(0){%><%if(talk[i].count){%><%if(MI.user.account){%>\
						<span>|</span><a href="/p/z/<%=talk[i].id%>" class="zfNum zfNumShow" target="_blank">点评<b class="relayNum">(<%=talk[i].count%>)</b></a>\
					<%}else{%>\
						<em class="zfNum zfNumShow">点评<b class="relayNum">(<%=talk[i].count%>)</b></em>\
					<%}}else if('+V1+' && '+V1+'.count){if(MI.user.account){%>\
						<span>|</span><a href="/p/z/<%='+V1+'.id%>" class="zfNum zfNumShow" target="_blank">点评<b class="relayNum">(<%='+V1+'.count%>)</b></a>\
					<%}else{%>\
						<em class="zfNum zfNumShow">点评<b class="relayNum">(<%='+V1+'.count%>)</b></em>\
					<%}}else{if(MI.user.account){%>\
						<span>|</span><a href="/p/z/<%=talk[i].id%>" class="zfNum" target="_blank">点评<b class="relayNum"></b></a>\
					<%}else{%>\
...
MI.Bos = function(op,value){ //Boss Using : MI.Bos('frontClick');
	try{
		...
		value = value || MI.boss;
		MI.Bos.pic.src = 'http://btrace.qq.com/collect?sIp=&iQQ=' + ouin + '&sBiz=microblog&sOp=' + op + '&iSta=0&iTy=214&iFlow=0' + (value ? '&sServerIp=&iBackInt1=&iBackInt2=&sBackStr1=' + value : '');
	}catch(e){}
}
MI.Uin = function(){
		...
		if (!Self.talkBox.display) {
			...
			if (type != 0) {
				var numberType = type == 1 ? '转播' : '点评',
					link = type == 1 ? '?t=1' : '?t=2';
				Self.talkBox._num.innerHTML = '<a href="/p/t/' + sourceId + '/' + link + '" target="_blank">' + 
