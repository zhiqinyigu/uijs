/*
	...
	LocalStorage:
		...
		tips_[type]_[account] 提示/广告：1显示、-1隐藏 （type = relayList,list,college,QQx360）
*/
/*
	MI.Bos('');
			1 /list/xxxx
	...
	side:
		...
		btnFollowrecommendTag 收听我可能认识的人（同道中人）
		btnViewrecommend 查看我可能认识的人
		btnViewrecommendTag 查看我可能认识的人（同道中人）
	...
	tag:
		...
		btnTagTalk 把标签告诉大家
		btnTagGoTopic_1 推荐订阅标签相关话题
		btnTagFollow 收听通道中人
		btnTagUnFollow 取消收听通道中人

		btnTagCardGoSearch 资料卡片中点击标签
		btnTagFromBMW 宝马定制功能来源
	...
	music:
		...
		btnMusicPlayerPreview1 音乐-收藏试听
		btnMusicPlayerWinBlock 音乐-播放器被拦截
		btnMusicPlay 音乐-页内播放器播放
		btnMusicAddTalkBox 音乐-添加音乐至TalkBox
		btnMusicAddTalkBox1 音乐-添加收藏音乐至TalkBox
	...
	potential:
		btnPotentialDelPeople 不再出现（可能认识）
		btnPotentialGoPeopleMore 去看看给你推荐的人（可能认识）
		btnPotentialGoInvite 去邀请朋友来微博（可能认识）
		btnFollowKnowUsersPotential 收听-可能认识
		btnUnFollowKnowUsersPotential 取消收听-可能认识
		btnFollowAllPotential 一键收听（可能认识）

	search:
		btnSearchTag_1 - btnSearchTag_5 高级找人页卡点击
		
	login:
		btnLoginTopic 登录页话题
		btnLoginUser 登录页人
		btnLoginHotUser 登录页人排行

	list:
		btnListAdd 创建名单（右侧）
		btnListAddInAdmin 创建名单（管理）
		btnListAddInList 创建名单（下拉）
		btnListAdmin 管理（右侧）
		btnListAll 查看所有（右侧）
		btnListMore 查看更多名单（右侧）
		btnListLess 收起更多名单（右侧）
		btnListEdit 编辑名单
		btnListDel 删掉名单
		btnListButton 名单按钮
		btnListJoin 拉入名单
		...
		btnListFollow 订阅名单
		btnListView 名单切换查看
		btnListViewAdd 名单切换中创建
		btnListViewAdmin 名单切换中管理
		btnListSearch 名单添加成员搜索

	ad:
		btnAdNBMobile 宁波移动小banner

	other:
		btnTopMenu_1 - btnTopMenu_8 顶部导航
		btnTopMenuNews 广播大厅下拉菜单
...
MI = {
	...
	tmpl : {
		...
		numAll=numRelay+numComt;%><li id="<%=talk[i].id%>"<%if(fav){%> fav="<%=talk[i].favortimestamp%>"<%}%> rel="<%=talk[i].timestamp%>">\
			...
			<%}%>\
			<div class="msgBox">\
				<div class="userName" rel="<%=talk[i].name%>"><strong><a href="/<%=talk[i].name%>"'+V7+'><%=talk[i].nick%></a>'+MI_icon('talk[i].flag')+'<%=talk[i].icon%><%if(!'+V1+'){%>:<%}%></strong><%if('+V1+' && talk[i].type==2){%>转播:&nbsp;<%}else if('+V1+' && talk[i].type==7){%>点评:&nbsp;<%}else if('+V1+' && talk[i].type==4){%>对<strong class="userTo"><a href="/<%='+V1+'.name%>" title="<%='+V1+'.nick.toTitle()%>(@<%='+V1+'.name%>)"><%='+V1+'.nick%></a>'+MI_icon(V1+'.flag')+'说:<%}%></strong>\
				</div>\
					...
					<%if(Music){%>\
						<div class="musicBox" song="<%=talk[i].music[0].sMusicName%>" singer="<%=talk[i].music[0].sMusicAuthor%>" songurl="<%=talk[i].music[0].sMusicUrl%>" songid="<%=talk[i].music[0].dwMusicID%>">\
							<a href="#" class="mThumbsBox" thumbs="<%if(iconPic){%>0<%}else{%>1<%}%>">\
								...
								<span class="mSimple" title="<%=talk[i].music[0].sMusicName%>-<%=talk[i].music[0].sMusicAuthor%>" style="display:none;"><img class="crs" crs="http://mat1.gtimg.com/www/mb/images/mSimple.png" /></span>\
							</a>\
							<div class="mBox">\
					...
					<div class="replyBox">\
						<div class="msgBox">\
								...
								<%if(Music){%>\
									<div class="musicBox" song="<%='+V1+'.music[0].sMusicName%>" singer="<%='+V1+'.music[0].sMusicAuthor%>" songurl="<%='+V1+'.music[0].sMusicUrl%>" songid="<%='+V1+'.music[0].dwMusicID%>">\
										<a href="#" class="mThumbsBox" thumbs="<%if(iconPic){%>0<%}else{%>1<%}%>">\
											...
											<span class="mSimple" title="<%='+V1+'.music[0].sMusicName%>-<%='+V1+'.music[0].sMusicAuthor%>" style="display:none;"><img class="crs" crs="http://mat1.gtimg.com/www/mb/images/mSimple.png" /></span>\
										</a>\
										<div class="mBox">\
											<button class="btn_mPlay" title="播放" style="display:none">播放</button>\
											<button class="btn_mPause_hover" title="暂停" style="display:">暂停</button>\
							...
							<div class="pubInfo"><span class="left"><a class="time" rel="<%='+V1+'.timestamp%>" href="/p/t/<%='+V1+'.id%>" target="_blank"><%='+V1+'.time%></a> <%='+V1+'.from%><%if(1 && '+V1+' && numAll){if(MI.user.account){%>\
								 <a href="/p/t/<%='+V1+'.id%>" class="zfNum" target="_blank">查看转播/点评(<b class="relayNum"><%=numAll%></b>)</a>\
							<%}else{%>\
								 <em class="zfNum">查看转播/点评(<b class="relayNum"><%=numAll%></b>)</em>\
							<%}}%></span></div>\
						</div>\
					</div>\
				<%}%>\
				<div class="pubInfo"><span class="left"><a class="time" href="/p/t/<%=talk[i].id%>" target="_blank"><%=talk[i].time%></a> <%=talk[i].from%><%if(1 && !'+V1+' && numAll){if(MI.user.account){%>\
					 <a href="/p/t/<%=talk[i].id%>" class="zfNum" target="_blank">查看转播/点评(<b class="relayNum"><%=numAll%></b>)</a><%}else{%>\
					 <em class="zfNum">查看转播/点评(<b class="relayNum"><%=numAll%></b>)</em><%}}%><%if('+V1+' && '+V1+'.content && '+V1+'.content.length && talk[i].type==4){%>\
					<a class="view" href="/p/r/<%=talk[i].id%>">查看对话</a><%}%><%if(MI.user.fun.report && talk[i].name != MI.user.account){%> <a'+V5+' class="alarm">举报</a><%}%></span><div class="funBox"><%if(MI.user.account){%><a'+V5+' class="fav<%if(fav){%> light<%}%>" title="<%if(fav){%>取消<%}%>收藏"></a><span>|</span><%if(MI.user){if((!guest || guest == MI.user.account) && MI.user.account==talk[i].name){%><a'+V5+' class="delBtn">删除</a><span>|</span><a href="/p/t/<%=talk[i].id%>" class="comt" num="<%=numComt%>">点评</a><span>|</span><a'+V5+' class="relay" num="<%=numRelay%>">转播</a><%}else{%><a'+V5+' class="reply">对话</a><span>|</span><a href="/p/t/<%=talk[i].id%>" class="comt" num="<%=numComt%>">点评</a><span>|</span><a'+V5+' class="relay" num="<%=numRelay%>">转播</a><%}}}%><%if(0){%><%if(talk[i].count){%><%if(MI.user.account){%>\
					...
					<%}}%><%}%><%if(0 && MI.user.account){%><span>|</span><a href="/p/t/<%=talk[i].id%>" class="comt" target="_blank">点评</a><%}%></div>\
				</div>\
	...
	viewList : function(id,el){
		...
		if(!UI.hasClass(arrow,'lopen') && !noList) {
			show();
		}
		else{
			hide();
	...
	openMusic : function(url){
		...
		var playUrl = songData.stream;//'http://upload'+songData.stream+'.music.qzone.soso.com/'+(parseInt(songData.songID)+12000000)+'.wma';
...
MI.Bos = function(op,value){ //Boss Using : MI.Bos('frontClick');
	try{
		...
		MI.Bos.pic.src = 'http://btrace.qq.com/collect?sIp=&iQQ=' + ouin + '&sBiz=microblog&sOp=' + op + '&iSta=0&iTy=214&iFlow=0' + data;
	}catch(e){}
}
MI.Uin = function(){
...
MI.TalkList = function(id){ //Talk List
	...
	Self._comt = UI.html(MI.tmpl.reply.replace('<div class="left"></div>','<div class="left"><label for="replayListCheckbox" style="display:none"><input id="replayListCheckbox" type="checkbox" class="check1">同时转播给你的听众</label></div>'))[0];
		...
		if (!Self.talkBox.display) {
			...
			if (type != 0) {
				var numberType = type == 1 ? '转播' : '点评',
					...
					link = type == 1 ? '?t=1' : '?t=2';
				Self.talkBox._num.innerHTML = '<a href="/p/t/' + sourceId + '/' + link + '" onclick="MI.Bos(\'btn' + bossName + 'View\')" target="_blank">' + 
...
MI.TalkList.musicPlay = function(music){
	...
	*/
	...
	if(typeof playSongWithNoCheck == 'function'){
		setTimeout(function(){
			UI.A(MI.TalkList.lastMusic,'play','1');
			MI.TalkList.musicScroll(MI.TalkList.lastMusic);
...
MI.TalkList.musicMarqueeStop = function(o){
	...
	UI.C(MI.TalkList.musicLastMarquee,'marginLeft','-20px');
...
MI.TalkList.musicPause = function(a){
	...
	if(a){
		MI.TalkList.musicPlayBtn(MI.TalkList.lastMusic);
	}
	UI.A(MI.TalkList.lastMusic,'play','0');
	MI.TalkList.musicMarqueePause();
	MI.TalkList.musicState = 2;
}
...
MI.TalkList.musicClose = function(a){
	...
	try{
		...
		//alert(UI.A(musicBox,'play'))
		MI.mediaMutex(mBox,1);
	}catch(e){}
}
MI.TalkList.musicEvent = function(musicBox){
	...
	for (var i = 0,num = musicBox.length;i < num;i++) {
		...
		mThumbsBox.onclick = function(){
			UI.hide(mPlay);
			UI.show(mPause);
			MI.TalkList.music(this);
			return false;
		}
		mPlay.onclick = function(){
			UI.hide(this);
			UI.show(mPause);
			MI.TalkList.music(mThumbsBox);
			return false;
		}
