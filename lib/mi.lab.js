...
MI = {
	...
	newCount : function(type, url, time){
		...
		function newCout(){
			...
			if (fail < 6) { //6次未登录后不再拉取
				...
				var Time,Time_1,Time_2,Time_3;
				Time = + new Date();
				if (0 && MI.user.fun.newCount){
					urlData.cb = 'MI.newCountShow'; //回调函数
					reqUrl = 'http://t.qq.com' + reqUrl;
				...
				UI.each(data.info,function(o,i){
					...
					if ((o.type == 3 || o.type == 5 || o.type == 6 || o.type == 7 || o.type == 16 || o.type == 32) && !MI.noNewCount) {
						if (MI.talkList && MI.talkList._new) {
							...
							_new.innerHTML = '<a href="javascript:void(0)" tabindex="4">' + _('约{0}新广播，点击查看',tip) + '</a>';
	...
	addQQ : function(id){
		...
		if (window.id_af_show){
			...
			UI.getScript('http://id.qq.com/af/js/id_af.js',function(){
				id_af_init({"appid": 10021, "lang": 2052, "defaultlogin": true});
				id_af_show(id,1);
			});
		}
	},
			...
			if (Self._video) {
				...
				*/
				...
				Self._videoUpload.onclick = function(){
					...
					window.open(MI.url.uploadVideo,'wbVideo','height=' + height + ',width=' + width + ',top=' + top + ',left=' + left + ',scrollbars=yes,resizable=yes');
			...
			if (Self._pic) {
				...
				*/
				var imageUploadName = 'imageUpload_' + MI.random(),
					html = UI.html('<div class="cutBox" style="display:none;"><p  style="display:none;"><span class="ico_cut"></span><a href="#" class="txt mOut" title="'+_("支持粘贴QQ截屏等剪贴板内容") +'">'+_("截屏") +'</a></p><p><span class="ico_pUrl"></span><a href="#" class="txt mOut" title="'+_("粘贴图片链接")+'">'+ _("链接") +'</a></p><p><span class="ico_pics"></span><a href="#" class="multi txt mOut">多图</a></p></div><span class="pidLoad" style="display:none">' + _("上传中") + ' <a href="#" class="cancel">[' + _("取消") + ']</a></span><span style="display:none" class="preview"><span class="link"></span><a href="#" class="del">[' + _("删除") + ']</a></span><iframe id="' + imageUploadName + '" name="' + imageUploadName + '" src="about:blank" style="display:none"></iframe>'),
				...
				Self._multi = $$(Self._pic,'.multi')[0];

				Self._multi.onclick = function(){
...
MI.TalkBox.prototype = {
	...
	send : function(){
		...
		if (!Self.type && !Self.pic && !Self.sourcePic && !Self.topic && content.match(/^@[a-zA-Z]{1}[\w|-]{0,19}$/g)) {
			...
			document.location.href = 'http://t.qq.com/' + content.slice(1) + '?from=1';
			return;
		}
...
MI.photoFilter = {
	...
	pushImg:function(){
		...
		if(document.FilterPhoto && document.FilterPhoto.sendImgUrl_ToFlash) {
			document.FilterPhoto.sendImgUrl_ToFlash(Self.sourceUrl);	//调用flash提供的方法，传递原始
		} else {
	...
	addEvent : function(el){
		...
		if (comt) { // 评论
			comt.onclick = function(){
				...
				Self.curSource = Self.curSource.match(/[0-9]*[0-9]/g)[0];
				...
				comtList.innerHTML = '';
				var messageUrl = 't=2&id=' + Self.curSource + '&viewModel=' + Self.iconPic + '&r=' + MI.random();
				...
				Self.xhr.comt = UI.get('/message_relay_frame.php', messageUrl,function(data){
					Time_1 = + new Date() - Time;
					pageLink(data);
		...
		if (viewRelay && !viewRelay.innerHTML.hasString(_('对话'))) { // 查看转播
			...
			viewRelay.onclick = function(e){
				...
				UI.A(this,'rel',id);
				relayListNum = this;
				if ((relayList.display && Self.cur == li) || (relayList.display && Self.relayListMulti)) {
					...
					Time = + new Date();
					var messageUrl = 'id=' + id + '&viewModel=' + Self.iconPic + '&r=' + MI.random();
...
MI.app({
	Base : function(){
		if (MI.host == 't.qq.com' || MI.host == 'qun.t.qq.com'){
			setTimeout(function(){
				MI.navMenu();
...
UI.ready(function(){
	//MI.user.fun 的默认值
	if (MI.user.fun) {
		var fun = {search:1,goTop:1,mergePic:1};
		for (var i in fun) {
