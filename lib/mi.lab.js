...
MI = {
	...
	tmpl : {
		...
		videoThumbs : '<a href="#" class="vThumbs" url="$VideoPlayUrl"><span class="mask" style="display:block"></span><img src="$VideoPicUrl" /></a>',
...
MI.videoThumb = function(o,size){
    size = size ? size : 50;
	o.src = 'http://mat1.gtimg.com/www/mb/images/video_nothumb.gif';
}
		...
		if (viewRelay && viewRelay.innerHTML.hasString('转播')) { //View Relay List
			var relayList = Self._relayList,
				...
				createRelayBox = function(){
					UI.hide(this);
					var relay = UI.html(MI.tmpl.reply.replace('talkWrap','zfWrap').replace('talkSuc',''))[0],
			...
			viewRelay.onclick = function(e){
				...
				if (relayList.display && Self.current == li) {
					...
					UI.get('/message_relay_frame.php','id=' + id + '&viewModel=' + Self.iconPic + '&r=' + MI.random(),function(data){
						pageLink(data);
					});
...
MI.TalkList.video = function(a,b){ //Video Event
	...
	var box;
	if(b == 1){
		var picurl = unescape(_this.getAttribute('minipicurl')),playurl = unescape(_this.getAttribute('playurl')),realurl = unescape(_this.getAttribute('realurl')),title = _this.getAttribute('title')?_this.getAttribute('title'):_this.getAttribute('reltitle'),box = $$(_this.parentNode.parentNode,'.videoBox')[0],vWrap = $$(box,'.vWrap')[0],vBox = $$(box,'.vBox')[0],vSimple = $$(box,'.vSimple')[0],vThumbs = $$(box,'.vThumbs')[0];
		if(vSimple){
...
MI.TalkList.videoClose = function(){
	var v = $$(this.parentNode.parentNode,'.vSimple')[0] || $$(this.parentNode.parentNode,'.vThumbs')[0];//this.parentNode.nextSibling.firstChild;
