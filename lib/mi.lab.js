		...
		if (viewRelay && !viewRelay.innerHTML.hasString(_('对话'))) { // 查看转播
			var relayList,
				...
				createRelayBox = function(){
					...
					if (!this.appended) {
						...
						{
							isRelay = 0;
							relay = UI.html(MI.tmpl.reply.replace('talkWrap','talkWrap').replace('<div class="left"></div>','<div class="left" style="display:none"><label for="replayListCheckbox"><input id="replayListCheckbox" type="checkbox"' + (isRelay ? ' checked' : '') + ' class="check1">' + _('同时转播给你的听众') + '</label></div>').replace('talkSuc',''))[0];
						}
			...
			viewRelay.onclick = function(e){
				...
				if ((relayList.display && Self.cur == li) || (relayList.display && Self.relayListMulti)) {
					...
					Self.xhr.relay = UI.get('/message_relay_frame.php',messageUrl,function(data){
						Time_1 = + new Date() - Time;
						pageLink(data);
