...
MI = {
	...
	appLoadDo : function(i){ //Run Callback Function
		...
		for (var j in MI._appLoad[i]){
			if (MI._appLoad[i] && UI.isFunction(MI._appLoad[i][j])) {
				MI._appLoad[i][j]();
			}
		}
		delete MI._appLoad[i];
		...
		UI.ready(function(){
			...
			if (Self._video) {
				...
				*/
				...
				var html = UI.html('<div class="vloadWrap" style="display:none"><form name="videoUploadFrom" id="videoUploadFrom" class="videoUploadFrom" method="get"><a href="#" class="close" title="' + _('关闭') + '">' + _('关闭') + '</a><p>' + _('粘帖视频播放页地址') + '<span class="cNote">(' + _('优酷、土豆、56等网站视频可直接播放') + ')</span></p><p class="clear"><input type="text" class="inputTxt" /><input type="submit" class="btn" value="' + _('确定') + '" /></p><p class="cError" style="display:none">'+Self._errMsg[3]+'</p></form></div>'),frag = document.createDocumentFragment();
			...
			if (Self._pic) {
				...
				*/
				var html = UI.html('<p class="cutBox" style="display:none;"><span class="ico_cut"></span><a href="#" class="txt mOut" title="' + _('支持粘贴QQ截屏等剪贴板内容') + '">' + _('截屏') + '</a></p><span class="pidLoad" style="display:none">' + _('上传中') + ' <a href="#" class="cancel">[' + _('取消') + ']</a></span><span style="display:none" class="preview"><span class="link"></span><a href="#" class="del">[' + _('删除') + ']</a></span><iframe id="imageUpload" name="imageUpload" src="about:blank" style="display:none"></iframe>'),
				...
				if (UI.hasClass(document.body,'ipad')){ //Disable Upload
					Self._pic.onclick = function(){
						alert(_('对不起，您的浏览器由于安全限制不支持图片上传，建议您使用其他浏览器。'));
...
MI.TalkBox.prototype = {
	...
	delayVideoTime : null,
	url : '/publish.php',
	type : null, //Value: null(Talk)  1(Relay)  2(Reply)  3(Message)  4(Timeline Comment)  5(Comment)  6(Note)
				 //Value In PHP: 1-原创发表、2-转载、3-私信 4-回复 5-空回 6-提及 7-点评
	...
	send : function(){
		...
		//Post Data
		obj = {content:content.replace(/＠/g,'@').replace(/＃/g,'#'),countType:Self.countType,viewModel:Self.iconPic};
		if (Self.code) {
	...
	addPic : function(o){
		...
		if (o.result == 0) {
			...
			if (MI.user.fun.turnPic && o.info.path) {
				...
				Self._picLink.innerHTML = '<a href="#" class="fileName" onclick="return false">' + fileName + '<img src="' + pic + (normalPic ? '/160' : '') + '" /></a>';
			}
...
MI.TalkList = function(id){ //Talk List
	...
	Self._comt = UI.html(MI.tmpl.reply.replace('<div class="left"></div>','<div class="left"><label for="replayListCheckbox"><input id="replayListCheckbox" type="checkbox" class="check1">' + _('只作为点评发表') + '</label></div>'))[0];
...
MI.TalkList.prototype = {
	...
	reply : function(id,type,number){ //type : 0(reply) 1(relay) 2(comt)
		...
		if (!Self.talkBox.display) {
			...
			var className,curTitle,
				...
				//comtTitle = _('点评内容仅发送给原文作者，同时转播则发送给原文作者和你的听众'),
				comtTitle = '',
				delayAddReply;
			...
			if (replyBox && name2 && (type == 1 || type == 2)) { //添加转播点评
			...
			if (type != 0) {
				var numberType = type == 1 ? _('转播') : _('点评'),
	...
	addEvent : function(el){
		...
		if (share) {
			share.onclick = function(e){ //分享
				Self.share(this.parentNode,el.id);
				console.log(Self._share.style.display);
				if (UI.hasClass(this.parentNode,'shareDis')){
					UI.E(e).stop();
				}
				return false;
			}
		...
		if (fav) { // 收藏
			fav.type = 1;
			fav.title = (UI.hasClass(fav,'light') ? _('取消') : '') + _('收藏');
