...
MI = {
	...
	tmpl : {
		...
		videoHTML5: '<div class="vBox" style="display:block"><video width="460" height="362" poster="$MiniPic" controls="controls" autoplay="autoplay"  src="$VideoPlayUrl"><source type="video/mp4" src="$VideoPlayUrl"></source></video></div>',
...
MI.TalkList.videoOpen = function(vCon,cType,vUrl,vType){
	...
	if(cType == 1){
		...
		var tTools = $$(box,'.vTools')[0],oTools = UI.html(MI.tmpl.videoTool.replace('$VideoTitle',title).replace('$VideoRealUrl',realurl))[0],flash = UI.html(_videoObject.replace(/\$VideoPlayUrl/g,playurl).replace(/\$MiniPic/g,picurl))[0],vClose = $$(oTools,'.vClose')[0];
		if(vThumbsBox){
		...
		UI.after(UI.html(_videoObject.replace(/\$VideoPlayUrl/g,url))[0],_this);
	}
	box.play = 1;
...
UI.ready(function(){
	...
	//setTimeout(function(){
		...
		setTimeout(function(){
			var gotTop = $$('.goTop')[0],link = $$('#UIn a,.SM a,.mobileBox a,.proposal a'),hotTopic = $$('.hotTopicList ');
			if (gotTop) {
