		...
		var url = 'http://mat1.gtimg.com/www/mb/',
			js = {
				...
				mi : {
					...
					SettingEdu : '110307',
					SettingWork : '110308',
					TalkListUpdate : '110311b',
					Scroll : '101111',
					Capture : '110307',
					Music : '110311a',
					QQMusicInstance : '110307',
					QQMusicPlayer : '110307',
					QQMusicWmpPlayer : '110307',
...
MI.Load = function(id,url,type,boss){
	...
	function loaded(id,html){
		...
		UI.removeClass(btn,_loading);
		if(id =="recommend") MI.Card.build(el,'.userPic img',id);
		if(id == "star") MI.Card.build(el,'.imgList img',id);
	}
}
...
MI.TalkList = function(id){ //Talk List
	...
	Self._comt = UI.html(MI.tmpl.reply.replace('<div class="left"></div>','<div class="left"><label for="replayListCheckbox"><input id="replayListCheckbox" type="checkbox" class="check1">' + _('同时转播') + '</label></div>'))[0];
...
MI.TalkList.prototype = {
	...
	card : function(){
		...
		MI.app({
			Base : function(){
				setTimeout(function(){
					MI.Card.build(Self._body,'.userPic img','talkListAvatar');
					MI.Card.build(Self._body,'.msgBox strong a,.msgBox em a','talkListAccount');
				},100);
			}
		});
	},
	...
	reply : function(id,type,number){ //type : 0(reply) 1(relay) 2(comt)
		...
		if (!Self.talkBox.display) {
			...
			var className,curTitle,
				...
				comtTitle = _('点评原文，点评内容不会发送给你的听众'),
				delayAddReply;
	...
	addEvent : function(el){
		...
		if (reply) { // 对话
			reply.onclick = function(){
				Self.creatBox();
				Self.setArrowX(UI.parents(this,'share')[0] || this,time);
				Self.reply(el.id,0);
				Self.replyCont = UI.A(this,'cont');
				MI.Bos('btnReply');
				return false;
