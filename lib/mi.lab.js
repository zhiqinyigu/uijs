				...
				Self._musicAnchor.onclick = function(){
					if(!MI.Music){
						...
						UI.getScript(MI.version.Music,function(){
							...
							MI.Music.build(Self);// = new MI.Music({target:$('tagsList'),call:showTip});
						});
					}else{
						MI.Music.show();
					}					
				...
				function(data){
					...
					if (data.result == 0){
						...
						if(data.url){
							...
							Self._txt.value += title + '：http://url.cn/'+data.url;
							clearTimeout(Self.delayVideoTime);
...
MI.TalkList = function(id){ //Talk List
	var Self = this,_more = $('moreList'),comtIsRelay = MI.S('option_relayListCheck_' + MI.user.account) != -1;
	...
	Self._comt = UI.html(MI.tmpl.reply.replace('<div class="left"></div>','<div class="left"><label for="replayListCheckbox"><input id="replayListCheckbox" type="checkbox" class="check1">同时转播给你的听众</label></div>'))[0];
	...
	Self.comtBox.url = '/comment/publish.php';
	...
	reply : function(id,type){ //type : 0(reply) 1(relay) 2(comt)
		...
		if (!Self.talkBox.display) {
			...
			if (!Self.comtBox._relayCheck.checked && type == 2) { //评论
				this.talkBox.talkId = Self.curSource;
				this.talkBox.type = 1;
			}
			/*if (relay) {
			...
			var relayList = Self._relayList,
				...
				createRelayBox = function(){
					...
					if (!this.appended) {
						...
						UI.after(relay,this);
						relayBox._body.ondblclick = relayBox._body.onmousedown = function(e){
							UI.E(e).stop();
						}
						setTimeout(function(){
							relayBox._txt.focus();
						...
						//同时转播给你的听众
						$$(relayBox._body,'.check1')[0].onclick = function(){
							var checked = this.checked;
