...
MI = {
	...
	confirm : function(obj){ //obj = {type:'success',title:'',content:'',confirm:function(){},cancel:function(){}}
		var str;
		if(obj) {
			obj.type = obj.type ==  'success' ? 's' : 'e';
			str = '<div class="blackTip"><h2><span class="ico_tsW"><span class="ico_t' + obj.type + '"></span></span>' + obj.title + '</h2><p>' + obj.content + '</p><p><input type="button" value="确定" id="__confirmBtn__" /><input type="button" value="取消" id="__cancelBtn__" /></p></div>'
...
MI.TalkList.video = function(a,b){ //Video Event
	...
	var vUrl = unescape(_this.getAttribute('playurl'));
	var vMp4Url;
...
MI.TalkList.videoOpen = function(vCon,cType,vUrl,vType){
	...
	var id = getListID(_this);
	var box,vBox,flash;
	var vBoxCon = UI.html('<div class="vBox" style="display:block"></div>')[0];
	if(cType == 1){
		var picurl = unescape(_this.getAttribute('minipicurl')),playurl = vUrl ? vUrl : unescape(_this.getAttribute('playurl')),realurl = unescape(_this.getAttribute('realurl')),title = _this.getAttribute('title')?_this.getAttribute('title'):_this.getAttribute('reltitle'),title = MI.string.cut(title,40),box = $$(_this.parentNode.parentNode,'.videoBox')[0],vWrap = $$(box,'.vWrap')[0],vBox = $$(box,'.vBox')[0],vThumbsBox = $$(box,'.vThumbsBox')[0];
		...
		var url = vUrl ? vUrl : _this.getAttribute("playurl");
		var tools = _this.parentNode.previousSibling;
		box = _this.parentNode.parentNode;
		vBox = $$(box,'.vBox')[0]
		var vThumbsBox = $$(box,'.vThumbsBox')[0];
		var vWrap = $$(box,'.vWrap')[0]
		flash =_videoObject.replace(/\$VideoPlayUrl/g,url);
		if(box.play){
			box.play = 0;
			return false;
		}
...
UI.ready(function(){
	...
	setTimeout(function(){
		if (MI.user.college) {
			UI.getScript('http://mat1.gtimg.com/www/mb/js/mi.College_100826.js',function(){
				MI.College.build();
