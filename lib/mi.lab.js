...
(function(){
var V1='talk[i].source',V2='info.source',V3='<div class="SA"><em>◆</em><span>◆</span></div>',V4='%><a href="/certification" target="_blank" class="vip" title="腾讯认证"></a><%',V5=' href="#"',V6=' onerror="MI.Pic(this,50)"',V7=' title="<%=talk[i].nick.toTitle()%>(@<%=talk[i].name%>)"',V8='%><a href="http://blog.qq.com/zt/2010/2010expo/shibovol.htm" title="2010上海世博志愿者" target="_blank" class="ico_expo"></a><%';
MI_icons = ['auth','expo']; //Set Icon's Order
MI_icon = function(obj){
	return '<%for(var k=0,num=MI_icons.length;k<num;k++){if(MI_icons[k]=="auth"&&'+obj+'[MI_icons[k]]){'+V4+';break;}else if(MI_icons[k]=="expo"&&'+obj+'[MI_icons[k]]){'+V8+';break;}}%>';
}
MI = {
	...
	tmpl : {
		...
		black : '<div class="blackTip"><h2><span class="ico_tsW"><span class="ico_te"></span></span>确定将<%=sex%>加入黑名单？</h2><p><span class="fs14">加入之后...</span><br />自动解除你们的收听关系，并且<%=sex%>无法重对你进行收听<br /><%=sex%>无法再对你进行任何骚扰</p><p><input type="button" value="确定" id="blackTipBtn" /><input type="button" value="取消" onclick="MI.dialog.hide()" /></p></div>',
		unblack : '<div class="blackTip"><h2><span class="ico_tsW"><span class="ico_te"></span></span>你真的原谅<%=sex%>了吗？</h2><p><span class="fs14">移出之后...</span><br /><%=sex%>可以重新对你进行收听<br />不排除继续对你进行骚扰的可能性</p><p><input type="button" value="确定" id="blackTipBtn"/><input type="button" value="取消" onclick="MI.dialog.hide()"/></p></div>',
	...
	black : function(o){
		var btn,isBlack = o.target.innerHTML.hasString('取消') ? 1 : 0,type = isBlack ? 'un' : '';
			...
			if (!Self.sending) {
				UI.ajax({
					...
					success : function(data){
						data = MI.json(data);
						if (data.result == 0) {
							o.target.innerHTML = '<b></b>' + (isBlack ? '拉黑名单' : '取消拉黑');
							MI.dialog.hide();
							if (o.call) {
								o.call();
							}
