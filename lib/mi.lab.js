...
MI = {
	...
	tmpl : {
		...
		black : '<div class="blackTip"><h2><span class="ico_tsW"><span class="ico_te"></span></span>确定将<%=sex%>拉入黑名单？</h2><p><span class="fs14">拉入之后...</span><br/>自动解除你们的收听关系，并且<%=sex%>无法重对你进行收听<br /><%=sex%>无法再对你进行任何骚扰</p><p><input type="button" value="确定" id="blackTipBtn" /><input type="button" value="取消" onclick="MI.dialog.hide()"/></p></div>',
		unblack : '<div class="blackTip"><h2><span class="ico_tsW"><span class="ico_te"></span></span>你真的原谅<%=sex%>了吗？</h2><p><span class="fs14">取消之后...</span><br/><%=sex%>可以重新对你进行收听<br />不排除继续对你进行骚扰的可能性</p><p><input type="button" value="确定" id="blackTipBtn"/><input type="button" value="取消" onclick="MI.dialog.hide()"/></p></div>',
...
MI.Card.prototype = {
	...
	show : function(el){
		...
		function show(o){
			var notSelf = Self.account != MI.user.account,btn = o.follow ? Self._unfollow : Self._follow,icon = '',gender = o.flag.gender ? '他' : '她';
			if (o.block) {
