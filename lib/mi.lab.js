...
UI.ready(function(){
	...
	if (setTheme) {
		setTheme.onclick = function(){
			...
			if (!UI.A(T,'loaded')) {
				UI.getScript('http://mat1.gtimg.com/www/mb/js/mi.Theme_100902a.js',function(){
					MI.Theme.build();
					UI.A(T,'loaded',1)
				});
			}
	...
	setTimeout(function(){
		if (MI.user.college) {
			UI.getScript('http://mat1.gtimg.com/www/mb/js/mi.College_100902a.js',function(){
				MI.College.build();
				if (MI.user.college.finished) {
					UI.addClass(MI.College._btn,'finished');
				}
			});
		}
	},100);
