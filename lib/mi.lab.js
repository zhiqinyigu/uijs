...
MI = {
	...
	tmpl : {
		...
		msg : '<li id="<%=info.id%>" rel="<%=info.timestamp%>"><div class="msgBox"><div class="userName">发给 <a href="/<%=info.account%>" title="<%=info.nick%>(@<%=info.account%>)"><%=info.nick%></a>'+MI_icon('info.flag')+'</div><div class="msgCnt"><%=info.content%></div><div class="pubTime"><span class="time"><%=info.time%></span></div><p class="btnBox"><a class="btn delBtn"'+V5+'>删除</a></p></div></li>',
		...
		UI.ajax({
			...
			success : function(data){
				...
				if (MI.user && data.info) {
					...
					data.info.flag = data.info.flag || MI.user.flag;
