...
MI = {
	...
	tmpl : {
		...
		var numAll,numRelay,numComt;numRelay=talk[i].count[0]||0;numComt=talk[i].count[1]||0;\
...
MI.TalkList.prototype = {
	...
	reply : function(id,type,number){ //type : 0(reply) 1(relay) 2(comt)
		...
		if (type != 0) {
			...
			Self.talkBox._num.innerHTML = (number != 0 && number) ? '共有 ' + number + ' 条' + numberType : '';
		}
...
UI.ready(function(){
	...
	setTimeout(function(){
		var link = $$('#UIn a,.SM a,.mobileBox a,.proposal a'),hotTopic = $$('.hotTopicList a');
		if (link) {
