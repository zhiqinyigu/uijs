/*
	...
	Cookie:
		...
		mb_vT_[account] 查看消息列表类型（viewType）： 只看原创|简约模式|只看我收听的人
*/
/*
	MI.Bos('');
...
(function(){
var V1='talk[i].source',V2='info.source',V3='<div class="SA"><em>◆</em><span>◆</span></div>',V4='%><a href="/certification" target="_blank" class="vip" title="腾讯认证"></a><%',V5=' href="#"',V6=' onerror="MI.Pic(this,50)"',V7=' title="<%=talk[i].nick.toTitle()%>(@<%=talk[i].name%>)"',V8='%><a href="/certification" target="_blank" class="ico_expo" title="上海世博"></a><%';
MI_icons = ['auth','expo']; //Set Icon's Order
MI_icon = function(obj){
	return '<%for(var k=0,num=MI_icons.length;k<num;k++){if(MI_icons[k]=="auth"&&'+obj+'[MI_icons[k]]){'+V4+'}else if(MI_icons[k]=="expo"&&'+obj+'[MI_icons[k]]){'+V8+'}}%>';
}
MI = {
	...
	tmpl : {
		//list : '<li id="<%=info.id%>" rel="<%=info.timestamp%>"><div class="userPic"><a href="/<%=info.account%>" title="<%=info.nick.toTitle()%>(@<%=info.account%>)"<%if('+V2+'.type==2){%> class="masPic"<%}%>><img src="<%=info.userPic%>"'+V6+'/></a><%if('+V2+'.type==2){%><%='+V2+'.cusPic%><%}%></div><div class="msgBox"><div class="userName"><strong><a href="/<%=info.account%>" title="<%=info.nick.toTitle()%>(@<%=info.account%>)"><%=info.nick%></a>'+MI_icon('info.flag')+'<%if(!info.action){%>:<%}%></strong><%=info.action%></div><div class="msgCnt"><%if('+V2+'.type){%> <%}%><%=info.content%>&nbsp;</div><%if('+V2+'.type==1){%><div class="replyBox"><%='+V2+'.content%></div><%}%><%if(info.image && info.image.length){%><div><%for(var i=0;i<info.image.length;i++){%><div class="picBox"><a href="<%=info.image[i]%>/460" class="pic"><img class="crs" crs="<%if(info.iconPic){%>http://mat1.gtimg.com/www/mb/images/vT.png<%}else{%><%=info.image[i]%>/160<%}%>"></a></div><%}%></div><%}%><div class="pubInfo"><span class="left"><%if('+V2+'.type==1){%><span class="time"><%=info.time%></span><%}else{%><a class="time" href="/p/t/<%=info.id%>" target="_blank"><%=info.time%></a><%}%> <%=info.sorurce%> <%if('+V2+'.type==2){%><a class="view zfNum" href="/p/r/<%=info.id%>">查看对话</a></b><%}%><%if(!info.flag.auth){%> <a'+V5+' class="alarm">举报</a><%}%></span><div class="funBox"><%if(UI.isNumber(info.comt)){%><a href="#" class="comt">评论(<em><%=info.comt%></em>)</a><span>|</span><%}%><a'+V5+' class="delBtn">删除</a><span>|</span><a'+V5+' class="relay">转播</a><span>|</span><a'+V5+' class="fav"></a></div></div><%if('+V2+'.type==2){%><div class="talkDetail"><b><%='+V2+'.nick%>:</b> <%='+V2+'.content%></div><%}%></div></li>',
		listAll : '<%for(var i=0,n=talk.length;i<n;i++){%><li id="<%=talk[i].id%>" rel="<%=talk[i].timestamp%>"><%if(!guest){%><%if('+V1+' && talk[i].type==4){%><div class="userPic"><a href="/<%=talk[i].name%>"'+V7+' class="masPic"><img src="<%=talk[i].pic%>"'+V6+'/></a><a href="/<%='+V1+'.name%>" title="<%='+V1+'.nick.toTitle()%>(@<%='+V1+'.name%>)" class="cusPic"><img src="<%='+V1+'.pic%>"'+V6+'/></a></div><%}else{%><div class="userPic"><a href="/<%=talk[i].name%>"'+V7+'><img src="<%=talk[i].pic%>"'+V6+'/></a></div><%}%><%}%><div class="msgBox"><div class="userName" rel="<%=talk[i].name%>"><strong><a href="/<%=talk[i].name%>"'+V7+'><%=talk[i].nick%></a>'+MI_icon('talk[i].flag')+'<%if(!'+V1+'){%>:<%}%></strong><%if('+V1+' && talk[i].type==2){%>转播:&nbsp;<%}else if('+V1+' && talk[i].type==4){%>对<strong class="userTo"><a href="/<%='+V1+'.name%>" title="<%='+V1+'.nick.toTitle()%>(@<%='+V1+'.name%>)"><%='+V1+'.nick%></a>'+MI_icon(V1+'.flag')+'说:<%}%></strong></div><div class="msgCnt"><%=talk[i].content||"&nbsp;"%></div><%if(talk[i].image && talk[i].image.length){%><div><%for(var j=0;j<talk[i].image.length;j++){%><div class="picBox"><a href="<%=talk[i].image[j]%>/460" class="pic"><img class="crs" crs="<%if(iconPic){%>http://mat1.gtimg.com/www/mb/images/vT.png<%}else{%><%=talk[i].image[j]%>/160<%}%>"></a></div><%}%></div><%}%><%if('+V1+' && talk[i].type==2){%><div class="replyBox"><div class="msgBox"><div class="msgCnt"><strong><a href="/<%='+V1+'.name%>" title="<%='+V1+'.nick.toTitle()%>(@<%='+V1+'.name%>)"><%='+V1+'.nick%></a>'+MI_icon(V1+'.flag')+':</strong><%='+V1+'.content%></div><%if('+V1+'.image && '+V1+'.image.length){%><div><%for(var j=0;j<'+V1+'.image.length;j++){%><div class="picBox"><a href="<%='+V1+'.image[j]%>/460" class="pic"><img class="crs" crs="<%if(iconPic){%>http://mat1.gtimg.com/www/mb/images/vT.png<%}else{%><%='+V1+'.image[j]%>/160<%}%>"></a></div><%}%></div><%}%><div class="pubInfo"><span class="left"><a class="time" rel="<%='+V1+'.timestamp%>" href="/p/t/<%='+V1+'.id%>" target="_blank"><%='+V1+'.time%></a> <%='+V1+'.from%> <%if(MI.user.account){%><a href="/p/z/<%='+V1+'.id%>" class="zfNum" target="_blank"><b class="relayNum"><%='+V1+'.count%></b>人转播</a><%}else{%><span class="zfNum"><b class="relayNum"><%='+V1+'.count%></b>人转播</span><%}%></span><%if(MI.user.account&&UI.isNumber('+V1+'.comt)){%><div class="funBox"><a href="/p/c/<%='+V1+'.id%>" class="comt">评论(<em><%='+V1+'.comt%></em>)</a></div><%}%></div></div></div><%}%><div class="pubInfo"><span class="left"><%if('+V1+' && talk[i].type==2){%><span class="time"><%=talk[i].time%></span><%}else{%><a class="time" href="/p/t/<%=talk[i].id%>" target="_blank"><%=talk[i].time%></a><%}%> <%=talk[i].from%><%if(talk[i].count){if(MI.user.account){%> <a href="/p/z/<%=talk[i].id%>" class="zfNum" target="_blank"><b class="relayNum"><%=talk[i].count%></b>人转播</a><%}else{%> <span class="zfNum"><b class="relayNum"><%=talk[i].count%></b>人转播</span><%}}%><%if('+V1+' && '+V1+'.content && '+V1+'.content.length && talk[i].type==4){%> <a class="view zfNum" href="/p/r/<%=talk[i].id%>">查看对话</a><%}%><%if(!talk[i].flag.auth){%> <a'+V5+' class="alarm">举报</a><%}%></span><div class="funBox"<%if(!MI.user.account){%> style="display:none"<%}%>><%if(UI.isNumber(talk[i].comt)){%><a href="#" class="comt">评论(<em><%=talk[i].comt%></em>)</a><span>|</span><%}%><%if(MI.user){if((!guest || guest == MI.user.account) && MI.user.account==talk[i].name){%><a'+V5+' class="delBtn">删除</a><span>|</span><a'+V5+' class="relay">转播</a><%}else{%><a'+V5+' class="reply">对话</a><span>|</span><a'+V5+' class="relay">转播</a><%}%><span>|</span><a'+V5+' class="fav<%if(fav){%> light<%}%>" title="<%if(fav){%>取消<%}%>收藏"></a><%}%></div></div><%if('+V1+' && talk[i].type==4){%><div class="talkDetail"><b><%='+V1+'.nick%>:</b> <%='+V1+'.content%></div><%}%></div></li><%}%>',
		...
		card : '<div class="uCard"><div class="tip">这是你自己</div><div class="userPic"><a></a><p class="btn"><input type="button" class="addAttention" value="" /><a href="#" class="delAttention" title="取消收听">取消</a></p></div><div class="uCardcnt"><p class="userName"></p><p class="nums loading"></p><p class="btn"><a href="#" class="reply">对话</a><a href="#" class="msg">私信</a></p></div></div>'
	},
			...
			if (word) {
				...
				if (matchLength < 40) {
					for (var i in users) {
						...
						if (( i.slice(1).match(regAll) || users[i].slice(1).match(regAll) ) && !i.hasString(users[i])) {
...
MI.Dialog = function(o){ //Dialog
	...
	var wrap = Self._wrap;
	Self._title.onmousedown = function(e) { //Move
...
MI.Dialog.prototype = {
	...
	show : function(o){
		...
		Self.display = 1;
		UI.C(document.documentElement,'overflowX','hidden');
	},
	hide : function(){
		UI.hide(this._body);
		this.hideLoad();
		...
		this.display = 0;
		UI.C(document.documentElement,'overflowX','auto');
	},
	showLoad : function(){
...
MI.TalkBox.prototype = {
	...
	send : function(){
		...
		//Go To User's Page
		if (!Self.type && !Self.pic && !Self.topic && content.match(/^@[\w|-]{0,64}$/g) && content.length > 1) {
		...
		//Post Data
		data = {content:content,pic:Self.pic,countType:Self.countType,viewModel:Self.viewModel};
		if (Self.type) {
...
MI.Card.prototype = {
	...
	show : function(el){
		...
		function show(o){
			...
			Self._num.innerHTML = '广播' + num[0] + '条<span>' + (o.flag.vivi ? '' : '|</span>听众' + num[1] + '人<span>');//|</span>收听了' + num[2] + '人
			Self._name.innerHTML = '<a href="' + url + '">' + name + '</a>' + (o.flag.auth ? '<a class="vip" target="_blank" title="腾讯认证" href="/certification"></a>' : '') + (o.flag.expo ? '<a class="ico_expo" target="_blank" title="上海世博" href="/certification"></a>' : '') + '<br /><span class="en">@' + account + '</span>';
			if (notSelf) {
