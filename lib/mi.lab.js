...
MI = {
	...
	openMusic : function(url){
		...
		//var aList = '[[\"'+songData.songID+'\",\"\",\"'+songData.url+'\",\"'+escape(UI.url.decode(songData.song))+'\",\"\",\"'+escape(UI.url.decode(songData.singer))+'\",\"'+songData.songID+'\",\"\",\"\",\"\"]]';
		...
		var url = 'http://music.soso.com/url_player.html?song='+escape(UI.url.decode(songData.song))+'&singer='+escape(UI.url.decode(songData.singer))+'&url='+escape(playUrl)+'&stream=0&songID=0';
		var win = window.open(url, "_webSosoMusicPlayer", MI.listen_feature);
		if(!win){
