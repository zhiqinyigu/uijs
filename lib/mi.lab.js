		...
		listAll : '<%for(var i=0,n=talk.length;i<n;i++){%><li id="<%=talk[i].id%>"<%if(fav){%> fav="<%=talk[i].favortimestamp%>"<%}%> rel="<%=talk[i].timestamp%>">\
			...
			<div class="msgBox">\
				...
				<%var Imgage=talk[i].image && talk[i].image.length,Videos=talk[i].videos && talk[i].videos.length,Music=talk[i].music && talk[i].music.length;if(Imgage || Videos || Music){%><div class="mediaWrap">\
					...
					<%}%>\
					<%if(Music){%>\
						<div class="musicBox" song="<%=talk[i].music[0].sMusicName%>" singer="<%=talk[i].music[0].sMusicAuthor%>" stream="<%=talk[i].music[0].dwMusicLocation%>" songid="<%=talk[i].music[0].dwMusicID%>">\
					...
					<div class="replyBox">\
						<div class="msgBox">\
							...
							<%var Imgage='+V1+'.image && '+V1+'.image.length,Videos='+V1+'.videos && '+V1+'.videos.length,Music='+V1+'.music && '+V1+'.music.length;if(Imgage || Videos || Music){%><div class="mediaWrap">\
								...
								<%}%>\
								<%if(Music){%>\
									<div class="musicBox" song="<%='+V1+'.music[0].sMusicName%>" singer="<%='+V1+'.music[0].sMusicAuthor%>" stream="<%='+V1+'.music[0].dwMusicLocation%>" songid="<%='+V1+'.music[0].dwMusicID%>">\
										<a href="#" class="mThumbsBox" thumbs="<%if(iconPic){%>0<%}else{%>1<%}%>">\
				...
				<div class="pubInfo"><span class="left"><a class="time" href="/p/t/<%=talk[i].id%>" target="_blank"><%=talk[i].time%></a> <%=talk[i].from%><%if(0 && talk[i].count){if(MI.user.account){%>\
					...
					<a class="view" href="/p/r/<%=talk[i].id%>">查看对话</a><%}%><%if(MI.user.fun.report && talk[i].name != MI.user.account){%> <a'+V5+' class="alarm">举报</a><%}%></span><div class="funBox"><%if(MI.user.account){%><a'+V5+' class="fav<%if(fav){%> light<%}%>" title="<%if(fav){%>取消<%}%>收藏"></a><span>|</span><%if(MI.user){if((!guest || guest == MI.user.account) && MI.user.account==talk[i].name){%><a'+V5+' class="delBtn">删除</a><span>|</span><a'+V5+' class="relay">转播</a><%}else{%><a'+V5+' class="reply">对话</a><span>|</span><a'+V5+' class="relay">转播</a><%}}}%><%if(talk[i].count){%><%if(MI.user.account){%>\
...
MI.Crs = function(reset){
	...
	var scrollY = UI.scrollY(),y = scrollY + UI.windowHeight() * 1.5,
		noCrs = 1,
		className = 'crs',
		delay = 0;
	...
	UI.each(MI.crs,function(o){
		...
		if (UI.hasClass(o,className) && _y && _y < y && _y > scrollY) {
...
MI.TalkList = function(id){ //Talk List
	...
	Self.replyBox.successStart = Self.relayBox.successStart = Self.comtBox.successStart = function(){
		...
		if (viewRelay && !viewRelay.innerHTML.hasString('对话')) { //View Relay List
			var relayList = Self._relayList,
				...
				createRelayBox = function(){
					UI.hide(this);
					var relay,isRelay = 0;//MI.S('option_relayListCheck_' + MI.user.account) != -1
					if (!this.appended) {
...
MI.TalkList.music = function(a,b){ //Music Event
	...
	var arg = {
		location : UI.A(musicBox,'stream'),
		songID : UI.A(musicBox,'songid'),
		songName : UI.A(musicBox,'song'),
		singerName : UI.A(musicBox,'singer')
	}
	var link = '<a class="mPlayerPlaying" href="http://music.soso.com/url_player.html?song=%esong%&singer=%esinger%&url=http://upload%stream%.music.qzone.soso.com/%songid%.wma&stream=0&songID=0" target="_blank" title="%song%-%singer%"><span style="margin-right:10px;">%song%-%singer%</span></a>';
	
	link = link.replace(/\%esong\%/gi,escape(UI.A(musicBox,'song'))).replace(/\%esinger\%/gi,escape(UI.A(musicBox,'singer'))).replace(/\%song\%/gi,UI.A(musicBox,'song')).replace(/\%singer\%/gi,UI.A(musicBox,'singer')).replace(/\%stream\%/gi,UI.A(musicBox,'stream')).replace(/\%songid\%/gi,UI.A(musicBox,'songid'));
