	...
	addPic : function(o){
		...
		if (o.result == 0) {
			...
			if (MI.user.fun.turnPic && o.info.path) {
				...
				Self._picLink.innerHTML = '<a href="#" class="fileName" onclick="return false">' + fileName + '<span class="simg"><img  src="' + pic +  "/160" + '" /><span class="useFilter" style="dispaly' + (UI.B.ipad ? 'none':'inhert' ) + '" href="javascript:void(0)">' + _('使用魔法滤镜') + '</span></span></a>';
...
MI.PicTurn = {
	...
	set: function(src) {
		this._imgCt.innerHTML = '<img src="' + src + '/160" /><a href="#" style="display:' + (UI.B.ipad ? 'none':'inline-block') + ';float:none;padding:5px 0;" class="useFilter">' + _('使用魔法滤镜') + '</a>';
...
MI.TalkList.prototype = {
	...
	remove : function(id,type,veriCode){
		...
		var Self = this,
			talk = $(id),
			url = Self.removeUrl,
			data = {id:parseInt(id),r:MI.random()},
			favBtn;
		if (veriCode) {
	...
	addEvent : function(el){
		...
		if (comt) { // 评论
			comt.onclick = function(){
				...
				Self.reply(elId,2,UI.A(this,'num'));
		...
		if (relay) { // 转播
			relay.onclick = function(){
				...
				Self.reply(elId,1,UI.A(this,'num'));
				Self.replyCont = this.cont || UI.A(this,'cont');
				Self.replyTarget = this;
		...
		if (moreFun) { // 更多
			...
			moreFun.onclick = function(e){
				...
				moreFunOpenDelay = setTimeout(function(){
					Self.moreFun(T.parentNode,elId);
...
MI.App = {
	 ...
	 * }
	 *            @example
	 *            var relayList = MI.App.relayList({target : $('qz-talkList'),id:'24106133577475'});
	 */
