...
MIIco = ['expo','cic']; //Set Icon's Order
MIIcoHtml = ['<a href="http://blog.qq.com/zt/2010/2010expo/shibovol.htm" title="2010上海世博志愿者" target="_blank" class="ico_expo"></a>','<a href="http://t.qq.com/group.php?g=cic2010" onclick="MI.Bos(\'btnIconCic\')" class="ico_net" title="互联网大会认证名人"></a>'];
MIIcoWC = function(id){
...
//				var s = txtLen;
	...
	lotteryStart : function(a){
		...
		if(a){
			...
			else
			{
				...
				rnd =  rnd > l ? rnd - l : rnd;
				Self.lot.info.talk = Self.delLuckyData(data.info.talk, rnd);
			}
	...
	delLuckyData : function(a, num){
		if(num >= a.length){
			return a;
		}
		var tmp = [];
		for(var i =0; i<a.length; ++i){
			if(i != num)tmp.push(a[i]); 
		}
		return tmp;
	},
