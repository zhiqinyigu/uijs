...
MI.CicTalkList.prototype = {
	...
	addNewly : function(json){
		...
		if (data.result == 0 && data.info.talk.length > 0) {
			...
			Self.talkheight = (cfg.showOne) ? Self._singleHeight : UI.height(Self.newtalk);
				...
				setTimeout(function(){
					...
					for(var i = 0; i < 6; i++){
						...
						try{
							...
							if (replyBox) h += UI.height(replyBox);
							if (pubInfo) h += UI.height(pubInfo);
						}
						catch(e){
						}
						if ( h > Self.talkheight ){
							var s = parseInt(username.style.fontSize) - step;
							UI.each(cont,function(o,i){
								o.style.fontSize = parseInt(s) + 'px';
								o.style.lineHeight = parseInt(s) + 17 + 'px';
								username.style.fontSize = parseInt(s) + 'px';
