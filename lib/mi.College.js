...
MI.College = {
	...
	build : function(){
		...
		UI.append(MI.College._body,document.body);

		MI.College._close = dom('.close');
		MI.College._btn = dom('.openSteps');
		MI.College._cont = dom('.beginnerCon');
		MI.College._success = dom('.beginnerSuc');
		MI.College._finish = dom('.finish a');
		MI.College._scrollTxt = dom('.scrollTxt');
		MI.College._follow = UI.html('<input type="button" class="addAttention">')[0];
		MI.College._cont.innerHTML = '<a href="#" class="close" title="关闭" onclick="MI.S(\'tips_college_\'+MI.user.account,-1);MI.College.hideCont(this.parentNode);return false"></a><h2></h2><ul class="giftWrap"><li><h3 class="g2">纪念版皮肤</h3><p>精美纪念版皮肤<br />优质主播专享！</p></li><li><h3 class="g3">手机达人勋章</h3><p>点亮你的手机勋章<br />多种方式玩微博！</p></li><li><h3 class="g4">醒目位置推荐</h3><p>将你推荐到找人页面让更多人关注你！</p></li></ul><div class="HSR rstepWrap"><div class="top"><span class="left"></span><span class="right"></span></div><div class="cntBox"><h3>需要完成的几件事</h3><ol class="clear"><li class="done"><h4><b>1</b>有头有脸</h4><p>上传头像，做有头有脸的主播，让听众们对你有良好的第一印象！</p><a href="' + step[0] + '"><em></em>现在去完成&gt;&gt;<span class="bg"></span></a></li><li><h4><b>2</b>全面印象</h4><p>尽量详细填写你的个人资料、教育和工作情况，让听众对你印象更全面！</p><a href="' + step[1] + '"><em></em>现在去完成&gt;&gt;<span class="bg"></span></a></li><li><h4><b>3</b>初试啼声</h4><p>发送你的第一条广播，不管是生活奇遇，还是现在心情，看有没有听众来响应！</p><a href="' + step[2] + '"><em></em>现在去完成&gt;&gt;<span class="bg"></span></a></li><li><h4><b>4</b>新鲜达人</h4><p>用wap、短/彩信、客户端等任意无线方式发广播，即可点亮手机达人勋章！</p><a href="' + step[3] + '"><em></em>现在去完成&gt;&gt;<span class="bg"></span></a></li><li><h4 class="last"><b>5</b>领取礼包</h4><p>领取礼包，晋升优质主播，更有想不到的小惊喜等你发现！</p><a href="#" onclick="MI.College._finish.onclick();return false"><em></em>现在去完成&gt;&gt;<span class="bg"></span></a></li></ol></div><div class="bot"><span class="left"></span><span class="right"></span></div></div>';
		MI.College._steps = $$(MI.College_body,'.beginSteps .lstepWrap li');
		MI.College._stepsBig = $$(MI.College_body,'.beginnerCon .rstepWrap li');

		MI.College._close.onclick = function(){ //关闭
			if (confirm('确定关闭广播学院？\r\n\r\n关闭广播学院后你还可以在其他设置里重新打开。')) {
				UI.hide(MI.College._body);
				UI.ajax({url:'/asyn/userAttrSave.php',data:{t:17,v:1},success:function(){}})
				MI.College.hideBg();
			}
			return false;
		}
		MI.College._btn.onclick = function(){ //展开领取步骤
			var T = this,
			...
			if (!UI.hasClass(P,'showSteps')) {
				//if (!finished) {
						...
						UI.get('/asyn/college.php','r=' + MI.random(),function(data){
							...
							if (data.result == 0) {
								...
								UI.each(data.info.step,function(o,i){
									...
									if (o) {
										className = 'done';
										link = MI.College._stepsBig[i].lastChild;
										...
										UI.hide(MI.College._stepsBig[4].lastChild);
									}
									if (i == MI.College.cur - 1) {
										className += ' cur';
										MI.College._steps[i].firstChild.onclick = function(){
											return false;
										}
									}
									UI.addClass(MI.College._steps[i],className);
									UI.addClass(MI.College._stepsBig[i],className);
								});
								MI.College._finish.finished = data.info.finished;

								//Scroll Text
								var user = data.info.user;
								if (user) {
									var text = user.nick + ' ' + user.time + '前晋升为优质主播';
									MI.College._scrollTxt.innerHTML = '<a href="/' + user.name + '" title="' + text + '" style="display:inline-block;margin-left:60px">' + text + '</a>';
									MI.College._scrollTxtCont = MI.College._scrollTxt.firstChild;
									MI.College._scrollTxtCont.onmouseover = function(){
										MI.College.scrollStop = 1;
									}
									MI.College._scrollTxtCont.onmouseout = function(){
										MI.College.scrollStop = 0;
									}
								}

								MI.College.data = data;
								...
								UI.animate(P,'height',height,function(){
									if (MI.S('tips_college_' + MI.user.account) != -1) {
										MI.College.showCont(MI.College._cont);
									}
								});
							}
							T.sending = 0;
					...
					if (finished) {
						MI.College._finish.innerHTML = '<em></em>我的礼包';
						MI.College._finish.parentNode.className = 'done';
					}
				//}
				//else {
				//	MI.College._finish.onclick();
				//}
			}
			T.blur();
			return false;
		}
		MI.College._finish.onclick = function(){ //领取礼包
			var T = this,
				successNotDisplay = MI.College._success.style.display == 'none';
			if ((successNotDisplay && MI.College._finish.finished) || UI.hasClass(MI.College._btn,'finished')) {
				if (MI.College._success.innerHTML != '') {
					...
					UI.get('/asyn/collegePodium.php','r=' + MI.random(),function(data){
						...
						if (data.result == 0) {
							UI.evalScript(js);
							MI.College._success.innerHTML = data.info;
							show();
						}
						T.sending = 0;
					});
			...
			else if (MI.College._cont.style.display == 'none' && !MI.College._finish.finished) {
				MI.College.showCont(MI.College._cont);
			}
			function show(){
				...
				UI.hide(MI.College._cont);
				MI.College.showCont(MI.College._success);
			}
			T.blur();
			return false;
		}
		function dom(className){
			return $$(MI.College._body,className)[0];
		}

		//Position Event
		MI.College.position();
		UI.EA(window,'resize',MI.College.position);

		//Auto Open
		setTimeout(function(){
			if (MI.S('tips_collegeStep_' + MI.user.account) == 1) {//!UI.hasClass(MI.College._btn,'finished') && 
				MI.College._btn.onclick();
			}
		},0);

		//Current Step
		var pathName = document.location.href;
		UI.each(MI.College.step,function(o,i){
			if (!MI.College.cur && pathName.hasString(o)) {
				MI.College.cur = i + 1;
			}
		});
		...
		setInterval(function(){
			if (!MI.College.scrollStop) {
				var cont = MI.College._scrollTxtCont,
					left,
					width;
				if (cont) {
					...
					width = UI.width(cont);
					if (left < -width) {
						left = UI.width(MI.College._scrollTxt); 
					}
					cont.style.marginLeft = left - MI.College.scrollStep + 'px';
				}
			}
		},MI.College.scrollSpeed);
	},
	clear : function(){
	...
	position : function(){
		...
		if (headWrap) {
			if (!MI.College.width) {
				MI.College.width = UI.width(MI.College._body) - (UI.B.ie ? 0 : 4);
			}
			MI.College._body.style.cssText += ';left:' + (UI.getX(headWrap) - MI.College.width) + 'px;top:' + UI.height(headWrap) + 'px';
		}
	},
	...
	showBg : function(){
		UI.C(MI.College._body,'zIndex','9999');
	...
	hideBg : function(){
		...
		MI.dialog.display = 0;
		UI.C(MI.College._body,'zIndex','99');
	},
	follow : function(){
		if (window.GetGroupData) {
			...
			if (account.length) {
				MI.follow(account.join(','),MI.College._follow,function(){
					MI.tip('收听成功');
				});
			}
		}
	...
	talk : function(pic){
		if (!MI.College.talk.sending) {
			UI.ajax({
				url : '/publish.php',
				data : {
					pic : pic,
					content : MI.College.talkTxt,
					countType : '',
					viewModel : 0
				},
				success : function(){
