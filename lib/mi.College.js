	...
	build : function(){
		...
		Self._btn.onclick = function(){ //展开领取步骤
			var T = this,
				...
				height,
				finished = MI.user.college.finished;
						...
						UI.get('/asyn/college.php','r=' + MI.random(),function(data){
							...
							if (data.result == 0) {
								...
								UI.animate(P,'height',height,function(){
									if (tips_collegeStep != 1 && MI.S('tips_college_' + MI.user.account) != -1) {
		...
		Self._finish.onclick = function(){ //领取礼包
			...
			if ((successNotDisplay && Self._finish.finished) || MI.user.college.finished) {
