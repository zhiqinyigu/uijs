...
MI.SettingEdu = {
	...
	build : function(){
		...
		//Apply Dom
		Self._applySchool = UI.html('<div class="applyScholl"><p class="cNote">你可以申请添加<span id="applySchoolTypeTxt"></span>，申请通过后，相关信息将出现在对应的选择列表内，添加后通过这些信息可以方便的找到更多的同学。</p><p>你发出的申请会以广播的形式发到<a href="http://t.qq.com/k/%E6%95%99%E8%82%B2%E4%BF%A1%E6%81%AF%E7%94%B3%E8%AF%B7" target="_blank">#教育信息申请#</a>话题中。</p><form onsubmit="MI.SettingEdu.applySchool();return false"><table border="0" cellspacing="0" cellpadding="0">\
		<tbody><tr>\
		<th>学历：</th><td><select id="applySchoolType" onchange="MI.SettingEdu.applySchoolType(this)"><option value="6">博士</option><option value="5">硕士</option><option value="4" selected>大学</option><option value="3">高中</option><option value="2">初中</option><option value="1">小学</option></select></td></tr>\
		...
		<tr><th><span id="applySchoolMust">*</span>学校名称：</th><td><input type="text" class="inputTxt" id="applySchoolName"> <span class="error" id="applySchoolNameTip" style="display:none">这里一定要填</span></td></tr>\
		<tr id="applyDepartment"><th><span id="applyDepartmentMust" style="display:none">*</span>院系名称：</th><td><input type="text" class="inputTxt" id="applyDepartmentName"> <span class="error" id="applyDepartmentNameTip" style="display:none">这里一定要填</span></td></tr>\
	...
	applySchoolDisplay : function(type){ //1-School 2-Department
		var Self = this;
		MI.dialog.show({
			title : '<h1 class="mainTit">申请添加' + (type == 1 ? '学校/' : '') + '院系</h1>',
			width : 540,
		...
		if (type == 1) {
			...
			$('applySchoolTypeTxt').innerHTML = '学校或院系';
			...
			$('applySchoolTypeTxt').innerHTML = '院系';
	...
	applySchoolType : function(el){
		...
		if(el.value > 3){
			...
			this.applyCitySelect.first('中国,1');
			this.applyCitySelect.second('东城,1');
			UI.show(d);
			$('applyDepartmentName').value = ''
		}
		else {
			this.applyCitySelect.build(MI.cityAll);
			this.applyCitySelect.first('北京,1');
			this.applyCitySelect.second('东城区,1');
			UI.hide(d);
		}
	},
	...
	findSchool : function(el){
		...
		if (content.innerHTML == '') {
			UI.get('/asyn/schoolmates.php','school=' + school + '&r=' + MI.random(),function(data){//edu_oper.php
				data = MI.json(data);
				/*
				data = {
					result:0,
				...
				if (data.result == 0) {
					var empty = '<div class="noResult">暂时找不到该学校的校友，你可以邀请他们来微博。<a href="/invite/friends.php">现在去邀请<span class="ffsong">&gt;&gt;</span></a></div>';
					if (data.info && data.info.users.length) {
						var tmpl = '<div class="title clear"><span class="left">共有<b><%=num%></b>位本校校友</span><a href="/search/user.php?high=1&school=<%=school%>" class="right ffsong" target="_blank"<%if(num<11){%> style="display:none"<%}%>>查看全部&gt;&gt;</a></div>\
							<ul class="imgList"><%\
								...
								for(var i=0;i<n;i++){\
									%><li><a href="/<%=users[i].name%>" title="<%=users[i].nick%>(@<%=users[i].name%>)" target="_blank"><img src="<%if(users[i].pic){%><%=users[i].pic%>/50<%}else{%>http://mat1.gtimg.com/www/mb/images/head_50.jpg<%}%>"><%=MI.string.cut(users[i].nick,8,"")%></a><div class="btn"><input type="button" value="<%=users[i].name%>" class="addAttention"></div></li><%\
								}\
							%></ul>';
