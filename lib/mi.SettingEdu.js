...
MI.SettingEdu = {
	...
	findSchool : function(el){
		...
		if (content.innerHTML == '') {
			UI.get('/asyn/schoolmates.php','school=' + school + '&department=' + (department ? department : '') + '&r=' + MI.random(),function(data){//edu_oper.php
				data = MI.json(data);
				/*
				data = {
					result:0,
				...
				if (data.result == 0) {
					...
					if (data.info && data.info.users.length) {
						var tmpl = '<div class="title clear"><span class="left">共有<b><%=num%></b>位本校校友</span><a href="/search/user.php?pos=704&high=1&school=<%=school%>" class="right ffsong" target="_blank"<%if(num<11){%> style="display:none"<%}%>>查看全部&gt;&gt;</a></div>\
							<ul class="imgList"><%\
								var n = users.length;\
								if (n>8) {\
									n=8;\
