...
MI.SettingEdu = {
	...
	selectSchool : function(){ //Select School
		...
		MI.dialog.show({width:630,title:'<h1 class="mainTit">' + _('选择学校') + '</h1>',html:Self._schoolList});
	...
	applySchoolDisplay : function(type){ //1-School 2-Department
		var Self = this;
		MI.dialog.show({
			title : '<h1 class="mainTit">' + _('申请添加') + '' + (type == 1 ? _('学校/') : '') + _('院系') + '</h1>',
			width : 540,
	...
	selectDepartment : function(){ //Select Department
		...
		if ($('s').value == '') {
			...
			MI.dialog.show({width:630,title:'<h1 class="mainTit">' + _('选择院系') + '</h1>',html:Self._departmentList});
