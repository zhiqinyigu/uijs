...
MI.SettingEdu = {
	...
	build : function(options){
		...
		settings.validate = typeof(settings.validate) !== 'undefined' ? settings.validate : true;
		settings.d = settings.d || 'd';//由于不能多实例化，当页面中有2个学校选择器实例时会造成冲突，临时将院系容器id值通过参数传入，默认仍然是'd'
		var Self = this;
		//Form Dom
		...
		MI.app({
			Base : function(){
				Self.schoolKeyBox = new MI.SmartBox('s',function(){
					$('s').focus();$('s').blur();
					settings.schoolSelect && settings.schoolSelect($('s').value);
				},'',{left:-1,top:UI.B.ie?1:1,width:202});
				Self.departmentKeyBox = new MI.SmartBox('d',function(){
					$(settings.d).focus();$(settings.d).blur();
				},'',{left:-1,top:UI.B.ie?1:1,width:202});
				setSmartBoxUrl();
				UI.EA($(settings.d),'focus',function(){
					setSmartBoxUrl();
				});
			}
		});
		...
		if(Self._schoolType) {
			Self._schoolType.onchange = function(){ //School Type
				...
				var department = $(settings.d).parentNode.parentNode;
				if (this.value < 4) {
					UI.hide(department);
				}
				else {
		...
		function pageLink(data){
			...
			UI.each($$(MI.dialog._cont,'.selList a'),function(o){
				o.onclick = function(){
					...
					$('s').blur();
					MI.dialog.hide();
					settings.schoolSelect && settings.schoolSelect(this.title);
					return false;
				}
			});
		}
				...
				success: function(data) {//edu_oper.php
					...
					if (data.result == 0) {
						...
						if (data.info && data.info.users.length) {
							var tmpl = '<div class="title clear"><span class="left">' + _('共有<b>{0}</b>位本校校友','<%=num%>') + '</span><a href="/search/user.php?pos=704&high=1&school=<%=encodeURIComponent(school)%>" class="right ffsong" target="_blank"<%if(num<11){%> style="display:none"<%}%>>' + _('查看全部') + '&gt;&gt;</a></div>\
								<ul class="imgList"><%\
									var n = users.length;\
									if (n>8) {\
										n=8;\
	...
	editSchool : function(el){ //Edit School
		...
		$(settings.d).value = $$(el,'.eduDepartment')[0].innerHTML;
