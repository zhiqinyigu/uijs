...
MI.TalkList.shareCollection = (function(){
	var collectionTab = '<div class="DshareWrap"><div class="tabStyle1 shareTitle"><ul><li class="select"><a href="#">' + _('私信分享') + '</a></li><li><a href="#">' + _('邮箱分享') + '</a></li><li><a href="#">' + _('QQ分享') + '</a></li><li><a href="#">' + _('空间分享') + '</a></li></ul></div>',	
		tmpHtml = collectionTab + MI.tmpl.shareMsg +  MI.tmpl.shareMail + MI.tmpl.shareQQ + MI.tmpl.shareQzone + '</div>',
		...
		tabArray = ['shareMsg','shareMail','shareQQ','shareQzone'],  //和tab对应
		boxArray = [msgBox,mailBox,qqBox,qZoneBox],  	//和tab对应
		lastIndex = 0,			//最后一次点击的tab
		currId = '';			//当前id

	function init(){
		MI.TalkList.shareCollection[tabArray[lastIndex]](currId,1);  //初始化时显示私信分享
		//init = function(){};
	}
...
MI.TalkList.shareCollection.shareMsg = (function(){  //私信分享
    ...
    function getContent(id){
        ...
        txt +=  '” - ' + _('原文地址:') +  _time[_time.length-1].href;
        return txt;
    }
    ...
    function RunOnce(){
        ...
        var cancelBtn = UI.html('<button class="btn_cancel" style="margin:0 0 5px 10px;position:relative;top:-5px;">' + _('取消') + '<button>')[0];
        UI.after(cancelBtn,sendBtn);
        sendBtn.innerHTML = _('分享');
        cancelBtn.parentNode.style.cssText = 'width:260px;margin-left:-60px;text-align:right;';
        cancelBtn.onclick = function(){MI.dialog.hide()};
    }
