MI.SettingWork = {
	...
	findSchool : function(el){
		...
		if (content.innerHTML == '') {
			UI.get('/asyn/colleagues.php','company=' + school + '&r=' + MI.random(),function(data){//edu_oper.php
				data = MI.json(data);
				/*
				data = {
					result:0,
				...
				if (data.result == 0) {
					var empty = '<div class="noResult">暂时找不到该公司的同事，你可以邀请他们来微博。<a href="/invite/friends.php">现在去邀请<span class="ffsong">&gt;&gt;</span></a></div>';
					if (data.info && data.info.users.length) {
						var tmpl = '<div class="title clear"><span class="left">共有<b><%=num%></b>位本公司同事</span><a href="/search/user.php?high=1&c=<%=school%>" class="right ffsong" target="_blank"<%if(num<11){%> style="display:none"<%}%>>查看全部&gt;&gt;</a></div>\
							<ul class="imgList"><%\
								var n = users.length;\
								if (n>10) {\
									n=10;\
