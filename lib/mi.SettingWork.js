			...
			UI.get('/asyn/colleagues.php','company=' + school + '&department=' + (department ? department : '') + '&r=' + MI.random(),function(data){//edu_oper.php
				...
				if (data.result == 0) {
					...
					if (data.info && data.info.users.length) {
						var tmpl = '<div class="title clear"><span class="left">' + _('共有<b>{0}</b>位本公司同事','<%=num%>') + '</span><a href="/companyInfo.php?c=<%=school%>" class="right ffsong" target="_blank"<%if(num<11){%> style="display:none"<%}%>>' + _('查看全部') + '&gt;&gt;</a></div>\
							<ul class="imgList"><%\
								var n = users.length;\
								if (n>8) {\
									n=8;\
