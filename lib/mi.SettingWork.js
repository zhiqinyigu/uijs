			...
			UI.get('/asyn/colleagues.php','company=' + school + '&department=' + (department ? department : '') + '&r=' + MI.random(),function(data){//edu_oper.php
				...
				if (data.result == 0) {
					...
					if (data.info && data.info.users.length) {
						var tmpl = '<div class="title clear"><span class="left">共有<b><%=num%></b>位本公司同事</span><a href="/search/user.php?pos=706&high=1&c=<%=school%>" class="right ffsong" target="_blank"<%if(num<11){%> style="display:none"<%}%>>查看全部&gt;&gt;</a></div>\
							<ul class="imgList"><%\
								var n = users.length;\
								if (n>8) {\
									n=8;\
