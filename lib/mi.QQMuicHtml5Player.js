...
var S_UNDEFINE= 0,S_STOP = 1,S_PAUSE = 2,S_PLAYING = 3,S_BUFFERING = 4,S_PLAYBEGIN = 5,S_PLAYEND = 6, S_TIMEUPDATE = 7;
/**
 * audio播放对象控制类
 */
function H5AudioPlayer() {
	...
	this.initialize = function(playStateChangeCallback) {
		//alert("initialize 1");
		if (!this.checkPlayer()) {
			if (this.mInstall) {
		...
		this.mInit = true;
		//alert("initialize 5");
		this.bindPlayEvent(playStateChangeCallback);//绑定播放器事件
		//alert("initialize 6");
		return true;
	};
	...
	this.bindPlayEvent = function(playStateChangeCallback) {
		var oplay = (this.mPlayerName);
		//var oPlaySrc = (this.mPlayerSrc);
		var _this = this;
		// 开始加载
		EventUtil(oplay,"loadstart",function() {
			...
			if (playStateChangeCallback) playStateChangeCallback(S_PLAYBEGIN);
			//alert("loadstart");
		});
		//开始播放
		...
		EventUtil(oplay,"playing",function() {
			...
			if (playStateChangeCallback) playStateChangeCallback(S_PLAYING);
		});
		// 暂停
		EventUtil(oplay,"pause",function() {
			var oplay=VH5Player.mPlayerName;
			oplay.setAttribute("state", S_PAUSE);
			if (playStateChangeCallback) playStateChangeCallback(S_PAUSE);
		});
		//取回媒介数据过程中（延迟）存在错误
		EventUtil(oplay,"stalled",function() {
		    ...
		    if (playStateChangeCallback) playStateChangeCallback(S_STOP);
		});
		// 加载失败
		...
		EventUtil(oplay,"ended",function() {
			...
			*/
			...
			if (playStateChangeCallback) playStateChangeCallback(S_PLAYEND);
		});
		...
		EventUtil(oplay,"timeupdate",function(){
			...
			*/
			...
			if (playStateChangeCallback) playStateChangeCallback(S_TIMEUPDATE);
		});
	}
