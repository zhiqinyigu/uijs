	...
	toggle : function(o){
		...
		if (id) { //Toggle Dialog
			...
			for (var i in Self.cache) {
				...
				else if (Self.cache[i].displayTmp) {
					Self.cache[i].show();
					Self.current(Self.cache[i].id);
				}
			}
