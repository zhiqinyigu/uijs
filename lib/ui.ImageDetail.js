UI.ImageDetail = {
	...
	config : { //Config
		...
		tmplList : '<% for ( var i = 0; i < list.length; i++ ) { %><li><a href="javascript:void(0)" onfocus="this.blur()" onclick="UI.ImageDetail.showBox(<%=i%>,this);return false;" class="box" style="background-image:url(<%=list[i][2].replace(/3\\./gi,"2.")%>)"></a><a href="javascript:void(0)" onclick="UI.ImageDetail.showBox(<%=i%>,this);return false;" class="txt"><%=list[i][1]%></a></li><% } %>',
	...
	init : function(o){
		...
		//Change Option For Window's Height
		if (UI.windowHeight() < 725) {
			this.config.numPerPage = 4;
			this.config.bigImg.height -= this.config.heightPerImg;
			UI.addClass(this._box,'photo_box_small');
		}
		...
		function init(){
			...
			if (o.current) {
				UI.hide(Self._listAll.parentNode);

				Self.listImg = UI.GT(Self._list,'img'); //List Image
				Self.current = o.current == 0 ? data[1][0][0] : o.current; //Current ID
				Self.pageTotal = Math.ceil(Self.numTotal/Self.config.numPerPage); //Page Number
				Self.getIndex(); //Get Index

				Self.buildList(0);
				Self._list.builded = true; //If has builded
				
				if (Self.numTotal == 1) {
					UI.hide(Self._imgAll);
				}
			}
			else {
				UI.hide(Self._imgAll);
				...
				Self.buildList(1);
				Self._listAll.builded = true; //If has builded
			}
			...
			setTimeout(function(){ //Scroll To Picture Area
				UI.scrollTo(document.body,0,UI.getY(Self._numTotal));
			},0);
		}
	},
	build : function(){
		...
		document.title = (this.list[index][1] ? this.list[index][1] + '_' : '') + this.title;
		...
		//List Page
		var indexMiddle = Math.ceil(this.config.numPerPage / 2) - 1;
	...
	buildList : function(type){
		switch (type) {
			case 0 :
				...
				break;
			case 1 :
				this._listAll.innerHTML = new UI.tmplString(this.config.tmplList)({list:this.data[1]});
				break;
		}
	},
	prev : function(){
	...
	showList : function(){ //Show All Image In List
		UI.hide(this._box);
		UI.show(this._listAll.parentNode);
		UI.hide(this._imgAll);
		document.title = this.title;
		if (!this._listAll.builded) {
			this.buildList(1);
			this._listAll.builded = true;
		}
		UI.addClass(this._listAll.childNodes[this.index],'on');
	},
	showBox : function(index,el){ //Show Image Box
		UI.show(this._box);
		UI.hide(this._listAll.parentNode);
		if (this.numTotal != 1) {
			UI.show(this._imgAll);
		}
		if (!this._list.builded) {
			this.buildList(0);
			this.listImg = UI.GT(this._list,'img');
			this._list.builded = true;
		}
		this.index && UI.removeClass(this._listAll.childNodes[this.index],'on');
		this.show(index,el);
	},
	...
	positoin : function(el,small){
		...
		if (!small) {
			UI.ImageDetail._title.style.cssText += ';display:block;margin-top:' + UI.height(el) / 2 + 'px;';
			if (this.animate) {
