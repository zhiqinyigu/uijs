 ...
 */
 MI.Sidebar = {
	...
	_subRel : '', //mi.load加载的url分类名称
 	...
 	_listHtml : UI.html('<div class="tjTypeList"><a href="#" class="mobileBox" name="'+_('精选') + '" rel="" type="20">'+_('精选')+'</a><a href="#" class="foFun" name="'+_('娱乐')+'" rel="1.2.4" type="21">'+_('娱乐')+'</a><a href="#" class="foFun" rel="1.2.12" name="'+_('科技')+'" type="22">'+_('科技')+'</a><a href="#" class="foFun" rel="1.2.5" name="'+_('财经')+'" type="23">'+_('财经')+'</a><a href="#" class="foFun" rel="1.2.13" name="'+_('游戏')+'" type="24">'+_('游戏')+'</a><a href="#" class="foFun" rel="1.2.1" name="'+_('体育')+'" type="25">'+_('体育')+'</a><a href="#" class="foFun" rel="1.15.1" name="'+_('传媒')+'" type="26">'+_('传媒')+'</a><a href="#" class="foFun" rel="1.2.6" name="'+_('动漫')+'" type="27">'+_('动漫')+'</a><a href="#" class="foFun" rel="1.2.2" name="'+_('文化')+'" type="28">'+_('文化')+'</a><a href="#" class="foFun" rel="1.11.1" name="'+_('政府')+'" type="29">'+_('政府')+'</a><a href="#" class="foFun" rel="1.2.7" name="'+_('星座')+'" type="30">'+_('星座')+'</a><span  onclick="MI.Bos(\'btnRecommendClass_more\'); MI.feedBack({\'scene\':1,\'type\':\'31\'});" type="31"><a href="http://t.qq.com/people" class="foFun" rel="more" target="_blank" name="'+_('更多')+'">'+_('更多')+'<em class="ffsong">&gt;&gt;</em></a></span></div>')[0],
 	//初始化sidebar部分的dom
 	build : function(id){
 		 var Self = this;
 		 Self._id = id;
				...
				var bossName = 'btnRecommendClass_';
	 			...
	 			if(sublist!='more'){
					...
					$$($(id).parendNode,'.btn_refresh')[0].style.display='';
		 			...
		 			MI.feedBack({"scene":scene,"type":typeNo});
		 			return false;
	 			}
	 		};
		 	...
		 	listBtn.onclick = function(){	 			
		 		...
		 		if(tjtype){
					...
					UI.hide($$($(id).parendNode,'.btn_goTypes .sep')[0]);
			 		...
			 		MI.Bos('btnStarList');
			 		MI.feedBack({"scene":scene,"type":13}); //上报说明，13为分类
		 		}
		 		return false;
		 	}
		 }
 	...
 	clickEvent : function(id,scene){
		...
		if(moreBtn){
			moreBtn.onclick = function(){
				...
				MI.feedBack({"scene":scene,"type":7});
			};
		}
		UI.each($$(targetDom,'.foFun'),function(o,i){
			...
			o.onclick = function(){
				...
				if(reason == '') reason = '0';
				reasons = account + '.' + reason;
				MI.feedBack({"scene":scene,"type":3,"reasons":reasons});
				return false;
			}
		...
		UI.each(tempL,function(o){
			$$(o,'a')[0].onclick = function(){
					...
					var reason = UI.A($$(UI.parents(o)[1],'.BossHref')[0],'reason');
					if(reason == '') reason = '0';
					var reasons = account + '.' + reason;
		
					MI.feedBack({"scene":scene,"type":2,"reasons":reasons});
			}
		});
		UI.each($$(targetDom,'.BossHref'),function(o){
			o.onclick = function(){
				...
				if(reason == '') reason = '0';
				var reasons = account + '.' + reason;		
				MI.feedBack({"scene":scene,"type":5,"reasons":reasons});
			}
		});
 	},
 	//点 叉 的动作逻辑
	 ...
	 * @param {String} String 推荐分类的类别代号 =》 1.2.4
	 	...
	 	UI.each($$(target,'.knownInfo .del'),function(o){
			o.onclick=function(){
				...
				if(P.parentNode.childNodes[userNum]){
					UI.after(P.parentNode.childNodes[userNum], P);
				}
				...
				if(reason == '') reason = '0';
				var reasons = account + '.' + reason;
				MI.feedBack({"scene":scene,"type":6,"reasons":reasons});
				return false;
			}
		});
 	},
 	...
 	refresh : function(id,scene,userNum,subName){
 		...
 		Self._refresh.onclick=function(){
 			MI.feedBack({"scene":scene,"type":9});
 			var li = $$($(id),'li');
		 ...
		 MI.app({
			...
			for(var i=0;i<len;i++){
				if(!(UI.A(arr[i],'style') && arr[i].style.display=='none')){
					...
					if(reason == '') reason = '0';
					var account = UI.A(arr[i],'name') + '.' + reason;
			...
			MI.feedBack({"scene":scene,"type":1,"reasons":reasons});
			}
		});	
 	},
 	img : function(target){
