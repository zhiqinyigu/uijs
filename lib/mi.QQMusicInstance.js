...
function getQusicURL(sPlayUrl)
{
	...
	else
	{
		...
		if(pos!=-1)
		{
			...
			sPlayUrl = sPlayUrl.substring(0,pos+14)+ "/"+qusidt+".wma";
		}
...
function getQusicH5URL(sPlayUrl)
{
	...
	if(pos!=-1)
	{
		...
		sPlayUrl = "http://stream" +(parseInt(getStreamID(sPlayUrl))+10)+ ".qqmusic.qq.com/"+qusidt+".mp3";
	}
...
function playOneSong(name,urlin,qusid,ifexp,qzonekey,notCheck,singerName,iSongId,iSongType)
{
...
{
	initMusicR(function(){playOneSong(name,url,qusid,ifexp,qzonekey,true,singerName,iSongId,iSongType)});
}
//更新播放器
...
function openDownloadDiv()
{
	...
	top.QZONE.FrontPage.popupDialog(MI.Music.tipsMsg[2],strHTML, 476, 311);
...
function initMusicR(cb)
{
	...
	if(!window.musicInitReady)
	{
		...
		else if(Browser.isFirefox)
		{
			...
			if(ua.windows >= 6.0 || ua.firefox >= 3.6)
			{
				MI.alert(MI.Music.tipsMsg[0]);
				MI.Music.stop();
				QZONE.music.isFirstPlay = false;
				return false;
			}
