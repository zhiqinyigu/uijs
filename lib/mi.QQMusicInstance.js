...
MUSIC.userAgent = (function() {
	var vie, vff, vopera, vsf, vqq, vqqwebkit, vapple, wintype, mactype, vair, vchrome;//vqq-QQ浏览器，vqqwebkit-QQ浏览器高速模式
	var discerned = false;
	...
	if (agent) {
		...
		//判断是否是QQ浏览器
		var QQagent = (/QQBrowser.(\d+(?:\.\d+)?)/).exec(navigator.userAgent);
		if(QQagent) {
			vqq = QQagent[1] ? parseFloat(QQagent[1]) : NaN;
		} else {
			vqq = NaN;
		}
		vqqwebkit = NaN;
		...
		if (!isNaN(vsf)) {
			...
			//判断是否是QQ浏览器高速模式
			vqqwebkit = !isNaN(vqq) ? 1 : NaN;
		}
	} else {
		vie = vff = vopera = vsf = vapple = vair = vqq = vqqwebkit = NaN;
	}
	if (os) {
		wintype = !!os[1];
		mactype = !!os[2];
...
var Browser = 
{
	...
	isSafari : !!ua.safari,
	isOpera : !!ua.opera,
	isQQBrowser : !!ua.qqbrowser,
	isQQBrowserWebkit : !!ua.qqbrowserWebkit
};
/**
 * namespace 处理
 * 
	...
	{		
		...
		{ 
			if((Browser.isSafari || (ua && !!ua.chrome))&& top.VH5Player)
			{
				...
				noticeToolbar("play");
				/*
				 * @modified samlu 这里已经不再需要手动绑定回调函数了，回调函数的绑定移动到VH5Player的initialize()函数中去了。
				if(call){
					EventUtil(top.VH5Player.mPlayerName, "play", call);
					EventUtil(top.VH5Player.mPlayerName, "playing", call);
					EventUtil(top.VH5Player.mPlayerName, "pause", call);
					EventUtil(top.VH5Player.mPlayerName, "ended", call);
					EventUtil(top.VH5Player.mPlayerName, "error", call);
				}*/
			}
			/*else 
			{
			...
			else 
			{
				...
				*/
				MediaPlayer.runPlayer(url);
				// 设定播放事件处理函数
				MediaPlayer.playStateChange = call;
...
function initMusicR(cb,call)
{
	...
	if(!window.musicInitReady)
	{
		...
		else if(Browser.isFirefox || Browser.isOpera || Browser.isQQBrowserWebkit)
		{
			//alert("Browser.isFirefox");
			//FF下暂时屏蔽vista和win7,目前的控件有可能crash.
			if(ua.windows >= 6.0 || ua.firefox >= 3.6 || Browser.isOpera || Browser.isQQBrowserWebkit)
			{
				if(!window.loadMusicWmp)
				{
					includeJS(MI.version.QQMusicWmpPlayer,function(){
			...
			if(!window.loadH5Audio)
			{
				includeJS(MI.version.QQMuicHtml5Player,function(){
					try{
						...
						VH5Player.initialize(call); // 初化过程中同时绑定插件音频事件回调函数
						window.musicInitReady = true;
						if(cb){cb();}
					}catch(e){}
				})
